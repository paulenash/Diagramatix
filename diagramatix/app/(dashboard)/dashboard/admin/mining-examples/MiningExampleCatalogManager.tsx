...
export function MiningExampleCatalogManager() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/mining-examples");
  ...
  async function patch(id: string, body: Record<string, unknown>) {
    ...
    try {
      const res = await fetch(`/api/admin/mining-examples/${id}`, {
    ...
    <div className="h-screen flex flex-col overflow-hidden">
        ...
        {rows.map((r) => (
          <div key={r.id} className="rounded-lg border border-gray-200 bg-white p-4">
            ...
            <div className="flex items-center gap-3 flex-wrap text-sm">
              ...
              <button onClick={() => act(`/api/admin/mining-examples/${r.id}/duplicate`, "POST")} disabled={busy !== null} className="text-xs rounded border border-gray-300 px-2.5 py-1 text-gray-700 hover:bg-gray-50 disabled:opacity-50">
                Duplicate
              </button>
      ...
      {confirmDelete && (
        <ConfirmDialog
          ...
          onConfirm={() => { const r = confirmDelete; setConfirmDelete(null); act(`/api/admin/mining-examples/${r.id}`, "DELETE"); }}
        />
      )}
    </div>
  );
