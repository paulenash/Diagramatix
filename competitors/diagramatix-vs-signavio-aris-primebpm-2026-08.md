...
The July edition listed process mining as **suite-only** and told Diagramatix "don't compete here." That has changed: Diagramatix ships **Diagramatix Miner**, an in-tool process-mining module.
...
4. **Process mining (Diagramatix Miner)** incl. **object-centric (OCEL)** logs, that **calibrates the simulator** — a mine→model→simulate loop.
