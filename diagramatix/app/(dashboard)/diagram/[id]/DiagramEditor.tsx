...
export function DiagramEditor({
  ...
  // Two files are downloaded: <diagramName>.xml and diagramatix-export.xsd.
  async function handleExportXml() {
    ...
    // Best-effort fetch of the latest XSD (with version placeholders resolved).
    let xsdText = "";
    ...
    try {
      ...
      if (resp.ok) {
        ...
        // Pull the appVersion out of the served XSD's annotation
        const m = xsdText.match(/Generated by Diagramatix ([\d.]+)/);
        if (m) appVersion = m[1];
      }
    } catch {
      // ignore — we'll still export the XML with the local schema version
    }
    ...
    // Trigger XSD download alongside (if we managed to fetch it)
    if (xsdText) {
      const xsdBlob = new Blob([xsdText], { type: "application/xml" });
      const xsdUrl = URL.createObjectURL(xsdBlob);
      const a2 = document.createElement("a");
      a2.href = xsdUrl;
      a2.download = `diagramatix-export-v${appVersion}.xsd`;
      document.body.appendChild(a2);
      a2.click();
      document.body.removeChild(a2);
      setTimeout(() => URL.revokeObjectURL(xsdUrl), 1000);
    }
  }
        ...
        <div className="relative" ref={exportDropdownRef}>
          ...
          {exportDropdownOpen && (
            <div className="absolute right-0 top-full mt-1 w-44 bg-white border border-gray-200 rounded shadow-lg z-50">
              ...
              <button
                onClick={() => { handleExportXml(); setExportDropdownOpen(false); }}
                className="w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-50"
