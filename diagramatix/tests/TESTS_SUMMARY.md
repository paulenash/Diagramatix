...
**As at:** 2026-09-16  ·  **Document version:** 7.0  ·  **Suite:** 507 test files · 3,745 tests (all green)  ·  **Runner:** Vitest  ·  **CI:** enforced on every PR + push to `main`  ·  **Highest ref:** T4545  ·  **Plus:** a Playwright browser e2e suite — see [Layer 11](#layer-11--end-to-end-playwright-browser-tests)
...
| T4544 | `tests/ui/screen-brightness.test.ts` | 80% means 'leave the screen exactly as it is', so the scale is pct/80 — 100% a little brighter, 20% a long way darker — it is monotonic across the range, and an unreadable or out-of-range stored value falls back to the default rather than blacking out the screen. |
| T4545 | `tests/ui/screen-brightness.test.ts` | The dimmer is mounted at the app root so it covers the whole window, tints with backdrop-filter on a fixed pointer-events-none sheet rather than filtering an ancestor (which would re-anchor every position:fixed modal), renders nothing at the default, guards every localStorage access, and sits beside the zoom bar without moving it. |
