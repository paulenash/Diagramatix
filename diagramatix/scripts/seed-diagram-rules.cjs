...
R53: Use Event-based Gateways (gatewayType="event-based") when the prompt enumerates alternative EVENTS that can occur — e.g. "either Order Cancelled or Payment Received or Timeout is reached, whichever happens first". Both the splitting gateway (gatewayRole="decision") and the matching merge gateway (gatewayRole="merge") carry gatewayType="event-based" so both render with the event marker. The enumerated events sit directly between the two gateways (no intervening tasks); each event's outbound flow goes straight to the merge gateway. Do NOT use event-based gateways for plain if/else conditions — those remain Exclusive (R11).
...
R54: Event-based gateway wiring — Event-based gateways use the SAME decision/merge attachment as R35/R36, only the event marker differs. An Event-based Decision Gateway (gatewayType="event-based", gatewayRole="decision") attaches its incoming connector to the LEFT diamond vertex and its outgoing connectors, in AI order, to the TOP, RIGHT, then BOTTOM vertices (top and bottom only when there are exactly two branches). Each outgoing flow runs directly to its corresponding event with no intervening element. The matching Event-based Merge Gateway attaches its outgoing connector to the RIGHT vertex and its incoming connectors, in AI order, to the TOP, LEFT, then BOTTOM vertices, each from the corresponding event. Both the decision and merge gateways carry the event marker; neither is labelled (the marker is self-explanatory — no "Test?" question).
