...
const CHAPTERS: Chapter[] = [
  {
    ...
    sections: [
      ...
      { heading: "Codebase size & composition (lines of code)", body: [
        "A snapshot of the hand-written codebase, counted from **tracked files** (`git ls-files` + `wc -l`) as at `SCHEMA_VERSION` **1.35**. The **generated Prisma client** (`app/generated/`) is git-ignored and so excluded, as are `node_modules` and non-code assets (JSON fixtures, `.bpmn`/`.vsdx`/`.pdf` samples, images).",
        "",
        "**By language** (all tracked source — application + tests + scripts + contracts):",
        "",
        "| Type | Files | Lines |",
        "|---|--:|--:|",
        "| TypeScript (`.ts`) | 599 | 89,008 |",
        "| React (`.tsx`) | 198 | 66,152 |",
        "| Node scripts (`.cjs` / `.mjs`) | 45 | 4,755 |",
        "| XML Schema (`.xsd`) | 6 | 3,068 |",
        "| SQL (`.sql`) | 7 | 1,725 |",
        "| Prisma schema (`.prisma`) | 1 | 1,428 |",
        "| CSS (`.css`) | 1 | 88 |",
        "| **Total** | **857** | **166,224** |",
        "",
        "*(Tailwind supplies almost all styling, so there is only a single small hand-written CSS file.)*",
        "",
        "**By area** — application source only (`.ts` + `.tsx` under `app/`; excludes tests, scripts and contracts):",
        "",
        ...
        "| `app/hooks` | 2 | 9,058 | The `useDiagram` reducer store (a single large file) + one more |",
        "| **Application total** | **599** | **135,060** | |",
        "",
        "Alongside the application code sit **~13,700 lines of Vitest tests** (137 files) and **~10,000 lines of scripts** (88 files — idempotent seeds, migrations and tooling). The two tables above are different cuts and do **not** cross-sum: the by-language table spans the whole tracked source, while the by-area table is application `.ts`/`.tsx` only.",
        "",
        "**Reading the shape:** `app/lib` is the largest area — the app's value is in its **engines** (layout, mining, simulation, PCF, GRC), not its screens. The **206 API route files** reflect the fine-grained one-file-per-endpoint App-Router convention. The **hand-built SVG canvas** (no third-party diagramming library) concentrates in `app/components` + the `useDiagram` reducer. To refresh these figures, re-run `git ls-files` + `wc -l` grouped by extension / directory (excluding `app/generated`).",
      ].join("\n") },
    ],
  },
  {
