...
**As at:** 2026-09-16  ·  **Document version:** 7.0  ·  **Suite:** 507 test files · 3,745 tests (all green)  ·  **Runner:** Vitest  ·  **CI:** enforced on every PR + push to `main`  ·  **Highest ref:** T4804  ·  **Plus:** a Playwright browser e2e suite — see [Layer 11](#layer-11--end-to-end-playwright-browser-tests)
...
| T4405 | `tests/diagram/assist-review-items.test.ts` | **Messages by number** — bare: every task, collapsed subprocess and black-box pool numbered (never a white-box pool or gateway); selected task → black-box pools; selected pool → tasks/subprocesses; neither → error; answers "3 to 7 labelled X" / "to 2 labelled X" / "from two, called Y" parse, one-sided pair does not. |
...
| T4408 | `tests/diagram/assist-badges-and-selection.test.ts` | **Where the green numbers sit** — `badgePlaceFor`: activities below, events above, pools/lanes in the header; both collectors (rename, message) stamp it. |
...
| T4616 | `tests/diagram/merge-gateway-sides.test.ts` | Driven through the REAL reducer, not just the pure rule: each band produces the right vertex, the far end faces the gateway instead of doubling back, a decision's incoming flows are untouched, a message flow is untouched, and an endpoint deliberately dragged elsewhere is respected. |
