...
| T1052 | (`tests/bpmn/ep-boundary-interrupt.test.ts`) R8.19 — a timer/interrupt on an EP-INTERNAL task whose flow leaves the EP is re-homed onto the EP (boundaryHostId === ep.id), its centre sits on the EP rim, its End event is fully right-of the EP, and no event→End waypoint lies inside the EP | A loop-terminating timeout ("10 working days elapsed") rendering inside the EP with its End event ("Lapse Application") stranded far left (x=300) | If R8.19 EP-boundary re-homing or the exit-target ranking regressed |
...
| T1057 | (`tests/bpmn/pool-gap.test.ts`) every vertical gap between stacked pools equals POOL_GAP (98 = 1.5 × Task height) — a final `restackPoolsR52()` after the lane hug keeps a shrinking white-box pool from ballooning the gap to the black-box pool below | The Company→Loan Ranger gap ballooning to 268px because the final lane hug shrank the pool after the last restack | If POOL_GAP or the post-hug restack regressed |
| T1058 | (`tests/bpmn/pool-gap.test.ts`) a message-flow label is centred horizontally on its (vertical) connector (labelOffsetX ≈ 0) | Message labels drifting off to the side of their connector instead of centred in the inter-pool gap | If the layout-time message-label centring regressed |
