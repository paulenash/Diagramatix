...
export function Canvas({
    ...
    if ((diagramType === "bpmn" || diagramType === "state-machine") && onSplitConnector && SPLITTABLE_DROPS.has(pendingDragSymbol)) {
      ...
      if (hit) {
        if (pendingDragSymbol === "gateway" || pendingDragSymbol === "task" || pendingDragSymbol === "subprocess"
            || pendingDragSymbol === "subprocess-expanded"
  ...
  if (((diagramType === "process-context" && pcHighlightEnabled) || diagramType === "archimate") && highlightSeeds.size >= 1 && !draggingConnector) {
    ...
    for (const c of data.connectors) {
      (adjacency.get(c.sourceId) ?? adjacency.set(c.sourceId, []).get(c.sourceId)!).push(c);
      if (c.targetId !== c.sourceId)
        (adjacency.get(c.targetId) ?? adjacency.set(c.targetId, []).get(c.targetId)!).push(c);
    }
    const visited = new Set<string>();
    let frontier: string[] = [];
    ...
    while (frontier.length && depth < maxDepth) {
      ...
      for (const id of frontier) {
        for (const c of adjacency.get(id) ?? []) {
          ...
          if (excluded.has(other) || visited.has(other)) continue;
          visited.add(other);
          next.push(other);
          assocHighlightElIds.add(other); // non-seed reached element → green ring
        }
      }
      frontier = next;
      depth++;
    }
    // Every connector whose BOTH ends are in the tree lights up — this also
    // catches loop / cross edges the frontier walk didn't traverse.
    for (const c of data.connectors) {
      if (visited.has(c.sourceId) && visited.has(c.targetId)) assocHighlightConnIds.add(c.id);
    }
    assocActive = assocHighlightConnIds.size > 0;
    if (assocActive) {
      for (const id of visited) assocActiveElIds.add(id);
    }
  }
    ...
    >
      ...
      >
        ...
        <g transform={transform} style={{ ...(displayMode === "hand-drawn" ? { fontStyle: "italic", fontFamily: "var(--font-caveat), 'Segoe Print', 'Comic Sans MS', cursive" } : undefined), ...(readOnly ? { pointerEvents: "none" } : undefined) }}>
          ...
          {collabCursors && (
            <>
              <CollabLiveEdits elements={data.elements} connectors={data.connectors} baselineElements={collabBaseline?.elements ?? data.elements} baselineConnectors={collabBaseline?.connectors ?? data.connectors} broadcast={collabBroadcast} />
