...
**As at:** 2026-09-16  ·  **Document version:** 7.0  ·  **Suite:** 507 test files · 3,745 tests (all green)  ·  **Runner:** Vitest  ·  **CI:** enforced on every PR + push to `main`  ·  **Highest ref:** T4657  ·  **Plus:** a Playwright browser e2e suite — see [Layer 11](#layer-11--end-to-end-playwright-browser-tests)
...
| T4514 | `tests/diagram/pool-drag-crossing.test.ts` | The moving group draws above everything it crosses, as a block, while its own internal order is still left to the depth sort so a pool does not cover its own contents. A lone element is not lifted. |
...
| T4639 | `tests/diagram/pool-left-edge-and-lift.test.ts` | Paul, 2026-09-21: "When a Pool is selected and the arrow keys are used to move it part of the content disappears. Lanes, sublanes and sequence connectors disappear, other elements remain visible." Nothing was hidden — it was COVERED. `moveElement` marks a container and its descendants as TRAVELLING so a dragged pool rides above the diagram (his 2026-09-18 rule) and the matching END clears the mark; arrow keys called the move and never the end, so a nudged pool stayed pinned to the top overlay for the session ("I have to click elsewhere and then again on the Pool header"). The overlay also lifted the pool and its tasks but not its lanes or its flows, so an opaque pool body drew over both. Both halves guarded: every arrow ends its move, and everything travelling rises together — once, held back from the normal pass. |
