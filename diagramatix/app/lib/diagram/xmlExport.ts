...
/**
 * Browser-side helper: fetch the latest XSD from /api/schema and trigger a
 * download next to whatever XML file the caller just exported. Returns the
 * resolved appVersion (parsed from the XSD's annotation) so callers can use
 * it elsewhere; falls back to the local SCHEMA_VERSION if the fetch fails.
 *
 * Best-effort: if /api/schema is unreachable the function silently does
 * nothing — the .xml is still useful on its own.
 */
export async function downloadMatchingXsd(localSchemaVersion: string): Promise<string> {
  try {
    const resp = await fetch("/api/schema");
    if (!resp.ok) return localSchemaVersion;
    const xsdText = await resp.text();
    const m = xsdText.match(/Generated by Diagramatix ([\d.]+)/);
    const appVersion = m?.[1] ?? localSchemaVersion;

    const xsdBlob = new Blob([xsdText], { type: "application/xml" });
    const xsdUrl = URL.createObjectURL(xsdBlob);
    const a = document.createElement("a");
    a.href = xsdUrl;
    a.download = `diagramatix-export-v${appVersion}.xsd`;
...
export function parseDiagramatixXml(xmlText: string): any {
  ...
  function parseDiagramData(dataEl: Element): any {
    ...
    const nameCode = dataEl.getAttribute("nameCode"); // v1.42 — Diagram Name Code
    ...
    if (nameCode != null && nameCode !== "") data.nameCode = nameCode;
