...
export async function POST(req: Request, { params }: Params) {
  ...
  if (!mapping?.caseId || !mapping?.timestamp || !mapping?.activity || !mapping?.state) {
    return NextResponse.json({ error: "Map the case id, activity, timestamp and state columns." }, { status: 400 });
  }
