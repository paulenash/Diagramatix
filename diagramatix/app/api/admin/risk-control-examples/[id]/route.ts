/**
 * Admin Risk & Control example entry API (SuperAdmin only).
 *   PUT    /api/admin/risk-control-examples/[id] — patch metadata / toggle published / replace package.
 *   DELETE /api/admin/risk-control-examples/[id]
 */
...
export async function PUT(req: Request, { params }: Params) {
  ...
  const existing = await prisma.riskControlExample.findUnique({ where: { id } });
  ...
  const example = await prisma.riskControlExample.update({ where: { id }, data });
...
export async function DELETE(_req: Request, { params }: Params) {
  ...
  const existing = await prisma.riskControlExample.findUnique({ where: { id } });
  if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });
  await prisma.riskControlExample.delete({ where: { id } });
