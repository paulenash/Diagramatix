...
--
-- This file is PURE SQL -- no psql meta-commands -- so it runs unchanged in
-- pgAdmin, the Azure query editor, DBeaver or any other client. (An earlier
-- version opened with `\set ON_ERROR_STOP on`, which only psql understands and
-- which anything else reports as: syntax error at or near "\".) It is not
-- needed: everything is one transaction, so the first error aborts the rest
-- and the closing ROLLBACK still runs.
--
