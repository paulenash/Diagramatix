...
**As at:** 2026-09-16  ·  **Document version:** 7.0  ·  **Suite:** 507 test files · 3,745 tests (all green)  ·  **Runner:** Vitest  ·  **CI:** enforced on every PR + push to `main`  ·  **Highest ref:** T4552  ·  **Plus:** a Playwright browser e2e suite — see [Layer 11](#layer-11--end-to-end-playwright-browser-tests)
...
| T4389 | `tests/diagram/assist-undo-confirm-selection.test.ts` | `pushHistory` asks the gate first; `beginHistoryGroup`/`endHistoryGroup` are exported; `runAbraCommand` applies only through `applyGrouped` — **one spoken command, one undo**. |
...
| T4399 | `tests/diagram/assist-cost-and-commands.test.ts` | **SuperAdmin tile "Abracadabra Commands"** — advertised in AdminClient, its page exists (reachable), is SuperAdmin-guarded, renders the same catalogue and links to what is editable. |
...
| T4403 | `tests/diagram/assist-review-items.test.ts` | The bar drags by its header (and sits bottom-centre until it is); Palette and PropertiesPanel take force-collapse props that remember and restore the user's state; the editor passes `abracadabraOn`. |
...
| T4489 | `tests/diagram/assist-gold-flash-wiring.test.ts` | Abracadabra is gated on the seeded feature key rather than on being an admin, the gate is enforced on the route and not only in the editor, and a SuperAdmin previewing a lower tier stops seeing it. |
...
| T4494 | `tests/diagram/gold-flash-trigger.test.ts` | An Abracadabra edit hands React a new elements array, so the effect that computes the flash actually runs — and a no-op move correctly returns the same state, which is why that effect must not be the only thing the feature depends on. |
...
| T4549 | `tests/bpmn/emie-label-default.test.ts` | All three creation paths get the rule, driven through the real reducer — a palette drop that snaps to a rim, a drag released on a rim, and the assist/Abracadabra addBoundary op — while an event that snapped to nothing keeps the ordinary centred label, and re-mounting onto a different edge re-places it. |
