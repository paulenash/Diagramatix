...
**As at:** 2026-09-16  ·  **Document version:** 7.0  ·  **Suite:** 507 test files · 3,745 tests (all green)  ·  **Runner:** Vitest  ·  **CI:** enforced on every PR + push to `main`  ·  **Highest ref:** T4899  ·  **Plus:** a Playwright browser e2e suite — see [Layer 11](#layer-11--end-to-end-playwright-browser-tests)
...
| T4784 | `tests/diagram/assist-add-word.test.ts` | One rule, one place: the parser and the hold read the same words. `isIncompleteCommand("and a message from review")` equals the clean form's (true) and agrees on every word-prefix of every misheard corpus sentence; `stitchFinals` joins "and a message from review" + "to customer labelled invoice" (2.5 s) into one `addMessage`. Every noun in SYMBOL_PHRASES and the shared container / participant / box / message lists (plus the boundary-event phrase) folds — so the fold is BUILT from those lists; the grammar reads the same participant, box and message words. Wiring: grammar and hold call `repairHeardWords`, never the single repairs; the hold imports `COMMAND_VERBS` and keeps no verb literal; the grammar spells none of the shared nouns; make/turn are NOT held. |
