...
export function NimbClient() {
  ...
  const undo = useCallback(() => {
    setHistory((h) => {
      ...
      setBoard(last.board); setTurn(last.turn); setHover(null); setSelKey(null);
      return h.slice(0, -1);
    });
  }, []);

  const restart = useCallback(() => { setBoard(0); setTurn(1); setHistory([]); setHover(null); setSelKey(null); }, []);
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4 inline-block">
            <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${n}, ${cellSize}px)` }}>
              {Array.from({ length: n * n }, (_, i) => {
                ...
                const chosen = !preview && (selected?.move.cells.includes(i) ?? false);
                return (
                  <div key={i} style={{ width: cellSize, height: cellSize }}
                    className={"flex items-center justify-center rounded border-2 text-2xl font-bold " +
                      (filled ? "bg-gray-800 border-gray-800 text-white"
