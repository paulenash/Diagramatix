...
Composite: "location" — a place/site (a box with a small map-PIN symbol in the top-right corner); "grouping" — an informal cluster (a DASHED-outline box). CLASSIFY BY THE DRAWN ICON, NOT THE NAME: a name like "Front Office", "Back Office", "Sydney HQ" or "Melbourne Office" is a "location" ONLY when it carries the map-pin symbol; if that shape is drawn as an Actor, Component or anything else, use THAT type — the icon wins over the name. A Location or Grouping commonly CONTAINS other elements (nest them with "parent"). A "location" may be nested ONLY inside another "location" or a "grouping" — never inside an actor or any other element.
Junctions: "and-junction" and "or-junction" — a SMALL CIRCLE that JOINS several relationships of the SAME type (a relationship split/merge node). A SOLID/FILLED small circle = "and-junction" (all connected relationships apply); an OPEN/HOLLOW small ring = "or-junction" (one or more apply). A junction has NO label (label ""). Wire the relationships THROUGH it: emit a connection from the source element to the junction AND a connection from the junction to each target, ALL of the SAME relationship "type" (e.g. two "triggering" edges into and out of the junction). Recognise a junction by the small circle sitting ON the relationship lines, NOT by any text.
...
- a hollow diamond at one end → "uml-aggregation"; a FILLED (solid) diamond → "uml-composition". Diamonds are often SMALL and FAINT — relax your threshold: if you see ANY diamond-ish marker at a line end, classify it as aggregation or composition (never miss one as a plain association); when it looks filled or you can't tell hollow-vs-filled, prefer "uml-composition". STRONG HINT: an end whose role reads "owner…" with a "{subsets owner}" constraint, opposite an end named "…Value"/"ownedElement" with "{subsets ownedElement}", is a COMPOSITION whose filled diamond sits at the owner end — classify it "uml-composition" even if the diamond is barely visible, and make the owner end the TARGET (the diamond end).
- TWO (or more) SEPARATE lines can join the SAME pair of classes (e.g. an upperValue link and a lowerValue link between MultiplicityElement and ValueSpecification). Emit EACH as its own connection with its own roles/constraints — never collapse them into one.
- SELF-RELATIONSHIP: a line that LEAVES a class and LOOPS BACK to the SAME class (a small rectangular/curved loop off one side, often with a role + multiplicity at each end) is a self-connector — set BOTH "sourceId" and "targetId" to that class's id, and set "sourceSide" = "targetSide" = the side the loop sits on ("top"/"bottom"/"left"/"right"). Keep its type (association/aggregation/composition), roles, multiplicities and constraints just like any other connection. ALSO report its geometry so it matches the drawing: "sourceOffsetAlong" and "targetOffsetAlong" = where the two ends attach along that side (0..1 fraction from the side's start — for a top/bottom side measured left→right, for a left/right side top→bottom), and "selfLoopDepthFrac" = how far the loop extends OUT from the boundary, as a fraction of the image WIDTH.
...
NAVIGABILITY (associations only) — an association end may carry a small OPEN arrowhead: a thin "V" / ">" pointing INTO the class (NOT a filled/hollow triangle, NOT a diamond). It marks that end as navigable. For EACH association, set "sourceArrow": true and/or "targetArrow": true for the end(s) that actually have an open arrowhead in the drawing (omit both when the line has plain ends). Only associations use these — generalisation / realisation / aggregation / composition / dependency have their own end markers, so never set sourceArrow/targetArrow on them.
...
- ROLE NAME → "sourceRole"/"targetRole". It is a NOUN at an END of the line beside a multiplicity, lower case; singular when the multiplicity is 1 or 0..1, plural when 0..*, 1..*, n..m. If a visibility marker (+, -, # or ~) prefixes the role (e.g. "+ownerUpper"), KEEP it in the role string — it is transcribed into the end's visibility.
