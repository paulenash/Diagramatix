...
export function SimDataPanel({ data, onApplyData, onFillMissing }: {
  ...
  const sources = data.elements.filter((e) => SOURCE_TYPES.has(e.type));
  ...
  let missing = 0;
  for (const s of sources) if (!getSimParams(s).arrival) missing++;
  for (const t of tasks) if (!getSimParams(t).cycleTime) missing++;
  for (const g of gateways) {
    ...
    if (edges.length > 1 && !hasCond && Math.abs(sum - 100) > 0.5) missing++;
  }
  ...
  return (
    <div className="flex flex-col gap-3 text-[10px]">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <span className={missing > 0 ? "text-red-300" : "text-green-400/60"}>
          {missing > 0 ? `${missing} item(s) need values` : "all required values set ✓"}
        </span>
      ...
      {/* Sources */}
      {sources.length > 0 && (
        <Section title="Arrivals (start / intermediate events)" cols={["", "element", "inter-arrival", "max arrivals"]}>
          {sources.map((s) => {
            const sim = getSimParams(s);
            return (
              <Row key={s.id}>
                <Cell>{flag(!sim.arrival)}</Cell>
                <Cell grow>{s.label || s.id}</Cell>
                <Cell><MatrixDist value={sim.arrival} onChange={(arrival) => patchEl(s.id, { arrival })} /></Cell>
                <Cell><input type="number" value={sim.maxArrivals ?? ""} placeholder="∞" onChange={(e) => patchEl(s.id, { maxArrivals: num(e.target.value) })} className={inp} /></Cell>
              </Row>
            );
          })}
        </Section>
      ...
      {/* Tasks */}
      {tasks.length > 0 && (
        <Section title="Tasks" cols={["", "element", "cycle time", "wait", "team", "units"]}>
          {tasks.map((t) => {
            const sim = getSimParams(t);
            return (
              <Row key={t.id}>
                <Cell>{flag(!sim.cycleTime)}</Cell>
                <Cell grow>{t.label || t.id}</Cell>
                <Cell><MatrixDist value={sim.cycleTime} onChange={(cycleTime) => patchEl(t.id, { cycleTime })} /></Cell>
                <Cell><MatrixDist value={sim.waitTime} onChange={(waitTime) => patchEl(t.id, { waitTime })} /></Cell>
                <Cell><input type="text" value={sim.teamId ?? ""} placeholder="team" onChange={(e) => patchEl(t.id, { teamId: e.target.value || undefined })} className={`${inp} w-20`} /></Cell>
                <Cell><input type="number" min={1} value={sim.resourceUnits ?? 1} onChange={(e) => patchEl(t.id, { resourceUnits: Math.max(1, parseInt(e.target.value, 10) || 1) })} className={`${inp} w-12`} /></Cell>
              </Row>
            );
          })}
        </Section>
      ...
      {/* Event subprocesses */}
      {eventEPs.length > 0 && (
        <Section title="Event subprocesses" cols={["", "element", "trigger (delay after scope start)"]}>
          {eventEPs.map((e) => {
            const sim = getSimParams(e);
            return (
              <Row key={e.id}>
                <Cell>{flag(!sim.eventTrigger)}</Cell>
                <Cell grow>{e.label || e.id}</Cell>
                <Cell><MatrixDist value={sim.eventTrigger} onChange={(eventTrigger) => patchEl(e.id, { eventTrigger })} /></Cell>
              </Row>
            );
          })}
        </Section>
      ...
      {/* Decision branches */}
      {gateways.length > 0 && (
        <Section title="Decision branches (probability % per outgoing flow)" cols={["", "gateway → target", "%", "default", "condition"]}>
          {gateways.map((g) => {
            const edges = branchSum(g);
            const hasCond = edges.some((e) => e.branchCondition);
            const sum = edges.reduce((a, e) => a + (e.branchProbability ?? 0), 0);
            ...
            return (
              <div key={g.id} className="border-t border-green-500/10">
                ...
                {edges.map((c) => (
                  <Row key={c.id}>
                    <Cell>{flag(bad)}</Cell>
                    <Cell grow>→ {labelOf(c.targetId)}</Cell>
                    <Cell><input type="number" value={c.branchProbability ?? ""} placeholder="—" onChange={(e) => patchConn(c.id, { branchProbability: num(e.target.value) })} className={`${inp} w-14`} /></Cell>
                    <Cell><input type="checkbox" checked={!!c.isDefaultFlow} onChange={(e) => patchConn(c.id, { isDefaultFlow: e.target.checked || undefined })} className="accent-green-500" /></Cell>
                    <Cell><input type="text" value={c.branchCondition ?? ""} placeholder="expression (optional)" onChange={(e) => patchConn(c.id, { branchCondition: e.target.value || undefined })} className={`${inp} w-36`} /></Cell>
                  </Row>
                ))}
              </div>
            );
