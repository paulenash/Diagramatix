...
**As at:** 2026-09-16  ·  **Document version:** 7.0  ·  **Suite:** 507 test files · 3,745 tests (all green)  ·  **Runner:** Vitest  ·  **CI:** enforced on every PR + push to `main`  ·  **Highest ref:** T4862  ·  **Plus:** a Playwright browser e2e suite — see [Layer 11](#layer-11--end-to-end-playwright-browser-tests)
...
| T4850 | `tests/diagram/template-attach-plan.test.ts` | Refused with the real reason, never placed half-joined: inside an expanded subprocess, after an End ("Done"), after a lane; Perform Regular Task ("it has no step a sequence flow can enter"); Simple Document Request Loop ("it brings a pool or lane of its own"); Expanded Subprocess Template after the merge gateway ("“Expanded 1” would stick out above “Warehouse”" — lanes grow down); a loose task beside a pool, where a flow CAN leave it, is refused by the dry run because the fragment would straddle the pool ("“Submit” would stick out left of “Us”"). |
