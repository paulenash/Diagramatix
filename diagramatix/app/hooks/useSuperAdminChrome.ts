...
/**
 * SuperAdmin "presentation mode". Ctrl+Shift+B toggles hiding of SuperAdmin-only
...
export function useSuperAdminChrome(isSuperAdmin: boolean): boolean {
  ...
  // Ctrl+Shift+B toggles it — SuperAdmins only, so it does nothing for anyone else.
  useEffect(() => {
    if (!isSuperAdmin) return;
    const onKey = (e: KeyboardEvent) => {
      // `e.code === "KeyB"` is keyboard-layout-independent (the physical B key).
      if (e.ctrlKey && e.shiftKey && !e.altKey && !e.metaKey && e.code === "KeyB") {
        e.preventDefault();
