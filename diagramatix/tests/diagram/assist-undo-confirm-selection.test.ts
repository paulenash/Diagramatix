...
describe("R1 — destructive commands ask first", () => {
  ...
  it("T4392 — clear, container deletes, multi-selection deletes and delete+compact ask; a single element does not", () => {
    ...
    expect(needsConfirmation([{ op: "delete", ref: "Warehouse" }], els)).toBe("delete the pool “Warehouse” and the 1 element inside it");
    expect(needsConfirmation([{ op: "delete", ref: "Sales" }], els)).toBe("delete the lane “Sales” and the 2 elements inside it");
