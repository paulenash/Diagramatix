...
export default async function OrgSopTemplatesPage({ searchParams }: { searchParams: Promise<{ from?: string }> }) {
  ...
  return (
    <div className="min-h-screen dgx-dashboard-bg">
      <header className="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-3">
        ...
        <h1 className="text-lg font-semibold text-gray-900">SOP Templates <span className="text-sm font-normal text-gray-400">— {org?.name ?? "Organisation"}</span></h1>
      </header>
      <p className="max-w-3xl mx-auto px-6 pt-4 text-xs text-gray-500">The organisation&rsquo;s SOP templates. The one marked Default is used for every SOP unless a project overrides it.</p>
      <SopTemplatesManager scope="org" scopeId={orgId} />
    </div>
  );
}
