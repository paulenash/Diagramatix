...
/** Whether Pain Point icons render on the canvas at all (diagram-level
 *  "Display Pain Points" master toggle). Defaults true. Provided by Canvas.tsx. */
export const ShowPainPointsCtx = createContext<boolean>(true);
...
function MarkerShape({ el, show, showDesc, stroke, numberColor, descColor }: {
  ...
  const numberFont = Math.round(Math.min(rx, ry) * (digits >= 2 ? 0.62 : 0.8) * fsc * 10) / 10;
