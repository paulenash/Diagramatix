...
**As at:** 2026-08-24  ·  **Document version:** 6.4  ·  **Suite:** 335 test files · 2,066 tests (all green)  ·  **Runner:** Vitest  ·  **CI:** enforced on every PR + push to `main`  ·  **Highest ref:** T2880  ·  **Plus:** a Playwright browser e2e suite — see [Layer 11](#layer-11--end-to-end-playwright-browser-tests)
...
| T2877 | `tests/games/nimb.test.ts` | n × n Nimb move rules: 1–4 ✕ on consecutive empty squares in one row or column; a single square is offered once, not once per orientation. |
| T2878 | `tests/games/nimb.test.ts` | The misère outcome (last ✕ loses), solved for n=1–4 — only 3×3 is a first-player win. |
