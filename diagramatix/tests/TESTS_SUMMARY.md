...
**As at:** 2026-09-16  ·  **Document version:** 7.0  ·  **Suite:** 507 test files · 3,745 tests (all green)  ·  **Runner:** Vitest  ·  **CI:** enforced on every PR + push to `main`  ·  **Highest ref:** T4684  ·  **Plus:** a Playwright browser e2e suite — see [Layer 11](#layer-11--end-to-end-playwright-browser-tests)
...
| T4629 | `tests/diagram/pool-boundary-rules.test.ts` | Half an event of clear space in front of the first element. Paul, 2026-09-21: adding a Lane or Sublane to a pool with elements in it must keep at least 18px between the lane header's right edge and the left-most element (normally the Start Event). A fresh pool leaves 40px and the first lane's own 36px header ate nearly all of it. The pool and every lane grow LEFTWARDS by the shortfall — measured from the DEEPEST header against ALL of the pool's content, because an element added before the lanes were stays parented to the POOL while sitting inside a lane, so a parentage walk finds nothing in the very lane that crowded it. Nothing inside moves. |
