...
| T0700 | (Diagram Portal denorm) `deriveDiagramDenorm` mirrors `DiagramData.pcf` + `.procedureDoc` onto the flat Diagram columns the Portal queries — classification (pcfId/hierarchyId/name), procedure URL with a name→URL fallback, clean nulls for absent/blank values | The Portal's category facet/search or procedure link going stale/blank because a save didn't sync the denormalised columns | If the save-path denormalisation regressed |
