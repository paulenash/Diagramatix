...
|---|---|
| **Created** | 2026-09-07 |
| **Version** | **3 &mdash; 14 September 2026.** Programme closed: every phase and every smaller item shipped. The examples programme is the open remainder. |
...
| **Progress log** | **2026-09-14 &mdash; V3 CLOSE-OUT.** All eight phases and six smaller items shipped (2.6.2479 &rarr; 2.8.2503). Since the last entry: **queue discipline and preemption got their doors** (2.10.2530 &mdash; two columns, two team-library controls, three run routes; the capability had been shipped, tested, correct and unreachable); **skills completed as five steps** on 11&ndash;12 September (`faa87105` master Skills list; `ce5426ec` seed + admin grids; `d286030f` people hold skills, tasks require them; `9d2ef937` skills read as ArchiMate Capabilities; `fc70e02e` Hire &amp; Onboard redrawn to the Capability pattern; `8bc73ae8` / `2972825d` stereotypes user-maintainable, holder counts); **the Hire &amp; Onboard capstone shipped** (2.8.2508, and the four bugs it found; asserted end to end in 2.8.2510) &mdash; the six gaps its plan found are closed; **seven silent gaps in the simulator** closed (2.8.2506); Simulation Data fits, lognormal selectable and the tree shows what a diagram carries (`df09f6d8`); the AI assessment arrives whole (`258d20d7`); a business case cannot be made from a scenario that never ran (`ec363521`). **Still open:** the six single-capability examples in the table below and the Aardwolf business-case inputs &mdash; the code is complete, the teaching is not. **2026-09-07** — **Phase 0 COMPLETE.** 0.1/0.2/0.3 `e290a9b4`; TESTS_SUMMARY header repaired `11eed0f1`; **0.4 closed by Paul’s decision — generator retired, committed examples are the source of truth** (this also supersedes the original "extend the generator" plan for new examples). 0.1 generator merges by slug (guarded by T3369, proven to fail on a simulated overwrite); 0.2 `studyRuns.ts` + `latestRunPerScenario` (T3370/T3371); 0.3 `facts/` established, `assessFacts.ts` moved, both importers repointed (no compat shim — there were only two). **New finding: 0.4** — the generator is stale relative to its own committed output; see below. 382 simulation tests green, typecheck unchanged. **2026-09-08 — ALL EIGHT PHASES SHIPPED.** Phase 8 (the tornado) closes the programme's code: `sensitivity.ts` + `POST .../sensitivity` + `TornadoPanel`, T3545–T3560, feature-only (product stays 2.8, XSD stays 46). **What remains is the examples programme below — 7 worked examples and the `aardwolf-loan-comparison` business-case inputs, none of them started.** The code teaches nothing on its own: every capability from Phase 2 onwards is currently reachable only by someone who already knows it is there. **Examples re-levelled (Paul):** `loan-origination` -> intro; `car-repair-rework-loop`, `aardwolf-loan-comparison`, `sales-marketing-drill-through` -> core, leaving `advanced` to mean the capstone. Guarded by T3561/T3562; the levels reach a gallery only when `seed-simulation-examples.ts` is re-run. **A capstone example is planned** - see `Example-Hire-and-Onboard-Plan.md`, which found SIX gaps while checking whether it could be authored at all, four of them blocking: skills, business-case inputs and non-root (ArchiMate) diagrams do not survive a package, and there is NO scenario lever for cross-training - `TeamOverride` is `{capacity, discipline}` only, so "what if we trained someone?" is currently unaskable. Also found: `SimNode.batch` is honoured by the engine but reachable from no diagram (Phase 5 item 03 landed engine-only). |
...
| [x] | **Hire & Onboard** | `hire-and-onboard` | **The capstone.** Calendars + holidays + named people + skills read from ArchiMate + priority + sweep + tornado + business case, in one process. Plan: [`Example-Hire-and-Onboard-Plan.md`](./Example-Hire-and-Onboard-Plan.md) | 2-8 | advanced |
