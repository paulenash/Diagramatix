...
| T4548 | `tests/bpmn/emie-label-default.test.ts` | An EMIE's label starts half a default label-width left of centre so the outbound connector is clear of it; on a vertical (left/right) boundary it sits just above the outgoing connector instead, on the side that connector travels, with the clearance scaled to the event's own height. |
