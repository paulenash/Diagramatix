...
| T2854 | (`tests/editor/parentage.test.ts`) The B47 "parentage" scan rule reports an element whose declared owner is not the container it actually sits in (pool-owned while inside a lane; left on the outer EP when an inner one encloses it), passes a correctly-owned diagram, and ignores boundary events (owned by their host) | Stale ownership that is invisible on the canvas but exports to the wrong swimlane and bills simulation work to the wrong team | If checkParentage's expected-owner resolution or its exemptions regressed |
...
| T2852 | (`tests/simulation/arrival-sources-and-team.test.ts`) deleting a lane re-homes its task into the surviving lane AND re-derives an AUTO-FILLED `sim.teamId` from the new lane; a hand-set teamId is preserved (assigning a task across lanes is a legitimate model) | A task still billed to a deleted lane's team, so the simulator keeps reporting a team that no longer exists | If retagAutoTeam or its reconcileLaneMembership wiring regressed |
