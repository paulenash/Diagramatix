...
export interface StencilProfile {
  ...
  name: "bpmn-m" | "diagramatix-v1.5" | "diagramatix-v1.6";
...
/**
 * Diagramatix v1.6 profile — bytewise-identical shape definitions to v1.5,
 * but every master's `BaseID` / `UniqueID` is a fresh UUID-v4 generated by
 * `scripts/build-v16-from-v15.cjs`.
 *
 * Why this exists: v1.5's masters reuse the SAME BaseID GUIDs as v1.4
 * (the v1.5 build copied from v1.4 without refreshing GUIDs). Visio
 * identifies masters by BaseID, not by stencil filename — so loading both
 * v1.4 and v1.5 into "My Shapes", then opening a v1.5-exported diagram,
 * causes Visio to resolve dragged shapes to v1.4's masters (no colour)
 * because the BaseIDs collide. v1.6 has fully unique BaseIDs that don't
 * collide with v1.4 OR v1.5.
 *
 * Master ID numbers (the numeric ones in `masterIds` below) are
 * identical to v1.5 — only the GUIDs changed in the generated artifacts.
 */
export const diagramatixV16Profile: StencilProfile = {
  name: "diagramatix-v1.6",
  templateFile: "bpmn-template-v16.vsdx",
  stencilFile: "BPMN Diagramatix Shapes v1.6.vssx",
  masterIds: { ...diagramatixV15Profile.masterIds },
  mastersToAdd: diagramatixV15Profile.mastersToAdd.map(e => ({ ...e })),
  disableBodyColourBake: true,
};
 ...
 *  parameter. v1.5 / v1.4 aliases route to the v1.5 profile (legacy,
 *  kept for backwards compatibility with old export URLs). v1.6 / v16
 *  route to the new fresh-GUID v1.6 profile. */
export function profileByName(name: string | null | undefined): StencilProfile {
  if (
    name === "diagramatix-v1.6" || name === "v1.6" || name === "v16"
  ) {
    return diagramatixV16Profile;
  }
  if (
