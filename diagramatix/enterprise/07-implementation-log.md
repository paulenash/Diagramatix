...
| **A3** Enterprise identity & privacy | GDPR erasure, AI retention, require-SSO + reg. hardening, pre-egress AI redaction (shipped); SAML/OIDC+SCIM, email-verify, dedicated-instance (deferred) | 🟡 **partly shipped** |
| B | Deployment tiers (dedicated instance) | ⬜ planned |
| C | Questionnaire pack + SOC 2 Type II | ⬜ planned |

**Findings closed so far:** ENT-03, ENT-05, ENT-07, ENT-08, ENT-10, ENT-11, ENT-12, ENT-13, ENT-14, ENT-16 (full); ENT-02 (audit + reason + time-box + HttpOnly — remaining: per-mutation logging), ENT-04 (require-SSO + domain-restricted reg — remaining: SAML/OIDC + MFA + email verify), ENT-06 (redaction on the 3 structured narrators — remaining: free-text/transcript NER), ENT-15 (partial).
**Still open (high):** ENT-01 (SuperAdmin emails → stored role + MFA, A2c deferred). ENT-06 mostly closed (A3c) — remaining: free-text raw-prompt + transcript redaction (NER-hard, best-effort). Deferred mega-items: full SSO/SAML/OIDC+SCIM, email verification, dedicated single-tenant instance tier.

**Resume here:** read this file top-to-bottom (status snapshot → phase sections → "continue here"/deferred blocks). Remaining work, roughly ordered: (1) A2c SuperAdmin stored role + MFA, (2) full SSO/SAML/OIDC per-org + SCIM, (3) email verification, (4) dedicated single-tenant instance tier + SOC 2 evidence pack. ENT-06 redaction shipped for the structured narrators (A3c); optional follow-up = NER over free-text prompts/transcript. Each is independently shippable; conventions at the end of this file.
...
- **A3c — Pre-egress AI redaction** (`__COMMIT__`, ENT-06): `Org.aiRedaction` (opt-in, default off). New pure engine **`app/lib/ai/redaction.ts`** — `makeRedactor(entities)` → `{ redact, restore }`: boundary-aware, longest-first, reversible pseudonymisation of a **known vocabulary** of literals (not NER) into `Entity_N` tokens; the map never leaves the tenant. Read via **`orgRedactionEnabled(session)`** (binds like other policy; a full-view SuperAdmin sees raw, which is how you demo the difference). Applied at the three **structured** narrators where the sensitive strings are known server-side/client-side, so the swap is exact: **mining Explain** (resource + run/reference names; activity/state labels kept as process vocabulary), **sim Assess** (scenario + team names in the facts JSON), **staff-narrative** (client sends `entityHints` = pool/lane/sublane/data-object labels). OrgAdmin toggle in the **Data & AI Governance** card. Tests T0938-T0946. **Scope:** free-text raw AI-Generate prompts + the audio transcript are NER-hard and remain best-effort (the transcript already role-anonymises in its own system prompt); see [09](09-ai-off-and-local-llm.md).
