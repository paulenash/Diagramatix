...
const BODY = [
  ...
  "",
  "**Load it.** Open the **File** menu → **DiagramatixMINER Examples**. The gallery lists ready-made studies; the Accounts Payable card shows *200 cases · 10 variants · 2 references*. Click **▶ Load & open**. Diagramatix copies the example into a brand-new project of your own — a ready mined run plus its two reference state machines — and drops you straight into the **⛏ DiagramatixMINER** console on that project (after a brief intro). Nothing you already have is touched.",
  "",
  "**1. Open the run.** In the **Mining runs** panel the run *Accounts Payable — January 2026* is already there. Click it. A summary expands: ~200 **cases**, ~990 **events**, 8 **activities**, 7 **states**, 10 **variants**, spanning about a month. These are the distinct real journeys invoices took, compressed with their frequencies.",
  "",
