...
export function DiagramEditor({
  ...
  async function handleSaveToSharePoint(sel: { driveId: string; itemId: string | null; name: string }) {
    setSpBusy(true);
    try {
      const { buildSingleDiagramXml } = await import("@/app/lib/diagram/xmlExport");
      const { SCHEMA_VERSION } = await import("@/app/lib/diagram/types");

      // Resolve the runtime app/schema version for the XSD filename + XML header.
      let appVersion = SCHEMA_VERSION;
      let xsdText = "";
      try {
        const resp = await fetch("/api/schema");
        if (resp.ok) {
          xsdText = await resp.text();
          const m = xsdText.match(/Generated by Diagramatix ([\d.]+)/);
          if (m) appVersion = m[1];
        }
      } catch { /* best-effort */ }
      ...
      await uploadOne(`${safe}.json`, "application/json", JSON.stringify(jsonPayload, null, 2));
      let count = 2;
      if (xsdText) { await uploadOne(`diagramatix-export-v${appVersion}.xsd`, "application/xml", xsdText); count++; }
      if (diagramType === "bpmn") {
        try {
          const vr = await fetch(`/api/export/visio-v3?diagramId=${diagramId}&profile=v1.6`);
          if (vr.ok) { await uploadOne(`${safe}.vsdx`, "application/vnd.ms-visio.drawing", await vr.blob()); count++; }
        } catch { /* skip vsdx on failure */ }
      }
      setSpMessage({ title: "Saved to SharePoint", body: `Saved ${count} file${count === 1 ? "" : "s"} to "${sel.name}".`, tone: "info" });
    } catch (err) {
  ...
  return (
    <div className={`flex flex-col h-screen ${isImpersonating ? "bg-orange-50" : "bg-white"}`}>
              ...
              >
                ...
                {fileSubmenu === "export" && (
                  <div className="absolute right-full top-0 mr-1 w-44 bg-white border border-gray-200 rounded shadow-lg z-50">
                    ...
                    {/* Save to SharePoint — XML + XSD + JSON (and .vsdx for BPMN) */}
                    <button
                      onClick={() => { setFileMenuOpen(false); setFileSubmenu(null); setSpPicker("save"); }}
                      className="w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-50"
                      title="Save the diagram's data files (XML, XSD, JSON, and Visio for BPMN) into a SharePoint or OneDrive folder"
                    >
                      SharePoint…
                    </button>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* SharePoint Save / Open picker */}
        {spPicker && (
          <SharePointPicker
            mode={spPicker === "save" ? "folder" : "file"}
            title={spPicker === "save" ? "Save diagram to SharePoint" : "Open diagram from SharePoint"}
            confirmLabel={spPicker === "save" ? "Save here" : "Open"}
            fileExtensions={spPicker === "open" ? [".json", ".xml", ".vsdx", ".bpmn"] : undefined}
            onCancel={() => setSpPicker(null)}
            onPick={(sel) => {
              const mode = spPicker;
              setSpPicker(null);
