...
export function layoutBpmnDiagram(
  ...
  // ── Two shapes a plan can carry that no layout can rescue ──
  //
  // Paul, 2026-09-03, on V22.07 (generated by kimi-k3 while Anthropic was
  // overloaded). Both are cases where the model described something real and
  // expressed it in a way BPMN does not allow, and the drawing is then
  // faithful to nonsense.
  {
    const poolOf = (id: string): string | undefined => {
      const el = aiElements.find(e => e.id === id);
      if (!el) return undefined;
      return el.type === "pool" ? el.id : el.pool;
    };
    const seq = aiConnections.filter(c => c.type !== "message");

    // (a) An intermediate event with an OUTGOING flow and no incoming one can
    //     never be reached — the flow starts at something nothing leads to.
    //     "Escalation response overdue (3 business days)" was drawn loose at
    //     the far left with a line out of it; Paul read it immediately: "should
    //     be an EMIE on Task 'Escalate to delegated approver' probably?". That
    //     is nearly always what the model meant.
    for (const e of aiElements) {
      if (e.type !== "intermediate-event" || e.boundaryHost) continue;
      const hasIn = seq.some(c => c.targetId === e.id);
      const out = seq.filter(c => c.sourceId === e.id);
      if (hasIn || out.length === 0) continue;
      diagnose({
        kind: "unreachable-event",
        elementId: e.id,
        label: e.label ?? e.id,
        detail: "nothing leads to this event, yet flow leaves it — it was probably meant to be edge-mounted on the preceding activity",
      });
    }

    // (b) A message flow must CROSS a pool boundary. Both ends in one pool
    //     usually means the external participant was never declared: V22.07
    //     sent an escalation package from a task to a start event in the same
    //     pool, which is why Paul asked "A missing Black-box Pool???".
    for (const c of aiConnections) {
      if (c.type !== "message") continue;
      const a = poolOf(c.sourceId), b = poolOf(c.targetId);
      if (!a || !b || a !== b) continue;
      const src = aiElements.find(e => e.id === c.sourceId);
      diagnose({
        kind: "message-within-pool",
        elementId: c.sourceId,
