...
## 2b. What AUTHORING found — four more bugs, three in shipped features

The plan said to build the gaps first because "authoring the example and *then* finding the package
cannot carry it is the expensive failure". That was right, but it understated the case: **building
the example is what proved the features worked at all.** Four defects, none of which any test caught:

| # | Bug | Why nothing noticed |
|---|---|---|
| **H** | **`skillsFromArchimate` never worked on a real diagram.** It matched `el.type` against `"business-actor"`, but the editor stores every ArchiMate element as `type: "archimate-shape"` with the concept in `properties.shapeKey` (`"business-business-actor-box"`). A fill from a real operating model returned **zero actors and zero skills** | Its tests build synthetic elements with a bare concept type — a shape no diagram in the product has |
| **I** | **`assemblePortfolio` threw the people away.** It never passed `teamUnits` down to `assembleFromDiagram`, and its team rebuild dropped `units`. So EVERY study run — the portfolio wrapper is what the run route uses — assembled counted pools, and every `requiredSkills` was silently granted to anyone | The skills tests call `assembleFromDiagram` directly, which honours units. The wrapper the app actually uses did not |
| **J** | **The tornado could not see authored waits.** `enumerateParameters` covered team capacity, task cycle time and source arrival — not `delay` nodes. In this model the two timers are the **two biggest levers by a factor of ten**, and the chart could not show them | No example had a timer worth testing |
| **K** | **The example suite ran every example without its people.** `exampleSeeds.test.ts` assembled with capacities only, so no example's skills were ever exercised — and it could not have caught H or I | It asserted throughput, which a counted pool produces perfectly well |

H and I together mean **the ArchiMate skills feature was inert end to end** from the moment it
shipped: the reader found nothing, and had it found something the runner would have discarded it.
Both are fixed, and K is fixed so the suite would now notice.

### And one trap that is not a bug

**An arrival mean is sampled in ELAPSED time, then pushed to the next open window.** So a source with
a working calendar is neither "per elapsed hour" nor "per open hour": below the open-window length it
behaves roughly like open-hours spacing, above it saturates at about one arrival per window. Both
obvious guesses were wrong by more than a factor of two — 2.35 (open hours) gave ~1060 hires/yr and
pushed every team into the eighties; 11.09 (8760/790) gave ~386/yr. The value has to be **measured**:
`scratchpad/calibrate-arrival.ts` sweeps it and prints volume, utilisation and the vetting queue.

---

## 3. The diagrams

> **As built.** Both diagrams are generated by `scripts/gen-hire-onboard-example.ts`, which feeds
> the process through `layoutBpmnDiagram` and the operating model through `layoutGenericDiagram` —
> the same functions the AI generator and the image importer use — so geometry, waypoints,
> attachment sides and routing fields are produced by the product rather than hand-written. Sim
> parameters are attached afterwards by element id, with a guard that fails if an id stops matching.
> Note `layoutBpmnDiagram` **rebuilds pools and lanes and drops any properties passed with them**, so
> the lanes' `teamId` must be attached after layout too; without it every task runs with unlimited
> capacity and the model looks perfectly reasonable.
...
> ### ⚠ MEASURED 2026-09-08 — three of these predictions were wrong
>
> The model is built and runs; the figures below are what it **actually produces**, at
> `seed 20270705`, 10 replications. Three corrections, all of them about *which metric carries the
> story*:
>
> **1. Total queue wait is dominated by CLOSED HOURS, not contention.** ~13.4 working days of it, at
> teams running under 60%. With a 7.5-hour day and ~13 seizing steps, most "waiting for a resource"
> is waiting for Monday. Absolute queue wait therefore proves nothing, and the assertions below use
> the wait **at the constrained node**, and the **difference** between scenarios, which cancels the
> calendar component.
>
> **2. The vetting queue is real but small against the authored waits.** 9.6 h → 4.2 h when a third
> person is trained (a 56% cut), against a flow time of ~1156 h of which ~840 h is the two timers.
> So the saving is ~0.5% of time-to-hire. That is the honest answer, and it is *more* interesting
> than the one predicted, because —
>
> **3. The tornado says the load-bearing assumptions are the two WAITING PERIODS**, at 19% and 13%,
> ten times anything else. `teamCapacity: HR Operations` comes back **`no-difference` with a swing of
> exactly 0.00**, which is the headline the example was built for, and `Payroll` and
> `IT Provisioning` come back **`not-testable`**. 13 of 20 tested parameters made no measurable
> difference.
>
> The example therefore teaches three things rather than one: headcount is not the lever, the
> accredited pair genuinely is a constraint, and the biggest lever of all is two waiting periods
> nobody had questioned. It only says the third because gap **J** was fixed.
>
> **Measured baseline:** 785 hires/yr · busiest team **HR Operations 60%** (Onboarding 54%,
> Talent Acquisition 51%, Hiring Manager 19%, Payroll 18%, IT 15%) · `bottlenecks[0]` =
> **HR Operations** · flow p50 987.6 h, p95 1158 h · cost/case 1437.76.
...
|---|---|---|
| 1 | Baseline: the busiest **team** is < 65% (measured **60%**), yet the wait at *Compliance & vetting review* is > 8 h | The whole premise — a healthy-looking team with a real queue at one step |
...
| 6 | Scenario 4 cuts the **vetting node's** wait by > 40% (measured 9.6 → 4.2 h) **while the control node *Prepare onboarding pack* moves < 10%** | The improvement is at the constrained step, not everywhere. The control is what rules out a global effect |
...
| 11 | Tornado: `HR Operations` capacity → **`no-difference`, swing exactly 0.00** | The headline. A capacity bar sitting flat next to two timers running the width of the chart *is* the example |
| 12 | Tornado: `Payroll` **and** `IT Provisioning` capacity → **`not-testable`** | 1 and 2 cannot go ±20%. Untested-vs-unimportant, landing on a real model |
| 13 | Tornado: the top two movers are the **two timer delays** (measured 19% and 13%), and *Compliance & vetting review* cycle time also **moves** | Ranking sanity — and the finding the example did not expect |
