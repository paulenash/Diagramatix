/**
 * Duplicate an example catalog entry into a new DRAFT (SuperAdmin) — a quick
 * "copy to extend" so an admin can fork a good example and tweak it without
 * disturbing the original. The full package is copied; the copy starts
...
export async function POST(_req: Request, { params }: Params) {
  ...
  const src = await prisma.simulationExample.findUnique({ where: { id } });
