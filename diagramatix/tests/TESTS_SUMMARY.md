...
**As at:** 2026-09-16  ·  **Document version:** 7.0  ·  **Suite:** 507 test files · 3,745 tests (all green)  ·  **Runner:** Vitest  ·  **CI:** enforced on every PR + push to `main`  ·  **Highest ref:** T4928  ·  **Plus:** a Playwright browser e2e suite — see [Layer 11](#layer-11--end-to-end-playwright-browser-tests)
...
| T1058 | (`tests/bpmn/pool-gap.test.ts`) Each generated message-flow label sits in the air gap next to its POOL end, 10px off that pool's edge and 6px beside (not on) its line, placed from FINAL routed geometry by the one message-label rule (messageLabel.ts). Until 2026-09-25 this pinned R05.09's centring on the line, which Paul's "always in the air gap … attached … diagonally to the left or right" ("Always — generated too") superseded | Message labels drifting out of the gap because their offsets were baked BEFORE the L→R sweep / lane hug / pool restack moved the pools; or a generated label placed by a rule of its own | If the post-routing placement stopped using the shared rule, or regressed |
...
## Layer 10b — AI Assist + Voice Assist (assist-while-you-draw + voice/typed command editing)
...
| T3151 | `tests/bpmn/v2505-layout-defects.test.ts` | A long connector label wraps to two BALANCED lines; a short one and an unbreakable word are left alone. |
| T3152 | `tests/bpmn/layout-corpus.test.ts` | The layout corpus is present and every captured AI plan still lays out without throwing. |
| T3153 | `tests/bpmn/layout-corpus.test.ts` | Readability violations across the corpus stay within budget — a ratchet, lowered as fixes land, never raised. |
| T3154 | `tests/bpmn/layout-corpus.test.ts` | Flow elements with NO sequence flow raise a diagnostic — a prod run showed "✓ 12el / 0conn" as a success. |
| T3155 | `tests/bpmn/layout-corpus.test.ts` | Negative control — a properly connected plan raises nothing. |
| T3156 | `tests/bpmn/layout-corpus.test.ts` | Message flows alone do not count as a process; only SEQUENCE flow does. |
| T3157 | `tests/bpmn/layout-corpus.test.ts` | A decision with ONE branch and its merge are both collapsed, and the flow is rejoined rather than severed. |
| T3158 | `tests/bpmn/layout-corpus.test.ts` | Negative control — a decision whose branches lead to DIFFERENT places survives. |
| T3159 | `tests/bpmn/layout-corpus.test.ts` | No 1-in/1-out gateway reaches the drawn diagram. |
| T3160 | `tests/bpmn/gateway-final-placement.test.ts` | R6.32 — a branch whose target is LEVEL leaves the decision by the right vertex, decided on final geometry. |
| T3161 | `tests/bpmn/gateway-final-placement.test.ts` | The other branch keeps a corner, so two branches never share one vertex. |
| T3162 | `tests/bpmn/layout-corpus.test.ts` | No connector in the corpus points at empty space — a pass that moves an element after routing must re-route it. |
| T3163 | `tests/ai/ai-errors.test.ts` | Overload, rate limit, timeout and 5xx are all recognised as transient. |
| T3164 | `tests/ai/ai-errors.test.ts` | Negative control — a real fault is NOT treated as transient, so it is not retried. |
| T3165 | `tests/ai/ai-errors.test.ts` | An overload reads as words and says it is temporary; no provider JSON reaches the user. |
| T3166 | `tests/ai/ai-errors.test.ts` | An unrecognised failure is truncated rather than pasted whole. |
| T3167 | `tests/ai/ai-errors.test.ts` | A key or billing problem is named, since retrying will not help. |
| T3168 | `tests/bpmn/layout-corpus.test.ts` | An event nothing leads to, with flow leaving it, is reported — it was meant to be edge-mounted. |
| T3169 | `tests/bpmn/layout-corpus.test.ts` | Negative control — a reachable event, and a properly mounted boundary event, are not reported. |
| T3170 | `tests/bpmn/layout-corpus.test.ts` | A message flow with both ends in ONE pool is reported: the participant was never declared. |
| T3171 | `tests/bpmn/layout-corpus.test.ts` | Negative control — a message flow that genuinely crosses to another pool is fine. |
| T3172 | `tests/valueChain/prompt-shapes.test.ts` | The master template states that a boundary event attaches only to an activity. |
| T3173 | `tests/valueChain/prompt-shapes.test.ts` | The master template states that a message flow must cross a pool boundary. |
| T3174 | `tests/valueChain/prompt-shapes.test.ts` | A prompt mounting a boundary event on an intermediate catch event is reported — the shape cannot be drawn. |
| T3175 | `tests/valueChain/prompt-shapes.test.ts` | Negative control — one mounted on a task is not reported. |
| T3176 | `tests/valueChain/prompt-shapes.test.ts` | A message flow written lane-to-lane within one pool is reported: it is a sequence flow, or the other party needs a pool. |
| T3177 | `tests/valueChain/prompt-shapes.test.ts` | Negative control — a flow that genuinely reaches a POOL is fine. |
| T3178 | `tests/valueChain/prompt-shapes.test.ts` | A wrapped instruction is rejoined before matching — line-by-line reading would miss most of the catalogue. |
| T3179 | `tests/valueChain/prompt-closing-forms.test.ts` | A branch closing "(continues to User task \"Assess claim\")" is accepted — the form whose absence made the model invent one-branch merge gateways. |
| T3180 | `tests/valueChain/prompt-closing-forms.test.ts` | Negative control — a branch naming only a lane is still reported. |
| T3181 | `tests/valueChain/prompt-closing-forms.test.ts` | Every activity noun the template offers is accepted; a bare quoted name with no type word is not. |
| T3182 | `tests/valueChain/prompt-closing-forms.test.ts` | The template requires a merge only where two or more branches converge, and forbids a gateway that merges one thing. |
| T3183 | `tests/valueChain/prompt-closing-forms.test.ts` | The template's wait rule and its boundary-event rule no longer contradict: a bare wait is an event, a wait that can time out is a receive task. |
| T3184 | `tests/valueChain/prompt-closing-forms.test.ts` | The template makes an exception path terminate in the same words a gateway branch uses. |
| T3185 | `tests/valueChain/prompt-closing-forms.test.ts` | The template forbids a gateway whose branches all name the same destination. |
| T3186 | `tests/valueChain/prompt-closing-forms.test.ts` | A parallel split must ALWAYS be joined — the converge-only merge rule is the exclusive case and must not be applied to it. |
| T3187 | `tests/valueChain/prompt-closing-forms.test.ts` | The template offers no non-interrupting boundary event, because bpmnLayout forces every edge-mounted event to interrupting anyway. |
| T3188 | `tests/ai/prompt-annotation-geometry.test.ts` | The purple AI-Prompt note keeps the position and width the user left it at when it is rebuilt. |
| T3189 | `tests/ai/prompt-annotation-geometry.test.ts` | With no previous note (a first generation) it is still placed against the diagram's content box. |
| T3190 | `tests/ai/prompt-annotation-geometry.test.ts` | Its height grows when the text no longer fits, and never shrinks below what the user set. |
| T3191 | `tests/ai/prompt-annotation-geometry.test.ts` | It wraps to its ACTUAL width, not the default — measuring against the wrong width is how a note ends up clipped. |
| T3192 | `tests/ai/technical-description-fidelity.test.ts` | The Technical Description states the task MARKER (User/Service/Send), so a regeneration need not infer it from the wording. |
| T3193 | `tests/ai/technical-description-fidelity.test.ts` | An unmarked task is called a plain Task rather than described with no type at all. |
| T3194 | `tests/ai/technical-description-fidelity.test.ts` | A wrapped label is flattened to one line — its hard breaks otherwise land at column 0 and read as new instructions. |
| T3195 | `tests/ai/technical-description-fidelity.test.ts` | Inside a subprocess it is the SUBPROCESS that ends, and an unnamed event offers no `<unnamed …>` placeholder as if it were a name. |
| T3196 | `tests/ai/technical-description-fidelity.test.ts` | A data artifact repeated beside a remote consumer is described ONCE, keeping every reader. |
| T3197 | `tests/ai/technical-description-fidelity.test.ts` | An intermediate event states its TRIGGER, so a timer catch is not read back as a message catch. |
| T3198 | `tests/ai/technical-description-fidelity.test.ts` | A subprocess states its loop marker instead of relying on its name happening to say "Repeat Until…". |
| T3199 | `tests/bpmn/layout-corpus.test.ts` | A gateway and an end event that share a name are reported — a description addresses elements BY NAME, so two of one name cannot be told apart. |
| T3200 | `tests/bpmn/layout-corpus.test.ts` | Silent once they are told apart, the end event naming where it goes next. |
| T3201 | `tests/bpmn/layout-corpus.test.ts` | A data artifact repeated beside a remote consumer is NOT reported — that duplication is deliberate. |
| T3202 | `tests/ai/aiModel.test.ts` | Without Moonshot credentials the resolver returns a model this deployment can actually reach, never an unreachable default. |
| T3203 | `tests/valueChain/prompt-truncated.test.ts` | A complete prompt is not reported as truncated. |
| T3204 | `tests/valueChain/prompt-truncated.test.ts` | A name opened and never closed is reported — the V22.07 shape, which every other check passed. |
| T3205 | `tests/valueChain/prompt-truncated.test.ts` | An unclosed FINAL line is reported even when the document totals balance. |
| T3206 | `tests/valueChain/prompt-truncated.test.ts` | A last line ending mid-sentence is reported; an ordinary finished line is not. |
| T3207 | `tests/valueChain/prompt-truncated.test.ts` | An empty prompt is not called truncated — that is a different fault, reported elsewhere. |
| T3208 | `tests/valueChain/prompt-truncated.test.ts` | generateMdPrompt refuses a max_tokens stop and never returns the partial text. |
| T3209 | `tests/valueChain/prompt-truncated.test.ts` | It also refuses text that LOOKS cut when the stop reason is clean — a dropped stream leaves no stop reason. |
| T3210 | `tests/valueChain/prompt-truncated.test.ts` | It accepts a complete response. |
| T3211 | `tests/config/e2e-server-visibility.test.ts` | The Playwright webServer does not discard its output — `stdout: "ignore"` is what hid a build failure behind a bare timeout. |
| T3212 | `tests/config/e2e-server-visibility.test.ts` | Its timeout is at least 600s, so runner load does not turn into a red build while a genuine hang is still caught. |
| T3213 | `tests/config/e2e-server-visibility.test.ts` | The e2e server times each step, so a timeout says WHICH step was slow. |
| T3214 | `tests/config/e2e-server-visibility.test.ts` | It still aborts on a failed step rather than serving a broken build until the timeout. |
| T3215 | `tests/config/e2e-server-visibility.test.ts` | The e2e server fetches tsx ONCE — `npx --yes` re-downloads it per call, which cost 134s, 213s and 142s while the DB answered in 1.7s. |
| T3216 | `tests/config/e2e-server-visibility.test.ts` | No workflow invokes `npx --yes tsx` per script either — the same bug sat in 32 places in the deploy and expired its Azure token. |
| T3217 | `tests/config/e2e-server-visibility.test.ts` | Negative control — the scan still catches a real invocation, so ignoring comments has not made it blind. |
| T3218 | `tests/valueChain/regeneration-targets.test.ts` | With no narrowing, a regenerate covers every process and the chain-level prompts. |
| T3219 | `tests/valueChain/regeneration-targets.test.ts` | It narrows to the ticked processes, emitted in chain order however the boxes were clicked. |
| T3220 | `tests/valueChain/regeneration-targets.test.ts` | Ticking processes SUPPRESSES the chain-level prompts — naming processes is the narrower instruction. |
| T3221 | `tests/valueChain/regeneration-targets.test.ts` | An empty selection means no narrowing, not nothing — the distinction that stops a silent no-op. |
| T3222 | `tests/valueChain/regeneration-targets.test.ts` | A process the chain does not have is reported, instead of regenerating nothing and reporting success. |
| T3223 | `tests/valueChain/regeneration-targets.test.ts` | A chain-level type on its own still works. |
| T3224 | `tests/bpmn/auto-fuse-guards.test.ts` | Auto-fuse does not splice an element that is ALREADY in the flow — a cosmetic nudge must not restructure the process. |
| T3225 | `tests/bpmn/auto-fuse-guards.test.ts` | No activity ends up with two inbound or two outbound sequence flows, which BPMN does not allow without a gateway. |
| T3226 | `tests/bpmn/auto-fuse-guards.test.ts` | Negative control — a LOOSE element is still spliced in, so the guards have not disabled the feature. |
| T3227 | `tests/bpmn/layout-corpus.test.ts` | The layout worklist is accurate: every named diagram is in the corpus and still has defects — a stale entry reserves headroom nobody is using. |
| T3228 | `tests/editor/assoc-hit-clip.test.ts` | The association hit area uses clip-path, which DOES affect pointer events. |
| T3229 | `tests/editor/assoc-hit-clip.test.ts` | It uses no mask — a mask renders identically and clips nothing, which is why the first fix shipped and changed nothing. |
| T3230 | `tests/editor/assoc-hit-clip.test.ts` | The clip is an even-odd path over a huge outer rect, so element boxes are holes rather than the whole hit area vanishing. |
| T3231 | `tests/valueChain/run-row-match.test.ts` | A run progress message finds its row by name and type, not by position — the rows and the run are ordered by two different code paths. |
| T3232 | `tests/valueChain/run-row-match.test.ts` | It does not claim the row that merely shares the message index — the defect, stated directly. |
| T3233 | `tests/valueChain/run-row-match.test.ts` | It distinguishes two diagrams sharing a name but not a type. |
| T3234 | `tests/valueChain/run-row-match.test.ts` | It falls back to the index only when the message carries no name. |
| T3235 | `tests/valueChain/run-row-match.test.ts` | The key is the same `type::name` shape the diagram picker filters on. |
| T3236 | `tests/valueChain/prompt-closing-forms.test.ts` | The template says a loop subprocess holds only the repeating steps, never the whole subprocess. |
| T3237 | `tests/valueChain/prompt-closing-forms.test.ts` | It contrasts a real loop condition with the subprocess's own outcome, by example. |
| T3238 | `tests/valueChain/prompt-closing-forms.test.ts` | A loop excludes the End event, the outcome task, and any gateway routing the main flow. |
| T3239 | `tests/bpmn/layout-corpus.test.ts` | Two END EVENTS sharing a name are not reported — an end event is terminal, so nothing has to find it by name. |
| T3240 | `tests/bpmn/layout-corpus.test.ts` | An end event sharing a name with a GATEWAY is still reported: that one is addressed by name. |
| T3241 | `tests/bpmn/layout-corpus.test.ts` | Two tasks sharing a name are still reported. |
| T3242 | `tests/valueChain/template-history.test.ts` | Every diagram type has a template history, versioned from 1, ascending, with dates that do not go backwards. |
| T3243 | `tests/valueChain/template-history.test.ts` | Every entry carries a date, a version number and a real description — the three things a reader needs. |
| T3244 | `tests/valueChain/template-history.test.ts` | The BPMN history records the changes that actually shipped, backfilled from git rather than memory. |
| T3245 | `tests/valueChain/template-history.test.ts` | A prompt generated before the newest change is stale; same-day counts as stale, erring towards regenerating. |
| T3246 | `tests/valueChain/template-history.test.ts` | A prompt with no date counts as stale — one nobody can date is one nobody can vouch for. |
| T3247 | `tests/valueChain/import-plan.test.ts` | The default reading of an .md file adds only the chains that are new, and destroys nothing. |
| T3248 | `tests/valueChain/import-plan.test.ts` | A ticked chain already in the library is replaced; an unticked one is untouched, new or not. |
| T3249 | `tests/valueChain/import-plan.test.ts` | Ticking everything adds the new chains and replaces the rest, with the prompt counts it would destroy. |
| T3250 | `tests/valueChain/import-plan.test.ts` | `replace:false` can never destroy, whatever is ticked — the fail-safe for a scripted caller. |
| T3251 | `tests/valueChain/import-plan.test.ts` | The preview reports what the library would LOSE, kept separate from what the file carries. |
| T3252 | `tests/valueChain/staleness.test.ts` | A chain NAMES the processes whose prompts predate the master template, rather than counting them. |
| T3253 | `tests/valueChain/staleness.test.ts` | A process with no prompt at all is reported missing, not stale — different fault, same remedy. |
| T3254 | `tests/valueChain/staleness.test.ts` | A stale chain-level prompt counts too and is reported separately from the per-process ones. |
| T3255 | `tests/valueChain/staleness.test.ts` | A fully current chain reports nothing, and still states the version it judged against. |
| T3256 | `tests/valueChain/staleness.test.ts` | A diagram generated before its prompt was regenerated is warned about in its Properties panel. |
| T3257 | `tests/valueChain/staleness.test.ts` | A diagram newer than its prompt says nothing — silence is the right answer for the common case. |
| T3258 | `tests/valueChain/staleness.test.ts` | An older master template is a separate warning, because the remedy is regenerate the PROMPT first. |
| T3259 | `tests/valueChain/staleness.test.ts` | A diagram with no stored AI plan says so as info: it cannot be replayed against a layout change. |
| T3260 | `tests/valueChain/staleness.test.ts` | A diagram never generated from a repository prompt says nothing at all. |
| T3261 | `tests/valueChain/staleness.test.ts` | The repository process code is read off the diagram name, including a " (2)" duplicate suffix. |
| T3262 | `tests/valueChain/staleness.test.ts` | Anything that is not exactly a repository code refuses to match — a wrong match warns the wrong diagram. |
| T3263 | `tests/bpmn/exception-path-clearance.test.ts` | R55.7: a boundary event exception task is not drawn on top of a decision gateway (V22.07, item T). |
| T3264 | `tests/bpmn/exception-path-clearance.test.ts` | It clears EVERY main-line element, not just the first in the way — clearing one reveals the next. |
| T3265 | `tests/bpmn/exception-path-clearance.test.ts` | The gateway branch then leaves by its own bottom vertex instead of detouring past its left edge. |
| T3266 | `tests/bpmn/exception-path-clearance.test.ts` | V22.07 lays out with no readability defect at all. |
| T3267 | `tests/valueChain/template-history.test.ts` | A prompt regenerated the morning AFTER a template change is not stale at UTC+10 — the end-of-day bug. |
| T3268 | `tests/valueChain/template-history.test.ts` | `templateVersionAt` reports the version in force at an instant, not the newest. |
| T3269 | `tests/valueChain/staleness.test.ts` | Regenerating a chain prompts clears the chain badge and the named list. |
| T3270 | `tests/valueChain/staleness.test.ts` | One minute after the change is current; one minute before is not. The boundary, stated once. |
| T3271 | `tests/valueChain/staleness.test.ts` | Regenerating the DIAGRAM clears the prompt-moved warning. |
| T3272 | `tests/valueChain/staleness.test.ts` | Regenerating the PROMPT first, then the diagram, clears both warnings; doing only the second leaves one. |
| T3273 | `tests/valueChain/staleness.test.ts` | Regenerating stores a plan, which clears the not-replayable note. |
| T3274 | `tests/valueChain/staleness.test.ts` | An unknown template version says nothing rather than claiming v0. |
| T3275 | `tests/valueChain/provenance-round-trip.test.ts` | The model and the generation date survive an export to .md and back, exactly. |
| T3276 | `tests/valueChain/provenance-round-trip.test.ts` | An unattributed prompt stays unattributed — an import never invents a date or a model. |
| T3277 | `tests/valueChain/provenance-round-trip.test.ts` | The prompt TEXT is untouched: no comment leaks into what the model is later sent. |
| T3278 | `tests/valueChain/provenance-round-trip.test.ts` | The chain, its processes and its prompt names survive alongside the provenance. |
| T3279 | `tests/valueChain/provenance-round-trip.test.ts` | A .md written before provenance existed still imports, as unknown rather than fabricated. |
| T3280 | `tests/valueChain/provenance-round-trip.test.ts` | A malformed provenance comment reads as unknown, never as a default. |
| T3281 | `tests/valueChain/provenance-round-trip.test.ts` | Nothing is written when there is nothing to say — no empty comment in every .md. |
| T3282 | `tests/ai/quota-exhausted.test.ts` | The exact spend-cap message Paul hit mid-run is recognised, so the batch stops instead of grinding. |
| T3283 | `tests/ai/quota-exhausted.test.ts` | It names the date access returns, so the message says when to come back. |
| T3284 | `tests/ai/quota-exhausted.test.ts` | A RATE limit is NOT exhaustion — abandoning a 277-prompt run over a momentary 429 would be worse. |
| T3285 | `tests/ai/quota-exhausted.test.ts` | Other providers ways of saying the same thing are recognised. |
| T3286 | `tests/ai/quota-exhausted.test.ts` | Nothing, and anything unrelated, is not a spend cap. |
| T3287 | `tests/bpmn/containment-inference.test.ts` | A clean plan produces NO diagnostics: containment follows sequence flow, never associations. |
| T3288 | `tests/bpmn/containment-inference.test.ts` | The loop subprocess holds exactly the three steps that repeat, not the seven around it. |
| T3289 | `tests/bpmn/containment-inference.test.ts` | The tasks a data-object association reaches stay OUTSIDE the loop, where the flow puts them. |
| T3290 | `tests/bpmn/containment-inference.test.ts` | A subprocess is never made its own parent — after which nothing places it. |
| T3291 | `tests/bpmn/containment-inference.test.ts` | V16.09 lays out with no readability defect. |
| T3292 | `tests/bpmn/boundary-reanchor.test.ts` | R8.14: every boundary event centre sits on its host rim after every pass has run. |
| T3293 | `tests/bpmn/boundary-reanchor.test.ts` | The one that strayed 504px is back on its host, and in its host container. |
| T3294 | `tests/bpmn/boundary-reanchor.test.ts` | It mounts on a side that is not walled in by a neighbour, off-centre toward its target. |
| T3295 | `tests/bpmn/boundary-reanchor.test.ts` | A boundary event already on its host is left alone — the rule repairs, it does not re-place. |
| T3296 | `tests/bpmn/boundary-reanchor.test.ts` | V16.06 lays out with no defect and no crossing. |
| T3297 | `tests/valueChain/provenance-round-trip.test.ts` | Exporting the library does not duplicate processes the narrative already heads. |
| T3298 | `tests/valueChain/provenance-round-trip.test.ts` | Every prompt comes back attached to the right process, with its model and date. |
| T3299 | `tests/valueChain/provenance-round-trip.test.ts` | Each process own write-up in the narrative is kept, not thrown away. |
| T3300 | `tests/valueChain/provenance-round-trip.test.ts` | A process the narrative never mentions is still carried rather than dropped. |
| T3301 | `tests/bpmn/end-event-row.test.ts` | R8.38: an End event is not left where a gateway centred on its branches lands on top of it. |
...
| T3324 | `tests/life/life.test.ts` | The same population behaves identically on a 400 and a 2000 grid — the size decides only where things die. |
| T3325 | `tests/life/life.test.ts` | A generation on a 2000 x 2000 grid follows the POPULATION, not the four million cells. |
| T3326 | `tests/life/life.test.ts` | Every methuselah settles at its published generation — eight independent figures, all exact. |
| T3327 | `tests/life/life.test.ts` | Rabbits outlasts the acorn threefold, at over 15,000 generations. |
| T3328 | `tests/ui/cascade-glyphs.test.ts` | A katakana cascade glyph draws one character and nothing else. |
| T3329 | `tests/ui/cascade-glyphs.test.ts` | A BPMN cascade glyph draws SHAPES and never falls back to a character. |
| T3330 | `tests/ui/cascade-glyphs.test.ts` | The Miner cascade really is half rocks and half BPMN, over 500 draws. |
| T3331 | `tests/ui/cascade-glyphs.test.ts` | Every glyph takes the colour it is handed — one cascade, green and brown skins. |
| T3332 | `tests/ui/cascade-glyphs.test.ts` | The Simulator says "Entering the Diagramatix Simulator" and cascades BPMN at half rate. |
| T3333 | `tests/ui/cascade-glyphs.test.ts` | The Miner cascades brown rocks and BPMN. |
| T3334 | `tests/ui/cascade-glyphs.test.ts` | The screensaver has no button and its key is Ctrl+ALT+M, not the browser-claimed Ctrl+Shift+M. |
| T3335 | `tests/ui/cascade-glyphs.test.ts` | The camera and video buttons take the space the M button left. |
| T3336 | `tests/ui/cascade-glyphs.test.ts` | Nothing anywhere in app/ still calls it DiagramMATRIX. |
| T3337 | `tests/ui/cascade-glyphs.test.ts` | The Simulator title is the feature icon and the product name, with no logo. |
| T3338 | `tests/ui/cascade-glyphs.test.ts` | The console background cascade is the same BPMN one as the entry. |
| T3339 | `tests/ui/cascade-glyphs.test.ts` | A panel may shrink below its content, so task fields scroll inside it rather than escaping. |
| T3340 | `tests/life/life3d.test.ts` | Three coordinates pack and unpack, negatives included. |
| T3341 | `tests/life/life3d.test.ts` | A cube has 26 neighbours — 6 by face, 12 by edge, 8 by corner — counted the way the engine counts. |
| T3342 | `tests/life/life3d.test.ts` | The face-only neighbourhood really is only the six faces; a corner-toucher does not count. |
| T3343 | `tests/life/life3d.test.ts` | A bounded cube is a CULL, not a wrap. |
| T3344 | `tests/life/life3d.test.ts` | A generation follows the population, not the volume — which is what makes 100³ affordable. |
| T3345 | `tests/life/life3d.test.ts` | Every 3-D pattern is re-verified against its own claimed category, period and travel. |
| T3346 | `tests/life/life3d.test.ts` | B6/S567 has a glider — Bays own test for whether a 3-D rule is a Life. |
| T3347 | `tests/life/life3d.test.ts` | The glider is still flying after twenty periods, displaced exactly 20 each way. |
| T3348 | `tests/life/life3d.test.ts` | The solid 2x2x2 cube never changes: every cube touches seven others, and 7 survives. |
| T3349 | `tests/life/life3d.test.ts` | The same glider does NOT fly under Conway rule — a pattern belongs to its rule. |
| T3350 | `tests/life/life3d.test.ts` | Born on three of twenty-six, a soup runs away; under B6/S567 it does not. |
| T3351 | `tests/life/life3d.test.ts` | Bays spelling B6/S567 reads as he wrote it. |
| T3352 | `tests/life/life3d.test.ts` | Counts above nine need a separator, and a count that cannot occur is refused. |
| T3353 | `tests/life/life3d.test.ts` | Rubbish notation is refused. |
| T3354 | `tests/life/life3d.test.ts` | A rule always formats unambiguously as a comma list. |
| T3355 | `tests/life/life3d.test.ts` | Every 3-D rule in the library parses and carries a real note; Conway is kept for comparison. |
| T3356 | `tests/life/life3d.test.ts` | The rules panel describes the ACTIVE rule and the right neighbourhood. |
| T3357 | `tests/ui/simulation-example-greeting.test.ts` | Loading a Simulation Example still lands on the project, carrying the example name. |
| T3358 | `tests/ui/simulation-example-greeting.test.ts` | The project reads the name and CLEARS the parameter, so a reload does not ask again. |
| T3359 | `tests/ui/simulation-example-greeting.test.ts` | The greeting says what Paul asked it to say, in those words. |
| T3360 | `tests/ui/simulation-example-greeting.test.ts` | Both answers act, and neither leaves the dialog up. |
| T3361 | `tests/ui/simulation-example-greeting.test.ts` | It is green and monospaced, and it is not a banned browser dialog. |
| T3362 | `tests/life/life3d.test.ts` | Two 3-D objects that do not touch are split into two — a glider in debris is invisible otherwise. |
| T3363 | `tests/life/life3d.test.ts` | Touching at a CORNER is one object under Moore, two under faces-only. |
| T3364 | `tests/life/life3d.test.ts` | A glider hitting a cube runs on and then leaves a glider — with its real numbers. |
| T3365 | `tests/life/life3d.test.ts` | Two gliders colliding leave exactly one. |
| T3366 | `tests/life/life3d.test.ts` | A plain still life leaves NO glider — the negative control for the whole category. |
...
| T3053 | `tests/bpmn/v2505-rules.test.ts` | B52 is silent once they are staggered by the label's full drawn height (30px for this two-line label; B52 now measures the wrapped box — see T4834). |
...
| T3372 | `tests/simulation/next-steps.test.ts` | Below three run scenarios the report says so rather than inventing advice. |
| T3373 | `tests/simulation/next-steps.test.ts` | An empty run history returns a reason, not a crash. |
| T3374 | `tests/simulation/next-steps.test.ts` | Only the latest run per scenario is used - older runs may not match their scenario overrides. |
| T3375 | `tests/simulation/next-steps.test.ts` | A team busiest in every run and never varied ranks first, with a capacity scenario attached. |
| T3376 | `tests/simulation/next-steps.test.ts` | A lever still improving at the highest value tried proposes the next value. |
| T3377 | `tests/simulation/next-steps.test.ts` | A lever whose whole range sits inside the noise is reported as not the constraint, with no action button. |
| T3378 | `tests/simulation/next-steps.test.ts` | A lever set once and never revisited is flagged as a single point. |
| T3379 | `tests/simulation/next-steps.test.ts` | Suggestion ordering is deterministic and independent of input order. |
| T3380 | `tests/simulation/next-steps.test.ts` | Levers are enumerated from stored metrics alone: teams, tasks and sources (not gateways). |
| T3381 | `tests/simulation/next-steps.test.ts` | A difference smaller than the run-to-run band is inside the noise; a larger one is not. |
| T3382 | `tests/simulation/next-steps.test.ts` | outcomeOf falls back to run-average percentiles for runs predating caseFlow. |
| T3383 | `tests/simulation/next-steps.test.ts` | leverHistory records values tried ascending, with the best by near-worst case. |
| T3384 | `tests/simulation/next-steps.test.ts` | The facts handed to the model contain only computed findings. |
| T3385 | `tests/simulation/next-steps.test.ts` | The deterministic summary renders the same findings when AI is off. |
| T3386 | `tests/simulation/next-steps.test.ts` | The path the next-steps panel posts to resolves to a route file. |
| T3387 | `tests/simulation/client-bundle-hygiene.test.ts` | No module reachable from a client component (StudyManager to nextSteps) imports the DB - Prisma in the browser bundle fails the production build. |
| T3388 | `tests/simulation/client-bundle-hygiene.test.ts` | The guard can fail: loadStudyRuns.ts does import the DB and is kept out of that graph. |
| T3389 | `tests/simulation/run-trend.test.ts` | With no baseline pinned the trend says so rather than picking one. |
| T3390 | `tests/simulation/run-trend.test.ts` | The baseline comes first and every later run is measured against it. |
| T3391 | `tests/simulation/run-trend.test.ts` | Runs before the baseline are excluded, so it reads as the starting point. |
| T3392 | `tests/simulation/run-trend.test.ts` | A slower run reads as slower - the sign of the percentage is not flipped. |
| T3393 | `tests/simulation/run-trend.test.ts` | Net movement is baseline to most recent, over the number of runs since. |
| T3394 | `tests/simulation/run-trend.test.ts` | A baseline alone is not yet a trend (no net figure). |
| T3395 | `tests/simulation/run-trend.test.ts` | Runs with no metrics are skipped rather than charted as zero. |
| T3396 | `tests/simulation/run-trend.test.ts` | A baseline run is never pruned, so the trend cannot lose its reference. |
| T3397 | `tests/simulation/wait-split.test.ts` | An authored waitTime is counted as process wait, never as queue wait. |
| T3398 | `tests/simulation/wait-split.test.ts` | Queueing for a busy team is counted as queue wait, never as process wait. |
| T3399 | `tests/simulation/wait-split.test.ts` | Adding capacity cuts queue wait and leaves process wait untouched. |
...
| T3422 | `tests/simulation/significance.test.ts` | A large difference against tight runs is reported as real, with a confidence interval. |
| T3423 | `tests/simulation/significance.test.ts` | A small difference against noisy runs is reported as inside the noise, not as a finding. |
| T3424 | `tests/simulation/significance.test.ts` | An inside-the-noise verdict says how many replications would settle it. |
| T3425 | `tests/simulation/significance.test.ts` | A run with no stored per-replication vector is reported approximate, never certain. |
| T3426 | `tests/simulation/significance.test.ts` | With no data at all it refuses to answer rather than guessing. |
| T3427 | `tests/simulation/significance.test.ts` | Two perfectly steady runs: any difference is real, an identical one is not. |
| T3428 | `tests/simulation/significance.test.ts` | Direction is described from the reader's point of view, not from the sign. |
| T3429 | `tests/simulation/significance.test.ts` | The t table is conservative between rows, never narrower than it should be. |
| T3430 | `tests/simulation/significance.test.ts` | mean and sample variance use n-1 and are zero-safe. |
| T3431 | `tests/simulation/significance.test.ts` | Resolving a smaller difference needs more replications; impossible ones return nothing. |
| T3432 | `tests/simulation/significance.test.ts` | An approximate comparison can still exceed the band, and is labelled approximate. |
| T3433 | `tests/simulation/significance.test.ts` | A zero band means the replications agreed exactly, not that nothing is known. |
| T3434 | `tests/simulation/significance.test.ts` | The real / inside-noise verdicts agree with exceedsBand. |
| T3435 | `tests/simulation/warmup.test.ts` | A run that settles gets a warm-up covering the transient. |
| T3436 | `tests/simulation/warmup.test.ts` | A run steady from the start needs no warm-up. |
| T3437 | `tests/simulation/warmup.test.ts` | Too few cases declines to answer rather than inventing a number. |
| T3438 | `tests/simulation/warmup.test.ts` | A run that never settles says so instead of suggesting a huge warm-up. |
...
| T3458 | `tests/simulation/queue-discipline.test.ts` | A model declaring no queue discipline runs bit-identically to before. |
| T3459 | `tests/simulation/queue-discipline.test.ts` | A segment-free model reports no per-segment split at all. |
| T3460 | `tests/simulation/queue-discipline.test.ts` | FIFO serves in arrival order regardless of priority. |
| T3461 | `tests/simulation/queue-discipline.test.ts` | A priority queue serves the urgent case first, and is FIFO within a priority. |
| T3462 | `tests/simulation/queue-discipline.test.ts` | Shortest-first serves the quick job first; an unknown length never jumps ahead. |
| T3463 | `tests/simulation/queue-discipline.test.ts` | Head-of-line blocking is kept: nothing is served past a request that does not fit. |
| T3464 | `tests/simulation/queue-discipline.test.ts` | Changing discipline mid-run re-orders those already waiting. |
| T3465 | `tests/simulation/queue-discipline.test.ts` | Discipline and queue order survive a snapshot. |
| T3466 | `tests/simulation/queue-discipline.test.ts` | An older snapshot with no discipline resumes as FIFO. |
| T3467 | `tests/simulation/queue-discipline.test.ts` | With no priorities assigned, a priority queue is a FIFO queue. |
...
| T3493 | `tests/simulation/validate.test.ts` | Identical distributions have a KS gap of zero. |
| T3494 | `tests/simulation/validate.test.ts` | Completely separated distributions have the largest possible gap. |
| T3495 | `tests/simulation/validate.test.ts` | A partial shift lands between the two, and the statistic is symmetric. |
| T3496 | `tests/simulation/validate.test.ts` | A matching model reports agreement and does NOT claim to be proven right. |
| T3497 | `tests/simulation/validate.test.ts` | A diverging model says so plainly and points at what to check. |
| T3498 | `tests/simulation/validate.test.ts` | Too few cases refuses to answer rather than returning a flattering figure. |
| T3499 | `tests/simulation/validate.test.ts` | The critical value uses the true case count, not the stored quantile vector. |
| T3500 | `tests/simulation/validate.test.ts` | Both distributions are summarised so the reader can see where they differ. |
| T3501 | `tests/simulation/validate.test.ts` | 101 quantiles reproduce the distribution closely enough to test against. |
| T3502 | `tests/simulation/validate.test.ts` | An empty sample set yields no quantile vector rather than a row of zeros. |
| T3503 | `tests/simulation/validate.test.ts` | The hold-back split is chronological: earlier cases fit, later cases test. |
| T3504 | `tests/simulation/validate.test.ts` | A zero holdout keeps everything for fitting and holds nothing back. |
| T3505 | `tests/simulation/validate.test.ts` | The holdout is capped, so a model can never be fitted on nothing. |
| T3506 | `tests/simulation/validate.test.ts` | Cases arriving out of order are sorted before splitting. |
| T3507 | `tests/simulation/validate.test.ts` | The path the validate panel posts to resolves to a route file. |
| T3508 | `tests/simulation/skills.test.ts` | A pool with no people declared is a counted pool, exactly as before. |
| T3509 | `tests/simulation/skills.test.ts` | Required skills change nothing when the team declares no people. |
| T3510 | `tests/simulation/skills.test.ts` | Only someone holding every required skill can take the work (AND). |
| T3511 | `tests/simulation/skills.test.ts` | Work requiring no skills can be taken by anyone on the team. |
| T3512 | `tests/simulation/skills.test.ts` | A skill nobody holds makes the work wait rather than going to anyone. |
| T3513 | `tests/simulation/skills.test.ts` | Releasing frees the right person: only the qualified one unblocks the work. |
| T3514 | `tests/simulation/skills.test.ts` | The generalist is kept free for the work only they can do. |
| T3515 | `tests/simulation/skills.test.ts` | Ties break on unit id, so the choice of person is reproducible. |
| T3516 | `tests/simulation/skills.test.ts` | A skilled pool serves work someone can do rather than leaving them idle. |
| T3517 | `tests/simulation/skills.test.ts` | Queue order is still respected among the requests that can be served. |
| T3518 | `tests/simulation/skills.test.ts` | A counted pool keeps strict head-of-line blocking, unchanged. |
| T3519 | `tests/simulation/skills.test.ts` | A skilled team closed by the calendar stops, however many qualify. |
...
| T3537 | `tests/simulation/skills.test.ts` | Required skills survive the diagram to assembler to network journey. |
| T3538 | `tests/simulation/skills.test.ts` | A team with no members declared assembles as a counted pool. |
| T3539 | `tests/simulation/skills.test.ts` | The model reaching the engine carries what the eligibility tests rely on. |
| T3540 | `tests/simulation/bpsim-skills.test.ts` | Skills survive a Diagramatix to BPSim to Diagramatix round trip. |
| T3541 | `tests/simulation/bpsim-skills.test.ts` | They travel in the dgx namespace, declared on the root so the file stays well-formed. |
| T3542 | `tests/simulation/bpsim-skills.test.ts` | A scenario with no skills emits no extension at all. |
| T3543 | `tests/simulation/bpsim-skills.test.ts` | A BPSim file from another tool has none, which reads as no constraint. |
| T3544 | `tests/simulation/bpsim-skills.test.ts` | Skill names are escaped, so a stray quote cannot break the document. |
| T3545 | `tests/simulation/sensitivity.test.ts` | Teams, task times and arrival rates are enumerated from the assembled network, not the diagram. |
| T3546 | `tests/simulation/sensitivity.test.ts` | A headcount of 1 cannot go down 20%, so it is reported as untested rather than unimportant. |
| T3547 | `tests/simulation/sensitivity.test.ts` | A headcount that rounds back onto itself is untested too; 5 does vary, to 4 and 6. |
| T3548 | `tests/simulation/sensitivity.test.ts` | A continuous parameter keeps its fractions. |
| T3549 | `tests/simulation/sensitivity.test.ts` | An untestable parameter says UNTESTED, never that it makes no difference. |
| T3550 | `tests/simulation/sensitivity.test.ts` | Each tornado point runs through the ordinary sweep override, not a special path. |
| T3551 | `tests/simulation/sensitivity.test.ts` | The widest swing comes first, and the statement names the load-bearing assumption. |
| T3552 | `tests/simulation/sensitivity.test.ts` | Untestable bars sort AFTER the ranked ones, never interleaved among them. |
| T3553 | `tests/simulation/sensitivity.test.ts` | The swing is reported against the baseline answer, in percent. |
| T3554 | `tests/simulation/sensitivity.test.ts` | A parameter that makes no difference stays in the chart and says a rough figure is safe. |
| T3555 | `tests/simulation/sensitivity.test.ts` | 'No difference' is a significance question, not a smallness one - the same swing on tight runs is real. |
| T3556 | `tests/simulation/sensitivity.test.ts` | When nothing moves it says so, rather than crowning a random winner. |
| T3557 | `tests/simulation/sensitivity.test.ts` | With nothing testable at all it says there is nothing to rank. |
| T3558 | `tests/simulation/sensitivity.test.ts` | The variation width is stated, since the plus-or-minus 20% is an assumption too. |
| T3559 | `tests/simulation/sensitivity.test.ts` | The URL the panel calls resolves to a POST handler on disk. |
...
| T3589 | `tests/simulation/hire-and-onboard.test.ts` | The example is in the catalog at advanced level, with a BPMN root and an ArchiMate companion that is not a root. |
| T3590 | `tests/simulation/hire-and-onboard.test.ts` | EXACTLY two people are accredited - the whole example rests on this. |
| T3591 | `tests/simulation/hire-and-onboard.test.ts` | Every task requiring a skill sits on a team that names people (or the requirement is silently ignored). |
| T3592 | `tests/simulation/hire-and-onboard.test.ts` | Every calendar anchors to a Monday and at least six exceptions land mid-run. |
| T3593 | `tests/simulation/hire-and-onboard.test.ts` | The business case carries the inputs a payback month needs. |
| T3594 | `tests/simulation/hire-and-onboard.test.ts` | The busiest team looks healthy (<65%) and it is HR Operations. |
| T3595 | `tests/simulation/hire-and-onboard.test.ts` | ...yet there is a real queue at the accredited step. |
| T3596 | `tests/simulation/hire-and-onboard.test.ts` | MORE DESKS is bit-identical: capacity cannot exceed the people who exist. |
| T3597 | `tests/simulation/hire-and-onboard.test.ts` | Hiring two more administrators does not touch the accredited queue. |
| T3598 | `tests/simulation/hire-and-onboard.test.ts` | Training one person halves it, and leaves the control step alone. |
| T3599 | `tests/simulation/hire-and-onboard.test.ts` | ...and it is the cheaper of the two answers. |
| T3600 | `tests/simulation/hire-and-onboard.test.ts` | The significance test agrees: hiring is noise, training is real. |
| T3601 | `tests/simulation/hire-and-onboard.test.ts` | The ArchiMate fill reaches every named person and every skilled task. |
| T3602 | `tests/simulation/hire-and-onboard.test.ts` | A role that aggregates others resolves to the skills it is made of. |
| T3603 | `tests/simulation/hire-and-onboard.test.ts` | Exactly two things deliberately do not match, and both are reported. |
| T3604 | `tests/simulation/hire-and-onboard.test.ts` | The goldens were captured at the config the example still ships. |
| T3605 | `tests/simulation/hire-and-onboard.test.ts` | The baseline reproduces its golden figures exactly. |
| T3606 | `tests/simulation/hire-and-onboard.test.ts` | More desks reproduces its golden figures exactly. |
| T3607 | `tests/simulation/hire-and-onboard.test.ts` | Hiring two more reproduces its golden figures exactly. |
| T3608 | `tests/simulation/hire-and-onboard.test.ts` | Training a third checker reproduces its golden figures exactly. |
| T3609 | `tests/mining/route-gating.test.ts` | The route enumeration finds the Miner's routes at all - a guard that matches nothing passes forever. |
| T3610 | `tests/mining/route-gating.test.ts` | Every mining route either calls gateFeature or is listed as deliberately open. |
| T3611 | `tests/mining/route-gating.test.ts` | The exemption list is honest: every entry still exists and carries a real reason. |
| T3612 | `tests/mining/route-gating.test.ts` | And the check can fail - the public webhook, which has no gate by design, is detected as ungated. |
| T3613 | `tests/mining/route-gating.test.ts` | The tiered sub-features are enforced where they are sold (OCEL, the example gallery, and calibrate's two products). |
| T3614 | `tests/mining/case-attributes.test.ts` | An unmapped column is DROPPED unless asked for - a spare column is as likely to hold a name as a region. |
| T3615 | `tests/mining/case-attributes.test.ts` | A kept column becomes a per-case attribute, taken from the case's FIRST event. |
| T3616 | `tests/mining/case-attributes.test.ts` | The attribute is offered as a dimension, with its values. |
| T3617 | `tests/mining/case-attributes.test.ts` | A column with too many values is marked unfilterable and says why, rather than vanishing. |
| T3618 | `tests/mining/case-attributes.test.ts` | A hashed column stores a token, never the original. |
| T3619 | `tests/mining/case-attributes.test.ts` | The digest is STABLE, so cases still group and join. |
| T3620 | `tests/mining/case-attributes.test.ts` | Masking the case id keeps the cases and their events intact. |
| T3621 | `tests/mining/case-attributes.test.ts` | An empty case id is still an unusable row, not the hash of nothing. |
| T3622 | `tests/mining/case-attributes.test.ts` | Per-event durations are stored, one shorter than the event count. |
| T3623 | `tests/mining/case-attributes.test.ts` | Resources are stored as dictionary indices, not repeated strings. |
| T3624 | `tests/mining/case-attributes.test.ts` | An event with no resource records -1, which is not the same as index 0. |
| T3625 | `tests/mining/case-attributes.test.ts` | Per-activity resource COUNTS survive, so the multi-team flag can be opened. |
| T3626 | `tests/mining/case-attributes.test.ts` | A log too big for the per-event vectors reports detail:counts rather than dropping them silently. |
| T3627 | `tests/mining/case-attributes.test.ts` | REGRESSION: a log asking for none of this produces identical stats, variants and analytics. |
...
| T3647 | `tests/mining/formats-xlsx.test.ts` | A plain worksheet becomes headers and rows. |
| T3648 | `tests/mining/formats-xlsx.test.ts` | A sparse row keeps its columns aligned - Excel omits empty cells, so cells are placed by their r= reference. |
| T3649 | `tests/mining/formats-xlsx.test.ts` | A blank row in the middle does not shift the rows after it. |
| T3650 | `tests/mining/formats-xlsx.test.ts` | The header row is the first row with anything in it (exports often carry a title above the table). |
| T3651 | `tests/mining/formats-xlsx.test.ts` | An unnamed column still gets a name, so it can be mapped. |
| T3652 | `tests/mining/formats-xlsx.test.ts` | Every sheet is returned, in workbook order. |
| T3653 | `tests/mining/formats-xlsx.test.ts` | An Excel serial date is handed through and read by the existing pipeline, not interpreted here. |
...
| T3763 | `tests/mining/twin-seam.test.ts` | A run imported WITH a hold-back still has one after a live refresh. |
| T3764 | `tests/mining/twin-seam.test.ts` | The split is RE-APPLIED to the grown log, not copied from the old fit. |
| T3765 | `tests/mining/twin-seam.test.ts` | A run imported without a hold-back does not acquire one. |
| T3766 | `tests/mining/twin-seam.test.ts` | A run with a calibrated twin gets a divergence date when the log moves on. |
| T3767 | `tests/mining/twin-seam.test.ts` | A run with NO twin is not marked stale - no noise on uncalibrated runs. |
| T3768 | `tests/mining/twin-seam.test.ts` | The FIRST divergence date is kept, not the latest refresh. |
| T3769 | `tests/mining/twin-seam.test.ts` | The twin is MARKED, never silently re-calibrated over a user's edits. |
| T3770 | `tests/mining/twin-seam.test.ts` | The hold-back the import route accepts is actually sent by the importer. |
| T3771 | `tests/mining/twin-seam.test.ts` | The study the calibrate route returns is used by its caller. |
| T3772 | `tests/mining/twin-seam.test.ts` | The Simulator can be opened ON a study, all six files down the chain. |
| T3773 | `tests/mining/handover.test.ts` | Transitions are ranked by TOTAL elapsed, not by median. |
| T3774 | `tests/mining/handover.test.ts` | The measured total is used when the run recorded one. |
| T3775 | `tests/mining/handover.test.ts` | A run imported before totals existed falls back, and SAYS it estimated. |
| T3776 | `tests/mining/handover.test.ts` | Each row carries its share of everything leaving that step. |
| T3777 | `tests/mining/handover.test.ts` | A transition's time IS the from-activity's time - a decomposition, not a second helping. |
| T3778 | `tests/mining/handover.test.ts` | The final event of a case contributes no transition. |
| T3779 | `tests/mining/handover.test.ts` | An edge with too few observations shows a frequency and no median. |
| T3780 | `tests/mining/handover.test.ts` | At the observation threshold the median appears. |
| T3781 | `tests/mining/handover.test.ts` | No edges is an empty view, not a crash or a zero-filled table. |
| T3782 | `tests/mining/handover.test.ts` | Only edges with enough observations get a median label on the model. |
| T3783 | `tests/mining/handover.test.ts` | The edge key cannot confuse "AB"->"C" with "A"->"BC". |
| T3784 | `tests/mining/handover.test.ts` | The median gap is labelled and the frequency badge is left alone. |
| T3785 | `tests/mining/handover.test.ts` | Arrow weight tracks TOTAL time, so the fattest arrow is the costliest path. |
| T3786 | `tests/mining/handover.test.ts` | A connector with no mined edge behind it is not touched at all. |
| T3787 | `tests/mining/handover.test.ts` | An edge with too few observations gets a weight but no median label. |
...
| T3948 | `tests/mining/cold-start-sample.test.ts` | The sample log the import screen advertises resolves to a real file. |
| T3949 | `tests/mining/cold-start-sample.test.ts` | It maps itself — a cold-start sample must not open with a mapping exercise. |
| T3950 | `tests/mining/cold-start-sample.test.ts` | It mines into enough cases and paths to be worth looking at. |
| T3951 | `tests/mining/cold-start-sample.test.ts` | It carries a resource column and spare columns, so no tab opens empty. |
| T3952 | `tests/mining/cold-start-sample.test.ts` | The screen offers it WITHOUT an adopted example — the cold-start defect, as a check. |
| T3953 | `tests/mining/compare-view.test.ts` | Each run in a series is judged under the SLA IT was judged under. |
| T3954 | `tests/mining/compare-view.test.ts` | No SLA means an UNKNOWN late rate, not a zero one. |
| T3955 | `tests/mining/compare-view.test.ts` | A run never conformance-checked has an unknown fitness. |
| T3956 | `tests/mining/compare-view.test.ts` | A deviation that matched nothing is not one the run had. |
| T3957 | `tests/mining/compare-view.test.ts` | An observation that cannot be scored still counts as an observation. |
| T3958 | `tests/mining/compare-view.test.ts` | The cron and the Compare view both CALL the shared alert-history assembly. |
| T3959 | `tests/mining/compare-view.test.ts` | Neither route builds its own alert points. |
| T3960 | `tests/mining/compare-view.test.ts` | The series route calls the alert rules rather than restating their thresholds. |
| T3961 | `tests/mining/compare-view.test.ts` | The Compare view computes nothing in the browser. |
| T3962 | `tests/mining/compare-view.test.ts` | A refusal REPLACES the delta table, it does not caption it. |
| T3963 | `tests/mining/compare-view.test.ts` | It says the comparison is of whole runs, beside a filter bar that is not. |
| T3964 | `tests/mining/compare-view.test.ts` | Durations use the shared clock-unit rule, not a local one. |
| T3965 | `tests/mining/example-catalog-teaches.test.ts` | The three-team example's hand-off map is measured, not approximated. |
| T3966 | `tests/mining/example-catalog-teaches.test.ts` | One hand-off dominates the elapsed time, and it is Finance → Legal. |
| T3967 | `tests/mining/example-catalog-teaches.test.ts` | The team causing the delay is not the team doing the work. |
| T3968 | `tests/mining/example-catalog-teaches.test.ts` | Exactly one ping-pong pair, and most cases do not bounce. |
| T3969 | `tests/mining/example-catalog-teaches.test.ts` | The credit check repeats, at a quotable per-case rate. |
| T3970 | `tests/mining/example-catalog-teaches.test.ts` | The task-mining ping-pong detector returns ZERO on a business log. |
| T3971 | `tests/mining/example-catalog-teaches.test.ts` | The waiting in the rework example is on the applicant, not the bank. |
| T3972 | `tests/mining/example-catalog-teaches.test.ts` | Order-to-Cash's two slicing columns survive the import. |
| T3973 | `tests/mining/example-catalog-teaches.test.ts` | A name and a unique number are deliberately NOT kept. |
| T3974 | `tests/mining/example-catalog-teaches.test.ts` | Slicing by Region changes the answer — the entire lesson. |
| T3975 | `tests/mining/example-catalog-teaches.test.ts` | Control failures concentrate in one channel without belonging to it. |
| T3976 | `tests/mining/example-catalog-teaches.test.ts` | Accounts Payable conformance genuinely improves across the three periods. |
| T3977 | `tests/mining/example-catalog-teaches.test.ts` | The three periods are comparable — the same process, not three. |
| T3978 | `tests/mining/example-catalog-teaches.test.ts` | Mined in the other direction they fire real alerts at the real thresholds. |
...
| T4001 | `tests/features/example-summaries.test.ts` | The example tiles MASK the cascade — no translucent card backgrounds. |
| T4002 | `tests/features/example-summaries.test.ts` | Neither gallery cascade is a dimmer version of the other. |
| T4003 | `tests/features/example-summaries.test.ts` | The Simulator cascade carries explicit colours rather than the default. |
| T4004 | `tests/features/example-summaries.test.ts` | The dashboard tile reads “Miner Examples” and keeps its pick icon. |
| T4005 | `tests/features/example-summaries.test.ts` | Both entry bursts read “Entering the Diagramatix <feature>…”. |
...
| T4105 | `tests/epc/import-aml.test.ts` | Both eEPC models in the sample export are found. |
| T4106 | `tests/epc/import-aml.test.ts` | Every EPC symbol survives the import — the sample uses all ten. |
| T4107 | `tests/epc/import-aml.test.ts` | Control, assignment and information arcs are separated. |
| T4108 | `tests/epc/import-aml.test.ts` | An object type it does not model is REPORTED, not dropped in silence. |
| T4109 | `tests/epc/import-aml.test.ts` | Attribute casing does not matter. |
| T4110 | `tests/epc/import-aml.test.ts` | A connection code it has never seen still lands, classified by its endpoints. |
| T4111 | `tests/epc/import-aml.test.ts` | Extra Group nesting does not hide the objects. |
| T4112 | `tests/epc/import-aml.test.ts` | A non-EPC model in the same export is skipped by name, not mangled. |
| T4113 | `tests/epc/import-aml.test.ts` | Assignments arrive as attributes on the function. |
| T4114 | `tests/epc/import-aml.test.ts` | The clean sample lays out with NO rule diagnostics. |
| T4115 | `tests/epc/import-aml.test.ts` | The deliberately broken model reports the planted faults. |
| T4116 | `tests/epc/import-aml.test.ts` | An imported EPC converts to BPMN by the same rules as a drawn one. |
| T4117 | `tests/epc/import-aml.test.ts` | The outcome events after each decision became branch labels. |
| T4118 | `tests/epc/to-bpmn.test.ts` | One department drawn twice is ONE lane, and both tasks are in it. |
| T4119 | `tests/epc/to-bpmn.test.ts` | One system drawn twice is ONE black-box pool. |
| T4120 | `tests/epc/to-bpmn.test.ts` | Folding two boxes into one loses no message flow. |
| T4121 | `tests/epc/to-bpmn.test.ts` | …and folding is not reported as losing the box. |
| T4122 | `tests/epc/import-aml.test.ts` | The editor offers ARIS (AML) on an EPC and calls the handler. |
...
| T4139 | `tests/epc/gateway-and-navigation.test.ts` | The refine pass is told to write the gateway’s question and the branch answers. |
| T4140 | `tests/epc/gateway-and-navigation.test.ts` | …and warned the answer-to-branch mapping must not move. |
| T4141 | `tests/epc/gateway-and-navigation.test.ts` | A join and a parallel gateway are left unlabelled — neither asks anything. |
| T4142 | `tests/epc/gateway-and-navigation.test.ts` | A refined label can only land on the flow it was written for. |
| T4143 | `tests/epc/gateway-and-navigation.test.ts` | Entity Drift is hidden on an EPC — it rings names an EPC cannot contain. |
| T4144 | `tests/epc/gateway-and-navigation.test.ts` | Which element carries the back marker is ONE decision, used by both render paths. |
| T4145 | `tests/epc/gateway-and-navigation.test.ts` | An EPC’s anchor is the event nothing flows INTO — assignments do not count. |
| T4146 | `tests/epc/gateway-and-navigation.test.ts` | The renderer no longer second-guesses the element type. |
| T4147 | `tests/epc/spec-doc.test.ts` | The eEPC specification’s generated tables are current — regenerating moves nothing. |
...
| T4155 | `tests/flowchart/label-fit.test.ts` | The renderer WRAPS a flowchart name instead of drawing one long line. |
| T4156 | `tests/flowchart/label-fit.test.ts` | It wraps to the shape’s usable width — a parallelogram’s taper is not free. |
| T4157 | `tests/flowchart/label-fit.test.ts` | A long name really does become several lines, in every fitted shape. |
| T4158 | `tests/flowchart/label-fit.test.ts` | A short name changes nothing. |
| T4159 | `tests/flowchart/label-fit.test.ts` | A name past the free-line count grows the box DOWNWARD only. |
| T4160 | `tests/flowchart/label-fit.test.ts` | The grown box actually holds the lines it was grown for. |
...
| T4193 | `tests/prompts/provenance-complete.test.ts` | DERIVED: no POST to /api/prompts omits how the text was written. |
...
| T4233 | `tests/ai/model-setting-keys.test.ts` | **The prod default-model script accounts for EVERY model setting key** — derived from the constants, so a third key cannot be forgotten. |
| T4234 | `tests/ai/model-setting-keys.test.ts` | The script reports a vision override rather than silently deleting one somebody set on purpose. |
| T4235 | `tests/ai/model-setting-keys.test.ts` | The script writes the ROW, which is what overrides the constant. |
| T4236 | `tests/simulation/assess-format.test.ts` | The AI assessment's output budget is big enough for the reply the prompt asks for (was 512, truncating mid-word). |
| T4237 | `tests/simulation/assess-format.test.ts` | **A truncated assessment is reported, not served as a finished one** — `stop_reason` carried from generator to route to screen. |
| T4238 | `tests/simulation/assess-format.test.ts` | The assessment is a verdict line plus short bullets, not one block of prose. |
| T4239 | `tests/simulation/assess-format.test.ts` | An assessment with no bullets degrades to a paragraph rather than an empty panel. |
| T4240 | `tests/simulation/hire-and-onboard.test.ts` | "Advertise for one week" reproduces its goldens exactly. |
| T4241 | `tests/simulation/hire-and-onboard.test.ts` | The two-lever scenario reproduces its goldens exactly. |
| T4242 | `tests/simulation/hire-and-onboard.test.ts` | Every scenario the package ships has goldens — the capture list is derived, not hand-kept. |
| T4243 | `tests/simulation/hire-and-onboard.test.ts` | **The example DEMONSTRATES something** — a scenario moves p50 >10%, while the staffing levers still move ~nothing. |
| T4244 | `tests/simulation/skill-catalog.test.ts` | **Two spellings of a skill are one skill** — the ambiguity the catalog exists to remove. |
| T4245 | `tests/simulation/skill-catalog.test.ts` | A duplicate is refused however it is spelled (the unique index is case-sensitive; this is not). |
| T4246 | `tests/simulation/skill-catalog.test.ts` | A retired skill still RESOLVES where it is used; it only stops being offered. |
| T4247 | `tests/simulation/skill-catalog.test.ts` | Re-adding a retired skill offers reactivation instead of making a second copy. |
| T4248 | `tests/simulation/skill-catalog.test.ts` | A skill belongs to one org and cannot be edited or deleted from another. |
| T4249 | `tests/simulation/skill-catalog.test.ts` | An empty or over-long skill name is refused. |
| T4250 | `tests/simulation/skill-catalog.test.ts` | The Skills screen exists and is linked from OrgAdmin — a catalog nobody can open is not extendible. |
| T4251 | `tests/simulation/skill-catalog.test.ts` | Writes are org-admin gated on every verb; reads stay open so the pickers work. |
| T4252 | `tests/simulation/skill-seed-parity.test.ts` | **The TS and SQL skill seeds plant the same list** — two seeds that drift give an org a different vocabulary per route. |
| T4253 | `tests/simulation/skill-seed-parity.test.ts` | The SQL supplies `id` and `updatedAt` — the two columns Prisma fills and Postgres has no default for. |
| T4254 | `tests/simulation/skill-seed-parity.test.ts` | Both seeds are insert-only and skip an org that already has a curated list. |
| T4255 | `tests/simulation/skill-catalog.test.ts` | **`?orgId=` is honoured for a SuperAdmin and ignored for everyone else** — a URL is not a permission. |
| T4256 | `tests/simulation/skill-catalog.test.ts` | Every write carries the target org, so a SuperAdmin cannot edit the wrong org's list. |
| T4257 | `tests/simulation/skill-catalog.test.ts` | The screen names the org whose list is shown, and offers the picker. |
| T4258 | `tests/simulation/skill-seed-parity.test.ts` | **Every skill a shipped example uses is in the seed, spelled the same way** — the seed shipped "Negotiation" beside the example's "Offer Negotiation". |
| T4259 | `tests/simulation/skill-seed-sql.test.ts` | **The seed SQL actually EXECUTES** against the database and rolls back — the regex-based parity test passed on a file missing every column comma. |
| T4260 | `tests/simulation/skill-seed-sql.test.ts` | Every seeded row carries a description. |
| T4261 | `tests/simulation/skill-catalog.test.ts` | A new skill can be given a description (the add form took name + category only). |
| T4262 | `tests/simulation/skill-catalog.test.ts` | An existing skill's description is editable — and the name is NOT, since references are by name. |
| T4263 | `tests/simulation/skill-catalog.test.ts` | A skill with no description is flagged rather than looking complete. |
| T4264 | `tests/simulation/skills-wiring.test.ts` | One picker serves both halves of the match — people and tasks draw from one list. |
| T4265 | `tests/simulation/skills-wiring.test.ts` | A skill already on a record shows even when the catalog no longer offers it. |
| T4266 | `tests/simulation/skills-wiring.test.ts` | The team editor says what naming NOBODY means — requirements are granted to anyone. |
| T4267 | `tests/simulation/skills-wiring.test.ts` | Every example names people, except automation pools (and H&O's deliberate counted pool). |
| T4268 | `tests/simulation/skills-wiring.test.ts` | **No example outside Hire & Onboard constrains a task** — step 3's promise. |
| T4269 | `tests/simulation/skills-wiring.test.ts` | Hire & Onboard KEEPS its accreditation constraints. |
| T4270 | `tests/simulation/skills-wiring.test.ts` | **Naming people changes nothing while no task requires a skill** — what makes step 3 safe. |
| T4271 | `tests/simulation/skills-wiring.test.ts` | A requirement is met by its holders and queues when they are busy, though the team is idle. |
| T4272 | `tests/simulation/skills-wiring.test.ts` | A requirement nobody holds never starts, rather than falling through. |
| T4273 | `tests/simulation/skills-wiring.test.ts` | **The engine passes a `key`** — without it the pool tracks headcount but not the individual, and the same person is handed out twice. |
| T4274 | `tests/simulation/archimate-capability-skills.test.ts` | **A person's Capabilities are their skills** — the pattern Paul specified. |
| T4275 | `tests/simulation/archimate-capability-skills.test.ts` | **A Business Role is a JOB and contributes no skill** — the inversion of the old reading. |
| T4276 | `tests/simulation/archimate-capability-skills.test.ts` | A team is read as a team and never offered as a person. |
| T4277 | `tests/simulation/archimate-capability-skills.test.ts` | An organisational capability is not somebody's skill. |
| T4278 | `tests/simulation/archimate-capability-skills.test.ts` | An unstereotyped model still yields skills (tolerance), and a marked one honours the marks. |
| T4279 | `tests/simulation/archimate-capability-skills.test.ts` | A capability bundle resolves to its leaves, not to itself. |
| T4280 | `tests/simulation/archimate-capability-skills.test.ts` | An association drawn the other way round still reads — Association carries no direction in meaning. |
| T4281 | `tests/simulation/archimate-capability-skills.test.ts` | **A legacy Role-as-skill model still reads, and is LABELLED legacy** — customer models are not invalidated. |
| T4282 | `tests/simulation/archimate-capability-skills.test.ts` | The capability pattern reads NO task requirements — the user chooses those (step 5). |
| T4283 | `tests/simulation/hire-and-onboard.test.ts` | The operating model supplies who can do what and never what the work requires (step 5). |
| T4284 | `tests/simulation/hire-and-onboard.test.ts` | **A POST is not a skill** — "Accredited Vetting Officer" is a role Grace holds; "Compliance Accreditation" is what she can do. |
| T4285 | `tests/simulation/hire-and-onboard.test.ts` | Teams are READ from the model's aggregation, and a team is never offered as a person. |
| T4286 | `tests/simulation/hire-and-onboard.test.ts` | An organisational capability hung off a team is nobody's skill. |
| T4287 | `tests/simulation/hire-and-onboard.test.ts` | **Every has-skill association is LABELLED** — the reader does not need it, the human does; omitted on the first redraw. |
| T4288 | `tests/simulation/archimate-stereotype-editing.test.ts` | An ArchiMate element can be given a stereotype in Properties (was UML-gated). |
| T4289 | `tests/simulation/archimate-stereotype-editing.test.ts` | An ArchiMate relationship can be given a label — "has skill" had no field at all. |
| T4290 | `tests/simulation/archimate-stereotype-editing.test.ts` | **A stereotype survives layout and reads back** — layout kept only `shapeKey`, silently dropping it. |
| T4291 | `tests/simulation/archimate-stereotype-editing.test.ts` | A stereotype is DRAWN in guillemets, not merely stored. |
| T4292 | `tests/simulation/skills-wiring.test.ts` | **Two holders → one makes the QUEUE worse while the team looks no busier** — run against the shipped example. |
| T4293 | `tests/simulation/skills-wiring.test.ts` | If the last holder loses the skill the work never starts, rather than falling through. |
| T4294 | `tests/simulation/skill-holder-counts.test.ts` | **The holder count follows the team the ENGINE would use** — a task inherits its lane's team, walking up nested containers. |
| T4295 | `tests/simulation/skill-holder-counts.test.ts` | No resolvable team means no count, not a count of nought — "nobody can" and "nobody said who" are different facts. |
| T4296 | `tests/simulation/skill-holder-counts.test.ts` | The count is shown in the dropdown before choosing, not only on the chip after. |
| T4297 | `tests/simulation/skill-holder-counts.test.ts` | **A bundle counts zero** — people hold its leaves, never its name; the reader reports it as a bundle. |
| T4298 | `tests/simulation/skill-holder-counts.test.ts` | The ArchiMate fill NAMES its bundles rather than flattening them silently. |
| T4299 | `tests/simulation/business-case-guards.test.ts` | A business-case side that completed nothing is FLAGGED, not treated as free work. |
| T4300 | `tests/simulation/business-case-guards.test.ts` | **No saving, percentage or payback is computed from an absent premise** — "100% reduction, payback immediate" was arithmetic on nothing. |
| T4301 | `tests/simulation/business-case-guards.test.ts` | The written case LEADS with the blocker, before any figure. |
| T4302 | `tests/simulation/business-case-guards.test.ts` | A normal comparison is untouched — the refusal must not fire on healthy data. |
| T4303 | `tests/simulation/business-case-guards.test.ts` | The prompt states which fields are MONEY and which are hours — £1,437.76 was reported as "1,437.76 hours". |
| T4304 | `tests/simulation/business-case-guards.test.ts` | The case has room to finish (was 700 tokens) and says so when it does not. |
| T4305 | `tests/diagram/connector-id-uniqueness.test.ts` | **Two transitions between the same pair of states get different ids** — the second was a ghost from the moment it was drawn. |
| T4306 | `tests/diagram/connector-id-uniqueness.test.ts` | A plan whose connections omit `type` is not doubled — the reconciler's guard failed OPEN. |
| T4307 | `tests/diagram/connector-id-uniqueness.test.ts` | The loosened guard still ignores a genuinely non-transition connection. |
| T4308 | `tests/diagram/connector-id-uniqueness.test.ts` | The whole curated shape survives layout with unique, resolvable connectors. |
| T4309 | `tests/diagram/connector-id-uniqueness.test.ts` | **Uniqueness must not cost determinism** — two identical layout calls give identical ids. |
| T4310 | `tests/ai/no-env-var-advice.test.ts` | **No user-facing text tells a reader to set a server env var** — advice they cannot follow; the same sentence was in 11 places. |
| T4311 | `tests/ai/no-env-var-advice.test.ts` | The advice points at something the reader can do — their own key, or an administrator. |
| T4312 | `tests/ai/new-ai-generate-console.test.ts` | **Every console tone is DERIVED from the configured AI colour** — a hardcoded tone looks right for violet and is invisible for anything else. |
| T4313 | `tests/ai/new-ai-generate-console.test.ts` | The accent is read from the feature-colour scheme and reaches the backdrop, the tones and the structure editor. |
| T4314 | `tests/ai/new-ai-generate-console.test.ts` | The renamed labels are the ones asked for — Transcript, Load Attachment, Load Image, Re-plan (not "Re-send to AI"). |
| T4315 | `tests/ai/new-ai-generate-console.test.ts` | **Load Image and Load Attachment are two choosers over ONE reader** — how a file is parsed must not depend on which button opened the dialog. |
| T4316 | `tests/ai/new-ai-generate-console.test.ts` | Close is at the right after the User Guide chip, and a dirty full-screen console confirms before discarding. |
| T4317 | `tests/ai/new-ai-generate-console.test.ts` | Save / New sit under the description, Refine under them, and Refine never auto-generates. |
| T4318 | `tests/ai/new-ai-generate-console.test.ts` | The JSON / structure editor is the SHARED pop-up, not a second copy that would drift. |
| T4319 | `tests/ai/new-ai-generate-console.test.ts` | **Prompt provenance is sent on CREATE only** — tidying the wording of something you dictated does not make it typed. |
| T4320 | `tests/ai/new-ai-generate-console.test.ts` | Answering the AI Tidy questions plans immediately — the only path that generates without a Plan click. |
| T4321 | `tests/ai/new-ai-generate-console.test.ts` | **`preservePositions` is derived from the PLAN's bounds**, not from the attachment still being attached. |
| T4322 | `tests/ai/new-ai-generate-console.test.ts` | The dictation stop-race guard survived — a Stop during startup must not leave a live microphone. |
| T4323 | `tests/ai/new-ai-generate-console.test.ts` | A failed plan's raw reply is kept and opens on Raw edit — the error alone says nothing about why. |
| T4324 | `tests/ai/new-ai-generate-console.test.ts` | The elevated-options modal renders NOTHING when neither gate is open, rather than listing what you may not do. |
| T4325 | `tests/ai/new-ai-generate-console.test.ts` | **Compare/export is SuperAdmin; model + prompt-from-diagram is Admin** — one blanket gate would hand four live generations to every org admin. |
| T4326 | `tests/ai/new-ai-generate-console.test.ts` | The new console is mounted BESIDE PlanPanel, for BPMN, behind its own button — both are live. |
| T4327 | `tests/ai/new-ai-generate-console.test.ts` | **PlanPanel shares no orchestration with the new console** — the comparison is worthless if one change moves both. |
| T4328 | `tests/ai/new-ai-generate-console.test.ts` | `PlanStructureModal`'s accent still defaults to the diagram-type colour, so the sidebar's use is unchanged. |
| T4329 | `tests/ai/new-ai-generate-console.test.ts` | **The NEW AI Generate button is coloured AT REST** — gating it on "active" made the true branch unobservable, because the console it opens covers the button. |
...
| T4350 | `tests/ai/fable-5-1.test.ts` | **Fable 5.1 and Fable 5 are two catalogue entries with two ids** — dateless ids are pinned snapshots; "Fable 5" never quietly becomes 5.1. |
| T4351 | `tests/ai/fable-5-1.test.ts` | Fable 5.1 is priced (ties with Fable 5), so an ordinary user can pick it — not only a SuperAdmin. |
| T4352 | `tests/bpmn/ep-siblings-no-overlap.test.ts` | **Two sibling Expanded Subprocesses never intersect, and each is DRAWN in its own lane** — Paul&#39;s Meeting Planner plan; R8.26 put the top-vertex EP&#39;s bottom ON the gateway line while the right-vertex EP straddled it (439×70px), and left "Invoice Customer" above the Accountant band. |
| T4353 | `tests/bpmn/ep-siblings-no-overlap.test.ts` | Three parallel EP arms off one gateway take separate rows — the mechanism isolated (two arms never showed it). |
| T4354 | `tests/bpmn/ep-siblings-no-overlap.test.ts` | A lone EP with a downstream task keeps its right-shift layout — the fix must not invent rows it never needed. |
| T4355 | `tests/bpmn/ep-siblings-no-overlap.test.ts` | **A branch EP in ANOTHER lane is drawn in that lane, and no lane carries an empty band** — R8.26 placed a cross-lane branch on another lane&#39;s gateway line and pulled EPs off their rows without re-hugging (267px empty band; Paul: "pushed downwards", "No joy"). |
| T4356 | `tests/ai/new-ai-generate-console.test.ts` | **The console reports NOT busy on unmount, for both signals** — closing on apply orphaned the editor&#39;s busy flag: spinning logo, "Running the layout engine", forever. |
| T4357 | `tests/ai/new-ai-generate-console.test.ts` | A successful apply clears busy BEFORE it closes — the `finally` ran on an unmounted component. |
| T4358 | `tests/ai/new-ai-generate-console.test.ts` | The editor&#39;s canvas overlay fails closed: it needs a mounted AI panel behind it. |
| T4359 | `tests/bpmn/laneless-pool-no-duplicates.test.ts` | **A lane-less white-box pool keeps its own elements** — Paul&#39;s transcript plan: the compounder&#39;s five elements were drawn TWICE with the same ids (once in the main pool&#39;s first lane via the "unassigned" fallback, once in their own pool). |
| T4360 | `tests/bpmn/laneless-pool-no-duplicates.test.ts` | The isolated shape: a second white-box pool with no lanes — no duplicates, elements in their own pool. |
| T4361 | `tests/bpmn/laneless-pool-no-duplicates.test.ts` | A GENUINELY unassigned element (no pool at all) still lands in the first white-box pool&#39;s first lane — the fallback keeps working. |
| T4362 | `tests/ui/project-menu-consolidation.test.ts` | **The Project ▾ menu holds the five moved items** (SOP Templates, Resources, Org Owner, Standard Operating Procedures, Entity Structure — renamed from Project Structure) and the old header/sidebar placements are gone. |
| T4363 | `tests/ui/project-menu-consolidation.test.ts` | The menu renders for VIEWERS; only the write actions are gated inside — moving viewer-visible items into an editor-only menu would have taken them away. |
| T4364 | `tests/ui/project-menu-consolidation.test.ts` | Both popups open from the menu and are mounted as dialogs with the edit gate. |
| T4365 | `tests/ui/project-menu-consolidation.test.ts` | `ListPopup`: a scrollable list with Continue bottom-right in a footer OUTSIDE the scroll region. |
| T4366 | `tests/ui/project-menu-consolidation.test.ts` | Both dialogs are built on the shared popup, not two copies; the structure dialog carries the new name. |
| T4367 | `tests/ui/project-menu-consolidation.test.ts` | **Risk &amp; Controls is in the Project menu too**, as a launcher popup (current risks and controls + the console&#39;s options); the header button and sidebar row are gone. |
| T4368 | `tests/ui/project-menu-consolidation.test.ts` | Project → Configuration covers EPC: an EPC colours tab, all fifteen EPC objects colour-editable (junctions stay white), and the typography note says it applies to EPC. |
| T4369 | `tests/ui/project-menu-consolidation.test.ts` | **An SOP says who generated it, when and with which model** — the list route returns provenance and the popup shows it ("they seem to be there without me creating them?"). |
| T4370 | `tests/pcf/framework-edit.test.ts` | **An APQC framework's name, variant and version are independent, editable fields** — `name` no longer silently overwrites `variant`, and version was not editable at all. |
| T4371 | `tests/pcf/framework-edit.test.ts` | A field sent blank is refused; division is tailored-only and may be cleared; unknown keys are not written. |
| T4372 | `tests/pcf/framework-edit.test.ts` | The edit persists field by field through Prisma; an edited version label leaves `familyKey`/`isCurrent` alone (upgrade pairing keys on the family). |
| T4373 | `tests/pcf/framework-edit.test.ts` | The admin Edit dialog offers Name, Variant, Version (+ Division for tailored), sends only what changed; the single-field Rename is gone; the route uses the pure helper. |
| T4374 | `tests/ui/tree-filter.test.ts` | **The navigation tree filter** — an empty filter matches everything. |
| T4375 | `tests/ui/tree-filter.test.ts` | Name is a case-insensitive substring; diagram type is exact. |
| T4376 | `tests/ui/tree-filter.test.ts` | Ticked badges are ANDed (has AI + has SI narrows), and the facets combine. |
| T4377 | `tests/ui/tree-filter.test.ts` | The tree filters in the ONE function every folder's list goes through; the control sits beside Sort with a one-click clear; an emptied folder says "filtered out". |
| T4378 | `tests/ui/superadmin-gates.test.ts` | **`tileVisibleTo` is an exact-email gate, narrower than SuperAdmin** — another SuperAdmin is exactly who it excludes. |
| T4379 | `tests/ui/superadmin-gates.test.ts` | The Fun tiles' owners list is paul@nashcc.com.au alone; every `funExtensions` tile carries it; the grid applies it once; the email comes from the session. |
| T4380 | `tests/ui/superadmin-gates.test.ts` | NEW AI Generate is behind `isActingAdmin` (presenting as a customer hides it); the original AI Generate button is untouched. |
| T4381 | `tests/entity-lists/entity-drift.test.ts` | **The Entity Drift button is offered on BPMN only** — the gate is `=== "bpmn"`, not merely `!== "epc"`; on any other type the check could only ever answer "no drift". |
| T4382 | `tests/ui/model-select-cost.test.ts` | **The model picker shows no cost unless `showCost` is set** — rendered markup carries labels only by default, prices only when asked (a SuperAdmin detail). |
| T4383 | `tests/ui/model-select-cost.test.ts` | The Properties-panel picker takes its cost flag from `isActingAdmin` (a customer view mode shows what a normal user sees — no prices); the three SuperAdmin-gated pickers keep `showCost`. |
| T4384 | `tests/infra/deploy-readiness.test.ts` | **`checkHealth` is a readiness answer** — 200 when the injected DB ping resolves, 503 with the error when it rejects, 503 "timed out" when it hangs; the commit is reported either way. |
| T4385 | `tests/infra/deploy-readiness.test.ts` | `GET /api/health` answers 200 against the real database, `Cache-Control: no-store`, and is NOT under a protected prefix (App Service's probe has no session). |
| T4386 | `tests/infra/deploy-readiness.test.ts` | Deploy workflow: no `az webapp restart` command after container set (it was the second stop); a guarded "Ensure the App Service health check path" step; the smoke test polls `/api/health` and requires THIS commit in the body; Dockerfile/build pass `GIT_COMMIT_SHA` through. |
| T4387 | `tests/infra/deploy-readiness.test.ts` | **The dashboard looks each user up once** — one lookup when not impersonating, two (target then signed-in) when impersonating, stale-target fallback clears the cookie and stops claiming to impersonate. |
| T4388 | `tests/diagram/assist-undo-confirm-selection.test.ts` | **The history-group gate** admits every push outside a group and only the FIRST inside; nesting ends once; an empty group leaves nothing behind. |
| T4389 | `tests/diagram/assist-undo-confirm-selection.test.ts` | `pushHistory` asks the gate first; `beginHistoryGroup`/`endHistoryGroup` are exported; `runVoiceCommand` applies only through `applyGrouped` — **one spoken command, one undo**. |
| T4390 | `tests/diagram/assist-undo-confirm-selection.test.ts` | Every `moveElements` in `applyAssistOps` is followed by `elementsMoveEnd()` — a voice move/nudge is committed (undo entry + connectors re-routed) instead of leaving a drag half-open. |
| T4391 | `tests/diagram/assist-undo-confirm-selection.test.ts` | **The working set**: `syntheticElement`/`withAdded`/`withDeleted`/`withLabel` are pure; the editor keeps a working copy so "add X and connect it to Y" resolves X mid-batch. |
| T4392 | `tests/diagram/assist-undo-confirm-selection.test.ts` | **Destructive commands ask first** — clear, container deletes (naming what is inside), multi-selection deletes, delete+compact; a single named element does not; yes/no/other parsed; the editor parks and re-applies on "yes". |
| T4393 | `tests/diagram/assist-undo-confirm-selection.test.ts` | **The selection is a reference** — this/these/the selection/the selected <kind> resolve to the mouse selection, a kind filter narrows it, several → ambiguous for one-target ops, nothing selected falls back; `[selected]` travels to the AI. |
| T4394 | `tests/diagram/assist-suggestion-seed.test.ts` | **The Assist "Suggestion" chip has a real target** — the intent row is RENAMED in place (RENAMES, not a duplicate), names the "Send Notification" template, and that template exists in the seed as a send task + message catch; the builder carries `taskType` through. |
| T4395 | `tests/diagram/assist-command-catalog.test.ts` | **The Commands reminder card cannot lie** — every non-voice example in `commandCatalog.ts` parses deterministically (the card behind the bar's Commands button and the SuperAdmin tile). |
| T4396 | `tests/diagram/assist-command-catalog.test.ts` | The catalogue's Voice words (stop / yes / no) are mic controls, not commands — the grammar must not parse them. |
| T4397 | `tests/diagram/assist-cost-and-commands.test.ts` | **Cost so far** — AI calls priced from the rate catalogue (cache read at a tenth, write at 1.25×), Deepgram voice by the minute, browser voice free, the open mic added live, unpriced models named, foreign rows ignored; `formatCostReport` wording. |
| T4398 | `tests/diagram/assist-cost-and-commands.test.ts` | The bar has Commands + Cost, the reminder card scrolls and drags by its title bar, the cost is labelled an estimate, the editor records when the mic opened and calls `/api/ai/command/usage`, and the route sums only this feature's rows. |
| T4399 | `tests/diagram/assist-cost-and-commands.test.ts` | **SuperAdmin tile "Voice Assist Commands"** — advertised in AdminClient, its page exists (reachable), is SuperAdmin-guarded, renders the same catalogue and links to what is editable. |
| T4400 | `tests/diagram/assist-review-items.test.ts` | **Nudge is 20 px** — "nudge/bump <any ref> up" parses as the small step, "move" as the element-span; the reducer translates a task in a lane by exactly dy and committing the move changes no geometry. |
| T4401 | `tests/diagram/assist-review-items.test.ts` | **The first word is no longer lost** — the PCM queue holds chunks until the socket opens, drains in order, bounds memory (oldest dropped); the mic opens in parallel with the token, capture is wired before onopen, onReady tells the bar to show "connecting…" then "listening…". |
| T4402 | `tests/diagram/assist-review-items.test.ts` | **"stop" means one thing** — mic stop words vs flow end words never overlap; spoken and typed "stop" stop the mic and drop any pick or parked question; the rename loop ends on "done"/"cancel" and says so. |
| T4403 | `tests/diagram/assist-review-items.test.ts` | The bar drags by its header (and sits bottom-centre until it is); Palette and PropertiesPanel take force-collapse props that remember and restore the user's state; the editor passes `voiceAssistOn`. |
| T4404 | `tests/diagram/assist-review-items.test.ts` | The Commands card says everything on it is instant — no dangling "green commands" reference. |
| T4405 | `tests/diagram/assist-review-items.test.ts` | **Messages by number** — bare: everything the message rule (canConnect.ts) lets a message start or end at, here the tasks, the subprocess and the black-box pool (never a white-box pool or gateway); selected task → the black-box pool, selected pool → the tasks/subprocess, each with the answer forms that can work (`dirs`); neither → error; answers "3 to 7 labelled X" / "to 2 labelled X" / "from two, called Y" parse, one-sided pair does not. |
| T4406 | `tests/diagram/assist-review-items.test.ts` | The grammar routes "add a message" / "… to the selected" to `addMessageByNumber` (the explicit form and the AI bail unchanged), `validateOps` keeps it, `#id:` refs resolve exactly, the editor feeds the flow, shares the badges and applies through the group; the card lists both forms. **Updated 2026-09-26:** the badges prop is now `renameBadges={onScreenBadges}`, from `badgesOnScreen(renameFlow, messageFlow, pickFlow)` — the same value the voice-debug recording saves (T4886, T4893) — so the pin checks that wiring and, behaviourally, that the rename, message and picker targets all reach it. |
| T4407 | `tests/diagram/assist-review-items.test.ts` | **A pool around everything brings no lane** — WRAP_IN_POOL makes one white-box pool, zero lanes, adopts the loose elements into the pool itself, padded around them. |
| T4408 | `tests/diagram/assist-badges-and-selection.test.ts` | **Where the green numbers sit** — `badgePlaceFor`: activities below, events above, pools/lanes in the header; both collectors (rename, message) stamp it. The message collector needs two participants (a white-box pool and a black-box pool), since a message runs between pools. |
| T4409 | `tests/diagram/assist-badges-and-selection.test.ts` | The canvas honours the place: above for events; for pools/lanes at the header's centre x just past the START of the rotated, centred name (cy + width/2), measured in the header's own face so it moves with the name. |
| T4410 | `tests/diagram/assist-badges-and-selection.test.ts` | **Selection protocol** — rename by number (after the name), plain rename, move, nudge and lane move all leave nothing selected. |
| T4411 | `tests/diagram/assist-nudge-label-swap.test.ts` | **Nudge in all four directions, 20 px, only the named element** — grammar accepts up/down/left/right (and "to the right"), the reducer moves the element plus its boundary events and nothing else, the editor computes dx as well as dy. |
| T4412 | `tests/diagram/assist-nudge-label-swap.test.ts` | **A selection nudges together (20 px) and moves together (100 px per step)**, then nothing stays selected. |
| T4413 | `tests/diagram/assist-nudge-label-swap.test.ts` | **"label selected <text>"** labels the selected connector and deselects; bare "label selected" opens a single-item name phase that ends the flow; **"label connectors / messages / tasks"** = rename by number. |
| T4414 | `tests/diagram/assist-nudge-label-swap.test.ts` | **"swap top and bottom"** grammar — EVERY ordered pair of top/bottom/middle(centre)/left/right (60 phrasings enumerated), no-op refused (grammar and validateOps); the lane swap still parses. |
| T4415 | `tests/diagram/assist-nudge-label-swap.test.ts` | The editor swaps on EVERY selected gateway, OUTGOING points on a decision and INCOMING on a merge, naming any gateway that lacks a connector at a point ("middle" = the flow side) through `updateConnectorEndpoint`; the reducer re-attaches each end to its new side with targets untouched. |
| T4416 | `tests/diagram/assist-fragments.test.ts` | **A command split at a pause waits for the rest** — a lone verb ("Swap."), a dangling connective or a half rename/connect is incomplete; whole commands (incl. the by-number "add a message" and "rename tasks") are not; the editor imports the pure module. |
| T4417 | `tests/diagram/assist-fragments.test.ts` | A comma straight after the verb is a breath, not syntax ("Swap, top and bottom." parses); commas inside a name list still separate names. |
| T4418 | `tests/diagram/assist-fragments.test.ts` | The AI fallback's prompt lists the gateway swap (and says it is NOT a lane swap), nudge directions, label-selected, label/rename-by-number and their op shapes. |
| T4419 | `tests/diagram/assist-surround-subprocess.test.ts` | "surround selected with an expanded subprocess called X" (and "wrap these in a subprocess", "put an expanded subprocess around …") parses with its label; "unwrap / dissolve the selected subprocess" parses; "delete selected" stays a delete; a bare "surround selected" is held for the rest; "the selected subprocess / expanded subprocess / EP" resolve as references. |
| T4420 | `tests/diagram/assist-surround-subprocess.test.ts` | The wrap plan: the group moves 90 px right and becomes the EP's children exactly as it was, everything right of it in the SAME lane moves 180 px, the other lane and the left do not move, the EP sits in the lane around the group, a Start faces the entry and an End faces the exit on the centre line, the one flow in / one flow out are re-pointed at the EP, the inner flow is kept; guards for nothing selected, a container, two lanes, not one-in-one-out, and an unselected element inside the area; the reducer applies the plan and ignores a failed one. |
| T4421 | `tests/diagram/assist-surround-subprocess.test.ts` | "Delete selected" on the EP is the exact reverse — every x, y and parent restored and the flow re-spliced (round trip, twice, no undo); a foreign EP without Start/End re-enters at the leftmost and leaves from the rightmost and still gives its room back; a non-EP and an empty EP are refused by message. |
| T4422 | `tests/diagram/assist-surround-subprocess.test.ts` | Editor wiring: both op handlers, delete routing an EP with contents to the unwrap, nothing left selected, the two reducer cases, the AI prompt's op shapes and the Commands card examples. |
| T4423 | `tests/config/public-page-db-access.test.ts` | Every page or route outside `app/api` that touches the database calls `auth()`, or is named in a short allow-list of deliberately public pages. SEC-28: `/test/db` read the whole User table with no auth and was reachable anonymously on prod. |
| T4424 | `tests/config/public-page-db-access.test.ts` | The deleted `app/test` scratch tree has not returned, and no file under `app/` hard-codes a Postgres connection string (two of those pages did). |
| T4425 | `tests/diagram/rich-text-sanitiser.test.ts` | SEC-26: the sanitiser parses untrusted HTML in a document with no browsing context, so a resource handler cannot fire while parsing. Pinned at source because the browser branch needs a DOM the node environment does not provide. |
| T4426 | `tests/diagram/rich-text-sanitiser.test.ts` | The sanitiser still strips disallowed tags and every attribute, keeps whitelisted tags, and preserves the text inside a stripped tag. |
| T4427 | `tests/ai/user-key-base-url.test.ts` | SEC-24: a bring-your-own AI endpoint pointing at loopback, private, carrier-grade-NAT, link-local or metadata addresses is rejected, as is plain http, a bad URL, and credentials embedded in the URL. |
| T4428 | `tests/ai/user-key-base-url.test.ts` | A customer public gateway is still accepted, including a non-default port and addresses just outside each private range. |
| T4429 | `tests/ai/user-key-base-url.test.ts` | The endpoint is validated on save AND re-checked at the point of use, so a row written before the guard existed is dropped rather than honoured. |
| T4430 | `tests/templates/import-thumbnail-regenerated.test.ts` | SEC-22: template import regenerates the preview from the template data and never stores the uploaded one, unconditionally rather than as a fallback. |
| T4431 | `tests/templates/import-thumbnail-regenerated.test.ts` | Both preview render sites still use dangerouslySetInnerHTML, which is why the write path is the place the trust boundary is enforced. |
| T4432 | `tests/security/wave-g-access-control.test.ts` | SEC-27: the reviewer grant is scoped to a live round (an allow-list, so an unknown status withholds access) and excludes a reviewer who declined. Reviewer rows are kept for history, so an unscoped match granted permanent write access to a closed review. |
| T4433 | `tests/security/wave-g-access-control.test.ts` | SEC-23: the cross-org gate runs over the FINAL bundle audience, after email-resolved users are folded in, and runs again when a pending invitation is promoted (the only point at which the invitee exists). |
| T4434 | `tests/security/wave-g-access-control.test.ts` | SEC-25: registration normalises the address before both the duplicate check and the create, matching the lowercased lookup login performs. |
| T4435 | `tests/simulation/engine-gateway-and-stall.test.ts` | SIM-02: a decision split with no probability, condition or default on ANY branch is drawn evenly from the seeded generator, matching what the readiness check promises the modeller; a configured default flow still wins. |
| T4436 | `tests/simulation/engine-gateway-and-stall.test.ts` | SIM-03: the runaway guard watches the clock as well as the token population, so a zero-duration cycle stops and is reported as a stall instead of hanging the run thread. |
| T4437 | `tests/simulation/bpsim-import-linear.test.ts` | IO-11: the BPSim block scanner is anchored — open tags found independently, each close searched from that offset, no lazy middle spanning to the close tag and no per-tag slice of the remainder. Structural rather than timing: see the file docblock for why, and for the measurements that established the defect. |
| T4438 | `tests/simulation/bpsim-import-linear.test.ts` | A malformed BPSim document still parses without throwing and yields nothing useful, with a loose elapsed-time canary. |
| T4439 | `tests/diagram/server-writers-bump-version.test.ts` | DATA-33: every raw SQL statement in app/ that writes Diagram.data also bumps version, so an editor holding a stale version cannot silently revert a server-side change. Found three writers a manual grep had missed. |
| T4440 | `tests/diagram/server-writers-bump-version.test.ts` | DATA-33: the four ORM writers of Diagram.data increment version as well. |
| T4441 | `tests/diagram/server-writers-bump-version.test.ts` | ENG-20: the reproduce-original-layout branch rejects an element naming itself as its parent, since it returns before the containment-cycle passes run. |
| T4442 | `tests/diagram/server-writers-bump-version.test.ts` | CANVAS-10: the canvas coordinate helper and every group-drag handler read pan/zoom through refs, so a memoised renderer cannot hold a viewport-stale closure. |
| T4443 | `tests/diagram/assist-wrap-container.test.ts` | The grammar takes wrap-the-selection for a pool or a lane, with a name, and with the homophones the recogniser returns (poll/pull for pool, line for lane); wrap-everything-in-a-pool and the subprocess wrap keep their own meanings. |
| T4444 | `tests/diagram/assist-wrap-container.test.ts` | A pool wraps loose elements and re-parents them, and REFUSES when a sequence flow would cross the boundary (naming what is on the other side) or when the elements are already in a pool. |
| T4445 | `tests/diagram/assist-wrap-container.test.ts` | A lane becomes a full-width band in the selection own pool, leaves elements outside its vertical extent alone, refuses to sweep in an unselected neighbour level with the selection, and refuses entirely outside a pool. |
| T4446 | `tests/diagram/assist-wrap-container.test.ts` | Both refuse an empty selection, a selection containing a container, and one spread across two parents. |
| T4447 | `tests/diagram/assist-spoken-number.test.ts` | A numbered pick reads a digit, a number word, or a filler prefix, keeping the rest as the name. |
| T4448 | `tests/diagram/assist-spoken-number.test.ts` | Known mishearings of a number resolve — lane, line, won and juan all pick item 1 — and the correction is reported. |
| T4449 | `tests/diagram/assist-spoken-number.test.ts` | Only the LEADING token is rewritten, so picking item 5 and naming it Lane Manager is untouched, and a full command is not mistaken for a pick. |
| T4450 | `tests/diagram/assist-spoken-number.test.ts` | Both halves are wired: the pick handler uses the shared reader, and the number words are boosted at least as strongly as `lane`, which is what caused the confusion. |
| T4451 | `tests/diagram/label-edit-affordances.test.ts` | The dashed label box is hidden while that label is being edited — on elements (events, gateways, data objects and stores) and on connectors — and no copy of the old condition survives to keep one border showing. |
| T4452 | `tests/diagram/label-edit-affordances.test.ts` | Every SymbolRenderer render pass is told whether the label being edited is its own, so no pass keeps its dashed box while you type. Counted against the number of renderers rather than a fixed list. |
| T4453 | `tests/diagram/label-edit-affordances.test.ts` | A double-click opens the label editor on every element that has a label: no handler silently does nothing, and the gateway fans a group selection out first and falls back to the editor only when there is no group to fan. |
| T4454 | `tests/diagram/label-edit-affordances.test.ts` | Double-clicking a connector LINE opens its label editor, not just the small label box, and the request is ignored on first render so a drawn connector does not open an editor by appearing. |
| T4455 | `tests/config/deploy-gates.test.ts` | Every statement that destroys data is recognised in the SQL a deploy is about to run against production: dropping a table, a column, a schema, a database, an enum type, truncating, and retyping a column. Includes the exact text Prisma emits for a removed field. |
| T4456 | `tests/config/deploy-gates.test.ts` | An ordinary additive release passes untouched, and dropping a constraint, a default, a NOT NULL or an index is not treated as destructive — a gate that fires on routine work is a gate someone switches off. |
| T4457 | `tests/config/deploy-gates.test.ts` | Only executable statements count. Prisma's own warning comments talk about dropping columns, and a string literal can contain anything; neither trips the gate, while a real statement after a comment still does. |
| T4458 | `tests/config/deploy-gates.test.ts` | The refusal message names every offending statement, names the override, and says the likeliest cause out loud: a renamed field reads as a drop plus an add. |
| T4459 | `tests/config/deploy-gates.test.ts` | The gate is actually wired into the deploy — it runs before the schema is applied, diffs against the live database rather than an empty one, and neither it nor the push can swallow a failure. The override is reachable only from the manual dispatch input, so no merge can set it. |
| T4460 | `tests/config/deploy-gates.test.ts` | The deploy waits for the test suite: it starts from a completed CI run rather than the push, refuses a run that did not pass, and checks out, tags and smoke-tests the commit CI actually tested rather than whatever the branch tip had become. CI is also barred from passing on an empty run, so a typo in the include glob cannot produce a green tick that ships everything. |
| T4461 | `tests/diagram/label-edit-zoom.test.ts` | Opening the name editor on a wide container now snaps the canvas in. Aimed at the whole box the snap silently did nothing, because it only ever zooms in and a container is already wider than the target. Stated both ways, so the old behaviour is on record as well as the new. |
| T4462 | `tests/diagram/label-edit-zoom.test.ts` | The zoom target is small enough to actually fire, and the editor still fits the screen at the zoom it snaps to. Guards the trap the first attempt fell into: a target at exactly fraction × viewport lands on the current zoom and never moves. |
| T4463 | `tests/diagram/label-edit-zoom.test.ts` | Pools, lanes, data objects and ordinary tasks keep the aim they already had, including a pool whose header bar has been widened. |
| T4464 | `tests/diagram/label-edit-zoom.test.ts` | The zoom maths stays inside its ceiling and its minimum-width floor, refuses to act on an unmeasured viewport, and clamps a malformed stored fraction. Each limit is tested where the others cannot mask it. |
| T4465 | `tests/diagram/label-edit-zoom.test.ts` | The canvas calls the shared planner rather than restating the geometry. Two copies of this rule is how pools came to be handled and expanded subprocesses did not. |
| T4466 | `tests/diagram/assist-selected-word.test.ts` | A misheard "selected" is put back after any of the command verbs that take a target, including all seven Paul named. Covers the filler word, the capitalisation, and the other words that lose the same leading syllable. |
| T4467 | `tests/diagram/assist-selected-word.test.ts` | The repair does not eat a real connect command, and does not touch the word "connector" — renaming a connector by name has to survive a repair aimed at the word inside it. |
| T4468 | `tests/diagram/assist-selected-word.test.ts` | A repaired utterance parses to exactly what the same command parses to when it is heard correctly, rather than merely being rewritten. |
| T4469 | `tests/diagram/assist-selected-word.test.ts` | The recogniser is told to expect "selected" at least as strongly as the word it was losing to. Without that the parser repair is a guess made after the fact. |
| T4470 | `tests/diagram/assist-selected-word.test.ts` | The words that already refer to the selection are recognised, and nothing else is. |
| T4471 | `tests/diagram/label-edit-affordances-2.test.ts` | Every hook in the connector label component is declared before the component can bail out. It returns null on three paths, so a hook below them runs on some renders and not others and React throws when an unlabelled connector is selected. |
| T4472 | `tests/diagram/label-edit-affordances-2.test.ts` | A group with no name can still be reached: there is a hit target on the name strip, not only on the dashed border, and the interior stays click-through so the elements inside remain usable. |
| T4473 | `tests/diagram/label-edit-affordances-2.test.ts` | A pool's header double-click target follows the header's real width instead of the 36px default, which stopped being true the moment anyone widened it. |
| T4474 | `tests/diagram/label-edit-affordances-2.test.ts` | No history entry is recorded for an edit that never opens — the snapshot runs after the types that bail have bailed. |
| T4475 | `tests/diagram/assist-name-case.test.ts` | A renamed item gets a capital on its first word — and the FIRST WORD ONLY, because a process step is a phrase and title case gives "Send To Customer For Approval". Empty and whitespace-only names survive. |
| T4476 | `tests/diagram/assist-name-case.test.ts` | A deliberate capital inside the first word is left alone, so iPhone, eCommerce and mRNA are not turned into IPhone, ECommerce and MRNA. A name starting with a digit or a bracket is left alone too, rather than reaching past it to the next letter. |
| T4477 | `tests/diagram/assist-name-case.test.ts` | Every spoken rename phrasing applies it — rename, relabel, change the name of, call, and label selected — while the REFERENCE is left exactly as spoken, since it has to keep matching what is on the canvas. |
| T4478 | `tests/diagram/assist-name-case.test.ts` | The guided rename reads the number off the utterance as spoken rather than a lower-cased copy, so a one-breath "14 Approve Invoice" no longer arrives with the name flattened before anything can capitalise it. |
| T4479 | `tests/diagram/assist-surround-nested.test.ts` | A selection being surrounded may contain a subprocess: Paul's case of an event plus the expanded subprocess it flows into is accepted, the inner subprocess goes inside the new one with its own contents intact rather than flattened out of it, its contents travel with it, and unwrapping gives the nest back. A pool or a lane is still refused. |
| T4480 | `tests/diagram/assist-surround-nested.test.ts` | The pool and lane wrap agree with surround on what may be selected: a subprocess is allowed in, a swimlane is not. |
| T4481 | `tests/diagram/assist-surround-nested.test.ts` | Selecting a subprocess AND something inside it is not an error — the child is already coming along with its parent, so the redundant pick is ignored and the result matches selecting the parent alone. |
| T4482 | `tests/diagram/assist-gold-flash.test.ts` | "Turn on gold flashing" is heard in either word order, on and off, plus the short forms. It does not swallow "turn on the lights" or a rename of something called Gold Standard, and an AI that returns the op without the on/off flag is rejected rather than guessed at. |
| T4483 | `tests/diagram/assist-gold-flash.test.ts` | Only the commands worth pointing at flash: added, enclosed, moved and nudged. A delete has nothing left to outline and a rename has just had you read a number off that very item, so neither does. |
| T4484 | `tests/diagram/assist-gold-flash.test.ts` | The toggle is off until turned on, is remembered per browser in a form a human can read, and reads as off when storage throws — a private window must never be the reason the editor fails. |
| T4485 | `tests/diagram/assist-gold-flash.test.ts` | Sparks are deterministic per item, so the same item sparks the same way each time and a row of them does not look like noise; two items differ; every spark stays inside sane bounds. |
| T4486 | `tests/diagram/assist-gold-flash-wiring.test.ts` | The two gold-flashing commands are absent from the card users read and present on the SuperAdmin tile, in both word orders. The hidden list is held to the same parse guarantee as the visible one, since a list nobody sees is where a dead phrase survives. |
| T4487 | `tests/diagram/assist-gold-flash-wiring.test.ts` | What the flash outlines, worked out by diffing the diagram before and after rather than trusting thirty op handlers to report themselves. Added and enclosed win over moved, so surrounding a selection does not also point at the dozen neighbours it shoved aside. Sub-pixel drift is ignored and the count is capped. |
| T4488 | `tests/diagram/assist-gold-flash-wiring.test.ts` | One ordering of the subscription tiers, with enterprise counted above expert — the ordering that already existed elsewhere omits it entirely, which would put the top tier below the one beneath it. An unknown name hides a feature rather than granting it. |
| T4489 | `tests/diagram/assist-gold-flash-wiring.test.ts` | Voice Assist is gated on the seeded feature key rather than on being an admin, the gate is enforced on the route and not only in the editor, and a SuperAdmin previewing a lower tier stops seeing it. |
| T4490 | `tests/diagram/assist-gold-flash-fixes.test.ts` | Gold sparks leave the item rather than the diagram's top-left corner. A CSS transform on an SVG shape takes its origin from the SVG user space and not the shape, so the scale that was in the keyframe dragged every spark toward the origin; only the rings were being seen, which is why it still read as working. |
| T4491 | `tests/diagram/assist-gold-flash-fixes.test.ts` | A rename flashes, from either path. The guided rename flow writes labels through its own code and never touches the op batch, so it arms the flash itself through a shared helper; and the before/after snapshot carries the label at both ends, since a rename moves nothing and would otherwise be invisible to the diff. |
| T4492 | `tests/diagram/assist-gold-flash-fixes.test.ts` | Switching gold flashing on or off works while a numbered pick is open. Every other utterance during a pick is a number, a name or "done", so the exception is checked before the pick swallows it — otherwise the words become the new name for whichever item was picked. |
| T4493 | `tests/diagram/assist-gold-flash-fixes.test.ts` | "Ten on gold flashing" is repaired to "turn". Only at the start of an utterance and only before on/off, a shape no sentence with a real ten has, so a numbered pick and "add ten lanes" are untouched. Number words are no longer boosted at the recogniser at all, which is what made it reach for "ten" in the first place. |
| T4494 | `tests/diagram/gold-flash-trigger.test.ts` | An Voice Assist edit hands React a new elements array, so the effect that computes the flash actually runs — and a no-op move correctly returns the same state, which is why that effect must not be the only thing the feature depends on. |
| T4495 | `tests/diagram/assist-wrap-messages.test.ts` | An expanded subprocess in the selection is accepted by both the subprocess wrap and the lane wrap, while a pool or a lane in the selection is still refused — the distinction being that a swimlane says who does the work and so cannot go inside a step of it. |
| T4496 | `tests/diagram/assist-wrap-messages.test.ts` | A refused wrap names what is actually in the way: both homes when the selection is spread across two, and the pool by name plus the way forward when a pool would nest. The old wording read as a complaint about the containers in the selection, which is what sent Paul looking in the wrong place. |
| T4497 | `tests/diagram/move-set-boundary.test.ts` | Nudging a pool no longer drags off the events mounted on a subprocess elsewhere. Paul's repro: five events carried a parentId of Pool 1 and a boundaryHostId of the Document Handling subprocess, so the container rule swept them along while their host stood still. |
| T4498 | `tests/diagram/move-set-boundary.test.ts` | What a move still takes with it: a container's contents, everything mounted on what is moving, a chain of mountings, and a boundary event the user picked outright. A mounting cycle does not hang it. |
| T4499 | `tests/diagram/move-set-boundary.test.ts` | The reducer really uses the shared rule, driven through MOVE_ELEMENTS on the shape from Paul's own export — the same path a mouse drag takes, which had the identical defect and nobody had noticed. |
| T4500 | `tests/diagram/move-set-boundary.test.ts` | The mismatched parentage behind it can be found: an edge-mounted element whose parent is neither its host, nor inside it, nor around it. Quiet about the ordinary cases. |
| T4501 | `tests/diagram/move-set-boundary.test.ts` | A container moves what it is drawn around, not what claims it as a parent. Paul's elements sat well above a 78px Pool 1 while naming it as their parent, so nudging the pool moved the whole diagram. Measured from the centre, so an element whose corner pokes out still counts as inside. |
| T4502 | `tests/diagram/assist-wrap-fragment.test.ts` | The selection wrap is understood with its leading verb dropped, which is how the recogniser delivered it. Without that the fragment reached the AI, came back as the whole-diagram wrap, and re-homed an entire diagram into an existing pool. The whole-diagram wrap still means what it meant. |
| T4503 | `tests/diagram/assist-wrap-fragment.test.ts` | Growing an existing pool to adopt every loose element asks first, naming the pool and the count. It destroys nothing, so no other guard would have stopped it, and it is one dropped word away from restructuring a diagram. A brand-new pool is not worth asking about. |
| T4504 | `tests/diagram/wrap-pool-sizing.test.ts` | Adopting loose elements into an existing pool actually resizes it around them, leaving room for the name strip and never cropping it around what it already held. Lanes are re-tiled end to end, including when the growth happens in the middle of the stack. |
| T4505 | `tests/diagram/wrap-pool-sizing.test.ts` | A container name said without "called" is understood — "surround selected with Pool 2" — and the container word is rejoined to a bare trailing number, since "Pool 2" is one name and the word had already been eaten identifying the container. The naming words are never taken as the name. |
| T4506 | `tests/diagram/wrap-pool-sizing.test.ts` | A pause does not become a second command. An utterance ending on a comma is held, because the recogniser punctuates on intonation and the comma is the speaker saying they have not finished; a tail beginning "called X" is held rather than handed to the AI, which turned it into a whole new pool. |
| T4507 | `tests/diagram/wrap-pool-sizing.test.ts` | The reducer really sizes the pool it adopts into, driven through WRAP_IN_POOL rather than trusting the helper is called. It used to hand the job to an enclosure pass that counts only lane children and so never moved the pool at all. |
| T4508 | `tests/diagram/pool-order.test.ts` | Moving and swapping pools exist as commands at last. Asking before this got the AI's nearest guess — "move Pool 1 above Pool 2" came back as "nudge Pool 1 up", twenty pixels. Neither rule claims a sentence unless BOTH sides are pools, so the lane move, the lane swap and the gateway-point swap keep theirs. |
| T4509 | `tests/diagram/pool-order.test.ts` | Moving a pool in the stack puts it where asked and makes room without a separate step, because the stack is laid out again from the top. The gaps stay at their slots rather than travelling with the pools, so the overall height is unchanged and nothing drifts; contents ride along; a move that would change nothing says so. |
| T4510 | `tests/diagram/pool-order.test.ts` | Swapping two pools exchanges their places, works on neighbours and distant pools alike, carries the contents, and reads "the selected pools" off the mouse — refusing anything that is not exactly two pools. |
| T4511 | `tests/diagram/pool-order.test.ts` | The reducer applies both, and returns the very same state object when a plan refuses, rather than one that merely happens to sort the same way. |
| T4512 | `tests/diagram/pool-drag-crossing.test.ts` | A pool dragged across the diagram picks nothing up. What travels with it is settled when the drag STARTS, so a pool rising up the page cannot acquire whatever it has come to be drawn around — which is unfixable while membership is a function of the current position. It still carries what genuinely lives in it, and never an event mounted on something standing still. |
| T4513 | `tests/diagram/pool-drag-crossing.test.ts` | The single-element drag uses the same rule as everything else: it is handed the travelling set rather than recomputing it, the parentId shortcut that sat beside the set is gone, and the set is cleared at the end so it cannot leak into the next drag. |
| T4514 | `tests/diagram/pool-drag-crossing.test.ts` | The moving group draws above everything it crosses, as a block, while its own internal order is still left to the depth sort so a pool does not cover its own contents. A lone element is not lifted. **Revised 2026-09-22:** the lifted group is now an event-free COPY drawn on top, and the originals stay in their normal passes — taking them out moved a travelling container to a different place in the tree, React unmounted the component that owned its drag, and every pool drag died after its first mousemove (T4658). |
| T4515 | `tests/diagram/pool-order.test.ts` | A message flow follows the pool it is attached to through a reorder. Its waypoints are in world coordinates, so moving the pool and leaving them alone left the message pointing at where the pool used to be. A connector whose two ends moved together is slid rather than re-routed, so waypoints the user placed are not thrown away. |
| T4516 | `tests/diagram/message-label-preserve.test.ts` | A message label keeps its place relative to the pool it is attached to when that pool moves. The label is stored as an offset from the line MIDPOINT, so moving one end slides it somewhere nobody put it; it is re-anchored to the end that moved, and mirrored when the pool crosses to the other side. Covers the sideways offset and a multi-line label. |
| T4517 | `tests/diagram/wrap-never-blackbox.test.ts` | A black-box pool is never grown to swallow loose elements. A participant is a deliberate statement that its insides are not modelled, so engulfing them contradicts the one thing it says and changes its type on the way. A new pool is created instead, and an unset pool type counts as black-box exactly as it does everywhere else. |
| T4518 | `tests/diagram/wrap-never-blackbox.test.ts` | A white-box pool is still grown as before, and a pool with lanes counts as white-box whatever its stored flag says, since having lanes IS being one. |
| T4519 | `tests/diagram/wrap-never-blackbox.test.ts` | A surround always makes a new pool whatever the AI returns: a whole-diagram wrap is rewritten to the selection wrap when the words the user said name the selection, and the rewrite happens before anything decides whether to ask — otherwise the confirmation would announce the wrong action. A genuine whole-diagram wrap is untouched. |
| T4520 | `tests/diagram/diagram-menu-items.test.ts` | NEW AI Generate, Generate SOP and both publish actions are in the Diagram menu, along with the version status line — the only place the current version number is shown, so burying the menu without it would lose it. |
| T4521 | `tests/diagram/diagram-menu-items.test.ts` | And are gone from the toolbar, with no copy left behind in either place, and nothing remaining of the old Publish dropdown's open/close state. |
| T4522 | `tests/diagram/diagram-menu-items.test.ts` | Each moved action closes the menu it is now in, rather than the dropdown it used to live in — which would close nothing and leave the menu sitting over the dialog it just opened. Each also keeps the gate it had, so moving something has not quietly widened who can see it. |
| T4523 | `tests/diagram/update-diagram-data.test.ts` | Every write of Diagram.data sets the same three things — the blob, a version bump and updatedAt — from one statement builder. The blob is always a parameter, and the version predicate appears only when a caller asks for one. |
| T4524 | `tests/diagram/update-diagram-data.test.ts` | A read-modify-write survives somebody else writing first: it guards the write with the version it read, and when it loses it re-reads and reapplies against the NEW data rather than clobbering it. It declines to write when the mutation has nothing to do, and gives up rather than looping forever under constant contention. |
| T4525 | `tests/diagram/update-diagram-data.test.ts` | A caller that names a version is told about a conflict instead of silently reapplying on top of someone else's change, and the error carries both versions so a route can answer 409 with them. |
| T4526 | `tests/diagram/update-diagram-data.test.ts` | Writing a blob the caller already holds: a compare-and-swap when a version is given, an honest unconditional write when it is not, and a missing row reported rather than thrown. |
| T4527 | `tests/diagram/one-diagram-data-writer.test.ts` | Nobody writes Diagram.data by hand any more. A tree scan over app/ with a short allow-list, each entry carrying the reason it keeps its own statement — and the allow-list is itself checked, since an entry that no longer writes the column is a stale exemption the next writer in that file would inherit. |
| T4528 | `tests/diagram/one-diagram-data-writer.test.ts` | Every remaining hand-written statement still bumps the version, touches updatedAt, and passes the blob as a parameter. Five of the thirteen were skipping updatedAt, so a diagram could change without anything that sorts by modification date noticing. |
| T4529 | `tests/diagram/concurrent-save-family.test.ts` | The simulator Fill lays ONLY the simulation parameters over what is actually in each child diagram, instead of writing back the snapshot taken when the console opened. Moves, renames and elements added since all survive, a deleted element is not resurrected, and a Fill that changes nothing writes nothing. |
| T4530 | `tests/diagram/concurrent-save-family.test.ts` | A SOP save cannot silently destroy another editor's sections: the save deletes and recreates every section, so it now demands the version it loaded, guards the write with a compare-and-swap, and answers 409 with the document that is actually there. A title-only save needs no token. The editor keeps its work when refused and says so — silence is what made the old behaviour dangerous, since it destroyed their work and returned 200. |
| T4531 | `tests/diagram/concurrent-save-family.test.ts` | The archive link-clearing and the mining calibrate read INSIDE the guarded step rather than holding a copy of the row, and every mining write still bumps the version — without which the editor's compare-and-swap never fires and the overwrite is invisible to both parties. |
| T4532 | `tests/diagram/black-box-pool-menu.test.ts` | The two flags a black-box pool carries — IT System and Collection — read and toggle from one shared module, so the right-click menu and the Properties panel cannot disagree; deselecting Collection clears the key rather than writing "single", which is what the Visio exporter and the canvas glyph test for. |
| T4533 | `tests/diagram/black-box-pool-menu.test.ts` | A black-box pool shows IT System / Collection instead of Generate SOP (it has no internals, so its role SOP would be empty), the menu stays open so both flags can be set in one right-click, and it still opens where SOP generation is not offered. |
| T4534 | `tests/config/mutating-route-guard.test.ts` | The read-only-impersonation ratchet: every mutating API route is guarded, on a reviewed exemption list with a reason, or on the frozen debt list — which may only shrink. A new mutating route that forgets the guard fails immediately; fixing a listed one fails until its line is deleted. |
| T4535 | `tests/config/mutating-route-guard.test.ts` | Each exemption is an exemption for the reason given: the session-less ones (webhook, cron, ingest key, anonymous sign-up) really resolve no session, and admin/impersonate stays unguarded on purpose because it is how an admin leaves view-only mode. |
| T4536 | `tests/config/mutating-route-guard.test.ts` | The shared wrapper decides the 403 from the session and cookie jar alone, the Risk & Control guards call it rather than keeping a second copy, and the feature slug is a parameter rather than a hard-coded riskControl. |
| T4537 | `tests/security/read-only-impersonation-family.test.ts` | The guard refuses a read-only impersonation write with 403, lets an ordinary user and an edit-mode admin through (support and repair still work), and ignores the cookie for a non-superuser. |
| T4538 | `tests/security/read-only-impersonation-family.test.ts` | The project wrapper answers the 403 before it reaches the database — proven with a project id that does not exist, which would answer 404 if the order were wrong — and does not block a read. |
| T4539 | `tests/security/read-only-impersonation-family.test.ts` | SEC-33: an oversized declared Content-Length is refused before the body is read, a chunked request still gets through, and in sop-templates the auth, the guard and the size check all precede formData(). |
| T4540 | `tests/security/read-only-impersonation-family.test.ts` | Each route the audit named calls the guard — the CALL, not just the import, and on the right side of the handler — with the Compare DELETE covered as well as its POST, and the SOP read path deliberately left open. |
| T4541 | `tests/diagram/it-system-task-types.test.ts` | Setting a black-box pool's IT System flag retypes the Send and Receive tasks that exchange messages with it to User, leaves every other task type alone, and reaches only the tasks on the other end of THIS pool's messages. |
| T4542 | `tests/diagram/it-system-task-types.test.ts` | Clearing the flag hands each User task back to its own traffic — Send if it only sends, Receive if it only receives, None if it does both — counted over all its message flows, not just those with this pool. Driven through the real UPDATE_PROPERTIES reducer, so the Properties panel and the right-click menu cannot diverge. |
| T4543 | `tests/diagram/it-system-task-types.test.ts` | Escape removes the pool alignment line and its green markers and keeps them away for the rest of the gesture — the drag re-proposes the guide every mouse-move, so clearing alone would put it straight back — and the next gesture shows it again. |
| T4544 | `tests/ui/screen-brightness.test.ts` | Each Display default means 'leave the screen as it is' — Brightness 80%, Contrast 50% — so each scale is pct/default; brightness goes a little brighter and a lot darker, contrast sits centred in its range, both are monotonic, and unreadable storage falls back to the default rather than blacking out the screen. |
| T4545 | `tests/ui/screen-brightness.test.ts` | The dimmer is mounted at the app root so it covers the whole window, tints with backdrop-filter on a fixed pointer-events-none sheet rather than filtering an ancestor (which would re-anchor every position:fixed modal), reads its filter from a CSS custom property in BOTH the standard and -webkit- form, and guards every localStorage access. |
| T4546 | `tests/ui/screen-brightness.test.ts` | Brightness and Contrast live in System ▸ Display on the dashboard with a Reset to default that greys itself out when there is nothing to reset, and the canvas toolbar no longer carries a copy — two places to set one thing is how they drift. |
| T4547 | `tests/ui/screen-brightness.test.ts` | A stored setting is in force at the FIRST paint of a hard reload: a blocking boot script in <head> paints it before React can hydrate. The script is run against the runtime for every combination of slider positions to prove the two implementations of pct/default cannot drift, and it cannot throw whether storage or the document is the thing that fails. |
| T4548 | `tests/bpmn/emie-label-default.test.ts` | An EMIE mounted on a TOP edge puts its label outside the host, above the event and left of it, clear of the flow leaving upward — below the event there would be inside the host, over its contents. A bottom mount shifts half a default label-width left; a vertical (left/right) boundary sits just above the outgoing connector on the side that connector travels. All three scale to the event's own size. |
| T4549 | `tests/bpmn/emie-label-default.test.ts` | All three creation paths get the rule, driven through the real reducer — a palette drop that snaps to a rim, a drag released on a rim, and the assist/Voice Assist addBoundary op — while an event that snapped to nothing keeps the ordinary centred label, and re-mounting onto a different edge re-places it. |
| T4550 | `tests/canvas/container-header-menu.test.ts` | A pool's or lane's header strip is read from its own stored width rather than assumed to be 36px (a pool must not read the lane key, or vice versa), spans only the container's own rows, and is now the single copy of a rule that was being re-derived in five places. |
| T4551 | `tests/canvas/container-header-menu.test.ts` | The right-click element matrix puts Pool/Lane straight after the Gateway and ends with Pain Point, Issue and Review Comment; every symbol it offers has a label; the Review Comment is withheld where the editor cannot build one, and is built through the editor rather than added as a bare shape because it carries an author and a timestamp. |
| T4552 | `tests/canvas/container-header-menu.test.ts` | Generate SOP opens only from a pool's or lane's header strip; a right-click in the body falls through to the element matrix. A black-box pool answers anywhere, having no body to put anything in. |
| T4553 | `tests/config/voice-assist-rename.test.ts` | Nothing in any source, doc, seed or test file — or any file or directory NAME — still carries the feature's OLD name, and the new one (Voice Assist) reaches the feature registry, the route gate and the command bar. VERSION_HISTORY, the schema changelog and audit/** are exempt: a release note keeps the name the thing had when it shipped. |
| T4554 | `tests/config/voice-assist-rename.test.ts` | Assist is available on every subscription level; Voice Assist keeps the Expert-and-above availability it had (a rename is not a re-pricing); both are plainly labelled in the matrix, and every seeded row is a key the registry knows. |
| T4555 | `tests/config/voice-assist-rename.test.ts` | The SQL half carries the feature states across rather than dropping them (isFeatureAvailable fails CLOSED, so a bare key rename would switch Voice Assist off for everyone who has it), can be run twice without hitting the unique index, renames the PUBLISHED catalogue columns as well as the draft, covers the Guide and Notes tables, and replaces the longer phrase first so no title is left reading "Voice Assist Mode". |
| T4556 | `tests/diagram/pool-move-label-roundtrip.test.ts` | ⚠ MEASUREMENT of an OPEN defect, not a guard: dragging a pool away and back must return its message-label offsets. Built from Paul's own three exports. A ONE-step drag round-trips perfectly, so the formula is sound; seven steps lands 213px out and thirty 174px, which is the label he found below the returned pool. Three assertions are `it.fails` — fixing the defect fails this file, which forces the measurement to be updated rather than left lying. |
| T4557 | `tests/canvas/pool-edge-gesture.test.ts` | A drag that starts on a container edge is read by its DIRECTION once it clears the 4px threshold: across the edge resizes, along it moves. Sign-independent, and a slightly untidy resize still resizes — only a clearly-along drag switches. |
| T4558 | `tests/canvas/pool-edge-gesture.test.ts` | The edge band is generous OUTSIDE (empty canvas, costs nothing) and scaled INSIDE, so Paul's 78px pool gives up 6px an edge instead of 10 — it used to lose a quarter of its height to a zone that could not be dragged. A tall EP keeps the full 10px and nothing drops below 4. Sweeps the pool top to bottom and asserts every band is draggable. |
| T4559 | `tests/canvas/pool-edge-gesture.test.ts` | The pool alignment guide belongs to its gesture: any new interaction retires it, via mousedown CAPTURE so a click on another shape clears it too (bubble phase never sees it — elements stopPropagation). Escape and a real gesture end both still clear it. |
| T4560 | `tests/canvas/pool-edge-gesture.test.ts` | The hit-zone owns the gesture rather than the element guessing: it classifies the drag, starts a move from the ORIGINAL press (handing it the live pointer would make the pool jump by the threshold), sizes all four edges from the shared rule, and no longer deselects a pool on a press that may become a drag. |
| T4561 | `tests/canvas/message-label-hidden-on-drag.test.ts` | Message labels come off while a POOL is dragged and return at the drop, in the place the settle chose — mid-drag the label is the old offset riding a line whose midpoint is moving, a number about to change. Scoped to the dragged pool (other pools' messages still tell the truth), messages only (not sequence flows, even ones on that pool), and not for a task drag, which would flicker on every nudge. The connector stays; only the text goes, and an open editor still wins. **Narrowed 2026-09-23** (Paul: "No need to hide message labels during Pool movements unless the Pool is moved over another Pool with which it has message connectors OR has message connectors with one or more of that Pool's child elements") — the labels now go only while the two pools actually OVERLAP; see T4691. |
| T4562 | `tests/ai/command-model-and-payload.test.ts` | The Voice Assist fallback payload is capped at 120 elements, keeping everything SELECTED first (those are the referents for "the selected task"), dropping any connector whose other end was cut, and stating how much was left out so the model knows the picture is partial. |
| T4563 | `tests/ai/command-model-and-payload.test.ts` | The command interpreter defaults to Haiku, not the generation model — it rewrites ONE sentence and the grammar re-parses the result, so a poor rewrite fails to parse rather than corrupting the diagram. Falls back where Haiku is unreachable, overridable in AI Model settings, blank means default, and the picker joins the dirty check. |
| T4564 | `tests/ai/command-model-and-payload.test.ts` | The fallback aborts after a bounded wait instead of hanging on "thinking…", clears its timer whichever way the call ends, and says it timed out in words rather than showing a stack message. |
| T4565 | `tests/diagram/assist-busy-and-ambiguity.test.ts` | R3: a DESTRUCTIVE command never guesses between candidates. A bare type noun still resolves to the most recent for ordinary commands (what makes "add a task after the gateway" quick) but reports the ambiguity under strict — while a single candidate, or a named reference, still answers. |
| T4566 | `tests/diagram/assist-busy-and-ambiguity.test.ts` | R2: an ambiguity names what it found — up to four labels and a count of the rest — instead of discarding the candidate list resolveRef had already computed and saying only "is ambiguous". |
| T4567 | `tests/diagram/assist-busy-and-ambiguity.test.ts` | B4: the voice path gates on a REF, which flips synchronously, so a sentence spoken during an AI call queues instead of racing it and applying against pre-call state. Drained through the ref so the queued command sees the NEW diagram, and "stop" bypasses the queue and clears it. **Changed by design 2026-09-26:** the `finally` used to run the next queued command in the same tick, before anything rendered — so, despite the ref, it resolved against the diagram from BEFORE the call, and the voice-debug recording folded both commands into one "after" (T4897). The `finally` now only wakes a drain effect, which runs one queued command per render through the ref (`runAbraCommandRef.current(next, true)`) and wakes itself again for a queued command handled on the spot; a sentence arriving while the queue drains joins its back (`if (!fromQueue && (voiceBusyRef.current || voiceQueueRef.current.length > 0))`), and "stop" is still checked before that guard. |
| T4568 | `tests/diagram/assist-greedy-grammar.test.ts` | B5: the four sentences the over-greedy rules used to swallow now decline — "collapse the subprocess", "swap Task A with Task B", a positional phrase read as a name — and "move the Assembly Line task up" is better than a fallback: the lane rule declines and the general element-move rule picks it up deterministically. A match BLOCKS the AI fallback, which is why declining is the fix. |
| T4569 | `tests/diagram/assist-greedy-grammar.test.ts` | And the phrasings that must survive the tightening: "compress Warehouse" (no pool word), "swap Sales with Picking", a lane move with the word leading or trailing, an explicit "called …" name that looks positional, and the pool nudge. Records that a BARE quoted name is not an escape hatch — clean() strips the trailing quote first — so "called" is. **Changed by design 2026-09-26 (T4900):** "compress the Customer pool" now keeps the kind word — `poolRef: "pool Customer"` — so the resolver binds it. |
| T4570 | `tests/diagram/assist-greedy-grammar.test.ts` | The guards themselves: a non-container kind spotted at either end of a reference, container words and ordinary names left alone, the lane word required to be ATTACHED rather than merely present (the recogniser spells "lane" as "line", which is why "Assembly Line" tripped it), and the apply-layer fallthrough that moves an element when the ref turns out not to be a lane. |
| T4571 | `tests/diagram/assist-disambiguation-picker.test.ts` | R2 second half: an ambiguous command is PARKED and its candidates numbered in reading order by the SHARED rule, so a badge means the same thing in all three flows that draw them. Declines to ask about fewer than two — a picker with one badge is a worse error message, not a better one — and keeps the whole command, not just the reference. |
| T4572 | `tests/diagram/assist-disambiguation-picker.test.ts` | Answering it: a bare number, the forgiving forms the rename pick takes, a rejection for a number not on the canvas or an utterance that is not an answer, and the choice substituted as an #id: reference — a NAME would be ambiguous again, which is why it asked. Matched by VALUE so it reaches ref / hostRef / laneA alike, without mutating the parked command. |
| T4573 | `tests/diagram/assist-disambiguation-picker.test.ts` | Wired in: a destructive command with candidates parks rather than failing (and a parked command is not reported as a failure), the next utterance resumes it through the ORDINARY apply path, a non-answer re-prompts rather than guessing, the badges use the existing renderer, and it can be left three ways — a flow-end word, Escape, or changing diagram. **Updated 2026-09-26:** the picker's badges reach the canvas through `badgesOnScreen` (shared with the voice-debug recording, T4893); the pin checks that wiring and that the picker's targets come out of it. |
| T4574 | `tests/diagram/assist-voice-reliability.test.ts` | V1 sends THIS diagram's names to the recogniser, timidly: multi-word phrases (the safe case), single words only when long and distinctive, never a bare common word, never anything the command vocabulary already owns, never a number, never a sentence — and NEVER with a boost suffix, which is what made "turn" come back as "ten". Capped, de-duplicated, phrases first, and sent after the command words. |
| T4575 | `tests/diagram/assist-voice-reliability.test.ts` | V2 resolves what the recogniser still gets wrong, without an AI call: a single mis-heard word (escalade → Escalate) and a word it split in two (where house → Warehouse). Measured first — multi-word mis-hears already resolved on token overlap — so the pass runs LAST and is deliberately narrow. Two short names that merely rhyme do NOT match, and several sound-alikes ask rather than guess. |
| T4576 | `tests/diagram/assist-voice-reliability.test.ts` | The phonetic key: one symbol per sound across the spellings English uses (ph/f, ck/k, qu/kw), a silent h after w, a leading vowel kept because it is the reliably heard part, and "ch" NOT collapsed into "ks" — the intermediate symbol is a digit so the letter rules cannot reach it. |
| T4577 | `tests/diagram/assist-voice-reliability.test.ts` | A token-overlap TIE is no longer settled by document order — "shop order" scored 0.5 against both Back Order and Ship Order and took whichever came first, a guess wearing a green tick. The SOUND settles it when it picks exactly one; otherwise the ambiguity is reported, which raises the picker. |
| T4578 | `tests/config/help-section-collection.test.ts` | Every seed that creates a `HelpSection` passes `collection` explicitly, and passes its CHAPTER's collection rather than any collection. The column defaults to "user-guide", so five `add-tech-notes-*.ts` seeds silently mislabelled 33 Technical Notes sections — and the User Guide's bulk save is `deleteMany({ collection: "user-guide" })`, which would have deleted them. Being right by accident of a default is what made the tech-design ones wrong. |
| T4579 | `tests/config/help-section-collection.test.ts` | `seed-voice-assist-content.sql` repairs the rows already written wrong, realigning any section whose collection disagrees with its chapter's, writes its own sections with the right collection for each of the two chapters, and does the repair INSIDE the seed's transaction so a failure cannot leave the database half-fixed. |
| T4580 | `tests/diagram/assist-convert-and-guards.test.ts` | B8 — a dictation-token 403 is the ORG'S DECISION and is refused (starting the browser engine sends the same audio somewhere else and defeats the policy just as thoroughly); a 503, a 401 and a network failure fall back and SAY the engine is the browser. `startDictation` branches on that outcome, releases the optimistically-opened mic on a refusal, and surfaces the notice — the defect was `if (r.ok)` collapsing all three into one silent path. |
| T4581 | `tests/diagram/assist-convert-and-guards.test.ts` | B6 — a sub-lane is a lane with a lane parent (every interactive path) OR `type: "sublane"` (the AI converter and importers). Both count, "lane" on its own means a band directly in a pool, and a "which one?" guard counts siblings at the same level. The delete guard used the type alone, so "delete the sublane Staff" left the word in the name and answered "which lane? there are 4". |
| T4582 | `tests/diagram/assist-convert-and-guards.test.ts` | M3 — the spoken form of the right-click type picker. Every subtype the menu can show is reachable by voice, glyphs and hyphens included ("event based gateway", "business rule task", "multi instance parallel"); a bare "none" is never meant; the menu and the grammar read the SAME table so they cannot come to offer different things. The TAIL discriminates, so an add phrasing keeps its verb. |
| T4583 | `tests/diagram/assist-convert-and-guards.test.ts` | M3 refuses by name what it cannot do: a shape change (task → gateway) is a different operation, so the grammar declines and the AI is asked. A subtype that exists but not for this element says which kinds it belongs to, writes exactly what the menu writes, and leaves nothing selected — the standing protocol. |
| T4584 | `tests/diagram/assist-convert-and-guards.test.ts` | R5 — the flash diff sees a subtype change. "Make this a user task" moves nothing, re-parents nothing and renames nothing, so the diff found no difference and flashed nothing: the rename defect of 18 September arriving through a different door. The fingerprint is derived from the shared subtype table, so a subtype added to the menu is covered the same day, and it is carried into BOTH snapshots. |
| T4585 | `tests/diagram/assist-convert-and-guards.test.ts` | R6 — a failed reference says what it nearly found, from words in common OR from how it sounds (two edits, deliberately further than the resolver will act on). The looseness is safe ONLY because these candidates go into a question: the acting pass keeps its one-edit limit. Capped, so a question does not become a wall. **Changed by design 2026-09-26 (T4902):** the near misses are bounded by the field's kind — `nearestRefs(ref, els, 3, opts.kind)` — since a suggestion of the wrong kind would be picked and refused. |
| T4586 | `tests/diagram/assist-convert-and-guards.test.ts` | R7 — the auto-connect inside `add` runs the same `canConnect` gauntlet the explicit `connect` op always did, so "add a task after Done" no longer draws a sequence flow out of an end event and reports it with a green tick. Checked against the state that WILL exist, and the element is still added with the log saying why it is unconnected. |
| T4587 | `tests/diagram/assist-correction-pairs.test.ts` | V3's first half — a failed command followed by a working re-issue is a correction pair, classed MISHEARD (said again, worked — the recogniser's fault) or REPHRASED (said differently, worked — the grammar's). The sound is compared as well as the words, because a short command like "delete escalade" scores only 0.5 on token overlap, which is exactly when the recogniser has least to go on. Bookkeeping ("stop", "undo", a bare number) is excluded, or every failure near the end of a session would look corrected; a success is claimed once, so three attempts is one pair. |
| T4588 | `tests/diagram/assist-correction-pairs.test.ts` | The tally that decides whether V3 should be built at all: a session full of MISHEARD says a personal phrase book has something to learn, one that is all REPHRASED says the effort belongs in the grammar instead. Shown beside the Cost readout because a command that had to be said twice is a cost the dollar figure does not show — and it STORES NOTHING: persisting the pairs is V3's second half, deliberately unbuilt until the measurement says V1 and V2 left something behind. |
| T4589 | `tests/config/sql-script-result.test.ts` | A MULTI-STATEMENT script pasted into the Database tile says what it did. `pgPool.query` with no params takes several statements and node-postgres then returns an ARRAY with `rows` undefined, so a 14-statement seed that had fully run and COMMITTED displayed "undefined completed — 0 row(s) affected" — indistinguishable from a script that did nothing, from the one tool used to change production data. The statement SHOWN is the last that returned rows (not simply the last), and every statement is listed. |
| T4590 | `tests/config/sql-script-result.test.ts` | Every `seed-*.sql` ends with a verification SELECT placed AFTER the COMMIT — before it, the query would report what the transaction was about to do rather than what was committed, and only one of those is evidence. The Voice Assist seed's trailing check covers all ten claims its own header makes, and the whole seed is wrapped in a transaction so a failure leaves nothing half-applied. |
| T4591 | `tests/diagram/assist-fill-and-pointer.test.ts` | M4 — reading order is ROWS, not raw coordinates: a BPMN diagram is laid out in bands, so sorting the selection by x alone interleaves two lanes and lands one lane's names on the other's elements. Elements within half a row-height are one row, each row reads left to right, and two elements at the same point order deterministically by id. |
| T4592 | `tests/diagram/assist-fill-and-pointer.test.ts` | M4 — "name these Receive, Check and Ship" pairs names with the selection in reading order, and a COUNT MISMATCH is refused rather than truncated: renaming three of four leaves a diagram that looks finished and is wrong. Parsed before the rename rule, so it is not read as a rename of something called "these". |
| T4593 | `tests/diagram/assist-fill-and-pointer.test.ts` | M4 — teams and Risk/Control attachment over the whole selection. "Team" must actually be said, so "move these right" and "wrap these in a pool" keep their meaning; a team belongs to an ACTIVITY and anything else is named and skipped. A catalogue code is normalised on both sides ("R-012" / "r 012" / "are 012" → R12, leading zeros never spoken), and matching is NEVER fuzzy — attaching the wrong control survives into an audit, so a synonym or a typo fails rather than resolving to a plausible neighbour. |
| T4594 | `tests/diagram/assist-fill-and-pointer.test.ts` | M5 — what the pointer is over: containers lose to what is inside them (a pool covers everything in it and is almost never what the hand means), but the bare part of a pool still finds the pool, and the container rule holds even when the container is the SMALLER of the two — an element taller than a lane that has not grown yet, which the "smaller wins" tie-break gets wrong alone. Position words ("here", "over there") and element words ("the one under the cursor") are told apart. |
| T4595 | `tests/diagram/assist-fill-and-pointer.test.ts` | M5 — the pointer NEVER outranks the selection. The review plan asked for "connect this to that" with *that* meaning the pointer; "this"/"that" already mean the selection (M1, shipped), and that sentence needs the two demonstratives to differ inside one utterance. So the selection still wins, "it" still means the last added, and the pointer is used only as a better last resort than document order. The explicit phrase resolves to the hovered element and to nothing when the pointer is over nothing. |
| T4596 | `tests/diagram/assist-fill-and-pointer.test.ts` | M5 — "put a task here". The position word is stripped BEFORE the name is read, or "add a task called Approve here" is named "Approve here"; a task someone really does want to call "Here" still works because stripping would leave a naming keyword bare. An explicit position draws no connector, keeps clear of what is already there, and REFUSES with a reason when the mouse has never been over the canvas rather than dropping an element at (0,0). Fed by a capture-phase handler into a ref, so tracking every mouse move renders nothing. |
| T4597 | `tests/diagram/assist-align-ghost.test.ts` | M7's align half — the same seven modes the Alignment ▾ menu dispatches, said out loud ("align these" = smart, "in a row" = one horizontal line, "in a column" = one vertical, plus the four edges). A BARE AXIS WORD is refused rather than guessed: "align these horizontally" means "lay them along a horizontal line" to some people and "move them horizontally" to others, so the grammar declines and the AI can ask. Distribute and "same size as this" are NOT claimed — those reducers do not exist, and distributing badly is worse than not distributing because it looks done. |
| T4598 | `tests/diagram/assist-align-ghost.test.ts` | M8 — take an Assist ghost by voice: the first (what Tab does), by position, or by label/type. "Yes" is deliberately NOT an accept word — it already confirms a parked destructive command, and a word that means two things depending on whether a ghost happens to be showing is how a diagram gets cleared by accident. A bare number is left alone too, because it answers a numbered pick. A pick that is not on offer lists what IS, since the ghosts are translucent and easy to misread. |
| T4599 | `tests/diagram/assist-align-ghost.test.ts` | B7's remainder — the AI prompt offers `addPool.relativeTo`, so "above" finally has something to be above; without it the model could name a position with no anchor and the pool landed relative to the existing stack. Also pins that the SYSTEM template literal contains no stray backtick, which ended the string and broke the production build on 2026-09-20. |
| T4600 | `tests/diagram/assist-align-ghost.test.ts` | Every op in the `AssistOp` union has a `validateOp` case. One added to the union and the grammar but not the validator works when TYPED and vanishes when the AI returns it — a failure that only shows on the expensive path, and only sometimes. |
| T4601 | `tests/diagram/assist-stranded-tail.test.ts` | A stranded "after X" / "before X" tail is a CONNECTION, not a new element. Paul, 2026-09-21: "Add a task called receive order" is complete so it ran, then "after the start." arrived with no sentence in front of it — the hold rules cannot help a first half that already finished, so it reached the AI, which saw the newest element, reconstructed the whole command and added a SECOND "receive order". Handled deterministically now, so it never reaches a model that could invent one. A whole command that merely begins with the connective is left alone. |
| T4602 | `tests/diagram/assist-stranded-tail.test.ts` | A named anchor that does not resolve STOPS the add. It used to push the error, set `anyFail`, and fall straight through to the recency fallback — so the log read "which 'receive order'? 2 match … — say the name; added check stock after check stock": the ambiguity named, and the element added anyway, anchored to whatever was last. Failing to find what the user asked for is not permission to pick something else. An ambiguous anchor now raises the picker; the recency fallback stays for when no anchor was named. |
| T4603 | `tests/diagram/assist-stranded-tail.test.ts` | Activity, gateway and event labels always start with a capitalised word (Paul, 2026-09-21) — enforced in the REDUCER (both `UPDATE_LABEL` and `ADD_ELEMENT`) so the palette, an inline edit, a spoken command and an AI apply all obey it; it was applied on a voice rename only, so "add a task called receive order" left a lower-case name. First word only, never title case, and a deliberate capital is left alone (iPhone, eCommerce, mRNA). Containers are excluded — they already carry the unique/never-bare-kind-word rules in the same branch. |
| T4604 | `tests/diagram/assist-ref-mentions.test.ts` | An element id never reaches the user. `serializeDiagram` hands the model every id so it can describe relationships, and a model given ids sometimes answers with one — the apply layer then matched `k3f9a2bx` against the labels, failed, and printed the id (Paul, 2026-09-21: "the element ids are confusing"). A bare id that EXISTS now simply resolves, since we handed it over; one that does not is our plumbing leaking and gets a human sentence instead, checked BEFORE "did you mean" because suggesting names for an id is noise on noise. A name that happens to look like an id still resolves to itself. **Changed by design 2026-09-26:** the pinned call is now `nearestRefs(ref, els, 3, opts.kind)` (T4902); the id guard still runs first. |
| T4605 | `tests/diagram/assist-ref-mentions.test.ts` | When the AI returns nothing, name what is MISSING instead of blaming the phrasing. "Add a task before review" with no Review on the diagram said "didn't understand that" — but it was understood: the grammar declined on purpose (B5), and there is simply no Review. Deliberately conservative, since a wrong "nothing is called X" is worse than the vague message: a name being CREATED is never reported ("called/named/labelled", and the second half of a rename), nor are pronouns, positions or kind words, nor an element id. Falls back to the old wording when it has nothing better to say. |
| T4606 | `tests/diagram/lane-delete-and-naming.test.ts` | A lane that absorbs a deleted neighbour's space takes its SUB-LANES with it. The absorb model changed the absorber's y and height and nothing else, so a lane with sub-lanes grew while its bands did not — Paul's export showed "shipping" at h=519 with sub-lanes still totalling 262, a void of exactly the deleted lane's 257px. The sub-lane nearest the edge that MOVED takes the space (bottom-most when it grew downward, top-most when upward), leaving every other band untouched: scaling them all proportionally would silently resize bands nobody mentioned. Recurses, because the band that grew may have its own. |
| T4607 | `tests/diagram/lane-delete-and-naming.test.ts` | Container names start with a capital too (Paul, 2026-09-21: "lanes are created without capitalised names"). Applied inside `uniqueContainerLabel`, so every container path gets it — create, rename, split, voice, AI — and applied BEFORE the uniqueness pass so "sales" beside an existing "Sales" is caught as the clash it is rather than slipping past on case. |
| T4608 | `tests/diagram/lane-delete-and-naming.test.ts` | The delete confirmation stops claiming the contents are deleted. The reducer has always cascade-deleted the container SUBTREE and re-homed every non-container child, but the prompt said "delete the sub-lane 'domestic' AND THE 7 ELEMENTS INSIDE IT" — so Paul said no to an operation that would not have touched his seven elements. It now says they are kept, and counts only real contents rather than nested lanes. A confirmation that overstates the danger trains the user to distrust the prompt. |
| T4609 | `tests/diagram/lane-delete-and-naming.test.ts` | "Participant box" is a black-box pool — the spec's own word for one — and "for X" names it, so "create a participant box for the courier above customer" is a pool, not a TASK named "Participant box for the courier above customer". The positional guard also had to look INSIDE an implicit label, not just at its start: the label did not begin with a positional word, so the whole sentence became a name. Only the unambiguous words are matched inside ("above", "between", "below"), since a real step name can contain "to" or "from". |
| T4610 | `tests/diagram/assist-block2-round2.test.ts` | The sentence shown back is in the USER'S vocabulary. The apply layer resolves a bare element id now, so "back order to the merge gateway" worked — but the log printed the AI's canonical verbatim: "connect qruut4v9 to ksjm25kj" (Paul, 2026-09-21). The command was right; the sentence describing it was written in our plumbing. Ids become names, an id with no element left becomes "something", and ordinary eight-letter words are untouched. |
| T4611 | `tests/diagram/assist-block2-round2.test.ts` | "Add a merge" made a TASK named "merge" — the word was in nobody's vocabulary, so the add rule took it as a name. Merge / merge gateway / join are gateways now, and per Paul's rule a bare gateway, decision or merge carries NO marker: the × is a choice, not a default, and the ROLE is what the reader needs. An explicitly-asked-for marker still works. |
| T4612 | `tests/diagram/assist-block2-round2.test.ts` | A decision gateway's label is a question and ends in one (Paul, 2026-09-21). A new decision already defaulted to "Decision?", but renaming it lost the mark — "In stock" not "In stock?" — so the diagram read differently depending on how the gateway got its name. Not doubled, not added over other punctuation the user chose, never added to an empty label (a bare "?" is worse), and never to a MERGE, which asks nothing. Enforced on both reducer label paths. |
| T4613 | `tests/diagram/assist-block2-round2.test.ts` | "Undo" inside a numbered pick gets you out. It was answered "say the number of the item to rename" three times in a row in Paul's log — the feature arguing with someone trying to leave. It now drops the pick AND undoes, and the prompt names the way out ("cancel" to stop) rather than only the way to finish. **Updated 2026-09-26:** the pin now also expects `armDebugBefore(elementsRef.current);` first — the undo changes the diagram, so the voice-debug recording is armed and saves its "after" on the undo's own line (T4893); the cancel-then-undo it pins is unchanged. |
| T4614 | `tests/diagram/assist-block2-round2.test.ts` | A pool still spans its lanes after a delete. A sub-lane delete grows the band that absorbs it, which grows the lane — and nothing re-fitted the ancestors, so Paul's pool sat at h=435 with its lanes reaching 529: "independent of its contents and behaves weirdly when resized". Every other lane action already ended with `ensureContainersEncloseChildren`; both delete returns now do too. |
| T4615 | `tests/diagram/merge-gateway-sides.test.ts` | The merge gateway connection convention (Paul, 2026-09-21): a branch from ABOVE meets the top vertex, one from BELOW the bottom, one LEVEL with it the left. It is the mirror of the decision fan-out the product already had, and uses the same band predicate — touching exactly counts as clear — so a merge and a decision never disagree about what "above" means. Applies to a merge only; a gateway's role defaults to decision. |
| T4616 | `tests/diagram/merge-gateway-sides.test.ts` | Driven through the REAL reducer, not just the pure rule: each band produces the right vertex, the far end faces the gateway instead of doubling back, a decision's incoming flows are untouched, a message flow to a merge is refused outright (a gateway carries no message — the old `if (c)` guard had made that case assert nothing), and an endpoint deliberately dragged elsewhere is respected. |
| T4617 | `tests/diagram/merge-gateway-sides.test.ts` | The guard that would have shipped DEAD. The wiring first asked `!action.payload.targetSide` to mean "the caller chose nothing" — but `addConnector` defaults `targetSide` to "left", so the payload always carries a value and the rule never fired, while every unit test of the pure function passed. The live discriminator is `targetSide === "left"`, which is also the convention's own answer for a level element, so refining it to top/bottom changes nothing that was deliberate. Also pins that the convention runs BEFORE R6.30's vertex coercion. |
| T4618 | `tests/diagram/assist-block2-test3.test.ts` | Naming the gateway no longer breaks the point swap. "Swap top and bottom" always worked, but every more EXPLICIT way of saying it — "swap selected gateway, top and bottom" — fell through to the LANE swap and answered "couldn't find 'selected gateway, top'", so the feature looked gone (Paul: "What happened to Swap selected Top and Bottom?"). The comma matters: only one after the FIRST word is stripped earlier. A real lane swap, two NAMES, is untouched. |
| T4619 | `tests/diagram/assist-block2-test3.test.ts` | One hesitation must not produce two outcomes. "Add two sublanes" is complete on its own, so it RAN — two sub-lanes in the wrong lane — and the tail "to shipping called domestic and international" reached the AI, which rebuilt it into a whole command and made two more. Four from one sentence. A lane count with NEITHER a target NOR names is under-specified and is now held; "add a lane" is deliberately not, being the one phrasing people use alone. |
| T4620 | `tests/diagram/assist-block2-test3.test.ts` | An implicit label that is really another command. "Put Surround everything with a pool" made a TASK with that sentence for a name — the container guard is anchored at the start of the remainder and this one starts with a verb. Needs BOTH an instruction word and a container word, because "Review everything" is a fine task name and "Surround" alone might be one. |
| T4621 | `tests/diagram/assist-block2-test3.test.ts` | Sub-lane containment, MEASURED rather than assumed. Paul reported that adding sub-lanes destabilises the pool; driven through the real reducer, once and twice over, containment is exact and the untouched lanes do not move. Kept as the measurement so a regression says so, and so the next investigation starts from what was already ruled out. |
| T4622 | `tests/diagram/assist-block2-test3.test.ts` | "Move top to bottom" — move ONE connector to a FREE gateway point, the companion to the swap, which needs both points taken (Paul, 2026-09-21). Any ordered pair, naming the gateway optional, acting on every selected gateway. Saying the wrong one of the pair names the other rather than refusing blankly. Does NOT swallow "move Approve right", the older and far more common command. |
| T4623 | `tests/diagram/sublane-split-fit.test.ts` | A sub-lane band is never negative and never hangs out of its lane. `SPLIT_LANE_EVEN` floored each band at 28px but gave the LAST one the remainder — so once the floor ate the lane the remainder went NEGATIVE and the stack ran past its parent: 40px ÷ 3 was [28, 28, −16], 80px ÷ 4 was [28, 28, 28, −4]. Two is almost always safe and three often is not, which is exactly what Paul reported ("adding 3 lanes does not work properly, but adding 2 is ok"). A lane asked to hold N bands now grows to fit them, and only when it has to. |
| T4624 | `tests/diagram/sublane-split-fit.test.ts` | The growth is carried up rather than left hanging: the lane below slides down instead of being overlapped, and the pool follows — the 41px of lane hanging out of the bottom of the pool in Paul's export. `SPLIT_LANE_EVEN` was the ONE lane action that never called `ensureContainersEncloseChildren`. Loose contents still join the first band. |
| T4625 | `tests/diagram/pool-resize-follows-lanes.test.ts` | A pool boundary cannot outrun its lanes. Each lane is clamped on resize (`Math.max(40, min, …)`, covering its label and contents) but the pool was written with the RAW drag — so the instant a lane refused to shrink further the pool carried on alone and the lanes hung out of the bottom of it (Paul's own diagnosis, 2026-09-21, after three reports no voice path could reproduce: it happens with manually added lanes too). A pool with lanes has no height of its own — it is exactly the stack — so the lanes are computed first, clamps and all, and the pool derived from what they came to. It comes to REST rather than creeping. |
| T4626 | `tests/diagram/pool-resize-follows-lanes.test.ts` | The edge you did not drag stays put. The height clamp raised `newH` without moving `newY`, so a top-edge drag pushed the BOTTOM down — and the top/bottom test then read the gesture as "both edges moved" and fell into the pro-rata branch, resizing all three lanes instead of the one grabbed. The clamp is a floor, not a cage: growing still follows freely. |
| T4627 | `tests/diagram/pool-boundary-rules.test.ts` | A pool boundary STOPS at the content; it never pushes it. Paul, 2026-09-21: "Pool boundary moves should never move elements up. They should just be prevented from moving when the boundary hits anything in the pool or lane." A lane's minimum covered its LABEL and not its contents, so an inward drag squeezed the lane past the process inside it and `clampChildrenToLane` then shoved every element along ahead of the edge — the user moved a container and their diagram moved with it. The drag is now capped at the content first (`clampRectToContent`), on all four edges, after which the clamps below have nothing left to do. |
| T4628 | `tests/diagram/pool-boundary-rules.test.ts` | A pool IS its lane stack, however it got there. T4625 derives it inside the resize branch, but a pool's height is written from a dozen places and Paul reported the dissociation three times from three different gestures — so the invariant is asserted on the containment pass all of them already end with (`poolFollowsLanes` inside `ensureContainersEncloseChildren`). Includes the exact broken geometry from his Block 2 Test 3 export (pool 552.14 tall, lanes 584.86). Only the POOL is rewritten: re-stacking lanes here would shift one out from under its contents, which is the shoving T4627 forbids. A lane-less pool keeps its own height. |
| T4629 | `tests/diagram/pool-boundary-rules.test.ts` | Half an event of clear space in front of the first element. Paul, 2026-09-21: adding a Lane or Sublane to a pool with elements in it must keep at least 18px between the lane header's right edge and the left-most element (normally the Start Event). A fresh pool leaves 40px and the first lane's own 36px header ate nearly all of it. The pool and every lane grow LEFTWARDS by the shortfall — measured from the DEEPEST header against ALL of the pool's content, because an element added before the lanes were stays parented to the POOL while sitting inside a lane, so a parentage walk finds nothing in the very lane that crowded it. Nothing inside moves. **Revised 2026-09-22:** the gap is made by moving the pool's CONTENT right, not by growing the pool leftwards — Paul: "Adding lanes to a Pool should not grow the Pool. The lanes must be added within the Pool." See T4685–T4687. |
| T4630 | `tests/diagram/pool-boundary-rules.test.ts` | The content clamp is inward-only and per-edge. Dragging an edge AWAY from the content is never limited; a corner drag still moves on the axis that is legal; and a container that ALREADY overlaps its content (an import, an older diagram) is not yanked back out — that would move a boundary on a gesture the user never made. It simply may not go any further in. |
| T4631 | `tests/diagram/pool-boundary-rules.test.ts` | The stop is stable: dragging 30 ticks past the obstacle leaves exactly the geometry 12 ticks left. Creep past a limit is how the original dissociation accumulated. |
| T4632 | `tests/diagram/pool-left-boundary.test.ts` | A pool's LEFT boundary moves in both directions again, taking its lanes and sublanes with it by the same delta so the header strips stay nested. Outward is unlimited; inward stops dead at the first content it meets — 300 (the start event) minus 18 (half an event) minus 108 (three header strips), and dragging further changes nothing. |
| T4633 | `tests/diagram/pool-left-boundary.test.ts` | ONLY boundaries move. Paul, 2026-09-21: "Make sure ONLY the Pool, Lanes and Sublanes boundaries are affected by the move. Nothing else must be affected at all." The white-box lockstep (kept, on his instruction) used to translate every DESCENDANT of the other pools, so nudging one pool's left edge in by 74px slid every task in every other pool 74px right. It now moves structural containers only, and — since those contents no longer travel — the drag stops at the first content ANY locked pool meets, which keeps the alignment the lockstep exists for. Nothing vertical changes at all. |
| T4634 | `tests/diagram/pool-left-boundary.test.ts` | A sub-lane is never shorter than its own name. Paul, 2026-09-21: "don't let sublanes be vertically minimised past their header width. Currently they can be narrowed so the name of the Sublanes overshoots the Sublane horizontal boundaries." The name is drawn ROTATED down the header strip, so its length is spent on the band's HEIGHT, and `SPLIT_LANE_EVEN`'s flat 28px floor gave "International Deliveries" a 28px strip to run down. Each band now claims its label-driven minimum FIRST and only the surplus is shared, so a long name is never funded out of a short one's floor and the bands still tile the lane exactly. |
| T4635 | `tests/diagram/pool-left-boundary.test.ts` | `clampRectToLimits` applies only the limits it is handed and leaves every other axis alone. It exists because a white-box pool's left and right edges are not that pool's business alone — the caller works out the minimum across every locked pool, this applies it. |
| T4636 | `tests/diagram/pool-left-edge-and-lift.test.ts` | A pool offers all four boundaries, west included. `resizeEdges.ts` is the single answer to "which edges may be dragged" — it replaced four hand-written copies of "pools never move their LEFT boundary" (two edge-zone filters, the w/nw/sw square handles, and a missing `onLeft` in the click-to-select edge band). T4632 had already fixed and tested the REDUCER; the gesture never reached it, which is why Paul reported "No joy with left boundary move!!" and then "It's not selectable?". |
| T4637 | `tests/diagram/pool-left-edge-and-lift.test.ts` | The canvas ASKS that rule rather than restating it — no hand-written west-edge suppression survives in `Canvas.tsx` or `SymbolRenderer.tsx`, every edge list calls `edgeIsResizable`, and the black-box pool's edge band has its `onLeft` case. The black-box assertion is anchored on that branch's own comment: three branches in the file test the same four edge names, and a first-match search passed happily while the one that mattered was still broken. |
| T4638 | `tests/diagram/pool-left-edge-and-lift.test.ts` | A connector rises with a dragged container only when BOTH its ends do; one that crosses out of the group stays put, because showing that it leaves is its job. Plus the lifted overlay's draw order — container, lane, connector, element — the diagram's own order, so a group that rises looks exactly as it did, only higher. |
| T4639 | `tests/diagram/pool-left-edge-and-lift.test.ts` | Paul, 2026-09-21: "When a Pool is selected and the arrow keys are used to move it part of the content disappears. Lanes, sublanes and sequence connectors disappear, other elements remain visible." Nothing was hidden — it was COVERED. `moveElement` marks a container and its descendants as TRAVELLING so a dragged pool rides above the diagram (his 2026-09-18 rule) and the matching END clears the mark; arrow keys called the move and never the end, so a nudged pool stayed pinned to the top overlay for the session ("I have to click elsewhere and then again on the Pool header"). The overlay also lifted the pool and its tasks but not its lanes or its flows, so an opaque pool body drew over both. Both halves guarded: every arrow ends its move, and everything travelling rises together — once, held back from the normal pass. **Revised 2026-09-22:** lifted lanes and flows are drawn in the overlay AS WELL, not instead; holding them back from their normal pass was the same unmount fault (T4658). |
| T4640 | `tests/diagram/sublane-divider-hold.test.ts` | Dragging a lane boundary moves THAT boundary and nothing else. Paul, 2026-09-21: "When moving the top or bottom boundary of a Lane with 2 or more sublanes the boundary moves ok but the middle sublane dividers also move. This does not happen with Lanes within a Pool. Only the boundary should move." The two levels disagreed: a pool resize picks ONE lane to absorb, `MOVE_LANE_BOUNDARY` spread the change proportionally over the sublanes. A stack must fill its container exactly, so the change goes to the band at the edge that MOVED — ABOVE's bottom edge is dragged so its LAST sublane absorbs, BELOW's top edge moved so its FIRST one does — and every other divider keeps its position. |
| T4641 | `tests/diagram/sublane-divider-hold.test.ts` | The drag stops at the band that is actually giving way, not at the lane. The old clamp asked whether the LANE had 40px spare — a 240px lane would have allowed a 200px drag and then squeezed its first sublane to 40, moving the next divider to make up the difference. It asks the absorbing band's own label floor now, and comes to rest there. |
| T4642 | `tests/diagram/sublane-divider-hold.test.ts` | The band arithmetic in `laneBands.ts`: `absorbAtEdge` gives the delta to one end and floors it at that band's minimum; `shrinkRoom` is RECURSIVE, because only the edge band changes size, so a stack's room is exactly what ITS edge band can give, all the way down. Summing every band's minimum over-states the room by whatever the untouched bands hold — worth two pixels of overflow in the nested case that caught it. |
| T4643 | `tests/diagram/sublane-divider-hold.test.ts` | Deeper nesting obeys the same edge, and the gesture stops on the floor of the deepest band that must give way. Also: a band that did NOT absorb travels with its own sublanes rather than being re-fitted — re-fitting would "correct" an already-inconsistent inner stack and move a divider three levels from the user's mouse, which was a second route to the same complaint. |
| T4644 | `tests/diagram/pool-boundary-voice.test.ts` | "{Move, Nudge} Pool {left, right, top, bottom} boundary {left, right, up, down}" (Paul, 2026-09-21). Every combination he named, plus a named pool, a possessive and a distance. Tried BEFORE the whole-pool nudge: "move the pool's left boundary right" ends in a direction word exactly as "move the pool right" does, and the nudge rule would have swallowed it and slid the entire pool. |
| T4645 | `tests/diagram/pool-boundary-voice.test.ts` | An impossible pairing is refused, not guessed. A left/right boundary is a vertical line and only slides sideways; "move the left boundary up" is not a near miss but two different gestures, so the grammar declines it and `validateOps` declines it again when the AI offers the same thing back. "Outward"/"inward" are left unrecognised for the same reason — they reverse meaning between opposite edges. |
| T4646 | `tests/diagram/pool-boundary-voice.test.ts` | The rect a spoken boundary move asks for: the named edge moves, the opposite one stays fixed, in every direction. It is handed to the same RESIZE_ELEMENT the mouse uses rather than written as new geometry, so a spoken move stops at the content (T4627/T4633), carries the lanes, and leaves the pool its lane stack (T4628) — one rule, one place. |
| T4647 | `tests/diagram/lane-divider-reachable.test.ts` | A lane / sub-lane divider can actually be grabbed. Paul, 2026-09-21: "Check that I can select the intermediate Lane and sublane boundaries. This is intermittent at the moment." Intermittent was the tell — the hit-strips were rendered with the lanes, BEFORE the connector layer and the elements, so any shape crossing the boundary took the mousedown. A divider runs the full width of its pool, so almost everything crosses one somewhere: the strip answered along the empty stretches and did nothing where the process was. Drawn in a top overlay now, beside the pool edge-zone overlay that exists for the same reason. |
| T4648 | `tests/diagram/lane-divider-reachable.test.ts` | The strip starts where the header ends. Both strips hardcoded the header width — 30 for a pool, 36 for a lane — instead of reading the container's own, so a multi-line pool name (header 58) left the divider starting INSIDE the header, covering the band you click to select the pool. |
| T4649 | `tests/diagram/pool-boundary-transcript.test.ts` | Every line of Paul's 2026-09-21 voice log, on the free rule path. The first cut was two regexes over a tidy sentence; the transcript is punctuated mid-command ("Nudge. Pull, left boundary, left."), spells pool as PULL, varies the word order, and drops the verb ("Left boundary, left."). Token scan instead of regex, the recogniser's pool spellings accepted as the rest of the grammar already accepts them, and the LAST sentence read when the recogniser punctuates a false start — so "Merge selected pool. Left boundary left." stops inventing a pool called "Merge selected". |
| T4650 | `tests/diagram/pool-boundary-transcript.test.ts` | The edge you did not name. "Nudge pool boundary left" takes the edge the movement is heading for — the only reading that does not require guessing at something unsaid. The DIRECTION is never inferred from the edge: left and right are both perfectly sensible for a left boundary, so that returns "needs-direction" and the sentence is declined. |
| T4651 | `tests/diagram/pool-boundary-transcript.test.ts` | A boundary sentence never becomes a 900px move. "Move the pool left boundary" used to fall through to the element move rule, which read it as "move the pool left" and moved by a whole element SPAN — for a pool, its own width. That is Paul's "the move of a pool appears to be way too large a move". Both greedy rules now decline a sentence naming a container boundary, and both are proven load-bearing separately: the nudge rule claims "…boundary up" (it ends in a direction), the move rule claims "…boundary right now" (it does not). |
| T4652 | `tests/diagram/pool-boundary-transcript.test.ts` | Half a sentence is still not a command — "Left.", "right.", "Nudge, pull one." produce no boundary op — and an unrelated "edge" in a label is not a pool boundary. |
| T4653 | `tests/diagram/assoc-hit-mask.test.ts` | A Data Object's association never takes a click off its own element. Paul, 2026-09-21. The clip that cuts shapes out of an association's hit path uses `evenodd`, which COUNTS crossings rather than unioning them — so a rectangle listed twice is inside an even number of subpaths and stops being a hole. `sourceBounds`/`targetBounds` were passed explicitly AND were already in `maskBounds`, so the only doubled rectangles were the association's own two endpoints, and the line stayed clickable inside exactly the shapes it belongs to. Centre-to-centre is why it surfaced: a routed association clips only the edge of its endpoints, while a centre-to-centre line runs through the middle of both, under the pointer. |
| T4654 | `tests/diagram/assoc-hit-mask.test.ts` | A pool header is a handle, not a connector source. Paul, 2026-09-21: "clicking on the Pool header should always just allow pool movement … and never go to connector create mode" — the connect overlay now starts where the header ends, and the header shows a grab cursor on every pool. Plus the gesture rule behind his "pools almost immediately go into connector creation mode": that overlay covers a SELECTED element and read a 5px movement as "start drawing a connector". Moving the mouse now MOVES the shape, as a drag does everywhere else; only a 300ms hold WITHOUT moving connects. **Revised 2026-09-22 — protocol item 4 decided:** a drag on an ALREADY-SELECTED shape draws a connector again ("click, click and press → connector creation mode"). The move-on-drag rule this row first described is reverted; the pool-header exclusion stays, so the report that prompted it stays fixed. |
| T4655 | `tests/diagram/canvas-help-and-header-drag.test.ts` | A press on a pool or lane HEADER takes hold of the container, whatever its selection state. Paul, 2026-09-22: "It would be good if it did grab and reliably then move the pool." The unreliability was one line — on an already-SELECTED container the press called `onSelect()` and returned, so the drag never began, and since clicking the header is also how you select it, every attempt after the first did nothing. Deselecting is now a CLICK rather than a press: under 4px it deselects as before, past 4px the container moves. |
| T4656 | `tests/diagram/canvas-help-and-header-drag.test.ts` | The header cursor can close to `grabbing`. It was an inline `style={{ cursor: "grab" }}`, and inline cursor cannot reach the `:active` pseudo — which is the entire reason the G05 scheme uses `.dgx-grab` / `.dgx-grab:active` classes in `globals.css`. Paul: "the cursor remains a hand." |
| T4657 | `tests/diagram/canvas-help-and-header-drag.test.ts` | The Canvas Help card — the pointer protocol in the same draggable window the Voice Assist commands use, opened from the status bar immediately before the zoom readout. `FloatingPanel` is now shared rather than copied. The catalog is data, so tests can hold it honest: every row names a cursor that has a glyph, and names only cursors the canvas actually sets — a card promising a cursor nothing produces is worse than no card. **Revised 2026-09-22:** the status-bar percentage is gone ("remove the status-bar percentage") — it showed the true scale while the zoom control shows the scale relative to how the diagram opened, so the two disagreed; Canvas Help is now the last item in the bar. |
| T4658 | `tests/diagram/grab-cursor-and-lift.test.ts` | A travelling container is never unmounted mid-drag. Paul, 2026-09-22: "I have to click and press on the header region twice in a row to move it … the green alignment dots and line appear and stop any further movement." The guide was a symptom. The first mousemove marks the pool travelling; the next render took it OUT of the container pass and drew it only in the lifted overlay — a different place in the tree, so React unmounted the SymbolRenderer that owned the drag, and its unmount cleanup removed the drag's listeners. The drag died after one tick, mouseup never arrived, move-end never fired, and the lift and the guide stayed on. The second press worked only because the pool was already stuck in the overlay. Tasks never had it: ordinary elements were never taken out of their pass. The overlay is now a `pointerEvents="none"` picture; the instance holding the gesture stays put. |
| T4659 | `tests/diagram/grab-cursor-and-lift.test.ts` | A selected element shows ✋ grab, not ✛. Paul: "click and press still does not produce a Grab cursor on elements." The crosshair was right when a drag on a selected element drew a connector and wrong since `77122bb3`, when the drag started MOVING it. Via the class, so it closes to ✊ on press. The Help card agrees. **Revised 2026-09-22:** the selected-element overlay is ✛ again, since the next press there connects; the ✊-during-a-move that Paul asked for comes from the canvas-wide gesture cursor (T4661) instead. |
| T4660 | `tests/diagram/grab-cursor-and-lift.test.ts` | Labels grab and close. Paul: "I'd like the grab cursor for labels as well." Both external element labels and connector labels were `grab` INLINE — which cannot reach `:active`, and which also outranked the body's "grabbing" the label drag sets, because a cursor on the element under the pointer wins over the body's. So they showed an open hand and never closed. They use `.dgx-grab` now. |
| T4661 | `tests/diagram/gesture-cursor.test.ts` | The canvas root carries the GESTURE's cursor for as long as the gesture lasts — `.dgx-dragging` while an element moves, `.dgx-connecting` while a connector is drawn. Paul's decided protocol, 2026-09-22: "1. click and press → Drag cursor. 2. click, click and press → connector creation mode, and when the targets are highlighted the cursor should change to '+'." Neither could hold its cursor from the shape it started on: pressing an unselected element SELECTS it, the selected state lays a crosshair overlay across it mid-drag, and that ✛ was the "+" he kept seeing through a move. A window mouseup clears the move state, so ✊ cannot be left on. |
| T4662 | `tests/diagram/gesture-cursor.test.ts` | …and it wins over every shape the pointer passes (`!important` on the root and all descendants), because every shape carries a cursor of its own and during a gesture the gesture is what the cursor has to describe. |
| T4663 | `tests/diagram/cursor-icons.test.ts` | The Canvas Help card draws the real cursors. Paul, 2026-09-22: "The icons do not match closely enough to the real cursor shapes." Emoji are drawings OF hands and arrows in their own style; the cursors are white shapes outlined in black, so `CursorIcon` draws them that way — every cursor on the card is an SVG in those two colours, except the 👆 pointer, which he asked to keep. The card renders the drawings, not the glyphs. **Revised 2026-09-22:** the pointer is drawn too now — "Change the pointer to the same style as the other hands" — so every cursor on the card is an SVG and no emoji is left. |
| T4664 | `tests/diagram/cursor-icons.test.ts` | The hands are the right hands, checked on the rendered geometry: the open hand's thumb sits LEFT of its fingers and the fingers are tilted apart ("thumb on the wrong side… fingers bigger and slightly spread"); the closed hand shows the BACK — short knuckles along the top, thumb on the left ("it grabs with the back of the hand towards the user") — and its thumb is drawn before the hand so it reads as attached, not the floating blob of the first draft. Also the POINTING hand: index finger up and tallest by far, the other three curled to knuckles, thumb on the left. |
| T4665 | `tests/diagram/cursor-icons.test.ts` | The rows he found missing: both corner handles (`nw-resize`, `ne-resize` — the names the handles actually set), separate rows for left/right and top/bottom edges, and a Zooming section. His item 7, "scroll when over a panel resizes all panels", is recorded as what it is: no Diagramatix code does it. The canvas keeps scroll for its own zoom; over a panel, Ctrl + scroll or a pinch reaches the BROWSER, which zooms the whole page. The test pins that there is still only the one wheel handler, so the entry cannot silently go stale. |
| T4666 | `tests/ui/canvas-control-row.test.ts` | The bottom-right canvas controls are ONE flex row — Bubble help · Zoom · Auto-connect — with the zoom bar beside Auto-connect. Paul, 2026-09-22: "Move the zoom control next to Auto-connect to allow more space." Each control was absolutely placed with a hard-coded offset, and the zoom bar's assumed Bubble help was always present ("0.5rem + 156px + 6px + 130px + 6px"), so wherever Bubble help is hidden it stood ~136px out across an empty gap. In a row each takes its own width and a hidden one leaves nothing behind; no child positions itself, and no offset survives on the canvas. T4546's zoom-bar check follows it into the row. |
| T4667 | `tests/ai/saved-prompt-filter.test.ts` | One filter rule for every saved-prompt list — the AI Generate console and both sidebar panels (`AiPanel`; `PlanPanel`, BPMN's "AI Plan"). It matches the NAME or any phrase in the TEXT, case-insensitively, and shows everything in order when empty. The console's private copy is gone, so the three cannot drift into matching different things. |
| T4668 | `tests/ai/saved-prompt-filter.test.ts` | Both sidebar panels: the filter sits on the Saved Prompts heading's own line, to its right, and the list is twice its old height (AiPanel `max-h-56` for `max-h-28`; PlanPanel 192px for 96, still drag-resizable 40–400). Paul, 2026-09-22: "Saved Prompts need a filter field on the same line as the heading to the right" and "Allow 2 x the height". PlanPanel's list now fills its section as a flex child instead of a `calc(100% - 16px)` that assumed a 16px heading — which stopped being true the moment an input sat beside it. |
| T4669 | `tests/diagram/cursor-note-tokens.test.ts` | No emoji cursor survives in the Canvas Help card's text. Paul, 2026-09-22: "2 occurrences of the grab cursor still in yellow in the Canvas Help under What the Cursor is Telling You." The icon column had been redrawn but the NOTES still typed ✊ — "closes to ✊ while you hold it" — and two more hid in the Moving and Connecting notes. Notes now name a cursor as a token (`{grabbing}`, `{crosshair}`) and `NoteWithCursors` draws the real white-and-black cursor there, inline; no cursor glyph may appear in any heading, row or note, and the card sends every note through it. |
| T4670 | `tests/diagram/label-follow.test.ts` | A sequence-flow label goes with the horizontal segment it sits by. Paul, 2026-09-22: labels "near a horizontal segment … should move with that horizontal segment as it is moved up or down. If they are further away then when they get very close they should then move with that segment." A label within 8px above or below the dragged segment (spans overlapping) moves the whole way with it, either direction; one further off stays put until the segment comes within 8px, is picked up at exactly that gap on the side it was on, and is carried from then on — even if one mouse sample jumps the segment clean past it. Driven through the real reducer (`UPDATE_CONNECTOR_WAYPOINTS`), which used to pin every label in place while its line slid away, because dragging a middle segment moves neither end and so not the anchor. Sequence flows only, as asked. **Revised 2026-09-22:** the rule no longer lives in `UPDATE_CONNECTOR_WAYPOINTS`; it runs in the reducer wrapper after every route-changing action (T4678–T4681), and these tests now pass through that one place. |
| T4671 | `tests/diagram/label-follow.test.ts` | The rule's parts: the moved segment is found by its unchanged x-span, or — when collinear fusion has merged it into a neighbour — by containment; a label not over or under the segment is ignored; one gap means both "close" and where a picked-up label lands. And the label ANCHOR, pinned to the renderer's own numbers: source point for a source-anchored label, cubic-bezier midpoint for a FOUR-point route, ends' midpoint otherwise. `connectorLabelBox` had been measuring four-point routes from the ends while the canvas drew them from the curve — found when this rule put a label somewhere the canvas did not. The checks, the reducer's `computeLabelAnchor` and these rules now share `baseLabelAnchor`, so the test pins it to the renderer rather than to the copies agreeing. |
| T4672 | `tests/diagram/label-follow.test.ts` | Moving a gateway leaves its top- and bottom-vertex branch labels where they are in the diagram; the MIDDLE-vertex branch's label, if there is one, still travels with it. Paul, 2026-09-22: "when moving the gateway the connectors originate from, do not move these labels as is currently done. Except the label on the middle vertex connector" — "if it exists!". A branch label is anchored at the gateway vertex, so every one used to go with the gateway even where the line beside it had not moved. Applied in the reducer wrapper after `MOVE_ELEMENT` / `MOVE_ELEMENTS`, where every exit of a move passes; a GROUP move — gateway and target together — still carries everything. |
| T4673 | `tests/diagram/pool-boundary-messages.test.ts` | Moving a pool's left boundary moves boundaries and nothing else. Paul, 2026-09-22: "Moving the Pool left boundary left causes message connectors FROM other pools TO elements in the pool … to move with the pool's left boundary." Reproduced exactly: when the white-box lockstep stopped moving other pools' CONTENTS it began recomputing every connector attached to a shifted pool, and a message ending on a POOL stores that end as a fraction of the pool's width — so the Customer → Receive line walked 422 → 390 → 365 → 409 across three drags while nothing it connects had moved. Now every message's drawn line stays put through repeated drags (pool → element, element → pool, element → element), no element in any pool moves over four drags in a row, and the boundaries still move in lockstep. |
| T4674 | `tests/diagram/pool-boundary-messages.test.ts` | `pinPoolMessageEnds` re-derives a message's pool-side fraction from where the end WAS, against the pool's NEW geometry, for every pool whose geometry changed — not just the one being resized — at either end; a message whose pool did not change, and anything that is not a message flow, come back untouched. Applied in both places a pool is reshaped under a message: the pool-resize branch and the white-box lockstep, which also runs when an element pushes a pool wider (a path no resize test reaches, so its wiring is pinned). Either layer alone holds the line; disarming the pin fails the behaviour test. |
| T4675 | `tests/diagram/pool-boundary-messages.test.ts` | A Dev Tools gesture trace, off unless switched on (`localStorage.setItem("dgx.traceGestures","1")` then reload, or `window.__DGX_TRACE_GESTURES = true`). Paul's "elements move left and sometimes downwards" on a second boundary drag does not reproduce in the reducer, so it is a question of which HANDLER takes the press — the left-edge resize zone or the header strip beside it, which moves the whole pool. The trace logs each press (which handler took it and where it landed against the edge and the header), each drag start (MOVE or RESIZE), how an edge press was decided, and after every move or resize every element that moved and by how much. Silent when off; `movedElements` reports exact deltas. |
| T4676 | `tests/diagram/edge-zone-owns-press.test.ts` | `edgeZoneAt` says which edge zone a press lands in, with the SAME bands the zones are drawn with (`edgeBand`: 14px outside, a height-scaled slice inside), so the shape's own press handler and the zone cannot disagree about who owns a press. Replays Paul's captured press — 2px outside a pool's left edge (worldX −55, edge −53) — and gets the LEFT zone; exact inner and outer limits; all four edges; a corner goes to the zone drawn on top, as the browser does; an edge the shape does not offer is ignored. |
| T4677 | `tests/diagram/edge-zone-owns-press.test.ts` | One press, one gesture. Paul's Dev Tools trace, 2026-09-22: a single press on a pool's left boundary logged "took: HEADER (moves the container)" AND "edge-zone decided RESIZE", then MOVE_ELEMENT (11 elements) and RESIZE_ELEMENT on every mousemove — "Problems when moving left boundary right!!!". Two faults: the header test had no lower bound (`x <= edge + header`), so a press outside the pool counted as its header; and the zone drawn over the pool does not stop the press, so the pool's handler ran too and both drags ran at once. Now the handler checks the zones FIRST, and a press in one only makes sure the pool is selected — never toggling it off, never starting a drag — leaving the zone to decide: across resizes, along moves. The header test is bounded on both sides. |
| T4678 | `tests/diagram/label-follow-route-change.test.ts` | Paul's case 1, 2026-09-22: "L-shaped connector enters a Task and attaches to the left boundary. Move the Task upwards, then the horizontal segment of that connector moves up but the label does not." The label-follow rule had run on ONE action — a segment dragged by hand — while a route changes through a dozen; moving an element recomputes its connectors through another. It now runs in the reducer wrapper after every route-changing action. A gateway's source-anchored top branch keeps its label 6px above the entry segment as the task goes up and down; a task-to-task flow, whose midpoint anchor moves only HALF as far as the segment, does too — which is why the label is PLACED relative to its segment rather than nudged. |
| T4679 | `tests/diagram/label-follow-route-change.test.ts` | Paul's case 2: "Re-routing a connector does not move the label with the horizontal segment." A segment pushed 40px off its natural height goes back under Re-route all, and its label comes back with it; an endpoint re-attached lower on the task's side moves the entry segment, and the label follows. |
| T4680 | `tests/diagram/label-follow-route-change.test.ts` | Only where a route changed, and only for sequence flows: a label on an untouched connector is the SAME object after an unrelated move; an association is left alone. The pass runs after every route-changing action — element moves and resizes, segment and endpoint drags, nudges, Re-route all, the connector-correction pass — and deliberately not after a whole-diagram replacement (SET_DATA), which places its own labels. |
| T4681 | `tests/diagram/label-follow-route-change.test.ts` | Which segment a label belongs to after the route changes. When a segment SPLITS — a gateway's straight middle branch becomes a Z as the gateway moves — the label stays with the half beside it, the one nearest where its line would be if it had moved with the label, not whichever half shares more of the old span (which would drag it back down). And a label never belongs to an INVISIBLE leader inside a shape: the rule looks only at the route as drawn, so a hidden edge-to-centre stub moving never moves a label. |
| T4682 | `tests/sharepoint/fail-gracefully.test.ts` | Paul, 2026-09-22: "trying to set a Sharepoint link on a Data Object should always fail gracefully if user not logged in to a Microsoft account, currently it can kill the app." How a failed SharePoint request is read for the user (`app/lib/microsoft/sharePointOutcome.ts`): the routes' no-token answer, and a token Microsoft REFUSES (Graph 401 — signed out, password changed, consent revoked), both mean "not connected" and offer to reconnect; the org-policy 403 means unavailable, with nothing to connect; an ended Diagramatix session is its own case; anything else keeps the picker open with the message. |
| T4683 | `tests/sharepoint/fail-gracefully.test.ts` | A Graph failure always gets a response the route can send: a status no HTTP response can carry (a network failure reports -1) becomes 502 instead of making the response constructor throw; a real status passes through with Graph's message or the fallback. All three SharePoint routes answer through the shared helpers, and a token lookup that throws reads as not connected rather than a 500. |
| T4684 | `tests/sharepoint/fail-gracefully.test.ts` | The editor is never taken down by a SharePoint window: "Connect" opens Microsoft's sign-in in a NEW tab (it used to navigate the whole editor away, stranding anyone not signed in to Microsoft), returning to a page that says the tab can be closed; a response that is not a list renders as an empty list; and every SharePoint picker and preview in the editor sits inside a `SafeBoundary`, so a render failure closes the window, not the diagram. |
| T4685 | `tests/diagram/lanes-within-pool.test.ts` | Paul, 2026-09-22: "Adding lanes to a Pool should not grow the Pool. The lanes must be added within the Pool." … "Never grow the Pool with these Lane and Sublane additions." Every way of adding a lane or sublane — first lane, another lane, a lane beside a named one, N lanes, sublanes, N sublanes — actually adds one and leaves the pool's position and size exactly as they were, every lane inside it, and the half-event gap in front of the first element still there. |
| T4686 | `tests/diagram/lanes-within-pool.test.ts` | The half-event gap (Paul's rule of 2026-09-21, which had grown the pool leftwards) is now made by moving the pool's content RIGHT as one ordinary move: every element by the same amount, the flow between them running between their new positions; nothing moves when the gap is already there. |
| T4687 | `tests/diagram/lanes-within-pool.test.ts` | With too little room on the right, the content moves only as far as there is room and never past the pool's right boundary; and the reducer has no path left that widens a pool to make the gap. |
| T4688 | `tests/diagram/lanes-within-pool.test.ts` | Every palette drop zone — pool top, pool bottom, a separator, inside a lane, a sublane inside a lane that has sublanes — adds its band WITHIN the container: carved out of the neighbour's empty space, the lanes still tiling the pool exactly, and every task still in the lane it was in and wholly inside it (a crowded edge slides the task into its own lane's free space rather than covering it). |
| T4689 | `tests/diagram/lanes-within-pool.test.ts` | With no room, nothing is added and nothing grows: a lane too small to give up a labelled band refuses, and so does a lane its process fills, rather than cover the process or push it out. The drop no longer draws a coloured insertion line (Paul: "No need for coloured highlights to indicate Lane or Sublane creation"), and no lane-adding path grows the pool. **Revised 2026-09-23** (Paul: "If this is not possible just divide the Pool in two … Only reject adding at any level if the new lane/sublane etc. has not enough space for the name in its header region") — a full lane is now HALVED rather than refused, and the name is the only refusal left. See T4701. |
| T4690 | `tests/sharepoint/fail-gracefully.test.ts` | Paul, 2026-09-23, linking a Data Object: `{"error":"SharePoint is not configured on this server."}` — a raw JSON error page where the Microsoft sign-in should have been. Two defects. The check that decided whether to OFFER SharePoint asked only for the Entra app's id and tenant, while connecting also needs the client secret and `MS_TOKEN_ENC_KEY` — so a server with the first two offered SharePoint everywhere and refused at the last step; every "is SharePoint available here" question now comes through `sharePointServerConfigured()`, which also rejects a key that is set but not 32 bytes. And the connect route, which is opened as a page, answered with a JSON body; it now redirects to a page with words for every outcome, while the picker asks `/api/microsoft/status` BEFORE offering to connect and refuses in a Diagramatix error popup (Paul: "Replace with a Diagramatix Error popup window"). |
| T4691 | `tests/canvas/message-label-hidden-on-drag.test.ts` | And only while it is over that pool: a pool moved anywhere else keeps its message labels, edges that merely touch are not "over", overlapping a pool it exchanges nothing with changes nothing, a pool-less element under the pool hides its own flow and only it, and a message wholly inside the dragged pool is never hidden (both ends travel with it). Which pool an endpoint belongs to is walked up the parentage, so a task-to-task flow between two pools counts as a flow between those pools. |
| T4692 | `tests/diagram/lane-drop-preview.test.ts` | Paul, 2026-09-23: "as the drag moves inside the Pool a ghostly image of the new Lane or Sublanes appears inside the Pool to show what would happen … If no new lane or sublane is going to be created, then the Pool boundary should turn red." The preview and the drop are ONE decision (`planLaneDrop`), so each zone — pool top, pool bottom, a divider, the middle of a lane, an empty pool — is planned and then dropped, and the band that appears is the band that was drawn, by rectangle and by name. Checked against the RESULT as well as the plan (the stack still tiles the pool exactly, every band fits its own name, every task still inside its lane), because a reducer that applies the plan would otherwise agree with a plan that lied. |
| T4693 | `tests/diagram/lane-drop-preview.test.ts` | "Nothing will happen" is the truth and is shown in red: a pool whose lanes are too full, a lane too short to split in two, and a band that could not fit its own name all plan to `none` — naming the pool, so the canvas knows whose boundary to redden — and the drop really does nothing. Outside every pool there is no pool to redden and the drop still makes a POOL. The canvas draws the plan's own rectangles, and a red outline when there are none. **Revised 2026-09-23:** the ghost is drawn from `previewBands`, which carries each band's own header width and its proposed name — see T4702. |
| T4694 | `tests/diagram/lane-drop-preview.test.ts` | One decision, drawn or done: the reducer's Pool/Lane drop branch applies the plan and no longer holds any zone arithmetic of its own, and `samePlan` keeps the ghost still while the answer has not changed (two points in the same zone are the same plan; a lane and a split are not). |
| T4695 | `tests/diagram/assist-log-2026-09-23.test.ts` | Paul's Voice Assist log, 2026-09-23: `Compact pool three.` → `which “three”? 2 match: “Pool 3”, “Lane 3”`. A spoken KIND word is a constraint, not decoration: "pool three" can only answer with a pool, "sublane one" only with a sub-lane, and when nothing of that kind matches the answer is "not found" rather than something of another kind. A whole NAME still wins over the kind word, so an element genuinely called "Pool cleaning" is still found, and a bare kind word still means "the one of that kind". |
| T4696 | `tests/diagram/assist-log-2026-09-23.test.ts` | The confirmation is pinned to the element it named. Five lines of the log read `Yes.` → `confirmed → which sub-lane? there are 2 — I couldn't match “one”`: the question resolved the reference, and the answer, an utterance later, resolved it again against a moved selection. `needsConfirmation` now returns the id, and the editor substitutes it into the parked ops the way the picker does, so "yes" acts on the thing the question named or on nothing. |
| T4697 | `tests/diagram/assist-log-2026-09-23.test.ts` | The container-delete guard — "never delete a container we can't confidently identify" — stopped firing on references that are already exact: an `#id:` from the picker or a pinned confirmation, the selection, the pointer. It had been answering `which lane? there are 3 — I couldn't match “#id:il8erk8a”`, putting an internal id in front of the user. It also compares numbers the way the resolver does, so "delete sublane one" is no longer refused against a lane called "Sublane 1". |
| T4698 | `tests/diagram/assist-log-2026-09-23.test.ts` | A new lane is named against the diagram and reported as named. The grammar asked for "Lane 1" — a name it could not know was taken — so the reducer quietly made "Lane 1 2" while the log said `added 1 lane to Pool 3: Lane 1`, twice. The grammar now asks for the bare kind and `containerNames.ts` numbers it; the log reads the same helper, so the name in the sentence is the name on the canvas. |
| T4699 | `tests/diagram/assist-log-2026-09-23.test.ts` | `Add a third lane to pool three.` created a TASK labelled "third lane to pool three" and reported success — the lane rule counts "a/one/two/three", not "a third", so the generic add rule swallowed it. A phrasing that still talks about lanes or pools after the container rules have declined it is a phrasing they could not read: it goes to the AI instead of becoming an element's name. "Assembly Line" and "Approve Invoice" are untouched. |
| T4700 | `tests/diagram/assist-log-2026-09-23.test.ts` | Asking WHICH one in a way that can be answered: `which “pool three”? 2 match: “Pool 3”, “Pool 3” — say the name` cannot be answered by name, so the container commands raise the numbered picker like the delete path does. And a sublane asked of a POOL (`Add sublane to pool` → `Pool 3 isn't a lane`) uses that pool's lane when there is only one, asks which when there are several, and says the pool has none when it has none. |
| T4701 | `tests/diagram/lanes-within-pool.test.ts` | Paul, 2026-09-23: "always add the new lane by trying to locate it at the top or the bottom where there are no existing elements, if possible. If this is not possible just divide the Pool in two and let the user resolve the lane divider issue… Only reject adding at any level if the new lane/sublane etc. has not enough space for the name in its header region." A lane asked for at the crowded end is placed at the empty one; with no empty space anywhere the neighbour is halved and nothing is moved, so an element may straddle the new divider — visible, and the user's to settle; the same rule applies one level down to sublanes. The earlier rule (never cover content, refuse otherwise) made a drop do nothing on exactly the pool a person is most likely to be reorganising. |
| T4702 | `tests/diagram/lane-drop-preview.test.ts` | Paul, 2026-09-23: "The ghost highlight … extends to highlight the parent header column. It would be clearer … to show a highlight of exactly what would be added. i.e. Just show the new child or children that will be created with their header regions and proposed initial names." The ghost is now one band per thing that will appear — two for a split, none for a refusal — each with its OWN header strip (the band's, not the parent's: pinned with a pool whose header is 60 and lanes 36) drawn INSIDE it at its left edge as `LaneShape` draws it, and the name it will be given. The name comes from the reducer's own naming, so the ghost and the canvas agree. Sublanes are named "Sub N" rather than "Sublane N" — the name runs down a 36px strip, so "Sublane 3" needs ~92px of band and "Sub 3" needs 58, which decides whether a sublane can be added at all. Existing "Sublane N" names are left alone and do not block "Sub 1". |
| T4703 | `tests/diagram/assist-reference-reliability.test.ts` | Paul, 2026-09-23: "spoken 'line' should always resolve quickly to 'lane'." The command grammar had spelled lane as `lanes?|lines?` and pool as `pool|poll|pull` for months; the reference RESOLVER kept its own narrower list, so "add a lane to poll three" worked and "delete line one" did not — the same word understood by one half of the feature and not the other. One list (`containerWords.ts`) now answers for both, including the plural "lines" and "sub line". |
| T4704 | `tests/diagram/assist-reference-reliability.test.ts` | A number heard as its homophone: "lane won" is Lane 1, "sub too" is Sub 2, "pool tree" is Pool 3. The phonetic pass could never reach these, because a name written with a DIGIT has no sound to compare. The fold runs only after a strict read has failed, so an element genuinely called "Won" keeps its name — proven by a diagram that has one. |
| T4705 | `tests/diagram/assist-reference-reliability.test.ts` | A reference to what is INSIDE a named container. "The top sublane in lane one" used to resolve to LANE ONE — the words it could not read were dropped until what was left named the parent, and a delete then offered to remove the lane and everything under it. It now reads the phrase properly: top/bottom/middle pick one, a plural names them all (which raises the picker), and it reaches non-containers by type or name ("the task in sub one"). Nothing inside means NOTHING — never the container itself. |
| T4706 | `tests/diagram/assist-reference-reliability.test.ts` | A picker answers to a name as well as a number, because the question shows the names and Paul's `Pool three.` came back "didn't understand that" right after being asked which "pool three" was meant. The name is read FIRST: `leadingSpokenNumber` forgives "lane" as a mis-heard "one", so "Lane 2" was being read as the number 1 and picking the wrong candidate. A container word that names none of them in particular is not an answer at all, and the question stands. |
| T4707 | `tests/diagram/assist-convert-and-guards.test.ts` | Paul, 2026-09-23: "No flashing occurred when this command executed: `Move company right boundary, right.` → moved Company's right boundary right 20px." `movePoolBoundary` was on the flashing list and the snapshot was armed; the DIFF read x and y alone, so it saw a left or top boundary move (which shifts the pool's origin) and was blind to a right or bottom one, which changes only the width or the height. Half the command flashed and half of it silently did not — on the one op where the flash matters most, because nothing inside the pool moves. A resize now counts as a change, at the same half-pixel epsilon as a move, so sub-pixel noise still flashes nothing. |
| T4708 | `tests/diagram/lane-drop-preview.test.ts` | Paul, 2026-09-23: "in addition move the current name of any sibling whose name will be affected, to its new position and show in the same ghostly way so that new names never appear over the top of old names." A new band takes its room from a neighbour, so that neighbour's name — set down the middle of its header strip — ends up somewhere else, and the ghost's new name could land on top of it. The ghost now also draws every band the drop MOVES, with its own name, at the box it is about to have, and that band's real name comes off the canvas while it does (`GhostMovedNameIdsCtx`). The positions come from `carveGeometry`, which is the same code the reducer applies — pinned by comparing every ghosted box with the real one after the drop, including a donor whose own sub-lanes re-fit. A split moves nobody, and a refusal ghosts nothing. |
| T4709 | `tests/diagram/template-voice-pick.test.ts` | Paul, 2026-09-24: "add template" opens "a large pop-up window … with all of the templates in their groups, divided by built-in and user … large icons of the actual template and our familiar green numbers marking each one." The numbering is ONE unbroken sequence in reading order — built-in first, groups alphabetical, ungrouped last within each half — because the number is the thing the user says and it has to mean one card whatever they are looking at. Each card knows the header it is drawn under. |
| T4710 | `tests/diagram/template-voice-pick.test.ts` | "That template is provisionally added and requires the user to confirm, or use a different number … for that tentative template to be replaced." A number picks; another number replaces; "yes" keeps it; "cancel" or "no" takes it away. "Yes" before anything is showing means nothing, so a stray word cannot close the window on an empty answer. A NAME works too and is read BEFORE the number, the trap the disambiguation picker hit on 2026-09-23 (`leadingSpokenNumber` forgives mis-heard number words, and template names are full of them); a phrase matching two names is no answer. The command itself parses to `pickTemplate` and still leaves "add a task called template" alone. |
| T4711 | `tests/diagram/template-voice-pick.test.ts` | "Do not include any templates that include White-box pools i.e. initial templates unless there are no white-box pools on the diagram, or it is an empty diagram … and Starters category as well." An initial template is recognised by what it IS — it carries a white-box pool, or it sits in a Starters group — rather than by a tick somebody has to remember; the API derives the pool half in SQL from the template's own data, in both the built-in and the user query. A black-box participant pool does NOT make a template initial. An empty diagram is offered everything. |
| T4712 | `tests/diagram/template-voice-pick.test.ts` | "Note that insert a template may require the current Lane and Pool to be expanded to accommodate the new template." It did not: the inserted elements arrived with no parent, the adoption pass never saw them, and a template bigger than the room left simply hung over the edge of the pool. `APPLY_TEMPLATE` now adopts by geometry, grows the lane it landed in (`growLaneToHeight`, lifted out of the label-fitting rule so both ask the same code), pushes the lanes below with their contents, grows the pool, and leaves a template that already fits exactly where it is. |
| T4713 | `tests/diagram/message-hold-2026-09-24.test.ts` | Paul, 2026-09-24: "Moving a Task up or down, in a white-box pool, that has messages attached to it causes the message labels to move up or down … the message labels should not move at all." `settleMessageLabels` carried the label along with whichever END had moved, which is right when a POOL is dragged (the label belongs to the pool's edge of the conversation) and wrong for everything else — a task shuffled inside its lane took the label with it. The settle now asks which of the moved elements are pools: one at either end and the label travels as before, otherwise `holdMessageLabel` re-expresses the offset against the new midpoint so the label's WORLD position is untouched. Up, down and sideways, and whichever end the task is. |
| T4714 | `tests/diagram/message-hold-2026-09-24.test.ts` | The other half of T4713, so the fix cannot quietly become "labels never move": a pool at either end still carries the label by the pool's own delta, and `holdMessageLabel` is proved to hold the world position rather than the offset. Planting the previous rule (any moved end carries the label) fails four of these. |
| T4715 | `tests/diagram/message-hold-2026-09-24.test.ts` | Paul, 2026-09-24: "any message originating on pools that were extended to the right also move to the right … This does not happen to messages terminating on any of the extended pools" — then, trying again, "I can't reproduce the pool right hand boundary move defect." Both are true. Dragging the boundary is `RESIZE_ELEMENT`, which pinned already; **Extend pools** sets every pool to a common width and recomputes with no pin at all, and a message sits at a FRACTION of its pool's width whose visible run takes its x from the SOURCE end — hence the asymmetry, exactly as reported. The pin now belongs to the width change itself, in the reducer wrapper, so `EXTEND_POOLS`, an enclosure pass, a layout re-run or an import cannot forget it. Note the shape of the evidence: waypoint 0 is an invisible leader at the element's centre and DOES follow the pool — reading it is what made a message that never moved look like it had. |
| T4716 | `tests/diagram/message-hold-2026-09-24.test.ts` | Paul, 2026-09-24: "The hover large template image appears under the drop down template menu. This image must always be clear of the drop-down menu itself. Further to the left is fine." `right-full` was not enough — the menu is 20rem wide and anchored to the RIGHT of the same wrapper, so it reaches far to the left of it and the preview landed behind. The panel is now offset from that same right edge by the menu's own width plus a gap, and painted above it. |
| T4717 | `tests/diagram/assist-command-log.test.ts` | Paul, 2026-09-24, asked for a debug feature that lets him "type a comment next to each attempted command to indicate whether it worked". `CommandLogEntry` moved out of `VoiceAssistBar.tsx` into `app/lib/assist/commandLog.ts` to be testable at all — the suite is node-only with no jsdom, so twenty-four assist tests can only assert on `.tsx` files as source text. Every field added for debugging is OPTIONAL, because `correctionPairs.ts` consumes the shape structurally as `LoggedCommand` and the session tally it computes is already on screen; a required field would have broken that readout silently. The guard is that an enriched log and a bare one produce an identical tally. |
| T4718 | `tests/diagram/assist-command-log.test.ts` | The debug toggle (`dgx.voiceDebug`, localStorage, the `goldFlash` pattern) departs from its model in two ways that look like bugs side by side, so both are pinned: it defaults **off** where gold flash defaults on — nobody should accumulate annotated sessions containing snapshots of their diagrams that they did not ask for — and it fails **closed** where gold flash fails open, because a wrong answer here silently starts capturing diagram content down an error path. Only the exact string `"true"` starts a recording. |
| T4719 | `tests/diagram/assist-command-log.test.ts` | `touchedFor` is a separate function from `goldFlash.flashTargets` and not a call to it, which looks like duplication until you see what each is for. Both diff before against after — the house technique, chosen because asking thirty-odd op handlers to report what they touched means handlers drift. But `flashTargets` DROPS DELETES ("there is nothing left to outline") and caps at forty so the screen is not covered in gold; evidence wants the opposite, because "it deleted the wrong lane" is exactly the report somebody needs to file. Delegating would have quietly lost every delete. Also pinned: every change an element had, not just the most interesting one, and sub-pixel noise is not a change. |
| T4720 | `tests/diagram/assist-command-log.test.ts` | `ok` is the system's opinion and `verdict` is the human's, and they must never be folded together. The pair `ok: true` + `verdict: "wrong"` is a **silent wrong answer** — the command reported success and did the wrong thing. It is the most dangerous failure mode there is, no generated test can ever find it (the system believes it passed), and it is invisible in the product today. `disputeCount` names exactly that pair: an admitted failure is not a dispute, "partly" is a grumble, and an unjudged entry is neither. |
| T4721 | `tests/diagram/canvas-snapshot.test.ts` | The snapshot's framing maths, extracted from `SopGenerateDialog.tsx` so the debug log and the SOP figure share one implementation — that file carries a hard-won comment that `html-to-image`'s `toPng` on a bare `<svg>` "fails silently and left SOPs with no figure", so the extraction had to keep the `<img>`+`<canvas>` route. An empty diagram returns **null** rather than a degenerate rectangle, because a zero-width bounds would divide to NaN pixels one call later; zero-size elements are ignored so a connector label at the origin cannot drag the frame into a corner; and the output aspect comes from the ORIGINAL bounds, not the clamped width, or a very wide diagram comes back squashed. |
| T4722 | `tests/diagram/canvas-snapshot.test.ts` | The `.dgx-voice.json` session file round-trips, carrying the comments, the verdicts, the inline PNG and the diagram JSON behind it — one artifact, readable in any editor, mailable. It refuses a file that is not one of ours instead of half-loading it, because this parses something a person chose off their disk. Counts (entries / failures / disputes) are computed here so a listing never parses the document. Also wiring, at source level since it lives in `.tsx`: the debug row is SuperAdmin-only, the debug diff is armed on EVERY command rather than only the ones `batchFlashes` outlines (those are precisely the ones whose effect nobody can check by looking), every entry is time-stamped whether recording or not, and a failed picture still stores the diagram — evidence with no picture beats no evidence. **Changed by design 2026-09-26 (Paul: "Make the snapshots in Voice Assist JSON, not SVG"):** the T4722b pin "a failed picture still stores the diagram" became "a snapshot is the diagram as JSON — no picture is taken" (`takeDebugSnapshot` saves a "marked" state through `captureState`, no `captureCanvasPng`, no `png`), and the time-stamp pin now reads the one `appendLog` helper; an old file with a PNG still parses (T4886–T4893). |
| T4723 | `tests/diagram/voice-debug-sessions.test.ts` | Phase 2: an annotated session survives the browser. The models are deliberately plain — `entries` and `tally` are `@db.Text` documents, written whole and read whole and never filtered on, so there is **no pgPool write path anywhere in this subsystem** and one fewer Prisma 7 rule to get wrong in new code; the numbers the listing sorts on are scalars beside them. The diagram is referenced by plain id and **not** by relation, because deleting a diagram must not delete the evidence about it — the most useful session is often about one somebody threw away. Snapshots cascade with their session; a snapshot whose picture failed still keeps its diagram JSON, because evidence with no picture beats no evidence. Route guards: every route checks the real `isSuperuser`, both mutating ones reach `blockReadOnlyImpersonation` (copying `api-harness` would have failed the ratchet — all six of those are frozen debt on `KNOWN_UNGUARDED`, not a pattern), the listing carries no documents and no bytes, the detail route sends a URL rather than the picture, and the picture is served `nosniff` + sandboxed + `no-store` because evidence about one moment must not come from a stale cache. |
| T4724 | `tests/diagram/assist-generator.test.ts` | Phase 3: cases are generated **from the ops outward** — pick an op shape, fill it from the fixture, render it to English — so the expected answer exists by construction and nobody labels a hundred sentences by hand. Deterministic in a seed (`rng.ts`, mulberry32 over an FNV-1a hash, no new dependency and the same hash-a-string idiom `goldFlash.sparksFor` already uses), because a red case nobody can reach again is a red case nobody can fix. Every ref a case emits must really resolve in the fixture, or it would fail for a reason unrelated to the grammar and be indistinguishable from one that mattered. The fixture's labels are chosen for the classes this feature has actually been caught by: `Lane 2` (a name ending in a digit, which the `lane:3` boost has beaten), `Sub 1`, `Review` (a bare common word), `Pick Items`, `Sales` (homophone-adjacent). |
| T4725 | `tests/diagram/assist-generator.test.ts` | A coverage ratchet over the op union: every `AssistOp` kind is either generated or on `NOT_GENERATED` **with a reason**, and the exclusion list may not go stale in either direction. Adding an op to `ops.ts` fails the suite until somebody makes a conscious choice — the same discipline `tests/backup/coverage.test.ts` applies to new tables, and for the same reason: the failure is not "you forgot", it is "decide". |
| T4726 | `tests/diagram/assist-generator.test.ts` | **The load-bearing test.** The generator may never import `commandGrammar.ts`. If the renderer were built from the parser then `parseCommand(render(op)) === op` would be a tautology and the entire corpus would prove nothing — one implementation marking its own homework. It MAY import `ops.ts`, which is the shared *vocabulary table* already used by the grammar, `resolveRef`, `convertPhrase` and the right-click menu; sharing a noun list is not sharing a parser, and the line goes exactly there. Comments are stripped before the check, because the module's own docblock explains this rule and therefore names the file it must not import. |
| T4727 | `tests/diagram/assist-generator.test.ts` | The round trip, as a **ratchet with a frozen `KNOWN_DISAGREEMENTS` list**, not a blanket assertion. An honest generator emits sentences the grammar refuses, and a permanently-red test gets deleted in week two — `mutating-route-guard.test.ts` says exactly that in its own docblock. So it fails BOTH ways: a new kind of disagreement fails, and a listed one that starts agreeing also fails, so the list cannot go stale. A failure ceiling catches regressions inside a family already listed. The one entry today is real and narrow: the WHOLE-DIAGRAM wrap drops its label ("put a pool around everything called Finance" → a bare `wrapInPool`, so the pool is created unnamed), while the SELECTION wrap keeps it. |
| T4728 | `tests/diagram/assist-generator.test.ts` | The scorer is a **waterfall — first failing layer wins, one label per row**. The most expensive failure mode in this feature's history is fixing the wrong layer, and a multi-label table is how that happens. Pinned: words wrong but answer right is `pass-despite-mishear` and **not a failure** (counting it as one makes the audio leg look far worse than it is); refused with the words intact is the grammar, refused with the words mangled is the recogniser; a wrong shape names the differing FIELD rather than making the reader diff two JSON blobs; and refs compare by what they RESOLVE to, never by text, so "the gateway" and "Approved?" are the same element. Note the scorer sorts object keys before comparing — the grammar builds op objects in branch order, and raw `JSON.stringify` reported a dozen identical ops as mismatches on the first run. |
| T4729 | `tests/diagram/assist-generator.test.ts` | **One scorer, two legs.** The free text leg and a perfect transcript must agree on every case in a seeded run; if they ever diverged, the cheap corpus and the recorded one could not be compared, which is the entire reason for recording anything. Also the tile: the text leg runs in the browser — the generator, scorer and grammar are pure, so a thousand cases cost nothing — and the ONLY call the page makes is the per-row `investigate`, which is SuperAdmin-guarded, blocks read-only impersonation, bills against the command model rather than the generate model, and treats a missing AI key as a state rather than a crash. |
| T4730 | `tests/dictation/asr-params-and-clips.test.ts` | Phase 4's precondition: every recogniser setting in ONE place (`asrParams.ts`), so the live microphone and a replayed clip cannot ask Deepgram for different things. Before this the live socket sent `language=en-AU`, 800 ms endpointing and a keyword-boost list while the batch route sent none of them — fine while they did unrelated jobs, and fatal the moment a clip is replayed to measure the live path, because the harness would faithfully report the accuracy of a configuration nobody ships. Pinned: the two builders agree on model/language/formatting; the live one keeps what only streaming has; **number words are still not boosted** (they were for one day — "turn on gold flashing" came back as "ten on gold flashing") and the reasoning travels with the constant; a meeting asks for diarisation and NO command bias while a replayed clip asks for the reverse; and `asrFingerprint` identifies a configuration rather than a laptop, excluding `sample_rate` so two machines do not look like a settings change. |
| T4731 | `tests/dictation/asr-params-and-clips.test.ts` | Nobody builds Deepgram parameters anywhere else — a source scan, because "one rule, one place" can only be asserted as an absence. This caught a third copy nobody had noticed: the **browser speech fallback** named `en-AU` separately, so an AU setting changed in one place and not the other would have been invisible until somebody noticed the fallback hearing differently. Also pins that the `en-AU` addition to the meeting transcribe path is documented where a bisect would find it: it is a real behaviour change to an existing feature, shipped named rather than smuggled in with a refactor. |
| T4732 | `tests/dictation/asr-params-and-clips.test.ts` | The WAV encoder, and the capture format that must not slip. Clips are raw 16-bit PCM because **the live path sends raw Int16 frames** — a corpus recorded through `MediaRecorder` would be Opus, a lossy generation the product does not have, and would measure a pipeline nobody ships. Pinned: every header field a decoder reads (RIFF/WAVE/fmt-16/PCM-1/mono/rate/byte-rate/block-align/16-bit/data), an explicit little-endian write, a sample round trip, a refusal on anything that is not ours, the same float clamp the live capture uses, and `peakLevel` — which is how the recorder refuses a **silent take**, the likeliest way to lose an hour of reading, since a muted mic looks exactly like a good take until the replay. The recorder is asserted to use the `ScriptProcessor` path and never `MediaRecorder`, and the upload route refuses anything but WAV so a future caller cannot quietly reintroduce it. |
| T4733 | `tests/dictation/asr-params-and-clips.test.ts` | The corpus survives the things that could lose it. **A retake is take 2, never an overwrite** — which take was the good one is only knowable later, so an upsert would throw away the answer before the question was asked, and the unique key enforces it in the database rather than by luck. **The sentence and the expected ops are denormalised onto the clip**: case ids shift whenever the generator changes, and a clip carrying a reference would silently lose its answer while a self-describing one survives — the mitigation for the second-largest hazard in the plan, with the reason recorded in the schema where the next person will read it. Plus: the audio decodes back to a WAV after a database round trip, the upload is SuperAdmin-guarded and blocks read-only impersonation with 413/415 caps, the listing never carries the audio, and the recorder does **one POST per accepted take** so a crash at clip seventy costs nothing. |
| T4734 | `tests/dictation/replay-and-stitch.test.ts` | Phase 5: `stitchFinals` reconstructs the utterances the live silence-buffer would have produced, from timestamped recogniser finals. Deepgram finalises after ~800 ms of quiet, so one sentence with a pause in it arrives as two finals — "rename Task 8 to" … "Approve" — and running the first half renames nothing and loses the second. Pinned: a pause inside a sentence does not split it; two complete commands with a real gap stay two; a short gap joins them; an unfinished sentence nobody finishes **runs anyway** once the hold budget is spent rather than silently swallowing everything after it; and whatever is left is always flushed when the audio stops, because `stopAbraListening` and the socket's `onEnd` both force a flush. |
| T4735 | `tests/dictation/replay-and-stitch.test.ts` | The live path and the replay share the buffering NUMBERS and the unfinished-sentence predicate — the parts that would actually drift — and the editor imports them rather than declaring its own, with no literal allowed to creep back. **The limit of the sharing is deliberate and is asserted to be written down:** these remain two implementations of one policy, because unifying them means rewriting the live timer state machine, which is the riskiest code in this feature, currently works, and whose fragment defects were expensive to find. Rewriting it so a test harness could share it would be the tail wagging the dog; the trade is recorded in `fragmentBuffer.ts` rather than left to be discovered. |
| T4736 | `tests/dictation/replay-and-stitch.test.ts` | The replay measures the live path rather than an approximation of it: the same socket, built by `liveStreamParams`, stitched by the shared policy, counting only `is_final` because interims are not what the editor runs. **Paced in real time**, because Deepgram's endpointing is wall-clock — blasting five seconds down the wire in fifty milliseconds would finalise everything as one segment and make the fragment behaviour invisible — with a timeout so one stuck socket cannot hang a hundred-clip run. It never throws: a bad WAV, an org that forbids cloud voice and a missing key are all rows in a table. The score takes the FIRST stitched utterance, not the concatenation, or a split command would be hidden by the very join it should expose. And the batch leg's blind spots — segmentation, streaming-vs-prerecorded, the command queue — are printed beside its own number in the UI, because that is the only reliable way to stop a green batch result being read as a green live one. |
| T4737 | `tests/dictation/replay-and-stitch.test.ts` | Paul, 2026-09-24: "show in the Test Voice Assist tile whether the Deepgram key is an Owner key … so when we push to prod I can easily see what type of key is installed there." The question cost an hour: local held a restricted key and production an Owner key, and the only symptom was live dictation **silently** using the browser engine — no `en-AU`, no keyword boosts — while a hundred replayed clips failed with "Dictation service unavailable". A restricted key is not an error, which is precisely why it must be STATED rather than thrown. The badge separates *valid* (`/projects`) from *can mint a browser token* (`/auth/grant`), names the symptom and the fix, and **never returns, logs or embeds the key**: the route may use it exactly once, in the Authorization header to Deepgram, and reports a one-way SHA-256 prefix so two environments can be compared without either value being disclosed. |
| T4738 | `tests/diagram/assist-generator.test.ts` | Two fixes the recorded corpus paid for. **The whole-diagram wrap keeps its name:** the grammar used `.test()` and threw away the "called X" tail, so "put a pool around everything called Finance" made an UNNAMED pool — while the selection wrap a few lines above had always kept its label, and `wrapInPool(op.label)` had always honoured one. Four clips in Paul's corpus were heard by the recogniser *exactly right* and still failed, which turned a curiosity into a priority. **A spelling variant is a pass that still shows as changed:** Deepgram writes "fulfillment" even under `language=en-AU`, and counting that as a broken command overstated the corpus by four points — but it scores `pass-despite-mishear`, never a silent clean pass, because hiding it would be moving the goalposts. A genuinely different word is not folded away. Note also what this fix did to T4727: the frozen `KNOWN_DISAGREEMENTS` list went RED on the repair and demanded its own entry be deleted, which is exactly what a list that cannot go stale is for. |
| T4739 | `tests/dictation/asr-params-and-clips.test.ts` | Keyword-boost profiles, so the list can be **measured** rather than argued about. Until Paul's corpus existed the shipped list had never met real audio — every local session fell back to the browser engine — and the first replay found it helping ONE clip and hurting EIGHT, with roughly half of all failures being a boosted word displacing the spoken one ("make"→"Lane", "delete"→"Selected", "swap"→"Pool"). Four profiles can now be run over the same hundred clips. The load-bearing guard: **`commandWords` is harness-only and live voice never passes it**, so a measurement setting cannot leak into the product — the microphone path is asserted to take no profile and to send exactly `COMMAND_KEYWORDS`. The alternatives are pinned to fix both observed faults: they carry singular AND plural container words (a singular-only boost competes with its own plural — "add two sublanes" came back "add two lane") and no weights at all (3 is strong enough to beat words that sound nothing like the boosted one), and they drop `selected` and `rename`, the two caught eating other words. Every profile must explain itself and say what to watch for, on the screen — a dropdown of four lists nobody can choose between is not a tool — and the run records which profile produced it, or two runs are two numbers nobody can attribute to a setting. |
| T4740 | `tests/diagram/voice-escape-and-blackbox.test.ts` | Paul, 2026-09-25: "I can't reliably exit the command after it mis-hears. It keeps adding words like 'stop', 'done', 'cancel' and 'exit'. The only way out is to stop and restart." The mechanism: a mis-hear leaves a half command in the buffer, `isIncompleteCommand` HOLDS it for up to three times 3.2s waiting for the rest, and every word said during that hold is appended — so "rename Task 8 to" + "cancel" became a complete command that renamed the element to **"Cancel"**. The escape word was being eaten by the thing it was trying to escape. `isMicStopWord` was already checked at the FRAGMENT, before the buffer could absorb it, which is exactly why "stop" worked and nothing else did; flow-end words now get the same treatment — buffer dropped, pending flush cancelled, microphone left on. With no flow open it says "cleared — listening for the next command" rather than sending a bare "cancel" to the metered AI. |
| T4741 | `tests/diagram/voice-escape-and-blackbox.test.ts` | Paul, 2026-09-25: "Sometimes a command will by default add a Task with a name like 'cancel this command!' to a black-box pool. This should NEVER occur." Not merely untidy — a black-box pool is a participant whose internals are deliberately NOT modelled, which is the entire meaning of the notation, so an element inside one is a contradiction that exports as a process with hidden content. It needed guarding in **both** places a parent is chosen: the `ADD_ELEMENT` container filter and the membership reconciler that re-homes elements after a move — the first fix was written in the reconciler alone and the test still failed, because `ADD_ELEMENT` has its own filter. `containerAccepts` could not make the call: it is given TYPES, and black-box is a property of the instance. |
| T4742 | `tests/diagram/voice-escape-and-blackbox.test.ts` | Paul, 2026-09-25: "just grow the Pool when the template is placed", and "assume the template will go on the end of the current elements". A pool is a HORIZONTAL BAND, so overflowing its right edge — exactly where "on the end" puts a template — does not put you outside it. `reconcileLaneMembership` adopts by full containment, so such a template was adopted by nothing, and with no children there was nothing to enclose. Vertical position now decides membership for an applied template while horizontal overflow just means the pool wants to be longer; scoped to `APPLY_TEMPLATE`, because a DRAGGED element released outside a pool is somebody deciding it goes outside. The widening is explicit, because `ensureContainersEncloseChildren` deliberately sizes a pool from its STRUCTURAL children only — lanes and sub-lanes — so a task was never going to grow it. Issue 6 (2026-09-26): membership is now decided for the template as ONE piece, by overlap (T4868); the left-of-the-pool and different-band cases are two separate templates. |
| T4743 | `tests/diagram/assist-generator.test.ts` | Paul, 2026-09-25, after two hundred recorded commands: "please generate commands with the following rules … Always use Verb phrases for Activities … Pool names should be Company or Department Names … Lane names should be names of Teams or Role names … Changing the generated commands will make the tests much more realistic." The first corpus handed out whatever names were convenient, which measured the recogniser against a vocabulary nobody speaks at a whiteboard — and a noun-phrase task name is a MODELLING ERROR, so a good share of the exam questions were sentences a modeller would never utter. Names are now typed by what is being named: a verb phrase for an activity, a team or role for a lane, a company or department for a pool, an external party or a product for a black box, a past participle for an event, an interruption for a boundary event, a document for a message. `newNameFor` reads the target's own type, because the rename template cannot know which kind it drew. The fixture also carries the UN-RENAMED defaults — `Task 1`, `Subprocess 3`, `Lane 3`, `Decision?`, `Expanded 2` — and the rename template prefers them, because "Rename Task 1 to Review Email" is the commonest real command there is and its reference ends in a DIGIT, the hardest thing for the recogniser to get right. Two renderer faults fell out of the change: `blackBoxPool` picked twice, taking the id from one pool and the name from another, so a case asked for "Customer" and expected Salesforce — a row that could never pass however well it was heard; and the bare `call X Y` form is now only rendered where the old name is a SINGLE word, since "call Pay Claim Send Invoice" has no word between the two names and splits two defensible ways. The corpus seed is bumped with it: a run against a different seed is a different exam, and the recorded clips carry their own seed so the two cannot be confused. |
| T4744 | `tests/diagram/assist-generator.test.ts` | No generated case may put something INSIDE a black-box pool. Since T4741 the reducer refuses to parent anything to one, so "add three lanes to Customer" is a broken exam question rather than a finding about the recogniser — it fails however perfectly it is heard, and a corpus that cannot tell the two apart teaches nothing. Scoped to the ops that actually fill a container (`addLanes`, `addSublanes`): "compress Customer" and "move Salesforce top boundary up" name a black box and are perfectly legal, because they resize it rather than fill it. The first version of this guard banned every mention and went red on `compress Customer`, which is the distinction worth keeping. |
| T4745 | `tests/ai/opus-5-5-and-rate-shadowing.test.ts` | Paul, 2026-09-25: "check with Anthropic for new models." Asked the account's own key rather than a docs page, and `GET /v1/models` returned **`claude-opus-5-5`**, released 2026-09-21 — four days earlier, and unknown to the model doing the asking, which is the entire argument for querying the API. Verified before shipping: 1M input, 128k output, `image_input` supported, and a smoke call answered. It is both NEWER and CHEAPER than Opus 5 ($4/$20 against $5/$25), which is pinned because it is unusual and because it is load-bearing — `allowedGenerateModels` gates ordinary users on cost, so a cheaper flagship is one they can actually reach rather than one only a SuperAdmin sees. |
| T4746 | `tests/ai/opus-5-5-and-rate-shadowing.test.ts` | Claude Sonnet 5 is **$2 / $10**, not $3 / $15. It had been recorded here at $3/$15 with a note calling $2/$10 an introductory price "through 2026-08-31" — but the rise scheduled for 1 September was CANCELLED, so the number was not stale, it was wrong, and the AI Usage report had been billing Sonnet 5 at 1.5x its real cost. The test pins both the numbers and the absence of any lingering conditional note, and asserts the word CANCELLED survives in `pricing.ts` — without the reason written down, $2/$10 reads like a typo and gets "corrected" back to the figure somebody half-remembers. |
| T4747 | `tests/ai/opus-5-5-and-rate-shadowing.test.ts` | **A rate row that merely COPIES a default is never stored.** This is the fault that made T4746 unfixable: `effectiveRates()` overlays `pricing.ts` with the `AiModelRate` table and THE TABLE WINS, and when the editable catalog shipped on 2026-07-23 it saved every rate on the screen — defaults included — writing ten byte-identical copies in one transaction. A copy of a number is a number that can go stale by itself, and correcting the constant changed nothing because the copy still said $3/$15. The PUT handler now deletes rather than stores a matching row, in the same transaction as the upserts, so setting a rate back to its default REMOVES the override instead of freezing today's figure against tomorrow's correction. `matchesDefault` compares the money and the provider, never identity — an unpriced model has no default to match, so a row for it is always a real choice. The last case states it as a property rather than a list: whatever `pricing.ts` says at any time, saving exactly those numbers stores nothing. Existing rows are cleaned up by `scripts/fix-stale-ai-rate-overrides.sql`, which removes only rows still holding the seeded values, so a genuine SuperAdmin edit survives it. |
| T4748 | `tests/diagram/assist-apply-layer.test.ts` | Paul, 2026-09-25: "The harness doesn't yet check whether the diagram itself came out right." It could not: the ~1,000-line body that applies Voice Assist ops lived inside the 8,700-line editor. It moved VERBATIM to `app/lib/assist/applyAssistOps.ts`, taking what it closed over as a context (diagram actions, screen hooks, refs) under the same names. The guard pins that the editor delegates and carries no op branch of its own — a copy left behind would be the one the user runs and not the one the harness scores — and that the editor keeps only its own memory ("again") and the display snapshots. The 38 older guards that read these lines from the editor now read both files through `tests/diagram/assistApplySource.ts`, so they follow the code rather than going blind. |
| T4749 | `tests/diagram/assist-apply-layer.test.ts` | `headlessDiagram` — useDiagram's actions over the real reducer, no React — dispatches, NAME FOR NAME, the reducer action type useDiagram's helper does, read from both sources. Payload shapes are held by the compiler; the type per name is not, and a headless `wrapInPool` that sent `WRAP_IN_CONTAINER` would type-check and score a different product. Also: a staged move is one undo entry committed at its end, as in the editor. Proved by planting `COMPRESS_POOLS` for `extendPools`. |
| T4750 | `tests/diagram/assist-apply-layer.test.ts` | **L4 over the 600-case corpus, as a ratchet** (`KNOWN_WRONG_EDITS`, failing both ways, like T4727). Each case is applied headless to `fixtureDiagram()` and `opEffects.ts` asks whether the edit the SENTENCE promised happened. Frozen on its first run: a named pool whose name goes nowhere when a pool exists; **"add a lane above Underwriters" landing at the BOTTOM of the pool** (the carve borrows from another lane — same code as the mouse lane-drop, so left for Paul); a lane whose sublanes fill it can neither give room for a new lane nor let its neighbour move; one correct refusal kept visible. Also pins that L4 runs only after L3 passed, so the other layers' counts do not move. |
| T4751 | `tests/diagram/assist-apply-layer.test.ts` | The checks are strict about what was asked and loose about geometry the reducer owns: a rename is checked against the NEW name; a gateway's added "?" is the reducer being right; convert compares the whole element (its marker is `taskType` on the element, not in `properties` — the first run misread every convert as a failure); compress passes a pool that grew to FIT its contents; display ops return `null` ("nothing to check"), never a pass. |
| T4752 | `tests/diagram/assist-apply-layer.test.ts` | **A command that changed nothing never says it worked.** ADD_LANE_AT (no room: the lane is carved from its neighbour, must fit its vertical NAME, and the pool never grows) and MOVE_LANE (needs a lane on BOTH sides; stops where the one giving way runs out) decline silently — right for a drag — and the voice log said "added a lane" / "moved" anyway. The apply layer now asks the REDUCER (`wouldChange`) rather than copying its room rule, and refuses by name: "no room below Claims Team for a lane called “Quality Assurance” …", "Claims Team is the top lane …". A move that can happen still does. Proved by disabling the MOVE_LANE check. |
| T4753 | `tests/diagram/assist-apply-layer.test.ts` | The corpus asks for things the fixture can do, or L4 scores a correct refusal as a wrong edit: disconnect names only pairs a flow joins; only a lane with neighbours on both sides is moved; no "add a start event after X" (nothing flows into one). The fixture holds nothing the reducer corrects on contact — black-box pools were 80px, under the 116px minimum, so their first edit GREW them and "move the bottom boundary up" read as moving it down — and it now has lanes with room and one element in no pool, so "put a pool around everything" has something to wrap. Recorded clips carry their own sentence and ops, so none of this orphans Paul's corpus. |
| T4754 | `tests/ui/superadmin-tile-filter.test.ts` | Paul, 2026-09-25: "when filtering SuperAdmin tiles keep the existing filter after coming back from entering a tile." The "Filter tools…" text is remembered in sessionStorage under `dgx.superadmin.tileFilter` and read back; clearing the box REMOVES the key rather than storing "". sessionStorage, not a `?q=` param, because about thirty tile pages return through a "← SuperAdmin" link to the BARE `/dashboard/admin`, which a query param would not survive. |
| T4755 | `tests/ui/superadmin-tile-filter.test.ts` | Unusable storage never breaks the grid: absent storage (SSR) or storage that throws on every call (private windows, blocked site data) reads as "" and writes silently — the grid just starts unfiltered, as it always did. |
| T4756 | `tests/ui/superadmin-tile-filter.test.ts` | The wiring: `SuperAdminToolsGrid` reads the remembered filter AFTER mount (so the server render matches) and persists in the setter rather than an effect (so the mount's empty state cannot wipe it first); the input and the × clear button both go through that persisting setter. |
| T4757 | `tests/diagram/assist-apply-layer.test.ts` | The fixture has room for every name `LANE_LABELS` can say — `laneMetrics(name).minHeight` fits the EMPTY space (less `MIN_LEFT_GAP`) above and below Underwriters and Lane 3. A new lane must be tall enough for its vertical name; without the room the carve goes elsewhere and voice refuses, correctly, which is noise in a corpus. Stricter than the planner on purpose: "Quality Assurance" once passed only because the planner fell back to halving the lane. |
| T4758 | `tests/diagram/assist-apply-layer.test.ts` | Paul, 2026-09-25, on the L4 finding that "add a lane above Underwriters" could land at the bottom of the pool: **refuse, voice only.** The apply layer previews the carve with the reducer and refuses unless the new lane sits against the named lane on the named side ("no room above Underwriters for a lane called “Billing Team” …"). The MOUSE drop keeps the planner's other-end fallback (his 2026-09-23 rule) — pinned by calling the reducer directly — because a drop shows where it will land and a sentence cannot. Halving at the named place is still allowed; it lands where it was named. Proved by planting the old `!carved` check. |
| T4759 | `tests/diagram/assist-apply-layer.test.ts` | Paul, 2026-09-25: a named wrap into an existing pool **grows it and ignores the name** — but the log now says “Accounts Payable” was not used; the pool keeps its name, so a spoken name never vanishes without a word. With no pool yet, the new pool still takes the name. L4 checks the name only in that case. Also: a lane beside one its sublanes fill is refused by decision ("keep refusing"), so the generator no longer asks for it, and `KNOWN_WRONG_EDITS` is empty with a ceiling of 0. |
| T4760 | `tests/diagram/wrap-in-pool-scenarios.test.ts` | Paul, 2026-09-25, on the harness row "no pool is called “Northwind Freight”": "I am not sure this is correct behaviour" — four cases, and the message is never a shrug. The rule now lives ONCE in `app/lib/diagram/wrapInPoolPlan.ts`; the reducer applies the plan and the voice log reports it (before, the log guessed on its own and said "grew Customer" while the reducer drew a new pool). **Case 1:** a white-box pool already holds everything → "everything is already in Claims Processing — nothing to put in a new pool", nothing changes. |
| T4761 | `tests/diagram/wrap-in-pool-scenarios.test.ts` | **Case 2:** only black-box pools, the loose elements together between them → ONE new pool, named as said, **as wide as the black-box pools**, fitted into the gap, the participants untouched ("put 2 elements in a new pool “Northwind Freight”, as wide as Customer"). Widens only if an element reaches past them. L4 agrees. Proved by planting the old tight-around-the-elements width: both the product test and L4 went red. |
| T4762 | `tests/diagram/wrap-in-pool-scenarios.test.ts` | **Case 3:** only black-box pools, the loose elements split above and below one → no pool can be drawn without swallowing a participant: "can't put one pool around them — they are split above and below Salesforce; move them to one side of it first". Also refused when an element sits ACROSS a black-box pool. Nothing changes. |
| T4763 | `tests/diagram/wrap-in-pool-scenarios.test.ts` | **Case 4:** no pools → a new pool round the loose elements, with the name said, as before. And across the cases L4 never reports "no pool is called …": its wrap check is rewritten from these four cases (not from the planner) — a white-box pool present grows and keeps its name; otherwise exactly one new pool carrying the name, as wide as and clear of any black-box pool, which must be unchanged. |
| T4764 | `tests/ui/voice-test-family-window.test.ts` | Paul, 2026-09-25: "Under Test leg - free: add a feature to popup a scrollable window that shows the details of the commands in a Family, and their statuses. Close button is outside the scrollable region." Each family name in the Text leg table opens `FamilyCasesWindow`: every case in that family, failures first — its status and what the status means, the sentence, the finding, and the ops expected beside the ops parsed. The list is the ONLY scrolling region, capped at 85vh; Close sits in a footer after it, so a long family can never push it out of reach. Escape and the backdrop close it too. The outcome colours moved to `outcomeStyle.ts` so the table and the window share one table. |
| T4765 | `tests/diagram/wrap-in-pool-scenarios.test.ts` | Paul, 2026-09-25, on a white-box pool with elements outside it: grow it and keep its name "as long as the elements can be enclosed by **widening** the existing pool". So the plan now refuses an element above, below or across the pool's edge by name ("can't take Do x into Claims Processing by widening it — it sits above it; move it level with the pool first"), and otherwise the pool widens with its top and bottom fixed (`growPoolToAdopt(..., { widenOnly: true })`). Each element joins the lane it sits LEVEL with — the deepest, a sublane beating its lane — not the first lane, which was stale parentage. Found on the way: a widened pool left its SUBLANES stopping short; they now run to its edge. SUPERSEDES T4507's upward growth (its test now pins the refusal) and moves T4518's element level with the pool. L4 fails a pool that grew taller. |
| T4766 | `tests/diagram/wrap-in-pool-scenarios.test.ts` | Paul, 2026-09-25: a pool is drawn round the PROCESS only — events, activities, gateways, data objects and **data stores** ("include data stores"). A diagram of annotations, a group and a review comment gets no pool: "there are no events, activities, gateways, data objects or data stores to put in a pool". Beside real process elements those are left where they are; a data store goes in with the rest, and one alone is enough for a pool. A boundary event rides its host and is never loose on its own account. One list, `PROCESS_TYPES` in `wrapInPoolPlan.ts`; L4 keeps its own copy, written from the rule. |
| T4767 | `tests/voice/spoken-text.test.ts` | Diagramatix Voice: what a line sounds like spoken. `speechTransform` is the cleanup every spoken string gets — quotation marks dropped (straight and curly) but APOSTROPHES KEPT ("can't" read as "cant" is audibly wrong), → read as "to", element ids never read aloud, a lone digit said as a word so the picker asks for "one or two" (longer numbers and digits inside words left alone). `spokenText` puts it behind the verbosity gate — off / questions / problems (Paul's default: questions and refusals, successes silent) / everything. Split so Animate can narrate bare labels, which the gate would silence. |
| T4768 | `tests/voice/speak-params.test.ts` | On Deepgram's `/v1/speak` the VOICE IS THE MODEL (`?model=aura-2-theia-en`). Slice 1 sent the fixed Theia model plus the chosen voice in a `voice` parameter Deepgram does not have, so all four voices spoke as Theia. Pins that every offered voice travels as `model` with no `voice` param, four voices make four different requests, the default is Theia, `narration` is an accepted purpose (one list, `SPEECH_PURPOSES`), and no file but the route builds a `/v1/speak` request in CODE (comments may name it) — the key stays server-side. |
| T4769 | `tests/voice/narration.test.ts` | What Animate says as each shape appears — the diagram's own labels in Animate's order, nothing invented. A step says its label; a container says what it is ("Pool, Customer", "Lane, Underwriters", "Sub-lane, …") because in speech only the prefix tells a pool from a step; an unlabelled shape is silent; notes are not the process (annotations, review comments, groups never read — Paul's PROCESS_TYPES boundary); data objects and data stores ARE read ("include data stores"); labels get the same cleanup as every spoken string. |
| T4770 | `tests/voice/speaker.test.ts` | The speaker: one voice at a time, and a stop that really stops. Browser faked at its edges (fetch, an AudioContext the test ends by hand; no IndexedDB in node, so it must work uncached). Replies play in order, never two at once; `isSpeaking` means SOUNDING, not fetching (the mic gate reads it). THE BUG: a `stop()` during the FETCH was ignored — the audio played after the user paused and a second pump could start beside it. Now a generation counter + aborted fetch: a stopped reply never plays, the next one plays alone, a mid-sentence stop reports once and forgets the queue. A 403 reaches the caller in the route's own words and the queue moves on; blank text is never sent. |
| T4771 | `tests/voice/speech-costs.test.ts` | Paul: "include it in the AI costs". A spoken reply's row carries CHARACTERS in `inputTokens`; each Aura-2 voice is priced in `PRICING` at the per-1k rate × 1000 per MILLION characters (derived from `DEEPGRAM_TTS_USD_PER_1K_CHARS`, so the two cannot disagree), output 0 — the AI Usage report costs speech with no special case and the rate is editable in the catalog. `providerOf("aura-…")` is now "deepgram" (it fell through to "anthropic"). The Voice Assist Cost button counts spoken replies via the same rate lookup, names an unpriced voice, mentions speech only when something was spoken (existing strings byte-identical), and counts only SUCCESSFUL replies. |
| T4772 | `tests/voice/speech-access.test.ts` | Who may hear Diagramatix speak — Paul: "on for SuperAdmin users, off for everyone else", granted per user. The ordinary feature gate FAILS OPEN (a missing matrix row resolves as available), so through it speech would have been on for every user from deploy until prod was seeded. `speechGranted` fails CLOSED: no row + no override → no; SuperAdmin → yes; the user's own override decides either way (beats a tier-wide row); an EXPLICIT available tier row → yes. `scripts/seed-voice-feedback-off.sql` makes the Feature Availability grid show the truth. |
| T4773 | `tests/voice/voice-slice-3-wiring.test.ts` | Wiring no pure test can see (read as code, comments stripped). The speak route refuses read-only impersonation BEFORE reading the body (speech spends money and writes a usage row — the ratchet in T4534 is honoured, not extended), gates on `speechGranted` and never the fail-open feature gate, records failures as well as successes, and files speech under provider "deepgram" with the voice as model and characters as input. Animate narration reads its script from `narration.ts`, WAITS for each line before revealing the next, cuts the voice off on pause/restart/close, reads speed through a ref so the slider cannot restart a line mid-sentence, shows the switch only to someone who can hear it, and on a refusal turns narration off with the reason while the tour carries on. |
| T4774 | `tests/voice/tts-settings-and-usage.test.ts` | The Text to Speech tile's two settings (`ttsSettings.ts`). The master switch `tts.enabled` is a BRAKE, not a gate: nothing stored means ON, and only an explicit "false" switches speech off — for everyone, SuperAdmins included. The default voice `tts.voice` is used when valid; one Diagramatix does not offer falls back to Theia (Paul's choice). A user who picked a voice for themselves keeps it (`voicePrefs.readVoice(fallback)`). |
| T4775 | `tests/voice/tts-settings-and-usage.test.ts` | What speech has cost, as the tile shows it (`speechUsage.ts`, pure). This month runs from the 1st UTC; the last 30 days is its own window; older rows drop out of both. Failures are counted but never costed — nothing was heard or billed. The last 30 days break down by use (voice.reply / voice.narration / voice.compare) and by person, dearest first. Priced by the same rate lookup as the AI Usage report, so they cannot disagree; a voice with no rate costs 0 and is named. A reply replayed from the browser's cache never reaches the server and is not counted — the page says so rather than inventing a figure. |
| T4776 | `tests/voice/tts-tile-wiring.test.ts` | Paul, 2026-09-25: "Text to Speech links to the Test Voice Assist tile". Slice 1's tile only redirected — to the wrong tab. Now: a plain link to its own SuperAdmin-only page (master switch, who can hear it, usage, side-by-side comparison), no special case in the grid, and the Test voice tab + panel removed so voice testing lives in ONE place. The master switch answers 503 BEFORE the grant check (SuperAdmins stopped too) and GET tells a surface no, so it draws no control. Each purpose records under its own point; a comparison play is SuperAdmin-only, goes straight to the route (a cached reply would report no delay) and is said exactly as the product would say it. Both admin routes are SuperAdmin-only and refuse read-only impersonation before reading a body; a grant changes ONE jsonb key atomically, never the whole override map; a search term is matched literally. |
| T4777 | `tests/dictation/replay-measurements.test.ts` | The silence before the voice in a recorded clip (`wavTools.ts`), measured because 7 of 12 prod replay failures lost the FIRST word and 4 of those sentences passed on another take. To the 10 ms frame, mono or stereo. A key click or mic-open pop is NOT the voice: the voice must hold for 30 ms, and the level is the 95th-percentile frame, so a click can neither start the voice nor set the bar (the recorder starts on a key press). A DC offset is removed first, measured AFTER the wake-up zeros (and the search starts after them), so subtracting it cannot turn those zeros into a signal and mark every clip clipped. Room noise below the floor is not a voice; a take that never rises has no onset. DEAD AIR (exact digital zeros while the device wakes) is measured on its own. What it cannot read (not 16-bit PCM) it does not guess at. |
| T4778 | `tests/dictation/replay-measurements.test.ts` | Padding a clip (`padWavStart`) adds exactly the silence asked for and nothing else: a valid header (RIFF and data sizes rewritten), the voice bytes untouched, the lead-in moved by exactly the padding. Zero padding returns the same buffer. |
| T4779 | `tests/dictation/replay-measurements.test.ts` | What a Replay run measures (`replayCompare.ts`). ONE choice, not three switches — a run with padding AND punctuation could not say which moved a clip: as recorded, padded 500 ms, punctuation on (batch only; falls back to as-recorded on stream rather than quietly doing something else), or compared in one pass with as-recorded first (batch: all three; stream: as recorded + padded, because the live product is the stream). First word compared as a word (case and punctuation ignored) — the split that tests the clipping theory, not pass/fail. FIRST WORD fixed/lost is counted on its own (question 2 is about first words: padding can fix the first word while another word is still wrong), beside whole-sentence recovered/broke, all against the same clips and leaving out any that never reached the recogniser. "Possibly clipped" means the voice began within 20 ms of the dead air ENDING — every real clip starts with ~51 ms of zeros, so a raw lead-in near 0 never occurs. |
| T4780 | `tests/dictation/replay-measurements.test.ts` | Punctuation on its own for the batch replay: full stops and commas without smart formatting, with its own fingerprint; unset it follows `prose`, so the meeting route and a plain replay are unchanged, and the live socket still sends punctuate=false. The run records the fingerprint of what was ACTUALLY sent — returned by the transcribe route and by `replayClip` — and the panel never rebuilds one: it used to stamp every run with the live socket settings and the shipped keywords, which is why older "none" and "tuned" runs look identical. Lead-in is measured on the clip as recorded; only the replay is padded. A replay that never reached the recogniser is NOT a mishear: one failing replay cannot take the clip's others with it, and it is kept off the pass rate and out of the saved run, and named. The batch caveat follows the leg of the RESULTS on screen, not the controls set for the next run. |
| T4781 | `tests/diagram/assist-add-word.test.ts` | Paul, 2026-09-25: "and message" and "Handle message" both got "didn't understand that" — "Should recognise 'Add'". Both now parse to `addMessageByNumber` and, applied headless on the fixture, open the numbered message picker and add nothing, exactly as "add message" does. The bare fold is EXACTLY `(and\|handle)[,] [a ]message[ flow]` plus punctuation: "and the message", "and messages", "handle the message" stay null (a conjunction must not open the picker, which captures the next utterances); "Handle Message 1", "handle message one", "connect Handle Message 1 to Customer" and "rename Handle Message 1 to …" are untouched. Only the approved words fold ("and", "handle"; the leading fold "and" only) — "at message", "had message" stay null. |
| T4782 | `tests/diagram/assist-add-word.test.ts` | The leading "and" fold recovers EXACTLY the clean parse: every generated (5 seeds × 300) and catalogue sentence starting "add a\|an …" or "add message …", said with "and", parses deep-equal to the clean sentence (corpus > 100). The production lines work: "and a start event called payment received" adds a start event; "And a boundary event called claim withdrawn to assess risk" is an `addBoundary` on "assess risk". |
| T4783 | `tests/diagram/assist-add-word.test.ts` | The fold's guards. "At" is NOT folded (Paul approved "and" only): every locative in the verdict ("at a gateway add a task …", "at a task called review", "at a lane called sales", "at an end event", …) is left as heard and stays null. A second command after the type word declines ("and a gateway, connect yes to approve", "… connect it to approve"), while a verb-led NAME still folds ("called send invoice", "called place order"). "New" is a modifier, never a type ("and a new one" null). Stranded tails and "and again" unchanged; counts, "the" and "an" are not folded. `containsAnotherCommand` / `startsWithCommandVerb` pinned. |
| T4784 | `tests/diagram/assist-add-word.test.ts` | One rule, one place: the parser and the hold read the same words. `isIncompleteCommand("and a message from review")` equals the clean form's (true) and agrees on every word-prefix of every misheard corpus sentence; `stitchFinals` joins "and a message from review" + "to customer labelled invoice" (2.5 s) into one `addMessage`. Every noun in SYMBOL_PHRASES and the shared container / participant / box / message lists (plus the boundary-event phrase) folds — so the fold is BUILT from those lists; the grammar reads the same participant, box and message words. Wiring: grammar and hold call `repairHeardWords`, never the single repairs; the hold imports `COMMAND_VERBS` and keeps no verb literal; the grammar spells none of the shared nouns; make/turn are NOT held. **2026-09-26:** the hold also imports `COMPRESS_COMMAND_VERBS` from the same module (T4902), still with no verb literal. The hold now also imports `EXPAND_COMMAND_VERB` (T4913). |
| T4785 | `tests/diagram/assist-add-word.test.ts` | A boundary event is never a task. "add a boundary event [called X]" is an `addBoundary` with NO host; the trigger word (from the right-click menu's `EVENT_OPTS` — none, terminate and link are refused as no BPMN boundary event carries them) and "non-interrupting" are read ("an error boundary event called Failed", "a MESSAGE boundary event to Review" — above the message bail now). Hosted forms unchanged. No boundary phrasing ever yields op "add" ("new boundary event called X", "… here", "give me a boundary event", "a big boundary event"). A name that runs into a second command goes to the AI. Not held, hosted or not (Paul chose the selection as host; a hold would delay every selection-hosted one ~12 s); a trailing "to"/"called" still is. |
| T4786 | `tests/diagram/assist-add-word.test.ts` | Paul, 2026-09-25: "Use the selected task". Headless on the fixture, a hostless boundary event mounts on the ONE selected task / collapsed / expanded subprocess, keeping its label, trigger and non-interrupting (`properties.interruptionType`); with nothing selected, a gateway, a pool, a start event or two tasks selected it refuses, adds nothing, and says `say which task or subprocess it goes on — “add a [timer] boundary event [called X] to <name>”`. A NAMED host still wins over the selection. `BOUNDARY_HOST_TYPES` is one set (boundaryHosts.ts), imported by useDiagram, nextSteps, applyAssistOps, Canvas, PropertiesPanel and the simulator's boundary race (simulation/assemble.ts — every host the editor accepts is raced), none keeping a copy. |
| T4787 | `tests/diagram/assist-add-word.test.ts` | Corpus ratchet: `repairHeardWords` changes no clean generated sentence (5 seeds × 300) and no catalogue phrase — a future widening that touches a correctly heard command fails here. |
| T4788 | `tests/diagram/assist-add-word.test.ts` | "add the message" and "add messages" open the numbered picker (they made a TASK called "message(s)"); "add messages from review to customer" is never an add. The hold agrees — the by-number forms are complete — because grammar and hold test the one `MESSAGE_BY_NUMBER` pattern (messagePhrase.ts). |
| T4789 | `tests/diagram/assist-add-word.test.ts` | A stranded "after X" asks the shared question (`hasCommandAfterName`, commandVerbs.ts — any command verb, convert verb or "new" after the reference's first word, a leading article aside): "after Send Invoice", "after Create Order" and "after the Send Invoice task" are connects (the private list sent "Create Order" to the AI — the path that invented T4601's duplicate); "after Review add a task called Approve", "after Review, connect it to Approve", "before Approve put a timer" are still left alone, and so is a verb before a BARE noun — "after Review Claim add Escalate", "… new task called Escalate", "before Check Coverage insert gateway", "… rename Check Coverage to Verify", "after the start add task Approve" — which, read as tails, connected the wrong pair and dropped the command. The grammar keeps no private verb list for it. |
| T4790 | `tests/diagram/assist-add-word.test.ts` | The hold waits only for a message that is being ADDED: it asks the grammar's own bail pattern (`ADD_MESSAGE_LEAD` — verb, optional article, optional new/another, then the message word). "add a task called Receive Messages" / "Process Messages" / "Send Message", "create a task called Collect Messages after Review Claim" and "add an intermediate event called Messages Received" are not held, so `stitchFinals` runs the next command ("connect it to Review Claim", "add a gateway", 3 s later) on its own instead of stitching it into the name. "add a [new\|another] message from review", "add messages from review", "send a message to customer", "add msg from review" and the misheard "and a message from review" still wait; "add a new / another message from X to Y" is never a task. The grammar's from/to message rules build on the shared `MESSAGE_VERB`, with no verb literal. |
| T4791 | `tests/routing/boundary-exit-r702.test.ts` | `pickBoundaryEventSide` is Paul's R7.02 ("exits from the event's connection point furthest from the host edge the event is mounted upon") with no near-corner exception: an event mid-edge, 36px from any corner (the assist's first slot), or overhanging a corner by 8px exits its mounted edge's outer point for targets in all eight directions; a target inside the host takes the inner point; only an event whose centre is ON a corner (under 1px) still faces its target. Wiring: `placeBoundaryEvent`'s first slot, on an EP and on a task, is never read as a corner. |
| T4792 | `tests/routing/boundary-exit-r702.test.ts` | ADD_CONNECTOR applies R7.02 whatever side the caller sent: with addConnector's "right"/"left" defaults, every event position in the T4791 table gives the mounted edge's outer side, centred (0.5), for every outside target; also under `force`; the flow's first visible point is the event's outer point and no waypoint lies on the host's edge line or inside it; a task→task flow keeps the sides it was given. |
| T4793 | `tests/routing/boundary-exit-r702.test.ts` | The same rule for the other roles: a boundary Start's flow into its EP leaves the inner point; a flow into a boundary Start arrives at its outer point; a flow into a boundary End arrives at its inner point; the compensation association from a compensation boundary event leaves its outer point. `boundaryEndpointSides` governs sequence flows and compensation links only (not messages or other associations). |
| T4794 | `tests/routing/boundary-exit-r702.test.ts` | Paul's case, L4 headless on his geometry (`_helpers/block2Test3.ts`): "connect event four to assess work completed" leaves Event 4's SOUTH point (bottom, 0.5; waypoints[1] is the south point) with no waypoint on the subprocess's bottom line or inside it — and it stays that way after the target is moved (the group-move end, ELEMENTS_MOVE_END: re-parent, re-fit, then the CORRECT_ALL_CONNECTORS pass). |
| T4795 | `tests/routing/boundary-exit-r702.test.ts` | Mounting or re-edging re-picks the side: a flow drawn from a free event (right) moves to the outer point when the event is mounted by SET_EVENT_BOUNDARY or by the MOVE_END drop snap; an event slid round each of the four corners (MOVE_ELEMENT case A) takes the new edge's outer point; a deliberately placed end (bottom, 0.3) survives a slide along the same edge and is re-picked (right, 0.5) round a corner; `withBoundaryEndpointSides` touches only the moved event's own end. |
| T4796 | `tests/routing/boundary-exit-r702.test.ts` | CORRECT_ALL_CONNECTORS leaves an already right-angled route exactly as it is: a bottom exit whose target lies up and to the right (which `rectifyWaypoints` WOULD have doubled back — asserted) and Event 4's south exit are unchanged; a route with a diagonal segment is still rectified to right angles; across the layout corpus no right-angled rectilinear route of 7+ points changes (over 100 checked). |
| T4797 | `tests/diagram/assist-boundary-follow-on.test.ts` | "add a task called fix it after event four" on Paul's geometry (Event 4 carrying its host as its parent): one log line ("added fix it after Event 4"); the task joins the HOST's container, the Sales lane, and lies inside it; the subprocess keeps its size; the lane grew and Marketing, its contents and the SalesForce pool below moved down with it (the pool gap kept); the flow is bottom→left, clear of the subprocess. A lane with room clamps the step in without growing (still an L below the event); a top-mounted event with no room above opens the lane at the top by sliding its contents down (the lane above untouched); an ordinary add after a task asks for no growth; a refused auto-connect is reported once, with no contradicting "after". |
| T4798 | `tests/diagram/assist-boundary-follow-on.test.ts` | `planBoundaryFollowOn` asks the reducer's rules: for an event on each edge, 36px from a corner and ON a corner, the planned side equals `pickBoundaryEventSide` for the planned spot AND the side ADD_CONNECTOR then draws. `followOnParentId` gives the host's container, never the host, and that container's scope equals canConnect's `flowScopeOf` for the event's outgoing flow at every depth (so the flow is legal); a boundary Start's step goes inside its EP, planned with the obstacle list the callers really pass (every element that is not a pool or lane). `clampExitTargetToBand` (R7.07) clamps with room and returns a growth without; `boundaryOuterSide` equals routing's `outerSideOfBox`. |
| T4799 | `tests/diagram/assist-boundary-follow-on.test.ts` | Wiring (one rule, one place): Canvas.tsx has no private `getBoundaryEventOuterSide`/`oppositeSide`, imports routing's per-end rule `boundaryEndSide` and asks it at the drag-start preview (and as the pointer moves), the drop (source and target by role), both endpoint re-attach branches (by the dragged end's role) and both auto-connect ranking boundary-Start overrides, with no `pickBoundaryEventSide` call of its own; DiagramEditor `acceptNextStep` uses `planBoundaryFollowOn` + `followOnParentId` + keepInLane and `attachTemplate` asks `planTemplateAttach` (templateAttach.ts), whose adoption uses `followOnParentId` (no `src.parentId`); applyAssistOps uses the same plan and parent; the reducer's ADD_CONNECTOR calls `boundaryEndpointSides` and canConnect's `flowScopeOf`, MOVE_ELEMENT / MOVE_END / SET_EVENT_BOUNDARY call `withBoundaryEndpointSides`, CORRECT_ALL skips aligned routes, ADD_ELEMENT honours keepInLane; no private scope copy remains; bpmnLayout uses the shared R7.07 clamp. |
| T4800 | `tests/routing/boundary-exit-r702.test.ts` | Generated corpus: V16.06's "GRC submission rejected" (24px from its host's corner, which the old 36px corner test treated as a corner and sent out of the right) now exits its mounted bottom edge, with no waypoint on or inside its host. |
| T4801 | `tests/routing/boundary-exit-r702.test.ts` | The diagram checks expect the side the reducer draws, corner included: an error event slid onto its host's bottom-right corner and connected east gets "right" from the reducer (the plain nearest-edge answer is "bottom") and B27 (`checkBoundaryIntermediateOutgoingOuter`) is clean; every flow the reducer draws from an edge-mounted intermediate event — mid-edge, 36px from a corner, overhanging a corner, ON each corner — to targets in all eight directions passes all five side checks (B27, the edge-mount incoming check, R8.05, R8.06, R8.07); so do flows into a Start, out of a Start, into an End and (forced) into an event ON each corner; a flow stored on the perpendicular point is still flagged, naming the outer point. |
| T4802 | `tests/diagram/assist-boundary-follow-on.test.ts` | One geometry for "which edge": emieLabel's `boundarySideOf` equals routing's `outerSideOfBox` at every point of a 1px grid round a host, corners included (over 70,000 checked, 0 differ). Wiring: diagramChecks.ts keeps no copy of the edge geometry or of `oppositeSide`, its `outerSideOfEdgeMountEvent` returns `outerSideOfBox`, and B27, R8.06 and the edge-mount incoming check expect `boundaryOutwardSide`'s answer; emieLabel returns `outerSideOfBox`; routing's `boundaryEndpointSides` asks `boundaryEndSide` for each end, which asks canConnect's `flowScopeOf`, and `pickBoundaryEventSide` delegates the outward case to `boundaryOutwardSide`. |
| T4803 | `tests/routing/boundary-exit-r702.test.ts` | A connector end on a boundary event takes its side by ROLE (`boundaryEndSide`): a Start's outgoing and an End's incoming end take the inner point wherever the other end is (canConnect's scope rule puts their flow inside the host), so a Start → End flow on one subprocess gets inner/inner from the rule where role-blind geometry gave outer/outer; the reducer gives a Start's flow to its own child the inner point even when the child's centre lies past the host's edge; a flow INTO a Start is outer; every other end is `pickBoundaryEventSide`'s; a non-boundary element gets null. |
| T4804 | `tests/diagram/assist-boundary-follow-on.test.ts` | A step after a boundary START goes inside its subprocess with the obstacles the callers really pass (the host is the step's container, not an obstacle; its children still are): voice "add a task called prep after kickoff" puts it in a roomy subprocess, which keeps its size, the flow leaving the inner point and staying inside; in a crowded subprocess it is placed IN the box (not below it) and the flow stays inside; the ghost accept's plan is inside too; a Start on a TASK's edge never makes the step the task's child (a task has no inside). |
| T4805 | `tests/diagram/message-flow-endpoints.test.ts` | Paul, 2026-09-25: "Should allow messages to Receive Intermediate events as well e.g. message 2 arrives" / "Also messages should be allowed FROM Intermediate and End events with trigger Send" / plain events "Convertible". The ONE message rule (canConnect.ts `whyCantSendMessage` / `whyCantReceiveMessage` / `messageFlowRefusal`), end by end against a task in another pool, 37 rows: tasks, collapsed and expanded subprocesses, black-box pools both ways; Message/plain/Multiple start receive only; Message/plain end send only; a Message or plain intermediate with no Flow Type (or "none") either way, "catching" receive only, "throwing" or legacy taskType send send only; a boundary Message event receives only; timer/error/escalation (boundary and inline), boundary start/end, a start inside an embedded subprocess, an event-subprocess shell, a compensation activity, gateways, lanes, white-box pools, data, annotations and anything inside a black-box pool never. canConnect's branch equals the rule and every refusal has a reason; the reasons name the end at fault ("“Message 1 Arrives” catches a message — it can only receive one"). |
| T4806 | `tests/diagram/message-flow-endpoints.test.ts` | The pair: different pools message, the same pool does not; a black-box pool messages another but never its own (hidden) contents; a pool-less element is its own participant only against a pooled one (the highlight's invisible pool) and two pool-less elements may not message; only an EXPLICIT black-box pool hides its contents; a precomputed `poolIdOf` (canConnect's new options argument) gives the same answer as the walk for every pair. |
| T4807 | `tests/diagram/message-flow-endpoints.test.ts` | Direction: `isThrowingEvent` (eventDirection.ts) is SymbolRenderer's reading (throwing, or taskType send with no Flow Type; an explicit Flow Type wins); only an explicit catching fixes a receiver. Convertible, through the reducer: a plain start becomes a Message catch start, a plain end a Message throw end, a plain intermediate faces the message; a no-Flow-Type Message event takes the direction it is used in; a Multiple trigger carries the message and keeps its marker; a timer is refused and stays a timer; a catching Message event is refused as a sender and stays a catch. |
| T4808 | `tests/diagram/message-flow-endpoints.test.ts` | "add message" on Paul's recorded diagram (snapshot qe8reyjr, `tests/fixtures/block2-test3-add-message.json`) numbers 20: both message events, the expanded subprocess, the plain Start/End/Rejected and the subprocess's inner end among them — never Timeout, Event 4, a gateway, a lane, My company, Data 1 or the embedded start. No second list: the numbers equal {x : some legal message has x as an end} computed from `messageFlowRefusal`, and a review comment on the diagram changes nothing (and anchors nothing — canConnect's review-comment yes is not the rule). The reminder card, docs and seed no longer say "collapsed subprocesses and black-box pools". |
| T4809 | `tests/diagram/message-flow-endpoints.test.ts` | The selected form: Customer → 19 counterparts incl. both message events and SalesForce; Message 2 Arrives (no Flow Type) → the two pools, either way; Message 1 Arrives (catching) and Start → offered "from" only, End "to" only; an anchor the rule refuses gets its reason (timer, gateway, white-box pool, embedded start, non-Message boundary event). `resolveMessageAnswer`: "from n" accepted, "to n" refused with the reason and "say “from n”"; in pair mode the receive-only end said first is refused with the order that works, said second accepted; a same-pool pair gets no "other way round"; no badge is no badge. |
| T4810 | `tests/diagram/message-flow-endpoints.test.ts` | Headless (L4) on Paul's diagram, `addMessage` asks canConnect first: Receive order → Pick items (one pool) ok=false, nothing drawn, no "added message"; SalesForce → Timeout refused and Timeout stays a timer; Message 1 Arrives → SalesForce refused, still a catch; Customer → a gateway refused; Customer → Message 2 Arrives added and `checkEffect` passes; Customer → the plain Start added and Start becomes a Message start. The prompts: "numbers on everything a message can start or end at …", the selected form offering only the answer that can work, and "select what the message starts or ends at first". |
| T4811 | `tests/diagram/message-flow-endpoints.test.ts` | The blue ring IS the rule: on Paul's diagram, for every source × target (33 × 33), `classifyDragTarget(…).message` equals "distinct and `messageFlowRefusal` is null". Customer lights both message events and the plain Start, not Timeout or a gateway; Timeout, a gateway and Start light nothing as sources; the white-box pool lights nothing; a task that already sends is still a target (the send-lock is gone); a review comment never lights blue either end. |
| T4812 | `tests/diagram/message-flow-endpoints.test.ts` | B42 (connector-legality, via canConnect with the diagram's connectors) under the one rule: a pool → gateway and a same-pool message are flagged, a message to a plain start is not (convertible); none of the eight messages on Paul's diagram is flagged; no message flow in any diagram of the seeded Order-to-Cash example (`o2cProjectExport.json`, 62 messages) is flagged. |
| T4813 | `tests/diagram/message-flow-endpoints.test.ts` | Wiring (one rule, one place): ADD_CONNECTOR asks `messageFlowRefusal` with `state.connectors` and has no inline boundary / white-box copy; canConnect's only messageBPMN branch is the rule; messageTargets.ts imports the rule and keeps no type list; the Canvas drop has no inline start-event / own-pool refusal, treats a pool-less source as crossing to a pool on BPMN, and asks canConnect with the diagram's connectors; the highlight gates blue on `isMessageHighlightTarget` with the drag's `poolIdOf` and connectors, has no send-lock, and reads direction through `isThrowingEvent` (as SymbolRenderer does); `addMessage` asks `messageFlowRefusal` (with the connectors) before `addConnector`; the editor resolves the answer with `resolveMessageAnswer(…, connectorsRef.current)` and returns on a refusal BEFORE closing the numbered flow. |
| T4814 | `tests/diagram/message-flow-endpoints.test.ts` | The live User Guide patch `scripts/sql/patch-voice-assist-message-numbering.sql`: its new phrase is in the seed and its old phrase is gone from it; the line as seeded into the live guide on 2026-09-20, put through the patch's replace(), is the seed's line now character for character (the patch replaces exactly the phrase its guard matches, so a guarded row is always a changed row); its guard matches the old phrase and not the new (idempotent); one transaction with the verification SELECT after the COMMIT. |
| T4815 | `tests/diagram/message-flow-endpoints.test.ts` | An intermediate event with no Flow Type that already has a message faces that way — the message is as definite as a Flow Type (the reducer stamps one when the editor draws the first message; generated and imported diagrams carry the message without it). The seeded O2C "Customer Responds" (Message, no Flow Type, receives from Customer): the rule refuses it as a sender with "already receives a message, so it catches — it can't send one too" (without the connectors only the Flow Type counts); voice numbers it "from" only; blue lights Customer → it and not it → Customer; voice's addMessage and the reducer refuse the other way, the event stays unflipped and B42 stays clean, while the receiving way still draws and stamps catching. The mirror (already sends → can't receive) for a Message and a plain event; B42 flags both messages of an event that does both, and a lone message never flags itself; an explicit Flow Type still wins over the messages. |
| T4816 | `tests/diagram/message-flow-endpoints.test.ts` | The review's loose ends: addMessage from a review comment is refused with the rule's reason (canConnect's review-comment yes would have drawn a review link and reported "added message"); a task that already exchanges a message the other way keeps its marker (Customer → "Do nothing" leaves it a Send task; a task with no message still becomes Receive; a Receive task that now sends stays Receive); refusal reasons read aloud take "an Error" / "an Escalation" / "a Timer"; no branch of the highlight computes blue and no other diagram type lights blue; the numbering judges each end once and asks only `sameMessageParticipant` per pair — on the 37-row world with events already facing each way, the same numbers as brute force over `messageFlowRefusal`, and `sameMessageParticipant` is exactly the rule's pair part. |
| T4817 | `tests/diagram/message-label-place.test.ts` | Paul, 2026-09-25: "The message lable should always be in the air gap between pools and attached closest to the Pool meesage endpoint diagonally to the left or right." `placeMessageLabel` (messageLabel.ts), the ONE rule: pool end above → the label's top 10px under the pool, its right edge 6px left of the line (left by default); pool end below → its bottom 10px above SalesForce; a pool at the SOURCE end attaches there, not at the task. `connectorLabelBox` of the stored placement is exactly the placed box. |
| T4818 | `tests/diagram/message-label-place.test.ts` | Paul's diagram before his renames (snapshot 18687imk): message 3 renamed "Order details" keeps its right side (753 + 6) — asked to go left it still goes right, because the left spot is across message 4's line at x 710.7; message 4 renamed "Request" stays left (710.7 − 6); both tops 10px under Customer (−166.7), inside the 58px gap [−176.7, −118.7]. `messageLabelSide` reads the side drawn now. |
| T4819 | `tests/diagram/message-label-place.test.ts` | Taken spots: a neighbour's label on the left sends it right; both sides taken → a row further in (14 + 2px), still inside the gap; every attached spot covered (an event's long name beside the line) → the same row stepped out along it (60px here), still to one side; nothing clear even stepped out → the candidate with the least overlap (a partly covered last row); a line 40px inside the pools' left end never puts the label outside them; a neighbouring message's LINE counts as taken. |
| T4820 | `tests/diagram/message-label-place.test.ts` | A gap too thin to attach in (20px < 10 + 14): the label is centred in it, one row; a two-line label (28px) in a 20px gap is centred and spills 4px into each pool — the documented limit. |
| T4821 | `tests/diagram/message-label-place.test.ts` | A name that wraps to two lines keeps its near edge 6px off the line (it grows away, never across) and stays in the gap. At a 12px connector font the label is sized as drawn (13 × 7.2 + 12) and clears its line; the same placement measured at 10px is drawn ACROSS its line at 12px — why the diagram's `connectorFontSize` is passed. |
| T4822 | `tests/diagram/message-label-place.test.ts` | Which gap: with a pool between the two ends the label sits in the gap next to the POOL end, not in the pool between; with the other end in no pool the gap runs to that element and the label is still attached at the pool (not at the midpoint); with neither end a pool, attached where the line leaves the SENDER's pool, either direction. |
| T4823 | `tests/diagram/message-label-place.test.ts` | The run at the pool end, whatever the route does further on: the verdict's free-form route that jogs part-way across the gap puts the label beside the run at the pool (top 90, right edge 234), never on the jog; a four-point route (anchored on the curve by the canvas) round-trips exactly; a diagonal line with no vertical run still gets a label 10px off the pool edge and clear of its own line; invisible leaders are not the line. |
| T4824 | `tests/diagram/message-label-place.test.ts` | Nothing to place: null with no (or a blank) label, for a sequence flow, for pools side by side and for overlapping pools. `messageLabelSide`: left, right, and none for a label centred on its line. |
| T4825 | `tests/diagram/message-label-rename-2026-09-25.test.ts` | Paul's rename sequence replayed on 18687imk with the EXACT payload the editor sent (`{ id, label, labelOffsetX: undefined, labelOffsetY: undefined, labelWidth: undefined }`): "Order details" lands right of message 3's line and "Request" left of message 4's, both 10px under Customer and inside the gap, offsets and labelWidth 80 stored — the old `{ ...c, ...payload }` spread erased them and dropped "Request" into My company. `{ id, label }` and the undefined-padded payload give the same diagram; `connectorLabelPayload` carries only supplied keys. |
| T4826 | `tests/diagram/message-label-rename-2026-09-25.test.ts` | Only a message's NAME re-places it: the Properties panel's blur on a sequence flow or an `archi-serving` relation keeps offsets, width and tether; a blur with the same text changes nothing; a drag stores exactly the dropped offsets and turns the tether off (a dragged message label too); the canvas editor's commit — new text with the stored position echoed, or none — places a message by the rule and leaves its tether as it was. |
| T4827 | `tests/diagram/message-label-rename-2026-09-25.test.ts` | A NEW message is placed by the same rule: drawn with the mouse (ADD_CONNECTOR, default "message 1") and said by voice (`addMessage` headless) both land 10px under Customer, 6px left of the line; a second message whose left spot is across the first one's line goes right (the old placement drew it across); free-form layout puts it beside the run at the pool, off every segment of its own route; pools side by side (no air gap) keep the old −45 / 0 default. |
| T4828 | `tests/diagram/message-label-rename-2026-09-25.test.ts` | Load heal (`healMessageLabels`, run by useDiagram's `useReducer` initialiser after the pool-header heal): Paul's saved "Request" — no position, drawn inside My company — is placed in the gap, left of its line; nothing else is touched (same element array, same connector objects); a diagram with nothing to heal, a hand-placed label however odd, and an unlabelled message come back as the same object. |
| T4829 | `tests/diagram/message-label-rename-2026-09-25.test.ts` | A two-line label placed by the rule survives its pool crossing over: `preserveMessageLabel` and the reducer's own black-box pool drag (MOVE_ELEMENT case A2 mid-drag, and MOVE_END's settle) mirror it by its DRAWN (wrapped) height, so it lands 10px off Customer's new top edge in the new gap; counting hard newlines only put it 4px inside the pool. |
| T4830 | `tests/diagram/message-label-rename-2026-09-25.test.ts` | By voice, headless (L4) on Paul's diagram: "label it Request" on the selected message 4 puts the label in the gap, attached, left of its line; "rename message 4 to Request" renames the MESSAGE (the resolver's loose match on "Event 4" no longer wins over a connector labelled exactly that); an element named exactly what was said still wins, and "rename Event 4 to …" still renames the event. |
| T4831 | `tests/diagram/message-label-rename-2026-09-25.test.ts` | Wiring: `computeMsgBpmnLabelOffsets` is gone; ADD_CONNECTOR and UPDATE_CONNECTOR_LABEL call `placeMessageLabel` (rename with `messageLabelSide`) and drop undefined fields; `updateConnectorLabel` dispatches `connectorLabelPayload(…)`; CASE A2 and `preserveMessageLabel` measure the wrapped height with `connectorLabelSize(…).h` (messageLabel.ts keeps no line count of its own); ConnectorRenderer's `commitEdit` echoes the STORED position; one label measure (`connectorLabelSize`) in the renderer, `connectorLabelBox`, `connectorLabelWidth` and the placement, and B52 asks `connectorLabelBox` at the diagram's `connectorFontSize`; bpmnLayout calls `placeMessageLabels`, keeps no R05.05 / R05.09 / R5.07 code, and its R5.12 label mover leaves message labels alone (it could only move them out of the air gap); headless builds the editor's payload; placing a placed label again changes nothing. |
| T4832 | `tests/diagram/assist-apply-layer.test.ts` | T4749 extended to the PAYLOAD: useDiagram's `updateConnectorLabel` and the headless diagram both build it with `connectorLabelPayload`; the editor's two-argument call sends exactly `{ id, label }`, as the harness does; renaming a connector with a stored position through each gives the same connectors, the position kept. |
| T4833 | `tests/bpmn/message-label-generated.test.ts` | Paul: "Always — generated too". Across the layout corpus, every labelled message with an air gap (measured independently of messageLabel.ts): the drawn box lies in the gap next to its POOL end, its near side is 10px off that pool's edge or a whole number of rows (h + 2) further in, it is not drawn across its own line, and no other message's line runs between it and its own line (for a label stepped out along its row, none through it either) — so it never reads as a neighbour's name; before the fix 8 of 207 generated labels were stepped out beside another message's line. More than 20 messages judged. (V15.01's label, beside an event whose nine-line name hangs into the gap, is stepped out along its row; generation's R5.12 no longer moves message labels, which had pushed it into the SCADA pool.) |
| T4834 | `tests/bpmn/v2505-rules.test.ts` | B52 measures the box the canvas draws (`connectorLabelBox`): two copies of a label drawn on two lines, 20px apart, now overlap and fire (the old check counted one line); one-line labels 20px apart are clear; two "Order details" labels 100px apart are clear at the default 10px and overlap — and fire — at `connectorFontSize` 12, as drawn. |
| T4835 | `tests/diagram/message-label-place.test.ts` | A label steps out along its row only while no other message's line lies between it and its own line (Paul: "attached closest to the Pool meesage endpoint"). Three messages from one task, lines at 440, 520 and 480: "message 1" and "message 2" sit left of their lines, and "Order details" stays attached 6px off 480 with neither neighbour's line between — the step out had put it at 526–616, 6px beside message 2's line. Both attached spots covered and a neighbour's line at x 360 on the left: every left step out passes it, so the label steps out RIGHT (x 466); without the neighbour it steps out left. |
| T4836 | `tests/diagram/message-label-rename-2026-09-25.test.ts` | The same through the reducer: ADD_CONNECTOR three times from a task at x 380–540 to Customer (sourceOffsetAlong 0.375, 0.875, 0.625 → lines 440, 520, 480; the third labelled "Order details"): the first two labels at 368–434 and 448–514; "Order details" is attached 6px off its own line, inside the gap, with no other message's line between it and its line, and not at the stepped-out x 526. |
| T4837 | `tests/diagram/message-label-rename-2026-09-25.test.ts` | A rename keeps the side the user sees: a new message's label (left, attached) dragged right of its line to x 427, then renamed "Invoice" — it is placed RIGHT, attached (x 406, top 90), although the left spot is clear (placed without a preference it goes left). Pins the reducer's `prefer: messageLabelSide(…)`. |
| T4838 | `tests/diagram/assist-apply-layer.test.ts` | The headless diagram opens a diagram the way the editor does: useDiagram's `useReducer(reducer, initialData, healOnLoad)` and headless's `healOnLoad(initial)` are the same exported heal; on Paul's saved diagram ("Request" with no position) `headlessDiagram(d).data` equals `healOnLoad(d)` and the label is placed; L4's `scoreApply` takes "before" from the healed diagram, so a heal is never scored as the command's effect. |
| T4839 | `tests/diagram/template-after-voice.test.ts` | Paul, 2026-09-25, of "add template after selected": "added a task called "Template". It should allow user to select a template then place the selected template After the selected gateway." Every phrasing opens the window anchored — `pickTemplate { afterRef }` for "add template after selected", "Add template after selected.", "add a template after review", "insert a template after it", "add template following the gateway", "add the template after selected gateway", "add a new template after review", "put a template behind Check Stock", "add template next to the gateway"; "use a template after this" is a template, not the ghost pick; "Add template," (held on its comma) stitched with "after selected." by `stitchFinals` parses the same. The anchor words are ONE list (`AFTER_WORDS`, placeWords.ts) read by the add rule too: "add a task called Pack following Review" still anchors, and the grammar keeps no private copy. |
| T4840 | `tests/diagram/template-after-voice.test.ts` | "before / ahead of / in front of / preceding X" give `pickTemplate { beforeRef }` (refused at apply time) — two of them used to make a GATEWAY named "template before the" / "template ahead of the"; "add template here", "put a template over there", "add a template right here" give `at: "pointer"`. |
| T4841 | `tests/diagram/template-after-voice.test.ts` | Unchanged: "add template", "templates", "add a new template" (the window); "add a template called Intake", "add template called X after Review", "add a task called template", "add a task Template Review", "add template review" (tasks — a name that begins with Template is a name). Never a task or gateway called "template…": "add a template to the selected", "add a template to the gateway", "add template after", "add template following", "add a template on Review" all go to the AI. `isIncompleteCommand` holds "add template after / before / following", and `stitchFinals` joins "add template after" with "selected". |
| T4842 | `tests/diagram/template-after-voice.test.ts` | `validateOps` keeps `afterRef`, `beforeRef` and `at: "pointer"` on pickTemplate and drops everything else (a stray label, a bad `at`, an empty ref); the AI prompt carries the canonical "add template after <name>" and the `afterRef` field. |
| T4843 | `tests/diagram/template-after-voice.test.ts` | Paul's diagram (the voice-debug snapshot, `tests/fixtures/block2-test3-template.json`, merge gateway 5njpff19 selected): "Add template after selected." applied headless reports ok, opens the window anchored (`template after 5njpff19`), and leaves the elements and connectors the very same objects — no task called Template, no pool resized (his report: both). |
| T4844 | `tests/diagram/template-after-voice.test.ts` | The anchor is resolved as the add op resolves one: nothing selected → "nothing is selected"; two selected → "select just one"; "after End" → "a template can’t follow “End” — no sequence flow can leave it"; a task inside an expanded subprocess → "templates can’t be attached inside an expanded subprocess yet" (the subprocess itself is fine); "before Check Stock" → the plain refusal and the diagram unchanged; an ambiguous "review" parks the numbered pick and the answer, re-run as an #id, opens the window after that one; "here" needs the pointer and then opens "template here". |
| T4845 | `tests/diagram/template-after-voice.test.ts` | Inside the open window (verdict-5, "Add template." … "After selected." split at a pause): "After selected.", "after the gateway", "and after Review", "put it after Check Stock" re-anchor it; "before the gateway" is refused; numbers, names, yes and cancel mean what they meant; "here" is not an answer. |
| T4846 | `tests/diagram/template-attach-plan.test.ts` | `templateAttachData`, hardened: the entry is a top-level flow node (no data, note or container, not boundary-mounted, not an event sub-process) with no incoming SEQUENCE flow, else the leftmost, else none. Data Input / Output enters at "Transform" (not its data object), Perform Regular Task and Automated Process have no entry, Single Approval still drops its start and enters at Submit, loops at their leftmost step, Non-Interruptible Process Pattern at its subprocess (not a task inside it). Across the 35 built-ins exactly eight entries change, all named. |
| T4847 | `tests/diagram/template-attach-plan.test.ts` | Single Approval after Paul's merge gateway (nothing going out): Submit at gateway right + 51, centres aligned — (646,107), the mouse's maths — start dropped; the dry run (`checkTemplateAttach`, the real reducer) puts every element in Warehouse and inside it, Warehouse grows from 116 past 200, and exactly ONE new sequence flow runs gateway → Submit, right@0.5 → left, routed; nothing is named Template. |
| T4848 | `tests/diagram/template-attach-plan.test.ts` | The placement the mouse attach made before it moved: after Check stock the fragment is nudged clear as one box to (320,391); after the decision gateway "Is it in stock?" (three branches) it is INLINE at (686,107), in Warehouse — the design's fan-out put Submit at (667,-89), between two pools, and its join-only dry run passed (verdict-5's blocker). |
| T4849 | `tests/diagram/template-attach-plan.test.ts` | After Paul's Event 4 (on the subprocess's bottom edge): the entry sits where `planBoundaryFollowOn` puts a step (R7), every element joins Sales (the host's lane, never the host), and after the dry run all of them are inside Sales, which grew — the lane pass used to hand a fragment below Sales' floor to Marketing — Marketing moved down, the host kept its size, and the join leaves bottom@0.5 → left with its first visible point below the event. |
| T4850 | `tests/diagram/template-attach-plan.test.ts` | Refused with the real reason, never placed half-joined: inside an expanded subprocess, after an End ("Done"), after a lane; Perform Regular Task ("it has no step a sequence flow can enter"); Simple Document Request Loop ("it brings a pool or lane of its own"); a loose task beside a pool, where a flow CAN leave it, is refused by the dry run because the fragment lands in the pool, away from its anchor ("“Submit” would land in “Us”, away from “Loose”"). Issue 6 (2026-09-26): Expanded Subprocess Template after the merge gateway is no longer refused ("would stick out above") — Warehouse grows at its top and the entry stays level with the gateway. |
| T4851 | `tests/diagram/template-attach-plan.test.ts` | APPLY_TEMPLATE's `join` is ADD_CONNECTOR's own work inside the one action: the same elements and connectors (ids aside) as APPLY_TEMPLATE followed by ADD_CONNECTOR; a join from an End is not drawn and the template is still placed; a lane parent the caller gave is kept (not re-homed by the lane pass) and that lane grows round the element. |
| T4852 | `tests/diagram/template-attach-plan.test.ts` | Each pick is planned on the diagram WITHOUT the one before: over the old preview Rework Loop is nudged away; on the base it lands exactly where the first pick did. `previewBase`: restore (the stored base) while the preview is exactly as shown, strip (by its ids — its elements, its connectors and the join — leaving exactly the original ids) when something else changed the diagram, fresh when Ctrl+Z already took it off. |
| T4853 | `tests/diagram/template-attach-plan.test.ts` | The swap, driven through a model of useDiagram's history that runs the hook's own steps (`runTemplateApply`, `runTemplateRemove`, `noteProduced`, `isStillShowing`) and `planTemplateShow`: three numbers then cancel → exactly the starting diagram, connectors and Warehouse height included, ONE history entry throughout; "yes" keeps the showing one and one undo removes it; Ctrl+Z mid-window → the next number is a fresh first pick, cancel → the start; a co-author's merge mid-window survives the next number (stripped, one new entry), and a second merge then cancel with an EMPTY history takes the template off by its ids with both merges kept — and, the recorded known limit (templatePreview.ts header), Warehouse stays grown; the plain window (its picks now dropped on the end of the current elements through `viewCentre`, issue 6b — T4876): the old undo-then-apply-in-one-tick puts the first and third templates on together (Paul's bug), the new swap shows only the third and cancel restores the start; a refused number (Perform Regular Task, nothing to join — Expanded Subprocess Template is no longer refused, issue 6) leaves the showing one untouched. |
| T4854 | `tests/diagram/template-attach-plan.test.ts` | `templatesToOffer`: attaching hides templates that bring a pool or lane, Paul's initial-template rule still applies, each hidden template counted once; `canAttachInline` is the mouse attach picker's filter too; the window's log line ("31 templates to add after “gateway” — say a number, 12 hidden (starters, and templates that bring a pool)") and footer wording. |
| T4855 | `tests/diagram/template-attach-plan.test.ts` | Wiring: `attachTemplate` asks `planTemplateAttach` + `checkTemplateAttach` and draws the join inside `applyTemplate` (no `addConnector`, one undo entry); the planner adopts via `followOnParentId`; no `previewTemplate` / `hadProvisional` left; `showTemplate` asks `planTemplateShow` and `templateStillShowing` and never undoes; `closeTemplateFlow` undoes only a still-showing pick, else removes by ids, and voice, Esc and the window's two buttons are its four callers; `pickTemplateCard` drops a pick that loads after a newer pick or a close (request number + window id); useDiagram's `applyTemplate` is `runTemplateApply` with every step wired and held — `snapshotData`, `pushHistory`, `topOfHistory`, the record into `templateShownRef`, the SET_DATA restore and the APPLY_TEMPLATE dispatch — and no push or dispatch of its own; `removeTemplate` is `runTemplateRemove`; `templateStillShowing` reads the same `topOfHistory()`; the hook reads back what it produced (`noteProduced`); APPLY_TEMPLATE ends with ADD_CONNECTOR; the apply layer opens the window only after `whyTemplateCantFollow`, and headless records "template after <id>". **Changed by design 2026-09-27 (T4928):** three routes close the window now — voice cancel, Esc, the Cancel button; the Keep button went when a pick became final. |
| T4856 | `tests/diagram/template-attach-plan.test.ts` | A white-box pool with no lanes grows round an attached template the way a lane does (Paul: "just grow the Pool when the template is placed"). Single Approval after a task in a 250px lane-less pool: every element parented to the pool and inside it with the 8px pad, the pool taller and its top unmoved, one join. Every built-in attached there at 150px and 250px through `planTemplateShow`: none refused for sticking out below (before: 25 and 21 of 29 were) or, since issue 6 grows the pool at its top round an anchored template, above; what is refused has nothing inline to join — all 29 with an entry attach at both heights (before issue 6: 23 and 26). The reducer: a child below a lane-less pool's floor grows it to child bottom + 8; in a pool with lanes the lane grows and the pool follows it. |
| T4857 | `tests/diagram/template-after-voice.test.ts` | The template ask is ONE phrase (templatePhrase.ts `TEMPLATE_PHRASE` = verbs + `TEMPLATE_NOUN`), read by the bare command, the anchored command and the add rule's decline: "add another template", "Add another template.", "add one more template", "put a template", "place a template", "drop in a template", "use another template" open the window ("add another template" made a TASK called "another template"); "add another template after Review" anchors; "put another template here" is the pointer; a full stop after the word is a pause like a comma ("Add template. After selected." → afterRef selected); "create another template", "create a new template", "give me another template", "create one more template after Review" go to the AI, never a task; "add a task Template Review" is still a name. Source: the grammar calls `isBareTemplateCommand` and keeps no template regex; the guard is built from `TEMPLATE_NOUN`. |
| T4858 | `tests/diagram/template-after-voice.test.ts` | One set of place words (placeWords.ts: `AFTER_WORDS`, `BEFORE_WORDS`, `HERE_WORDS`, `TAIL_LEAD_IN`, `cleanRef`) for the add rule, the template rule, the stranded-tail rule, the template window and the greedy guard. Outside the window "and after the start" and "that's after Review" are still stranded tails (connect … → the last); inside it "that's after the gateway" and "goes after Review" now re-anchor (they were "say a number…"), plus the window's own "move it after" / "then after"; M5 and the template rule share "right here" / "just there". Source: the grammar's `clean` is `cleanRef`, M5 and the stranded tail are built from the shared words, the window extends `TAIL_LEAD_IN`, the guard is built from all four lists, and no grammar module spells "(?:here|there)", "that'?s", "ahead of|in front" or the quote-stripping itself. |
| T4859 | `tests/diagram/template-attach-plan.test.ts` | One list of what no lane owns — containment.ts `isLaneUnowned` (pain point, issue, text annotation, review comment; not a task, data object, gateway, subprocess or start). `reconcileLaneMembership`, the parentage check (`diagramChecks.ts`) and the template attach (its adoption and its dry run) all call it and none keeps a list of its own. A note carried in a template stays unowned when attached after the merge gateway and does not stop the attach. |
| T4860 | `tests/diagram/template-after-voice.test.ts` | Inside the template window a template's WHOLE name beats the place words: "After Hours Escalation" (and "after hours escalation.") and "Before Close Checklist" pick those templates, while "after hours" and "after Single Approval" are still anchors. The editor's re-anchor resolves "selected" against the selection without the preview it selected, falling back to what was selected when the window opened (`selectionAtOpen`) — after a pick, "after selected" found only the template itself. |
| T4861 | `tests/diagram/template-attach-plan.test.ts` | The swap's order is ONE function, `runTemplateApply` (templatePreview.ts), run by useDiagram's `applyTemplate` and by T4853's model: a first preview pushes the diagram and THEN reads the entry (the entry is the pushed snapshot, stamp and `before` recorded); a swap over a preview still showing pushes nothing and restores the base BEFORE applying; a swap after another change pushes the stripped base, records it as the entry, restores it, then applies. `runTemplateRemove` pushes the diagram as it is and puts back the diagram without the preview, and does nothing when the preview is already gone. |
| T4862 | `tests/diagram/template-attach-plan.test.ts` | The Assist ghosts that attach a template (the "Template" ghost and the name-intent one) are offered only when `whyTemplateCantFollow(selectedElement, …)` is null — checked before either is pushed — so an End ("Done"), a data object ("Order") or a task inside an expanded subprocess never offers a picker that could only refuse; a task and Paul's merge gateway still do. |
| T4863 | `tests/diagram/template-insert-settles.test.ts` | Paul, 2026-09-25: "manually placing a template with an EP in it on or over existing diagram elements also does the same thing … We need a generic fix for this issue independent of voice assist." His own drop, rebuilt as a fixture (`tests/fixtures/voice-debug/block2-test3-template-insert.json` on `block2-test3-template.json`): APPLY_TEMPLATE at the viewport centre, on his process, passes the whole verification set (`_helpers/settleAudit.ts`: every connector end on its element, every rigidly-moved flow translated exactly, no new crossing, no pool overlap, every template element inside its parent, no unowned template flow element over a pool) — before, 22 connectors hung off their shapes, Company overlapped Pool 1 by 19.4px and the template was torn three ways. ONE piece in Warehouse (boundary events with their host's parent, "Re-work completed" no longer in Front office); Warehouse grows down, Marketing, its sub-lanes, Do nothing and Pool 1 move by exactly that, the Company/Pool 1 gap kept; his process does not move (no join); Company widens to the template's right + 40, the other pools keep their width; the template's flows keep their saved routes, translated. |
| T4864 | `tests/diagram/template-insert-settles.test.ts` | THE pools-below cascade (`cascadePoolsBelow`), computed once, top-down, from the original geometry: three black-box pools stacked 60px apart under a pool that grows, listed top-down or bottom-up, are each pushed ONCE by the growth, gaps and order kept (the per-pool loop it replaces pushed them two and three times and could reverse them); the 100-px rule on the ORIGINAL gap (260px away: nothing moves for +50, everything for +200); a pool that moved is not a pool that grew, and one that grew at its top pushes nothing; two grown pools push what is under both by the sum; a pushed pool takes its lanes, contents and boundary events; relaxed layout pushes only the pools the grown pool would cover, never swallowing one — a pool beside it stays. |
| T4865 | `tests/diagram/template-insert-settles.test.ts` | Every path that grows a lane pushes stacked pools below exactly once, in either creation order: APPLY_TEMPLATE, UPDATE_LABEL on a lane (a long name), SET_LANE_FONT_SIZE (both lanes grow; one push by the total), ADD_SUBLANE's first split, ADD_LANE's first lane and the palette's Pool/Lane drop into an empty pool, a step kept in its lane (issue 1's `makeRoomInLane`), and a lane-less white-box pool growing round a template (issue 5); the palette's lane split pushes nothing. |
| T4866 | `tests/diagram/template-insert-settles.test.ts` | Connectors follow ONCE (`connectorsFollow`): an eleven-point hand-shaped flow in a lane below the grown lane, and one in a PUSHED white-box pool, keep their exact shape, translated by the growth (re-routed, the router kept their old interior); a flow from the grown lane into the lane below is re-routed with both ends on and its "Yes" the same distance above its horizontal segment; nothing moved → the same connectors array; one end moved → re-routed and attached; a sub-lane that grows for its name takes the flows in the lanes below its PARENT lane with it (the old recompute stopped at the parent). |
| T4867 | `tests/diagram/template-insert-settles.test.ts` | ONE message-label rule (messageLabel.ts `followMessageLabel`): a message between two WHITE-box pools, its label placed 10px under the upper pool's edge, keeps exactly that place — in the gap, attached at the same end — when the upper pool grows 60px through APPLY_TEMPLATE and through `makeRoomInLane` (the old shift code counted an absent poolType as black-box and moved it half the growth); on Paul's diagram a legacy pool-to-pool message and Do nothing's two messages stay in the Company/Pool 1 gap, the pool-to-pool label riding with Pool 1 (verdict-6: 159.6px off with two rules chained); a message with NO air gap (pools side by side in a relaxed layout) keeps its label's place on its line when a lane rename moves its sender (measured against its old absolute box it was left 164px above its line). |
| T4868 | `tests/diagram/template-insert-settles.test.ts` | One host, by OVERLAP (templateAdoption.ts): a template straddling two lanes goes wholly into the one it overlaps most (a tie to the lower), which grows round all of it and pushes the lane below; centred just below the last lane it is that lane's (not left over the black-box pool below, as 33 of 35 built-ins were under the centre rule); over a sub-lane, the sub-lane's; to the right, taken in when it starts within `ADOPT_RIGHT_REACH` (120px) of the pool's edge, and not from 300px away; over a black-box pool with no white-box host it is moved clear (straight up or down) and stays unowned, and that pool stays black-box after an unrelated move; hanging into a white-box pool from its LEFT (centred left of its edge) it is not taken in and is moved left, `POOL_GAP` clear of it — every built-in dropped 700px left of Company, never left lying over it. ONE predicate, blackBoxPoolMenu.ts `isBlackBoxPool` (explicit black-box only; an absent poolType is white-box), in the adoption, `reconcileLaneMembership`, ADD_ELEMENT's container filter and the template window (`diagramHasWhiteBoxPool`). |
| T4869 | `tests/diagram/template-insert-settles.test.ts` | A template's own note moves with it (header shift and lowering) and no lane adopts it; an existing note follows the element it is associated to, else the lane its centre lies in, and one in the grown lane stays put; the header shift puts the piece exactly the half-event gap right of the lane header, the note with it; boundary events are parented as their host is, a sub-process's own rim start/end keep the sub-process, none of them chooses the host, but the lane holds each WHOLE (`_helpers/settleAudit.ts` `boundaryOutsideBand` — "End of Day" and "An Error" hung 10px below their lane); an EP reaching above its lane with an event on its top edge is lowered until the event clears the lane's top pad. |
| T4870 | `tests/diagram/template-insert-settles.test.ts` | Stale lane parents on the EXISTING elements are repaired before the lane grows: Do nothing, owned by Company but sitting in Slack team, is re-homed and goes down with Slack team (without the repair it stayed behind inside the grown Warehouse). |
| T4871 | `tests/diagram/template-insert-settles.test.ts` | A template that brings its own pools (Automated Process, Initial Multi-Lane, the two Document Request Loops), dropped at three places on Paul's diagram, is stacked under Pool 1 `POOL_GAP` (98) apart — one constant, poolLaneBounds.ts, shared with generation — whole, with its own parents, overlapping nothing and moving none of his pools, and every one of its labelled flows' labels travels with it (23 of 72 stayed behind over Company, up to 966px from their lines); a template that brings a LANE but no pool is stacked the same way, never taken in as a sub-lane of the lane it lands on (templateAdoption.ts `isTemplateContainer`, the same rule the attach refuses by). |
| T4872 | `tests/diagram/template-insert-settles.test.ts` | Sticking out ABOVE its lane: with no join only the template is lowered and Warehouse's own content stays put; joined after Paul's merge gateway (issue 5's attach), Expanded Subprocess Template is no longer refused — Warehouse grows at its top, its content and the template move down together, its top stays, the entry is level with the gateway and the join is one straight line, a free note lying in Warehouse is carried down with its content, the template's flows are still translated and its edge events lie wholly in the lane; joined, the template's flows keep the routes they were saved with (Paul's own template: "Re-work completed → Assess" translated, not re-routed — only the join is checked against obstacles, ADD_CONNECTOR `keepOtherRoutes`), and none of his own flows whose ends did not move changes; after the merge gateway no built-in is refused for sticking out, only for having nothing inline to join. |
| T4873 | `tests/diagram/template-insert-settles.test.ts` | Every built-in without a pool, dropped (a) into an empty lane (Good team) and (c) into a lane too small for it (Front office, the EP that does not fit its lane), passes the whole verification set; (b) centred ON Paul's process, everything but the crossings the drop itself makes (edge events wholly in their lane included) — the template's own flows cross nothing of the template's they did not, and existing flows that were translated or left alone cross no existing element they did not (the placement that keeps a template off the process is issue 6b). |
| T4874 | `tests/diagram/template-insert-settles.test.ts` | Wiring: `applyPoolBelowShift` / `applyPoolAboveShift` (and the unused sideways copy `applyPoolRightShift`) are gone and MOVE_ELEMENT pushes pools with `cascadePoolsBelow` (defined once, called by `settleGrowth` and by `refitContainersAfterMove`, the move re-fit MOVE_ELEMENT and the end of a group drag both run — issue 6b); every case that grows a lane (APPLY_TEMPLATE, UPDATE_LABEL, SET_LANE_FONT_SIZE, ADD_LANE, ADD_SUBLANE, ADD_ELEMENT, DELETE_ELEMENT) calls `settleGrowth` after its geometry, and `resizeLaneForLabel`, `makeRoomInLane`, `growLaneAtTop` and `growLaneToHeight` touch no connector; `settleGrowth` = cascade → notes → `connectorsFollow`, which uses `followMessageLabel`, `pinPoolMessageEnds` and `labelFollowOnRouteChange` and not `adjustMsgLabelOffset`; APPLY_TEMPLATE reconciles the existing elements only, has no band pass, plans with `planTemplateAdoption`, makes room for the whole fragment and joins after the settle with `keepOtherRoutes: true`; bpmnLayout imports `POOL_GAP`. |
| T4875 | `tests/diagram/template-insert-settles.test.ts` | `settleGrowth` with nothing grown returns the very same elements and connectors; SET_DATA(base) then APPLY_TEMPLATE equals APPLY_TEMPLATE on base (issue 5's swap composition). |
| T4876 | `tests/diagram/template-drop-group-drag.test.ts` | Paul, 2026-09-25: "assume the template will go on the end of the current elements". His template (6a's fixture), picked from the window with the middle of his screen at (495.41, 167.57): centred there as before it lay over Receive order, Check stock and Pick items (recorded); `dropBandAt` finds Warehouse and its last element, the merge gateway; `planTemplateDrop` plans the template's first step (`templateEntryOf`, the start event) ½ Task width right of the gateway and level with it, clear of every element and every flow, one piece, its top level joining Warehouse; applied, 6a's full verification set holds, Warehouse's own process does not move, the template lies wholly in Warehouse right of the gateway, nothing of his is under it and none of his flows runs through it — and what he sees: it is exactly where planned, lowered only as far as it reached above Warehouse (plus the 8px pad), so its start event ends that far below the gateway (with no join only the template is lowered — the issue 6 decision; carrying Warehouse's content down with it is Paul's open choice). |
| T4877 | `tests/diagram/template-drop-group-drag.test.ts` | Every built-in without a pool or lane of its own, dropped by `planTemplateDrop` with the middle of the screen over his process (Warehouse), over Front office (after Back order) and over the empty Good team: none lands on an element or a sequence flow that was there (over Front office 9 of 31 used to sit on "Back order → gateway"), its top level joins that lane, APPLY_TEMPLATE's full verification set (`auditInsert` — ends attached, rigid flows translated, no new crossings, no pool overlap, inside parents, edge events in their band, one piece) holds for all of them, none of his flows runs through a template element once applied (`existingFlowsThrough`, judged on the result — the audit's naive baseline already has the template where it was planned), and every element is where it was planned, lowered only by what reached above the lane; after the merge gateway and after Back order the first step is planned ½ Task width right of the last element and level with it (or further right when nudged clear, still level), and applied it is level, or lower by exactly what reached above the lane — most go in level, some are lowered. |
| T4878 | `tests/diagram/template-drop-group-drag.test.ts` | Which band and where in it (`dropBandAt` / `planTemplateDrop`): an empty diagram is centred exactly as before (`instantiateTemplate`); a lane with nothing in it takes the template at its left — the header's right edge plus MIN_LEFT_GAP — level with its middle, and adopts it (and the insert audits clean); a sub-lane is the band, not the lane it splits; a white-box pool with no lanes is a band; the last element is the step furthest right — an edge event, a step inside an expanded subprocess, a data object and a note are not steps (one rule with the template's first step, templates.ts `isProcessStep`); the last element's own sequence flow down to the next lane is not landed on — blind to the flows the first step sits on it (recorded), with them it goes further right, still level, and applied no existing flow runs through it; with nothing under the middle of the screen (a black-box pool, empty canvas, beside the pool) the nearest band is taken, never the black-box pool (Paul's diagram over Customer → Front office after Back order); a diagram with no pools puts it after its rightmost step, nothing adopted; a template with no step lines its box up instead; a template with pools of its own is centred as before (APPLY_TEMPLATE stacks it). |
| T4879 | `tests/diagram/template-drop-group-drag.test.ts` | Wiring: the toolbar list (`handleApplyTemplate`) plans with `planTemplateDrop` on the live diagram and no longer calls `instantiateTemplate`; the window (`showTemplate`) sends `at` only when the pointer gave one and always `viewCentre`; `planTemplateShow` centres on the pointer or drops by `planTemplateDrop` (behaviour checked too: its plain pick equals the drop, its pointer pick equals the centred instantiate); the attach and the drop both place the first step with `placeInline` and nudge + adopt through `nudgeAndAdopt` (`findFreeSlot` called once in templateAttach.ts); `templateAttachData` takes its entry from `templateEntryOf`. |
| T4880 | `tests/diagram/template-drop-group-drag.test.ts` | A GROUP drag ends like a single drag (ELEMENTS_MOVE_END): two tasks dragged together out of lane A into lane B, out of the pool onto empty canvas, and into an expanded subprocess get exactly the parent each gets dragged alone (MOVE_ELEMENT + MOVE_END) — B, released, the subprocess — with every connector end attached and nothing outside its parent; before the fix the group kept lane A wherever it went (recorded); the edge event on the first task stays mounted and belongs where its task now does, as a single drag leaves it; the subprocess that takes them in grows round them — and round the event on the first task's edge, now its own too — and stays in its lane; a moved container keeps its children as they are (a step drawn over a smaller subprocess inside it is not re-homed — only what the user moved is asked, as a single drag of the container leaves it) and finds its own parent the same way; the end is reconciled as MOVE_END is (a task drawn in lane B but owned by lane A is re-homed to B — CORRECT_ALL_CONNECTORS alone left it stale); a click on a selection moves nothing and ends as it always did — CORRECT_ALL_CONNECTORS, nothing re-parented or re-homed (headless). |
| T4881 | `tests/diagram/template-drop-group-drag.test.ts` | Paul's gesture (issue-6.md): his template dropped on his process, then his (681.43, 2.86) drag of all of it, then its end — every root he dragged has the parent a single drag gives it; Approved, Approved?, Assess and Rejected, whose centres left Company, are released (no lane owns them), connectors attached; what hangs out of its parent afterwards is exactly what hangs out when each is dragged alone — the subprocess, still Warehouse's, reaching past Company's right edge (a drag never widens a lane, one element or many). |
| T4882 | `tests/diagram/template-drop-group-drag.test.ts` | useDiagram's `moveElements` records every id it actually moved and `elementsMoveEnd` sends them in ELEMENTS_MOVE_END — CORRECT_ALL_CONNECTORS only when nothing moved — and forgets them; headlessDiagram does the same, and its staged two-step move ends in exactly the reducer's answer (released), one undo taking back the move and the release; each end asks only what its own gesture moved (a subprocess dragged over a task the gesture before released does not take it in); wiring: MOVE_ELEMENT, MOVE_END and ELEMENTS_MOVE_END all ask `pickDropParent` (its priority chain written once), MOVE_ELEMENT and ELEMENTS_MOVE_END re-fit through `refitContainersAfterMove`, and ELEMENTS_MOVE_END is in LANE_RECONCILE_ACTIONS and ROUTE_CHANGING_ACTIONS. |
| T4883 | `tests/diagram/template-drop-group-drag.test.ts` | A step dragged past its expanded subprocess's edge stays its step, one element or many, mouse or voice (review of 6b — the group end had released it, and its flows crossed the subprocess's edge): "File it" right, and "Check form" + "File it" down, in a 5-frame group drag keep the subprocess as parent and grow it to exactly the rectangle each single drag grows it to, nothing outside its parent, no sequence flow `canConnect` refuses, ends attached; by voice ("move File it right", "move File it down", "move Check form down" — parseCommand → applyAssistOps → headlessDiagram) the same; only a container inside the subprocess may take the step (dropped on a subprocess nested in it, the nested one takes it, as the single drag's drop gives it); data is never held (a data object dragged out in a group is released, as its single drag's drop releases it); a chevron dragged with another past its process group's edge keeps its group, as a single drag does; wiring: MOVE_ELEMENT's EP lock and process-group clamp and the group end's hold all ask `dragHolderOf` (defined once). |
| T4884 | `tests/diagram/template-drop-group-drag.test.ts` | An event on an element's edge goes where its host goes when a drop re-parents the host (the mount convention, `parentId: host.parentId` — the BPMN export files a flow node under its parentId's lane): a task dragged alone or in a group from lane A into lane B takes its edge event into lane B; Paul's template dropped on the end of Warehouse and group-dragged up onto Back order — its subprocess and every event on the subprocess's edge belong to Front office (before, "Re-work completed" stayed Warehouse's); wiring: MOVE_END and ELEMENTS_MOVE_END both call `edgeEventsFollowHosts` (defined once). |
| T4885 | `tests/diagram/template-drop-group-drag.test.ts` | A sub-process's own rim start and end keep it as their parent, whatever moves the sub-process (pre-push review: 6b's `edgeEventsFollowHosts` handed them to the host's lane on the first drag, undoing 6a's adoption rule — the export then put them outside the `<subProcess>` while their flows stayed inside, and the simulator found no body start). "Non-Interruptible Process Pattern" and "Expanded Subprocess Loop", each dropped on the end of Warehouse on Paul's Block 2 Test 3 diagram: adopted into Warehouse; after APPLY_TEMPLATE, a 5-frame drag of the sub-process, an arrow nudge of it alone, a 5-frame group drag of the template and an arrow nudge of the selected template, the rim start and end are parented to the sub-process, exported inside its `<bpmn:subProcess>` with every flow to or from them, the simulator's `bodyStart` is the rim start, no connector end is detached and nothing lies outside its parent; dragged into Front office alone or in a group, the same holds and the other events on its rim ("End of Day", "An Error") go to Front office with it; the rule (`containment.ts` `edgeEventParentId`): the host's parent, except a start or end its sub-process already owns — a rim start the sub-process does not own and an intermediate event (even one saved owned by its host) take the host's parent; wiring: `edgeEventsFollowHosts` and APPLY_TEMPLATE both ask `edgeEventParentId` (defined once, in containment.ts), and templateAdoption no longer restates the rule. |
| T4886 | `tests/diagram/voice-debug-json-snapshots.test.ts` | Paul, 2026-09-26: "Make the snapshots in Voice Assist JSON, not SVG. These can be better used for diagnosis." (reversing his 24 Sep "JSON and a PNG"). A snapshot's `diagramJson` is now the WHOLE `DiagramData`, flat — viewport, fonts, `relaxedLayout`, title, which change what a replayed command does — never `aiGeneration` (customer prompt text) or `_archive` (user emails and ids), plus one reserved key `_voiceDebug` { v: 2, role, diagramType, colorConfig, displayMode, ui }. `ui` holds the selection, pointer, "it", the numbered badges exactly as `badgesOnScreen` gives the canvas (rename pick, else message pick, else picker), and the open flow as a PROJECTION: a template window's `thumbnailSvg` and its preview's `base` (a whole second diagram) never get in — a spread would bring the pictures straight back. |
| T4887 | `tests/diagram/voice-debug-json-snapshots.test.ts` | Automatic capture (Paul's Q1 answer: once at the start, after every command that changed the diagram, before one when the mouse changed it) saves at most ONE copy per distinct state: the same object, or a new object holding the same diagram, is skipped, so one command's after-state is the next one's before-state. 📷 ("mark this moment") always saves — the badges or selection may be the point with the diagram unchanged. A running size total stops automatic saving under the save route's 40 MB cap (at 30 MB), reports it exactly once (the editor's log line), and 📷 still saves after. The total counts UTF-8 bytes, what the route's content-length cap counts — a string's length undercounts accented and CJK labels. |
| T4888 | `tests/diagram/voice-debug-json-snapshots.test.ts` | Any snapshot reads back: an old one (`{ elements, connectors }` only) as BPMN in the default colours with a default viewport; a new one as the diagram WITHOUT `_voiceDebug` plus its meta. Paul's Block 2 Test 3 snapshot (qe8reyjr), saved with Pick items selected, survives the session file, loads in `headlessDiagram` and replays "rename selected to Pick Stock" — and cannot be replayed without the saved selection, which is why a snapshot now keeps what was on screen. The old shape still replays by name. |
| T4889 | `tests/diagram/voice-debug-json-snapshots.test.ts` | A snapshot's Download JSON is the STANDARD export envelope, from the one builder the editor's own export and its SharePoint save now use (`app/lib/diagram/exportEnvelope.ts`): it passes the editor's Import JSON checks (non-empty `diagrams`, object `data`) and a project's (`project` + `diagrams`), and `checkSchemaCompatibility`; `_voiceDebug` stays out of `data` (Import would save it into the diagram) and the type, colours and display go where an export puts them. The import checks it mirrors are pinned as source so the mirror cannot drift, and no hand-written envelope is left in the editor. |
| T4890 | `tests/diagram/voice-debug-json-snapshots.test.ts` | The session file keeps the commands pretty-printed and puts each snapshot on ONE line — indenting a diagram doubled its size, and automatic capture makes snapshots most of the file. It round-trips through `parseDebugSession`, including an empty session and the viewer's GET-shaped object with extra keys. |
| T4891 | `tests/diagram/voice-debug-json-snapshots.test.ts` | The debug viewer: a picture only when the GET says `hasPicture` (`pngWidth != null` — sessions before 26 Sep 2026), and the viewer's only `<img>` is that picture, so a snapshot with no picture no longer shows a broken image. "draw" mounts the editor's own `<Canvas readOnly>` (the ProcessView pattern, badges redrawn from `renameBadges`; the `readOnly` PROP is pinned, not the word) through `next/dynamic`, one snapshot at a time (only `{drawn && reading && <SnapshotCanvas …>}` mounts one); Download JSON, Copy JSON (`navigator.clipboard`, no browser dialogs), and the session Download uses the bar's serialiser and `debugSessionFilename`. |
| T4892 | `tests/diagram/voice-debug-json-snapshots.test.ts` | Every command-log line goes through ONE `appendLog`, which stamps `at` (a dozen direct appends had skipped it) and carries the ops the command just applied — set in the editor's apply wrapper AFTER the debug arm (so the size-limit line cannot take them) and cleared on the next microtask — so a command's entry records its `ops`, as the plan promised and nothing ever did. |
| T4893 | `tests/diagram/voice-debug-json-snapshots.test.ts` | The editor's wiring: the "before" is saved when `armDebugBefore` runs, the "after" in an effect keyed on the diagram AND the log (so a command that changed nothing clears its arm rather than catching the next mouse drag), "start" when the bar opens with debug on (or the log is cleared, T4896), and 📷 as "marked" — all through `captureState`; the "after" is settled by `settleArm` (T4895). Every command that changes the diagram arms it: the op batch, a template pick, the guided rename, "undo" said in the rename pick, and a template taken off. The canvas and the recording read the badges from one value. No picture anywhere: `captureCanvasPng` and `prepareCanvasClone` are gone while the SOP figure's four helpers stay; the bar says "Save the diagram as it is now (JSON)". |
| T4894 | `tests/diagram/voice-debug-sessions.test.ts` | A JSON snapshot's `_voiceDebug` survives the save route into `VoiceDebugSnapshot.diagramJson` whole (the route keeps a fixed list of snapshot fields, which is why it rides INSIDE the document — no schema change), with the full DiagramData, no picture bytes and a null `pngWidth`, so the GET reports `hasPicture: false`. |
| T4895 | `tests/diagram/voice-debug-json-snapshots.test.ts` | A voice command's saved states are settled after its render by ONE pure rule, `settleArm` (`debugCapture.ts`): the "after" is saved only when the diagram is no longer the one the command started from — and saved even with no log line to tie it to, since it is the next command's "before"; the "before" (saved only when the mouse had changed the diagram) joins the command's line; the line's snapshot is its "after", else its "before", else none. Tested behaviourally with the real ledger, and the editor's wiring is pinned: the arm is cleared before settling (left set, every later mouse drag would be saved as that command's "after"), and both links are applied. |
| T4896 | `tests/diagram/voice-debug-json-snapshots.test.ts` | The Voice Assist bar's "clear" starts the debug recording afresh: it empties the saved states, the capture ledger and the arm with the log. Kept, they were states no line pointed at — invisible in the viewer yet counted against the 30 MB limit — and the ledger still held the last "after", so the next command got no "before". The new recording's "start" is saved by the one start effect, woken by a new epoch. |
| T4897 | `tests/diagram/voice-debug-json-snapshots.test.ts` | A command queued behind an AI call (B4) gets its own "before" and "after". The queue used to drain inside the AI call's `finally`, before React rendered, so the two commands shared one arm and one "after" and the AI command's line had no saved state (shown with the real ledger). The drain now runs in an effect after the render, declared after the effect that settles the command before, so each is settled before the next arms (T4567). |
| T4898 | `tests/dictation/replay-one-set.test.ts` | A recorded clip whose expected element is no longer in TODAY’S test diagram is **stale** — neither a pass nor a failure, counted apart and out of the pass rate. Before, `checkRefs` passed a MISHEARD clip whenever its expected ref resolved to nothing ("delete Ship Order" heard as "delete Review Claim" scored pass-despite-mishear) and failed the same clip heard perfectly. A selection word ("the selected task") still names nothing without a selection and keeps its old treatment; a misheard SHAPE is still judged; a case whose element exists scores exactly as before. The outcome has a colour and a sentence wherever cases are shown. |
| T4899 | `tests/dictation/replay-one-set.test.ts` | Replay replays ONE chosen set and saves the run under it. It used to list every clip in the database and file the run under whichever seed sorted first, so a second recorded set was silently mixed into the first set’s numbers (found 2026-09-26). The set list is counted in the database (`?sets=1`, a groupBy — never the capped listing); the panel fetches only the chosen set’s clips, keeps only clips of that set in the run, and posts `corpusSeed: runLabel.seed`. The default set: the one already chosen while it exists, else the current corpus, else the newest. |
| T4900 | `tests/diagram/assist-kind-binds.test.ts` | Paul, 2026-09-26: “"Compress" keyword are not recognised properly.” The recogniser was innocent (12 of 12 heard right since the boosts came out); the parser threw the word "pool" away. PARSED AND APPLIED through the headless editor (T4695 lived in the resolver alone): "Compact pool three." compresses Pool 3 with no "Pool 3 or Lane 3?"; "compress Pool 3" / "compressed pool three" work; "Move pool three top boundary down" moves Pool 3's edge (it had no picker at all); "add a lane to 3" can only mean a pool; a lane named where a pool must be is told so, never "isn't a pool"; with no kind word a name shared by a pool and a lane still asks; the field's kind (refKinds.ts) bounds every name pass; a near miss only asks. **Changed by design with T4903:** "compress lane three" now compresses Lane 3 instead of refusing. |
| T4901 | `tests/diagram/assist-kind-binds.test.ts` | What the compress rule reads (compressPhrase.ts): the kind word is KEPT and moved to the front ("the Customer pool" → "pool Customer"); a trailing "line" is a name; plural kinds, the gap, the diagram and an EP collapse are not a compress of one container; "delete X and compress" and "… and shrink" compact (they used to delete, drop the compaction and show a green tick); a bare "Compress lane." / "Compress the pool." is held for its name; the scorer resolves "pool three" exactly as the app does and L4 passes. **Changed by design with T4903:** "shrink the Sales lane" now parses to `compressLane`; the plural negatives also assert no `compressLane`. |
| T4902 | `tests/diagram/assist-kind-binds.test.ts` | Wiring: ONE compress verb list (commandVerbs.ts COMPRESS_VERBS / COMPRESS_COMMAND_VERBS / COMPRESS_VERB_SOURCE) read by the compress rule, the delete tail, greedyGuards, selectedWord, the keyterm collision set and the AI prompt — six copies had drifted apart; ONE kind table read by the apply layer (`resolveField` for all six pool fields) AND both scorers (`kind: refKind(`), so the harness resolves as the app does. L4 now counts a picker as a pass only when a ref really names several things — "compress pool three" asking which was scored a pass, which is why the harness never saw Paul's bug. |
| T4903 | `tests/diagram/assist-lane-compress-expand.test.ts` | The sentences: a lane or sub-lane word makes it a lane command (Paul, 2026-09-26: "Add commands Compress Lane <lane_name>, and, Expand Lane <lane_name>"). Both kind-word positions, a leading "line", sub-lanes, compress inflections; the pool word and bare names stay `compressPool`; "the selected lane" / "this lane"; expand/grow/enlarge with `by N [px|pixels]`; `validateOp` clamps both ops. |
| T4904 | `tests/diagram/assist-lane-compress-expand.test.ts` | A spoken kind word binds: "compress the Customer lane" and "expand the Customer lane" change the lane Customer Service and never the pool Customer; `refKind` for both lane fields. |
| T4905 | `tests/diagram/assist-lane-compress-expand.test.ts` | "Compress lane." / "Expand the lane." are held; the name after a pause is stitched on; a bare "compress lane" asks "which lane? (1–3)" and changes nothing until answered; one selected lane is used; a bare "compress the pool" asks unless one pool is selected. |
| T4906 | `tests/diagram/assist-lane-compress-expand.test.ts` | What is NOT a lane command: "compress all lanes" and the other plurals; "expand the subprocess / Review / the Production Line / pool three / the gap"; an unreadable "by …" ("by a hundred", "by 0") goes to the AI, never a default; "expand the pools" / "grow the pool" are still `extendPools`. |
| T4907 | `tests/diagram/assist-lane-compress-expand.test.ts` | Compress geometry (Paul's ruling, 2026-09-26: the bottom edge moves): the top stays, content slides up to ½ Task under it, the bottom comes up to ½ Task under the lowest content, lanes below close up, the pool shrinks, pools below stay put; sub-lanes fitted one by one; never below the lane's or the POOL's name floor; never grows; a second compress changes nothing; free notes move with their band. |
| T4908 | `tests/diagram/assist-lane-compress-expand.test.ts` | Expand: one Task row (64 px) at the bottom by default, or the number said; the last sub-lane takes the growth; the pools below are pushed by the 100-px rule; a stamped sub-lane stack; no-ops. |
| T4909 | `tests/diagram/assist-lane-compress-expand.test.ts` | Renaming a lane that has sub-lanes to a long name leaves no strip: the sub-lanes still tile it (the old `resizeLaneForLabel` bug, fixed by the one setter). |
| T4910 | `tests/diagram/assist-lane-compress-expand.test.ts` | The shared helpers widened to stamped sub-lanes, each on the mouse path it serves: the divider drag, the lane-drop carve, the pool bottom-edge resize floor, and `keepInLane` make-room through `growLaneToHeight`. |
| T4911 | `tests/diagram/assist-lane-compress-expand.test.ts` | L4 is written from the sentence and catches a wrong edit: taller, under its name, content outside, top moved, a lane above moved, a broken stack, loose margins, a pool under its name, expand by the wrong amount; no gold flash. |
| T4912 | `tests/diagram/assist-lane-compress-expand.test.ts` | Parse AND apply through the headless editor on the test diagram: "compress the Underwriters lane", "compress lane Claims Team", "expand lane Underwriters by 100" (Customer pushed), "compress Underwriters" with no kind word, "compress lane three" / "compress pool three" with no picker, "already fitted", one undo entry each. |
| T4913 | `tests/diagram/assist-lane-compress-expand.test.ts` | "expand" is a command verb now, and what that reaches: the hold, `looksLikeAnotherCommand`, `hasCommandAfterName`; "add a task called Expand Market" and "rename Task 1 to Expand Market" still work. |
| T4914 | `tests/diagram/assist-lane-compress-expand.test.ts` | Wiring: ONE setter (`setBandHeightAtBottom`, laneFit.ts; `growLaneToHeight` is its grow-only wrapper), ONE settle per case, one default step (`LANE_EXPAND_STEP`); the apply uses `resolveField`; the old "is a lane" refusal is gone; every card lane example resolves to one lane of the test diagram. |
| T4915 | `tests/diagram/assist-lane-compress-expand.test.ts` | The generated corpus says the lane commands and has a NEW realistic seed (the corpus reshuffles, so prod's clips keep their own seed): both families always with a lane word, Claims Team included, by-N distances; `compressPool` phrased with "Pool 3" / "pool three" / "compact" / "compressed"; a default-named Pool 3 is in the fixture. |
| T4916 | `tests/diagram/assist-lane-compress-expand.test.ts` | The two prod SQL patches (written, never run here): the User Guide patch writes exactly the seed's line, guarded, one transaction, verified after the commit; the assist-rules patch has three guarded UPDATEs and no DELETE/INSERT/TRUNCATE/DROP, and its lines match `seed-diagram-rules.cjs`. |
| T4917 | `tests/diagram/assist-lane-compress-expand.test.ts` | Room made at the TOP of a lane with sub-lanes goes to its first sub-lane, and only the non-lane content is carried down — it used to grow such a lane twice and overlap the lane below (nested and stamped sub-lanes). |
| T4918 | `tests/diagram/assist-lane-compress-expand.test.ts` | A lane's content is its shapes AND the routes drawn between them: a loop dragged under the row is never left below the lane's new bottom; connectors settle once. |
| T4919 | `tests/diagram/assist-lane-compress-expand.test.ts` | The name floors are the diagram's own fonts, in the reducer and in the question the apply layer asks it ("already fitted" was computed at default fonts). |
| T4920 | `tests/diagram/assist-lane-compress-expand.test.ts` | What a lane holds is what lies in it, re-derived before the fit moves anything: a step owned by a lane with stamped sub-lanes goes with the sub-lane it lies in; nothing is left behind, clipped, or re-homed into the wrong lane. |
| T4921 | `tests/diagram/assist-lane-compress-expand.test.ts` | The lane words are containerWords.ts's, mis-hears and the recogniser's full stops included: "Expand line." is held; "Compress lane. Customer." keeps its lane word and never touches the Customer pool. |
| T4922 | `tests/diagram/assist-lane-compress-expand.test.ts` | The cases the review's mutations showed were unguarded, each now pinned behaviourally (stamped sub-lanes keep their own x and width through a re-fit, among them). |
| T4923 | `tests/dictation/voice-test-sets.test.ts` | The voice test SET registry (corpusSets.ts). Paul, 2026-09-26: "there does not appear to be a way to create other sets of commands?" — a seed only reshuffles one generator; a set is a different exam. The Set drop-down on Test, Record and Replay lists "Realistic sample" (the generator at the current seed) and "Commands popup: every line", each explaining itself, plus "Custom seed…" (the generator reshuffled). A generated set or custom seed IS `generateCases`; the popup set is every line; the family list comes from the set. |
| T4924 | `tests/dictation/voice-test-sets.test.ts` | The popup set IS the popup (catalogCorpus.ts): every card line is a case or is left out with a written reason (the voice words, until the mic-onset question is settled), no stale exclusions or contexts; popup order; ids from the words (`dgx-voice-catalog#<slug>`), so an inserted line moves no other id. The answer key is FROZEN in `catalogCorpus.expected.json` as a file snapshot — a line that starts parsing differently fails until a person reviews it and re-freezes (`-u`). A parse-only line is judged on its parse alone. |
| T4925 | `tests/dictation/voice-test-sets.test.ts` | Recording RESUMES at the first case, in the set's own order, with no clip of its id AND sentence (a reworded line is a new case) — the clips route had always promised it; the listing sorts ids as strings and cannot be used. Replay names registry sets and labels every other recorded seed "Older recording: <seed> (replay only)"; registry sets first. Wiring: the three tabs read the one registry; the Recorder saves `corpusSeed` = the set; Replay scores the popup set against what the sentence parses to TODAY (a hearing test), noting lines whose parse has moved; the generator never imports the popup set. |
| T4926 | `tests/dictation/voice-test-sets.test.ts` | The card speaks the test diagram's names (Paul's choice, 2026-09-26): every line that is not parse-only resolves, with its context (a selection from the test diagram), to the test diagram — no stale, no ambiguous, no wrong element. Parse-only reasons are written out, and selections name real fixture ids. Found by this set and fixed: the card's own "make the selected event a timer event" found nothing whatever was selected — "the selected event" now names any event. |
| T4927 | `tests/diagram/template-after-voice.test.ts` | "add template" with ONE step selected goes after it — the window opens anchored on the selection, so the pick uses the Assist template's attach: a Start Event at the template's front is dropped, the template goes in the step's lane, the lane grows and one sequence flow joins them (Paul, 2026-09-27: the Assist template addition "is correct for the command 'add template' IF a element is selected"). A selected step no template can follow says why; nothing, several or a container selected still opens the plain window. |
| T4928 | `tests/diagram/template-after-voice.test.ts` | A pick in the numbered template window is FINAL (Paul, 2026-09-27: "remove the 'Keep it/cancel' step as this required mouse input"): the number applies the template and closes the window in the same step, reporting "added template …"; "undo that" takes it off. The window has no Keep button, and the prompts ask for a number or "cancel", never "yes to keep". |
