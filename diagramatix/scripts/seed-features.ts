...
export const FEATURES: Array<{ name: string; summary: string; details: string }> = [
  {
    ...
    details: [
      ...
      "- As-is vs To-be comparison — pin scenarios to different process variants and compare side by side with a plain-language verdict (e.g. \"28% faster, +12% throughput, $4.2k less per case, frees ~1.4 FTE\"), plus a grounded AI assessment that explains WHY in plain English (written only from the computed figures, so it never misstates a number)",
      ...
      "- True per-case results — the real distribution of individual case times: Typical (p50), Near-worst (p95) and Spread, with a distribution histogram — not a run-average that hides the tail; Monte-Carlo replications add a bottleneck ranking and a ± confidence on the mean",
      "- Run History — name and keep the runs that matter (e.g. \"Large Sales Team (25)\" vs \"Small Sales Team (3)\"), browse them, and compare any two saved runs head to head with the same charts and AI assessment",
      ...
      "- Ready-made examples — load a worked example (loan origination, car-repair rework loop, or an As-is vs To-be loan comparison) into your own project and demo in two clicks",
  ...
  {
    name: "Diagram Health Check & Connector Scan",
    summary: "Scan a whole project for BPMN connector and structure problems — especially handy after a Visio import — and fix them one at a time, each highlighted on its own diagram.",
    details: [
      "- Project-wide scan against shared BPMN correctness rules: sequence / association connectors illegally attached to pools or lanes, hanging message flows, single-lane pools, duplicate element names and other structural slips",
  ...
  {
    name: "Automatic Diagram Linking",
    summary: "Let Diagramatix wire up the sub-process links across a project for you — match each sub-process to the diagram that details it by name or process number, then confirm in one dialog.",
    details: [
      "- Scans every BPMN diagram in the project for un-linked sub-processes and finds the diagram that expands each one",
  ...
  {
    name: "Flowchart → BPMN Translation",
    summary: "Modernise legacy Standard Flowcharts into BPMN — translate an existing flowchart diagram into a proper BPMN process map in one step, so teams can move off the old flowchart standard onto the current business-process-modelling standard, BPMN.",
    details: [
      "- One-click, deterministic translation of a Diagramatix Standard Flowchart into a brand-new BPMN diagram — the original flowchart is never touched (one-way)",
