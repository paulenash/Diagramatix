...
R54: Event-based gateway wiring — for an Event-based Decision Gateway (gatewayType="event-based", gatewayRole="decision"), the incoming sequence connector attaches to the left diamond point; outgoing connectors fan out to the RIGHT side at top → right → bottom in AI order (visual top, middle, bottom on the right). Each outgoing flow goes directly to its corresponding event with no intervening element. For the matching Event-based Merge Gateway, the outgoing sequence connector attaches to the right diamond point; incoming connectors fan in from the LEFT side at top → left → bottom in AI order (visual top, middle, bottom on the left), each from the corresponding event. This top-middle-bottom fan distinguishes event-based gateways from Exclusive Decision gateways (R35), which use top-bottom-right ordering.
