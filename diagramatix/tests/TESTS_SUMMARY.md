...
**As at:** 2026-09-11  ·  **Document version:** 7.0  ·  **Suite:** 467 test files · 3,535 tests (all green)  ·  **Runner:** Vitest  ·  **CI:** enforced on every PR + push to `main`  ·  **Highest ref:** T4415  ·  **Plus:** a Playwright browser e2e suite — see [Layer 11](#layer-11--end-to-end-playwright-browser-tests)
...
| T4415 | `tests/diagram/assist-nudge-label-swap.test.ts` | The editor requires exactly one selected gateway, swaps OUTGOING points on a decision and INCOMING on a merge ("middle" = the flow side) through `updateConnectorEndpoint`; the reducer re-attaches each end to its new side with targets untouched. |
