...
function autoSizeForElement(el: DiagramElement): { w: number; h: number } {
  ...
  // EPC and Standard Flowchart: wrap to the shape's own width, keep that
  // width, and grow DOWNWARD when the wrapped block no longer fits. The same
  // function the layout and the renderer use, so a hand-typed name and a
  // generated one size identically — and a name typed into a box created from
  // an image behaves like one the layout produced.
  if ((el.type.startsWith("epc-") || el.type.startsWith("flowchart-")) && holdsInternalLabel(el.type)) {
    return fitShapeToLabel(el.type, el.label || "");
  }
...
/**
 * "Past line" element shift used by EP boundary growth: shift every
 * element whose relevant edge sits at-or-past `line` along `axis`, by
 * `delta`. Carries descendants and boundary events along (parentId +
 * boundaryHostId BFS). Skips any id in `excludeIds` (typically the new
 * element being placed and the EP itself).
 */
function shiftElementsPastLine(
  elements: DiagramElement[],
  axis: "x" | "y",
  line: number,
  delta: number,
  ...
  // Roots: any element whose far-side coord (left for x-axis, top for y-axis)
  // is at-or-past the line, NOT in excludeIds.
  const roots = new Set<string>();
  for (const e of elements) {
    if (excludeIds.has(e.id)) continue;
    const edge = axis === "x" ? e.x : e.y;
    if (edge >= line) roots.add(e.id);
  }
  // Expand via parentId + boundaryHostId.
...
/**
 * Span-aware bulk shift used by EP boundary growth. Roots are elements
 * past `line` along `shiftAxis` whose perpendicular projection overlaps
 * `[spanMin, spanMax]`. Descendants and boundary-event children of any
 * root come along regardless of their own span.
 *
 * Connector handling (fixes issue 2, where partial connectors crossing
 * the moving boundary lost their attachments):
 *   - Both endpoints in the shift set → translate waypoints by delta.
 *   - One endpoint in the shift set → recompute the route from the
 *     post-shift element positions.
 *   - Neither in the set → leave alone.
 */
function shiftElementsPastLineWithinSpan(
  elements: DiagramElement[],
  connectors: Connector[],
  shiftAxis: "x" | "y",
  line: number,
  ...
  if (delta === 0) return { elements, connectors };
  const roots = new Set<string>();
  for (const e of elements) {
    if (excludeIds.has(e.id)) continue;
    const edge = shiftAxis === "x" ? e.x : e.y;
    if (edge < line) continue;
    const perpMin = shiftAxis === "x" ? e.y : e.x;
    const perpMax = shiftAxis === "x" ? e.y + e.height : e.x + e.width;
    if (perpMax <= spanMin || perpMin >= spanMax) continue;
    roots.add(e.id);
  }
  const all = new Set<string>(roots);
  let changed = true;
  ...
  if (all.size === 0) return { elements, connectors };
  const dx = shiftAxis === "x" ? delta : 0;
  const dy = shiftAxis === "y" ? delta : 0;
  const newElements = elements.map((e) =>
    all.has(e.id) ? { ...e, x: e.x + dx, y: e.y + dy } : e,
  );
  const newConnectors = connectors.map((conn) => {
    const srcIn = all.has(conn.sourceId);
    const tgtIn = all.has(conn.targetId);
    if (srcIn && tgtIn) {
      return {
        ...conn,
...
/** Mirror of `shiftElementsPastLineWithinSpan` for elements BEFORE the
 *  line — used for NORTH / WEST resize handles. */
function shiftElementsBeforeLineWithinSpan(
  elements: DiagramElement[],
  connectors: Connector[],
  shiftAxis: "x" | "y",
  line: number,
  ...
  for (const e of elements) {
    if (excludeIds.has(e.id)) continue;
    const farEdge = shiftAxis === "x" ? e.x + e.width : e.y + e.height;
    if (farEdge > line) continue;
    const perpMin = shiftAxis === "x" ? e.y : e.x;
    const perpMax = shiftAxis === "x" ? e.y + e.height : e.x + e.width;
    if (perpMax <= spanMin || perpMin >= spanMax) continue;
    roots.add(e.id);
...
/**
 * When a Subprocess-Expanded is MOVED (translated, not resized), push
 * any outside element whose closest boundary is within `proximity` px
 * of the EP's leading edge in the direction of the move (issue 4).
 * "Leading edge" = the EP edge facing the move direction (bottom for
 * dy > 0, top for dy < 0, etc.). Span check on the perpendicular axis
 * keeps us from pushing elements far off to the side (issue 5).
 *
 * Excludes the EP itself, all EP descendants, and any boundary events
 * of the EP / its descendants (these all move with the EP already).
 */
function pushOutsideElementsForEPMove(
  elements: DiagramElement[],
  connectors: Connector[],
  epId: string,
  newEpRect: { x: number; y: number; width: number; height: number },
  dx: number,
  dy: number,
  proximity: number = 30,
): { elements: DiagramElement[]; connectors: Connector[] } {
  ...
  const epLeft = newEpRect.x;
  const epRight = newEpRect.x + newEpRect.width;
  const epTop = newEpRect.y;
  const epBottom = newEpRect.y + newEpRect.height;
  const pushIds = new Set<string>();
  for (const e of elements) {
    if (skipIds.has(e.id)) continue;
    // Skip pools/lanes — they're container ancestors of/around the EP,
    // not "outside elements" in the user's sense.
    if (e.type === "pool" || e.type === "lane" || e.type === "sublane") continue;
    let inProximity = false;
    if (dy > 0) {
      const horizOverlap = e.x + e.width > epLeft && e.x < epRight;
      const dist = e.y - epBottom;
      if (horizOverlap && dist <= proximity && dist >= -e.height) inProximity = true;
    }
    if (!inProximity && dy < 0) {
      const horizOverlap = e.x + e.width > epLeft && e.x < epRight;
      const dist = epTop - (e.y + e.height);
      if (horizOverlap && dist <= proximity && dist >= -e.height) inProximity = true;
    }
    if (!inProximity && dx > 0) {
      const vertOverlap = e.y + e.height > epTop && e.y < epBottom;
      const dist = e.x - epRight;
      if (vertOverlap && dist <= proximity && dist >= -e.width) inProximity = true;
    }
    if (!inProximity && dx < 0) {
      const vertOverlap = e.y + e.height > epTop && e.y < epBottom;
      const dist = epLeft - (e.x + e.width);
      if (vertOverlap && dist <= proximity && dist >= -e.width) inProximity = true;
    }
    if (inProximity) pushIds.add(e.id);
  }
  if (pushIds.size === 0) return { elements, connectors };
  // Carry descendants + boundary events of pushed roots along.
  const all = new Set<string>(pushIds);
  let changed = true;
  while (changed) {
    changed = false;
    for (const e of elements) {
      if (skipIds.has(e.id) || all.has(e.id)) continue;
      if (
        (e.parentId && all.has(e.parentId)) ||
        (e.boundaryHostId && all.has(e.boundaryHostId))
      ) {
...
/** Shift any connector waypoint whose coord is at-or-past `line` along
 *  `axis` by `delta`. Used in tandem with `shiftElementsPastLine` so
 *  free-floating connector segments don't get left behind. */
function shiftConnectorWaypointsPastLine(
  connectors: Connector[],
  axis: "x" | "y",
  line: number,
  delta: number,
...
/**
 * Whenever EP-related geometry changes (drop, move-in, resize), ensure
 * there's at least ½ × default-Task buffer between the deepest EP edge
 * (within this pool) and the pool's boundary on each axis. If the
 * current gap is already >= the buffer, the pool is left alone (no
 * shrink). This is what triggers the pool to follow when an EP edge
 * gets to within 33 px / 51 px of the pool boundary (user issue 3).
 *
...
function ensurePoolBufferFromEPs(
  ...
  let minEpLeft = Infinity;
  let minEpTop = Infinity;
  let hasEP = false;
  for (const id of descIds) {
    ...
    if (e?.type === "subprocess-expanded") {
      ...
      minEpLeft = Math.min(minEpLeft, e.x);
      minEpTop = Math.min(minEpTop, e.y);
    }
  }
  if (!hasEP) return elements;
  // Per-edge gap to nearest EP edge.
  const rightGap  = pool.x + pool.width - maxEpRight;
  const bottomGap = pool.y + pool.height - maxEpBottom;
  const leftGap   = minEpLeft - pool.x;
  const topGap    = minEpTop  - pool.y;
  // Outward-grow amounts (positive = pool needs to grow on that edge).
  const dRight  = rightGap  < bufW ? bufW - rightGap  : 0;
  const dBottom = bottomGap < bufH ? bufH - bottomGap : 0;
  const dLeft   = leftGap   < bufW ? bufW - leftGap   : 0; // grow LEFT (pool.x ↓)
  const dTop    = topGap    < bufH ? bufH - topGap    : 0; // grow TOP  (pool.y ↓)
  if (dRight <= 0 && dBottom <= 0 && dLeft <= 0 && dTop <= 0) return elements;

  // Width buffer: pool + ALL lane / sublane descendants widen on the
  // affected edge(s). For LEFT growth we also need to shift their x by
  // dLeft (since the pool itself moves left). Tasks etc. translate too.
  const widenIds = new Set<string>([poolId]);
  for (const id of descIds) {
    const e = elements.find((x) => x.id === id);
    if (e && (e.type === "lane" || e.type === "sublane")) widenIds.add(id);
  }
  // Bottom-most lane chain absorbs dBottom; top-most lane chain absorbs dTop.
  const bottomChain = new Set<string>([poolId]);
  if (dBottom > 0) {
    ...
    while (curId) {
      ...
      bottomChain.add(bottomLane.id);
      curId = bottomLane.id;
    }
  }
  const topChain = new Set<string>([poolId]);
  if (dTop > 0) {
    let curId: string | undefined = poolId;
    while (curId) {
      const childLanes = elements.filter(
      ...
      if (childLanes.length === 0) break;
      const topLane = childLanes.reduce((a, b) => (a.y < b.y ? a : b));
      topChain.add(topLane.id);
      curId = topLane.id;
    }
  }
  return elements.map((e) => {
    if (!descIds.has(e.id) && e.id !== poolId) return e;
    const isStructural =
      e.id === poolId || e.type === "lane" || e.type === "sublane";
    let nx = e.x;
    let ny = e.y;
    let nw = e.width;
    let nh = e.height;
    // The pool grows OUT around the existing EP — descendants stay at
    // their absolute positions, only the structural boundary (pool +
    // affected lane chain) expands. So we DON'T translate non-structural
    // descendants. This keeps the EP and its contents anchored while
    // the pool boundary slides outward to maintain the buffer.
    if (isStructural) {
      // X axis: pool + every lane/sublane descendant tracks the pool's
      // body. Shift x by -dLeft (pool's left moved left) and widen by
      // dLeft + dRight so the right edge moves to the new pool right.
      if (dLeft > 0) nx -= dLeft;
      if (dLeft > 0 || dRight > 0) nw += dLeft + dRight;
      // Y axis: top-chain absorbs dTop (y up, height up); bottom-chain
      // absorbs dBottom (height down). Other lanes stay put — their
      // y/height in the parent doesn't depend on the pool's outer edges.
      if (dTop > 0 && topChain.has(e.id)) {
        ny -= dTop;
        nh += dTop;
      }
      if (dBottom > 0 && bottomChain.has(e.id)) {
        nh += dBottom;
      }
    }
    if (nx === e.x && ny === e.y && nw === e.width && nh === e.height) return e;
    return { ...e, x: nx, y: ny, width: nw, height: nh };
  });
}

/** EP-to-pool buffers used by the cascade. Horizontal trigger is the
 *  user's "very close" criterion of 25 px (issue 3); vertical buffer
 *  remains ½ × default-Task height. */
const POOL_BUF_W = 25;
const POOL_BUF_H = 33;
 ...
 * shift left); `dRight` is the change in (pool.x + pool.width). For
 * each other pool, we shift its x by dLeft and its width by
 * (dRight - dLeft). Direct lane / sublane / sub-sublane descendants
 * widen too so lane boundaries stay flush with their pool. Tasks /
 * events / etc. just translate by dLeft to preserve their position
 * within their lane.
 *
 * Also updates connectors so message / association / sequence routes
 * with one or both endpoints in the shifted pools follow the shift
...
function applyPoolBoundaryShift(
  ...
  if (dLeft === 0 && dRight === 0) return { elements, connectors };
  const widthDelta = dRight - dLeft;
  const otherPools = elements.filter(
    (e) => e.type === "pool" && e.id !== excludePoolId,
  );
  if (otherPools.length === 0) return { elements, connectors };
  const affect = new Set<string>();
...
/**
 * Apply an EP boundary change (rect oldRect → newRect) and run the full
 * cascade: external shifts past/before the moved boundaries, re-snap
 * EP boundary events, bubble enclosure up the parent chain, enforce the
 * pool buffer, cascade pool-below / pool-above / pool-boundary shifts
 * to neighbouring pools, and recompute connectors anchored to the EP
 * or its boundary events.
 *
 * Internal contents do NOT translate — when the EP's top moves up, only
 * the EP's own rect changes plus the cascade upward through ancestors.
 * Anything inside the EP keeps its absolute position.
 *
 * `excludeId` is for callers that placed a new element near the moving
 * boundary (ADD_ELEMENT / MOVE_END). The bulk shift skips it so it
 * doesn't get shoved by the same delta.
 */
function applyEPBoundaryChange(
  elements: DiagramElement[],
  connectors: Connector[],
  epId: string,
 ...
 * Expanded Subprocess `epId`, and if so, decide which edge "wins" and
 * compute the resulting EP rect + element placement, then delegate the
 * cascade (external shifts, re-snap, ensure-enclose, pool buffer, pool
 * cascades, connector recompute) to `applyEPBoundaryChange`.
 *
 * Returns `null` if the drop is comfortably interior.
 *
 * Per-edge behaviour:
 *  - RIGHT  → keep ep.x fixed; grow EP rightward so the new element
 *              has a ½×newW buffer between its right edge and the new
 *              EP right.
 *  - BOTTOM → keep ep.y fixed; grow EP downward, ½×newH buffer below.
 *  - LEFT   → keep ep.right fixed; move ep.x LEFT so the new element
 *              fits at ep.x + ½×newW. Internal contents stay put — the
 *              EP grows OUTWARD into the parent and the cascade adjusts
 *              the parent pool / pools above.
 *  - TOP    → keep ep.bottom fixed; move ep.y UP so the new element
 *              fits at the new ep.y + ½×newH. Internal contents stay.
 *              Replaces the old "shift all internals down" approach.
 */
function expandEPForBoundaryEntry(
  ...
  const straddleBottom = xOverlap && newEl.y < ep.y + ep.height && newEl.y + newEl.height > ep.y + ep.height;
  ...
  // ½-element buffer between the new element's edge and the nearest EP
  // boundary on the entry axis.
  const bufW = newEl.width / 2;
  const bufH = newEl.height / 2;

  let newEpRect: { x: number; y: number; width: number; height: number };
  let placedX: number;
  let placedY: number;

  if (winner === "right") {
    placedX = Math.max(ep.x + bufW, newEl.x);
    placedY = Math.max(ep.y + bufH, Math.min(newEl.y, ep.y + ep.height - newEl.height - bufH));
    const newRightEdge = Math.max(ep.x + ep.width, placedX + newEl.width + bufW);
    newEpRect = { x: ep.x, y: ep.y, width: newRightEdge - ep.x, height: ep.height };
  }
    ...
    newEpRect = { x: ep.x, y: ep.y, width: ep.width, height: newBottomEdge - ep.y };
  }
  else if (winner === "left") {
    // EP grows OUTWARD on the left: keep ep.right fixed, move ep.x left
    // by 1.5×newW so the new element fits at ep.x_new + ½×newW with
    // existing internal contents untouched. Cascade pushes the parent
    // pool's left out via `applyEPBoundaryChange` (issue: replaces the
    // old "shift internals right by 1.5×newW" approach).
    const oldRight = ep.x + ep.width;
    const grow = newEl.width + bufW; // 1.5 × newW
    const newX = ep.x - grow;
    newEpRect = { x: newX, y: ep.y, width: oldRight - newX, height: ep.height };
    placedX = newX + bufW;
    placedY = Math.max(ep.y + bufH, Math.min(newEl.y, ep.y + ep.height - newEl.height - bufH));
  }
  else { // top
    // EP grows OUTWARD on the top: keep ep.bottom fixed, move ep.y up
    // by 1.5×newH so the new element fits at ep.y_new + ½×newH with
    // existing internal contents untouched. Cascade pushes the parent
    // pool's top up via `applyEPBoundaryChange` (replaces the old
    // "shift all internals down by 1.5×newH" approach).
    const oldBottom = ep.y + ep.height;
    const grow = newEl.height + bufH; // 1.5 × newH
    const newY = ep.y - grow;
    newEpRect = { x: ep.x, y: newY, width: ep.width, height: oldBottom - newY };
    placedX = Math.max(ep.x + bufW, Math.min(newEl.x, ep.x + ep.width - newEl.width - bufW));
    placedY = newY + bufH;
  }

  const r = applyEPBoundaryChange(elements, connectors, epId, oldEpRect, newEpRect, newEl.id);
  return { elements: r.elements, connectors: r.connectors, placedX, placedY };
}
...
function ensureContainersEncloseChildren(
  ...
  for (const e of sorted) {
    if (e.type !== "pool" && e.type !== "lane" && e.type !== "sublane" && e.type !== "subprocess-expanded") continue;
 ...
 *
 * The new band is carved out of the NEIGHBOUR it goes next to — the band
 * below it for a band on top, the band above it for one at the bottom, and
 * between two bands the taller of them. The neighbour gives up the empty
 * space at that edge, beyond its contents, up to half its height, so the
 * process inside it stays in the lane it was in. When that edge is too crowded
 * the donor's content slides away from it, into the donor's own empty space
 * at the far edge — still inside the same lane. It never gives more than its
 * own label (and its own sub-lanes' labels) can spare — the same floor a
 * boundary drag stops at — nor covers any content, and when that leaves too
 * little for the new band's label nothing is added.
 *
 * `index` is where the new band goes in the parent's top-to-bottom stack:
 * 0 = on top, bands.length = at the bottom. Returns null when there is no
 * room, or the parent has no bands yet (a first lane fills its parent, which
 * is its own case).
 */
function carveBandWithin(
  state: DiagramData,
  parentId: string,
  index: number,
  label: string,
  /** A neighbour to take the room from instead, when it is adjacent. */
  preferDonorId?: string,
): { state: DiagramData; bandId: string } | null {
  const parent = state.elements.find((e) => e.id === parentId);
  if (!parent || (parent.type !== "pool" && parent.type !== "lane")) return null;
  const bands = state.elements
  ...
  // Which neighbour gives, and from which of its edges.
  ...
  const content = contentBoundsOf(state.elements, donor.id);
  ...
  // The donor's content may slide away from the giving edge into its empty
  // space at the far one — never out of the lane it is in. Only a donor with
  // no sub-lanes: sliding content past fixed sublane dividers would change
  // which sublane it sits in.
  const hasSubs = state.elements.some((e) => e.type === "lane" && e.parentId === donor.id);
  ...
  const slide = Math.max(0, give - emptyNear);

  // The donor shrinks at that edge; its own stack re-fits there too.
  const donorY = edge === "first" ? donor.y + give : donor.y;
  const donorH = donor.height - give;
  let elements = state.elements.map((e) => (e.id === donor.id ? { ...e, y: donorY, height: donorH } : e));
  elements = refitStackAtEdge(elements, donor.id, donorY, donorH, donor.x, donor.width, edge, poolFs, laneFs);

  const headerW = parent.type === "pool" ? getPoolHeaderWidth(parent) : getLaneHeaderWidth(parent);
  const band: DiagramElement = {
    id: nanoid(), type: "lane",
    x: parent.x + headerW, y: edge === "first" ? donor.y : donor.y + donorH,
    width: parent.width - headerW, height: give,
    label, properties: {}, parentId,
  };
  ...
  if (slide > 0) {
    ...
    next = reducerImpl(next, { type: "MOVE_ELEMENTS", payload: { ids, dx: 0, dy: edge === "first" ? slide : -slide } });
  }
  return { state: next, bandId: band.id };
}

/**
 * Half an event's clear space in front of the first element, made INSIDE the
 * pool (Paul, 2026-09-21: "Make sure there is always a gap of at least 1/2
 * event width between the left edge of the left-most element and the
 * right-hand edge of the new Pool, Lane or Sublane").
 *
 * This first grew the pool and its lanes LEFTWARDS by the shortfall. Paul,
 * 2026-09-22: "Adding lanes to a Pool should not grow the Pool. The lanes
 * must be added within the Pool." So the pool stays exactly where it is and
 * its CONTENT moves right instead — as an ordinary MOVE_ELEMENTS, so
 * connectors, boundary events and an Expanded Subprocess's children travel
 * with it. The move never pushes content past the pool's right edge: where
 * there isn't room for the whole shortfall it moves as far as there is room.
 */
function withLeftGap(state: DiagramData, startId: string): DiagramData {
  ...
  const shortfall = leftGapShortfall(pool.x + inset, 0, content.x);
  const room = pool.x + pool.width - (content.x + content.width);
  const shift = Math.min(shortfall, Math.max(0, room));
  if (shift <= 0) return state;

  // The pool's own content: everything whose parent is the pool or one of
  // its lanes. MOVE_ELEMENTS brings their descendants along.
  const ownerIds = new Set([pool.id, ...containerIds]);
  const ids = elements.filter((e) => !containerIds.has(e.id) && e.parentId && ownerIds.has(e.parentId)).map((e) => e.id);
  if (ids.length === 0) return state;
  return reducerImpl(state, { type: "MOVE_ELEMENTS", payload: { ids, dx: shift, dy: 0 } });
}
...
// autoResizeUmlElement moved to app/lib/diagram/umlAutoSize.ts (imported above)
// so the image-import layout sizes UML boxes identically to the editor.
...
function reducerImpl(state: DiagramData, action: Action): DiagramData {
  switch (action.type) {
    ...
    case "ADD_ELEMENT": {
      ...
      if (action.payload.symbolType === "pool") {
        ...
        if (targetPool) {
          ...
          // Every lane or sublane a drop adds is carved out of its neighbour;
          // the pool keeps its size (Paul, 2026-09-22: "Never grow the Pool
          // with these Lane and Sublane additions"). No room → nothing added.
          const carveLane = (index: number) => {
            const carved = carveBandWithin(state, targetPool.id, index, `Lane ${totalLaneCount + 1}`);
            return carved ? carved.state : state;
          };
          const laneIndexAt = (y: number) => lanes.filter((l) => l.y < y - 0.5).length;

          // A pool with ONE lane always just gets a second one (Paul,
          // 2026-09-22: "Dragging the Pool/Lane symbol onto a Pool with one
          // single Lane should always just add a second Lane to the Pool").
          if (lanes.length === 1) return carveLane(1);
          ...
          if (dy <= TOP_BOTTOM_THRESHOLD) return carveLane(0);
          // B2: near bottom boundary → insert BELOW all lanes
          if (poolBot - dropPos.y <= TOP_BOTTOM_THRESHOLD) return carveLane(lanes.length);
          // B3: near a lane separator → insert BETWEEN
          for (let i = 0; i < lanes.length - 1; i++) {
            const sep = lanes[i].y + lanes[i].height;
            if (Math.abs(dropPos.y - sep) <= SEPARATOR_THRESHOLD) return carveLane(i + 1);
          }
          ...
          if (cursorLane) {
            ...
            // A sublane inside `cursorLane`, or a lane in the pool, at `insertY`
            // — each carved out of its neighbour, nothing grows.
            const insertSublaneAt = (insertY: number, labelN: number) => {
              const carved = carveBandWithin(state, cursorLane.id, sublanes.filter((sl) => sl.y < insertY - 0.5).length, `Sublane ${labelN}`);
              return carved ? carved.state : state;
            };
            const insertLaneAt = (insertY: number, _labelN: number) => carveLane(laneIndexAt(insertY));
            ...
            if (sublanes.length === 0) {
              ...
              // Otherwise (not the middle third): a sibling LANE below it.
        ...
        const containers = state.elements.filter(
          (b) => {
            ...
            if (b.type === "subprocess-expanded") {
              ...
              // True straddle: bounds cross an EP edge AND the element's
              // perpendicular projection overlaps the EP. Without the
              // perpendicular-overlap guard, an element placed FAR to the
              // right of the EP whose y-range (incidentally) crosses
              // the EP top edge would be treated as straddling — and
              // expandEPForBoundaryEntry would grow the EP for an
              // element nowhere near it.
              const xOverlap = newEl.x < b.x + b.width && newEl.x + newEl.width > b.x;
              const yOverlap = newEl.y < b.y + b.height && newEl.y + newEl.height > b.y;
      ...
      if (
        ...
        if (parent?.type === "subprocess-expanded") {
          ...
          if (grown) {
            ...
            workingConnectors = grown.connectors;
    ...
    case "RESIZE_ELEMENT": {
      ...
      // applyEPBoundaryChange handles the rect change, EP boundary-event
      // re-snap, span-aware external shifts, ensureContainersEncloseChildren
      // bubble-up, EP→pool buffer enforcement, and pool cascades (below /
      // above / horizontal lockstep).
      ...
      // Auto-grow ancestors when a contained element grew (e.g.
      // process-group adoption above). EP target is handled by the
      // dedicated branch earlier in this case statement and never falls
      // through to here.
    ...
    case "DELETE_ELEMENT": {
      ...
      if (el?.type === "lane" && el.parentId) {
        ...
        if (parent) {
          // Helper: promote a lane's direct sublanes into a slot, recursively
          // resize their descendants proportionally, re-home non-lane orphans
          // to the closest promoted lane, and cascade x/width through the
          // promoted hierarchy so each lane sits flush against its new
          // parent's header width.
          const promoteIntoSlot = (
            elementsArr: DiagramElement[],
            deletedLane: DiagramElement,
            newParent: DiagramElement,
            dSubs: DiagramElement[],
            directOrphanIds: string[],
          ): DiagramElement[] => {
            if (dSubs.length === 0) return elementsArr;

            const deletedY = deletedLane.y;
            const deletedH = deletedLane.height;
            const childrenTop = Math.min(...dSubs.map((c) => c.y));
            const childrenBottom = Math.max(...dSubs.map((c) => c.y + c.height));
            const childrenSpan = Math.max(1, childrenBottom - childrenTop);

            const promotedScale = new Map<
              string,
              { oldY: number; oldH: number; newY: number; newH: number }
            >();
            for (const child of dSubs) {
              const t = (child.y - childrenTop) / childrenSpan;
              const b = (child.y + child.height - childrenTop) / childrenSpan;
              promotedScale.set(child.id, {
                oldY: child.y,
                oldH: child.height,
                newY: deletedY + t * deletedH,
                newH: (b - t) * deletedH,
              });
            }
            ...
            const cascadeResize = (
              ...
              for (const e of elementsArr) {
                if (e.type === "lane" && e.parentId === parentLid && !promotedScale.has(e.id)) {
                  const ns = {
                    oldY: e.y,
                    oldH: e.height,
                    newY: parentScale.newY + (e.y - parentScale.oldY) * sf,
                    newH: e.height * sf,
                  };
                  promotedScale.set(e.id, ns);
                  cascadeResize(e.id, ns);
                }
              }
            };
            for (const child of dSubs) cascadeResize(child.id, promotedScale.get(child.id)!);

            const directSublaneIdSet = new Set(dSubs.map((c) => c.id));
            let result = elementsArr.map((e) => {
              ...
              return {
                ...
                y: sc.newY,
                height: sc.newH,
                parentId: directSublaneIdSet.has(e.id) ? newParent.id : e.parentId,
              };
            });

            for (const oid of directOrphanIds) {
              const orph = result.find((e) => e.id === oid);
              if (!orph) continue;
              const center = orph.y + orph.height / 2;
              let best: { id: string; dist: number } | null = null;
              for (const c of dSubs) {
                ...
                const d = Math.min(
                  Math.abs(center - sc.newY),
                  Math.abs(center - (sc.newY + sc.newH)),
                );
                if (!best || d < best.dist) best = { id: c.id, dist: d };
              }
              if (best) {
                const bestId = best.id;
                result = result.map((e) => (e.id === oid ? { ...e, parentId: bestId } : e));
              }
            }

            // Cascade x/width through promoted hierarchy: each promoted lane
            // (and its promoted descendants) sits at parent.x + headerWidth,
            // with width = parent.width - headerWidth.
            const queue: string[] = dSubs.map((c) => c.id);
            const seen = new Set<string>();
            while (queue.length) {
              const lid = queue.shift()!;
              if (seen.has(lid)) continue;
              seen.add(lid);
              const lane = result.find((e) => e.id === lid);
              if (!lane) continue;
              const par = result.find((e) => e.id === lane.parentId);
              if (!par) continue;
              const headerW = par.type === "pool" ? getPoolHeaderWidth(par) : getLaneHeaderWidth(par);
              const newX = par.x + headerW;
              const newW = par.width - headerW;
              if (lane.x !== newX || lane.width !== newW) {
                result = result.map((e) =>
                  e.id === lid ? { ...e, x: newX, width: newW } : e,
                );
              }
              for (const e of elementsArr) {
                if (
          ...
          // Helper: while `parent` has exactly one lane child, dissolve it.
          // If that lone lane has its own direct sublanes, promote them into
          // the lone lane's slot (filling parent's body) and remove the lone
          // lane. If it has no sublanes, flatten the lane into the parent.
          // Loops up to 10 times to handle deep promote-then-dissolve chains.
          const dissolveSingletons = (
            elementsArr: DiagramElement[],
            par: DiagramElement,
          ): DiagramElement[] => {
            ...
            for (let iter = 0; iter < 10; iter++) {
              const remaining = result.filter(
                (e) => e.type === "lane" && e.parentId === par.id,
              );
              if (remaining.length !== 1) return result;
              const last = remaining[0];
              const lastSubs = result
                .filter((e) => e.type === "lane" && e.parentId === last.id)
                .sort((a, b) => a.y - b.y);
              const lastOrphans = result
                .filter((e) => e.type !== "lane" && e.parentId === last.id)
                .map((e) => e.id);
              if (lastSubs.length > 0) {
                result = promoteIntoSlot(result, last, par, lastSubs, lastOrphans);
                result = result.filter((e) => e.id !== last.id);
              } else {
                result = result.map((e) =>
                  e.parentId === last.id ? { ...e, parentId: par.id } : e,
                );
                result = result.filter((e) => e.id !== last.id);
                return result;
              }
          ...
          // Direct sublane children of the deleted lane (read from original
          // state — line 2437's .map() unparented them to undefined).
          ...
          if (directSublanes.length > 0) {
            ...
            // deleted lane's vertical slice. Grandchildren and deeper
            // descendants resize proportionally to their parent's new
            // height. Sibling lanes at the deleted lane's level are
            // unaffected — they keep their original (y, height).
            const directOrphans = state.elements
              .filter((e) => e.type !== "lane" && e.parentId === id)
              .map((e) => e.id);
            elements = promoteIntoSlot(elements, el, parent, directSublanes, directOrphans);
            // After promotion, parent might have only one lane (e.g. the
            // deleted lane was the only sibling AND had only one sublane).
            // Recursively dissolve.
            elements = dissolveSingletons(elements, parent);
          } else {
            ...
            // parent if there are no siblings).
            const deletedY = el.y;
            ...
            // Replaces the original "second-last cascade-delete" rule.
            // Sublanes of the lone surviving lane are now PROMOTED rather
            // than cascade-deleted (e.g. delete L1 of P{L1, L2 with SL3,
            // SL4} → L2 absorbs → singleton dissolve promotes SL3, SL4 to
            // be lanes of P, filling its body).
            elements = dissolveSingletons(elements, parent);
          }
    ...
    case "ADD_LANE": {
      ...
      // by moving the content right, never by growing the pool.
      return withLeftGap({ ...state, ...next }, poolId);
    }
    ...
    case "ADD_SUBLANE": {
      ...
      if (existingSublanes.length > 0) {
        // One more sublane, at the bottom — carved out of the sublane above
        // it; the lane and the pool keep their size (Paul, 2026-09-22).
        const sublaneCount = state.elements.filter((e) => e.type === "lane" && e.parentId).length;
        const carved = carveBandWithin(state, laneId, existingSublanes.length, `Sublane ${sublaneCount + 1}`);
        if (!carved) return state;
        return withLeftGap(carved.state, laneId);
      } else {
    ...
    case "SPLIT_POOL_EVEN": {
      ...
      return withLeftGap({ ...state, elements: updatePoolTypes([...elements, ...placedNew]), connectors: state.connectors }, poolId);
    }
    ...
    case "ADD_LANE_AT": {
      ...
      // Carved out of the lane it is placed against; the pool keeps its size
      // (Paul, 2026-09-22: "Never grow the Pool with these Lane and Sublane
      // additions"). No room → no lane.
      const stack = state.elements
        .filter((e) => e.type === "lane" && e.parentId === poolId)
        .sort((a, b) => a.y - b.y);
      const refIdx = stack.findIndex((e) => e.id === refLaneId);
      if (refIdx < 0) return state;
      const carved = carveBandWithin(
        state, poolId, position === "above" ? refIdx : refIdx + 1,
        uniqueContainerLabel(state.elements, label, "Lane"), refLaneId,
      );
      if (!carved) return state;
      return withLeftGap(carved.state, poolId);
    }
    ...
    case "MOVE_LANE_BOUNDARY": {
      ...
      // change — see `refitStackAtEdge`.
      elements = refitStackAtEdge(elements, aboveLaneId, above.y, newAboveH, above.x, above.width, "last", poolFsNow, laneFsNow2);
      elements = refitStackAtEdge(elements, belowLaneId, newBelowY, newBelowH, below.x, below.width, "first", poolFsNow, laneFsNow2);
