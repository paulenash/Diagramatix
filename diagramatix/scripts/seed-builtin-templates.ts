...
// The full row set (new fragments + the carried-over existing 12).
type Row = { name: string; diagramType: string; group: string; description: string; data: TemplateData };
function buildAllRows(): Row[] {
  const rows: Row[] = FRAGMENTS.map((f) => ({ name: f.name, diagramType: "bpmn", group: f.group, description: f.description, data: buildFragment(f) }));
  ...
  if (fs.existsSync(file)) {
    ...
    for (const t of exp.templates) {
      rows.push({ name: t.name, diagramType: t.diagramType, group: t.group ?? "Process Fragments", description: EXISTING_DESCRIPTIONS[t.name] ?? "", data: t.data });
    }
  } else {
    console.warn(`  (existing .diag_tems not found at ${file} — the SQL will not carry the original 12)`);
  }
  return rows;
}

// ── SQL emitter ────────────────────────────────────────────────────────────
// Dollar-quoted literals so the big JSON/SVG blobs need no escaping. Idempotent:
// UPDATE metadata+thumbnail if the built-in exists (data left untouched, matching
// the DB path), else INSERT with data. New built-ins are owned by the oldest user.
const dq = (tag: string, s: string) => `$${tag}$${s}$${tag}$`;
function toSql(rows: Row[]): string {
  const out: string[] = [
    "-- Built-in BPMN template library — generated by scripts/seed-builtin-templates.ts --sql",
    "-- Idempotent. Run against prod:  psql \"<prod url>\" -f scripts/seed-builtin-templates.sql",
    "BEGIN;",
    "",
  ];
  for (const r of rows) {
    const n = dq("TN", r.name), g = dq("TG", r.group), d = dq("TD", r.description);
    const t = dq("TS", renderTemplateThumbnailSvg(r.data) || "");
    const j = dq("TJ", JSON.stringify(r.data));
    out.push(`-- ${r.name}  [${r.group}]`);
    out.push(`UPDATE "DiagramTemplate" SET "group"=${g}, description=${d}, "thumbnailSvg"=${t}, "updatedAt"=now()`);
    out.push(`  WHERE name=${n} AND "diagramType"='bpmn' AND "templateType"='builtin';`);
    out.push(`INSERT INTO "DiagramTemplate" (id, name, "diagramType", "templateType", "group", description, "thumbnailSvg", data, "userId", "createdAt", "updatedAt")`);
    out.push(`  SELECT gen_random_uuid()::text, ${n}, 'bpmn', 'builtin', ${g}, ${d}, ${t}, ${j}::jsonb, (SELECT id FROM "User" ORDER BY "createdAt" ASC LIMIT 1), now(), now()`);
    out.push(`  WHERE NOT EXISTS (SELECT 1 FROM "DiagramTemplate" WHERE name=${n} AND "diagramType"='bpmn' AND "templateType"='builtin');`);
    out.push("");
  }
  out.push("COMMIT;");
  out.push("");
...
async function main() {
  ...
  // --sql: emit a ready-to-run SQL file instead of writing to the DB.
  if (process.argv.includes("--sql")) {
    const outPath = path.join(process.cwd(), "scripts", "seed-builtin-templates.sql");
    fs.writeFileSync(outPath, toSql(rows), "utf8");
    console.log(`Wrote ${rows.length} templates to ${outPath}`);
    return;
  }
  ...
  try {
    for (const r0 of rows) {
      const r = await upsert(prisma, r0);
      r === "created" ? created++ : updated++;
      console.log(`  ${r === "created" ? "add   " : "update"} "${r0.name}"  (${r0.group})`);
    }
